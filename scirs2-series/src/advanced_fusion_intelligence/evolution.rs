...
impl<F: Float + Debug + Clone + FromPrimitive> EvolutionEngine<F> {
    ...
    pub fn evolve_generation(&mut self, fitness_evaluator: &FitnessEvaluator<F>) -> Result<()> {
        ...
        for i in (0..selected.len()).step_by(2) {
            ...
            // Crossover
            let (mut child1, mut child2) = if rand::random::<f64>() < self.crossover_rate.to_f64().unwrap() {
    ...
    fn roulette_wheel_selection(&self) -> Result<Vec<Architecture<F>>> {
        let total_fitness: F = self.population.iter()
    ...
    fn crossover(&self, parent1: &Architecture<F>, parent2: &Architecture<F>) -> Result<(Architecture<F>, Architecture<F>)> {
    ...
    pub fn get_population_stats(&self) -> (F, F, F) {
        ...
        let fitness_values: Vec<F> = self.population.iter()
            .map(|ind| ind.fitness_score)
            .collect();
        ...
        let max_fitness = fitness_values.iter()
            .fold(F::neg_infinity(), |acc, &x| if x > acc { x } else { acc });

        let min_fitness = fitness_values.iter()
...
impl<F: Float + Debug + Clone + FromPrimitive> Architecture<F> {
    ...
    pub fn calculate_complexity(&self) -> F {
        let layer_complexity: usize = self.layers.iter()
    ...
    pub fn validate(&self) -> bool {
        ...
        for connection in &self.connections {
            if connection.from_layer >= self.layers.len() || connection.to_layer >= self.layers.len() {
                return false;
            }
        }
...
impl MutationOperator {
    ...
    pub fn apply<F: Float + Debug + Clone>(&self, architecture: &mut Architecture<F>) -> Result<()> {
    ...
    /// Mutate layer parameters
    fn mutate_parameters<F: Float + Debug + Clone>(&self, architecture: &mut Architecture<F>) -> Result<()> {
    ...
    fn add_layer<F: Float + Debug + Clone>(&self, architecture: &mut Architecture<F>) -> Result<()> {
    ...
    fn remove_layer<F: Float + Debug + Clone>(&self, architecture: &mut Architecture<F>) -> Result<()> {
        if architecture.layers.len() > 2 { // Keep at least 2 layers
    ...
    /// Mutate connections
    fn mutate_connections<F: Float + Debug + Clone>(&self, architecture: &mut Architecture<F>) -> Result<()> {
