    ...
    F: Float + FromPrimitive + Debug + Display + ScalarOperand,
{
    ...
    pub fn smooth(&mut self, observations: &Array1<F>) -> Result<SmootherResult<F>> {
        ...
        for t in (0..(n - 1)).rev() {
            ...
            let temp = filtered_current.covariance.dot(&self.transition.transition_matrix.t());
            ...
            smoothed_states.insert(0, StateVector {
    ...
    pub fn compute_log_likelihood(&mut self, observations: &Array1<F>) -> Result<F> {
        ...
        for obs in observations.iter() {
            ...
            let temp = self.observation.observation_matrix.dot(&self.state.covariance);
            let innovation_cov = temp.dot(&self.observation.observation_matrix.t()) + &self.observation.observation_noise;
            ...
            log_likelihood = log_likelihood - F::from(0.5).unwrap() * (dim * two_pi.ln() + log_det + quadratic_form);
    ...
    F: Float + FromPrimitive + Debug + Display + ScalarOperand,
{
    ...
    pub fn fit_with_em<S>(&mut self, data: &ArrayBase<S, Ix1>, em_params: &EMParameters<F>) -> Result<EMResult<F>>
    where
        S: Data<Elem = F>,
    {
        let n = data.len();
        ...
        for iteration in 0..em_params.max_iterations {
            ...
            if em_params.verbose {
                println!("EM Iteration {}: Log-likelihood = {:.6}", iteration, new_log_likelihood);
            }
        ...
        if let Some(level) = &self.level {
            ...
            for i in 0..level_dim {
                for j in 0..level_dim {
                    transition_matrix[[current_idx + i, current_idx + j]] = level.transition_matrix[[i, j]];
        ...
        if let Some(trend) = &self.trend {
            ...
            for i in 0..trend_dim {
                for j in 0..trend_dim {
                    transition_matrix[[current_idx + i, current_idx + j]] = trend.transition_matrix[[i, j]];
        ...
        if let Some(seasonal) = &self.seasonal {
            ...
            for i in 0..seasonal_dim {
                for j in 0..seasonal_dim {
                    transition_matrix[[current_idx + i, current_idx + j]] = seasonal.transition_matrix[[i, j]];
                    process_noise[[current_idx + i, current_idx + j]] = seasonal.process_noise[[i, j]];
                }
            }
        }
    ...
    fn update_parameters(&mut self, smoother_result: &SmootherResult<F>, observations: &Array1<F>) -> Result<()> {
        let n = observations.len();
        
        // Compute sufficient statistics
        let mut sum_x = Array1::zeros(self.get_state_dimension());
        let mut sum_xx = Array2::zeros((self.get_state_dimension(), self.get_state_dimension()));
        let mut sum_x_lag = Array2::zeros((self.get_state_dimension(), self.get_state_dimension()));
        let mut sum_y = F::zero();
        let mut sum_yy = F::zero();
        let mut sum_xy = Array1::zeros(self.get_state_dimension());
        ...
        let residual_variance = (sum_yy - F::from(2.0).unwrap() * sum_xy[0] * sum_y / n_f + sum_xx[[0, 0]] * sum_y * sum_y / (n_f * n_f)) / n_f;
        ...
        if let Some(level) = &mut self.level {
            let level_variance = sum_xx[[0, 0]] / n_f - (sum_x[0] / n_f) * (sum_x[0] / n_f);
            if level_variance > F::zero() {
                level.process_noise[[0, 0]] = level_variance * F::from(0.1).unwrap(); // Scale down for stability
            }
        }
    ...
    pub fn seasonal(sigma_level: F, sigma_seasonal: F, sigma_obs: F, period: usize) -> Result<Self> {
        if period < 2 {
            return Err(TimeSeriesError::InvalidInput(
    ...
    F: Float + FromPrimitive + Debug + Display + ScalarOperand,
{
        ...
        S: Data<Elem = F>,
    {
        ...
        for iteration in 0..em_params.max_iterations {
            ...
            if em_params.verbose {
                println!("DLM EM Iteration {}: Log-likelihood = {:.6}", iteration, new_log_likelihood);
            }
    ...
    fn update_dlm_parameters(&mut self, smoother_result: &SmootherResult<F>, observations: &Array1<F>) -> Result<()> {
        let n = observations.len();
        let state_dim = self.transition.transition_matrix.nrows();
        
        // Compute sufficient statistics
        let mut sum_x = Array1::zeros(state_dim);
        let mut sum_xx = Array2::zeros((state_dim, state_dim));
        let mut sum_x_lag = Array2::zeros((state_dim, state_dim));
        let mut sum_xx_lag = Array2::zeros((state_dim, state_dim));
        let mut sum_y = F::zero();
        let mut sum_yy = F::zero();
        let mut sum_xy = Array1::zeros(state_dim);
        ...
        for t in 0..n {
            ...
            if t > 0 {
                let prev_state = &smoother_result.smoothed_states[t - 1];
                sum_x_lag = sum_x_lag + &outer_product(&state.state, &prev_state.state);
                sum_xx_lag = sum_xx_lag + &prev_state.covariance + &outer_product(&prev_state.state, &prev_state.state);
            }
        }
        ...
        if n > 1 {
            ...
            let predicted_transitions = self.transition.transition_matrix[[0, 0]] * sum_x_lag[[0, 0]];
            let process_residual_variance = (sum_xx[[0, 0]] - predicted_transitions) / n_transitions;
...
mod tests {
    ...
    fn test_kalman_smoother() {
        ...
        assert!(smoother_result.log_likelihood > F::neg_infinity());
    }
