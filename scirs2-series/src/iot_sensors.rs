...
impl EnvironmentalSensorAnalysis {
    ...
    pub fn with_temperature(mut self, data: Array1<f64>) -> Result<Self> {
        if data.iter().any(|x| !x.is_finite()) {
            return Err(TimeSeriesError::InvalidInput("Temperature data contains non-finite values".to_string()));
        }
    ...
    pub fn with_humidity(mut self, data: Array1<f64>) -> Result<Self> {
        if data.iter().any(|x| !x.is_finite()) {
            return Err(TimeSeriesError::InvalidInput("Humidity data contains non-finite values".to_string()));
        }
        ...
        if let Some(ref temp_data) = self.temperature {
            ...
            for i in 1..temp_data.len() {
                if (temp_data[i] - temp_data[i-1]).abs() < 0.01 {
                    ...
                    if consecutive_count > 20 { // 20 consecutive identical readings
    ...
    pub fn comfort_index(&self) -> Result<Array1<f64>> {
        let temp_data = self.temperature.as_ref()
            .ok_or_else(|| TimeSeriesError::InvalidInput("Temperature data required".to_string()))?;
        let humidity_data = self.humidity.as_ref()
    ...
    pub fn energy_optimization_recommendations(&self) -> Result<Vec<String>> {
        ...
        if let Some(ref temp_data) = self.temperature {
            ...
            if avg_temp > 25.0 {
                recommendations.push("Consider increasing cooling setpoint during peak hours".to_string());
            } else if avg_temp < 18.0 {
                recommendations.push("Consider decreasing heating setpoint during off-peak hours".to_string());
            }
            
            // Temperature stability analysis
            let temp_std = temp_data.std(0.0);
            if temp_std > 3.0 {
                recommendations.push("High temperature variation detected - check HVAC efficiency".to_string());
            }
        }
        ...
        if let Some(ref humidity_data) = self.humidity {
            ...
            if avg_humidity > 70.0 {
                recommendations.push("High humidity detected - consider dehumidification".to_string());
...
impl MotionSensorAnalysis {
    ...
    pub fn with_accelerometer(mut self, data: Array2<f64>) -> Result<Self> {
        if data.iter().any(|x| !x.is_finite()) {
            return Err(TimeSeriesError::InvalidInput("Acceleration data contains non-finite values".to_string()));
        }
    ...
    pub fn with_gps(mut self, data: Array2<f64>) -> Result<Self> {
        if data.iter().any(|x| !x.is_finite()) {
            return Err(TimeSeriesError::InvalidInput("GPS data contains non-finite values".to_string()));
        }
    ...
    pub fn activity_recognition(&self) -> Result<Vec<(usize, String)>> {
        let accel_data = self.acceleration.as_ref()
            .ok_or_else(|| TimeSeriesError::InvalidInput("Accelerometer data required".to_string()))?;
        ...
        for start in (0..accel_data.nrows()).step_by(window_size) {
            ...
            // Calculate features
            let magnitude: Array1<f64> = window.outer_iter()
    ...
    pub fn fall_detection(&self, threshold: f64) -> Result<Vec<usize>> {
        ...
        for (i, row) in accel_data.outer_iter().enumerate() {
            ...
            if magnitude > threshold {
                ...
                if check_samples > 0 {
                    let future_window = accel_data.slice(ndarray::s![i+1..i+1+check_samples, ..]);
                    let future_magnitudes: Array1<f64> = future_window.outer_iter()
                        .map(|row| (row[0]*row[0] + row[1]*row[1] + row[2]*row[2]).sqrt())
                        .collect();
                        
                    let mean_future = future_magnitudes.mean().unwrap();
                    if mean_future < 2.0 { // Low acceleration following high impact
                        fall_events.push(i);
                    }
                }
            }
    ...
    pub fn step_count(&self) -> Result<usize> {
        ...
        let magnitude: Array1<f64> = accel_data.outer_iter()
            .map(|row| (row[0]*row[0] + row[1]*row[1] + row[2]*row[2]).sqrt())
            .collect();
        ...
        for i in 1..filtered.len() - 1 {
            if filtered[i] > filtered[i-1] && filtered[i] > filtered[i+1] && 
               filtered[i] > min_peak_height && (i - last_peak) > min_peak_distance {
                steps += 1;
                last_peak = i;
            }
        }
...
impl IoTDataQualityAnalysis {
    ...
    pub fn new(data: Array1<f64>, sensor_type: SensorType, timestamps: Array1<i64>) -> Result<Self> {
    ...
    fn calculate_outlier_percentage(&self) -> Result<f64> {
        let mut sorted_data: Vec<f64> = self.data.iter()
            .filter(|&&x| !x.is_nan())
            .cloned()
            .collect();
        ...
        let outlier_count = self.data.iter()
    ...
    fn calculate_snr(&self) -> Result<f64> {
        let valid_data: Vec<f64> = self.data.iter()
            .filter(|&&x| !x.is_nan())
            .cloned()
            .collect();
    ...
    pub fn system_health_report(&self) -> Result<HashMap<String, String>> {
        ...
        if let Some(ref env) = self.environmental {
            ...
            if !recommendations.is_empty() {
                report.insert("Energy_Recommendations".to_string(), recommendations.join("; "));
            }
        }
        ...
        if let Some(ref motion) = self.motion {
            ...
            if motion.gps.is_some() {
                ...
                report.insert("Distance_Traveled".to_string(), format!("{:.2} meters", distance));
            }
        }
        ...
        for (_sensor, quality) in &self.quality_assessments {
            if quality.missing_percentage > 10.0 || quality.outlier_percentage > 5.0 || quality.snr < 10.0 {
                quality_issues += 1;
            }
        }
    ...
    pub fn predictive_maintenance(&self) -> Result<Vec<String>> {
        ...
        for (sensor_name, quality) in &self.quality_assessments {
            if quality.missing_percentage > 20.0 {
                maintenance_alerts.push(format!("Sensor '{}' may need replacement - high missing data rate", sensor_name));
            }
            
            if quality.outlier_percentage > 15.0 {
                maintenance_alerts.push(format!("Sensor '{}' may need calibration - high outlier rate", sensor_name));
            }
            
            if quality.snr < 5.0 {
                maintenance_alerts.push(format!("Sensor '{}' may have connectivity issues - low signal quality", sensor_name));
            }
        }
        ...
        if let Some(ref env) = self.environmental {
            ...
            for (sensor_type, issues) in malfunctions {
                if issues.len() > 100 {
                    maintenance_alerts.push(format!("{} sensor requires immediate attention - multiple malfunctions detected", sensor_type));
                }
            }
        }
...
mod tests {
    ...
    fn test_motion_sensor_analysis() {
        let timestamps = arr1(&[1, 2, 3, 4, 5]);
        let accel_data = arr2(&[[1.0, 2.0, 9.8], [1.1, 2.1, 9.9], [1.2, 2.2, 10.0], [1.3, 2.3, 10.1], [1.4, 2.4, 10.2]]);
    ...
    fn test_iot_data_quality() {
        ...
        let quality_analysis = IoTDataQualityAnalysis::new(data, SensorType::Temperature, timestamps).unwrap();
