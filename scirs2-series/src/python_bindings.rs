...
#[cfg(feature = "python")]
use pyo3::types::{PyList, PyDict, PyFloat, PyType, PyAny};
...
impl PyTimeSeries {
    ...
    #[new]
    fn new(values: PyReadonlyArray1<f64>, timestamps: Option<PyReadonlyArray1<f64>>) -> PyResult<Self> {
...
impl PyARIMA {
    ...
    fn forecast<'py>(&self, py: Python<'py>, steps: usize) -> PyResult<Bound<'py, PyArray1<f64>>> {
        match &self.model {
            Some(model) => {
                let forecasts = model.forecast(steps)
                    .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("{}", e)))?;
...
#[cfg(feature = "python")]
#[pyfunction]
fn apply_differencing<'py>(py: Python<'py>, data: &PyTimeSeries, periods: usize) -> PyResult<Bound<'py, PyArray1<f64>>> {
...
#[cfg(feature = "python")]
#[pyfunction]
fn apply_seasonal_differencing<'py>(py: Python<'py>, data: &PyTimeSeries, periods: usize) -> PyResult<Bound<'py, PyArray1<f64>>> {
...
#[cfg(feature = "python")]
pub fn create_pandas_dataframe(py: Python, data: HashMap<String, Array1<f64>>) -> PyResult<PyObject> {
...
#[cfg(feature = "python")]
pub fn create_pandas_series(py: Python, data: Array1<f64>, name: Option<&str>) -> PyResult<PyObject> {
    let pandas = py.import_bound("pandas")?;
