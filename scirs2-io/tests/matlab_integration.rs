...
use scirs2_io::matlab::{MatType, write_mat, read_mat};
...
fn test_matlab_write_read_roundtrip() {
    ...
    for (name, original) in &vars {
        let loaded = loaded_vars.get(name).expect(&format!("Variable {} not found", name));
        ...
        match (original, loaded) {
            (MatType::Double(orig), MatType::Double(load)) => {
                ...
                for (o, l) in orig.iter().zip(load.iter()) {
                    assert!((o - l).abs() < 1e-10, "Double values don't match: {} vs {}", o, l);
                }
            },
            (MatType::Single(orig), MatType::Single(load)) => {
                assert_eq!(orig.shape(), load.shape());
                for (o, l) in orig.iter().zip(load.iter()) {
                    assert!((o - l).abs() < 1e-6, "Single values don't match: {} vs {}", o, l);
                }
            },
            ...
            (MatType::Char(orig), MatType::Char(load)) => {
                assert_eq!(orig, load, "Character values don't match: {} vs {}", orig, load);
            },
