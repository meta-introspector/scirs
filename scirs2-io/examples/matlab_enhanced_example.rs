...
use scirs2_io::matlab::{MatType, read_mat, write_mat};
use scirs2_io::matlab::enhanced::{
    EnhancedMatFile, MatFileConfig, write_mat_enhanced, read_mat_enhanced,
    create_cell_array, create_struct
};
...
fn demonstrate_basic_mat_v5() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Character data
    vars.insert("text_data".to_string(), MatType::Char("Hello MATLAB!".to_string()));
    ...
    for (name, original) in &vars {
        if let Some(loaded) = loaded_vars.get(name) {
            ...
            println!("    {}: {}", name, if matches { "✅ OK" } else { "❌ MISMATCH" });
        } else {
...
fn demonstrate_enhanced_features() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let write_time = write_start.elapsed();
    println!("    Enhanced write time: {:.2}ms", write_time.as_secs_f64() * 1000.0);
    ...
    let read_time = read_start.elapsed();
    println!("    Enhanced read time: {:.2}ms", read_time.as_secs_f64() * 1000.0);
    println!("    Loaded {} variables", loaded_vars.len());
    ...
    println!("    Convenience write time: {:.2}ms", conv_write_time.as_secs_f64() * 1000.0);
    println!("    Convenience read time: {:.2}ms", conv_read_time.as_secs_f64() * 1000.0);
...
fn demonstrate_cell_arrays() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match &nested_cell_array {
        MatType::Cell(cells) => {
            println!("    Created nested cell array with {} top-level elements", cells.len());
        }
        _ => unreachable!(),
    }
...
fn demonstrate_structures() -> Result<(), Box<dyn std::error::Error>> {
    ...
    simple_struct.insert("name".to_string(), MatType::Char("Test Structure".to_string()));
    simple_struct.insert("data".to_string(), MatType::Double(Array1::from(vec![1.0, 2.0, 3.0]).into_dyn()));
    simple_struct.insert("flag".to_string(), MatType::Logical(Array1::from(vec![true]).into_dyn()));
    ...
    inner_struct.insert("x".to_string(), MatType::Double(Array1::from(vec![1.0, 2.0]).into_dyn()));
    inner_struct.insert("y".to_string(), MatType::Double(Array1::from(vec![3.0, 4.0]).into_dyn()));
    ...
    nested_struct.insert("timestamp".to_string(), MatType::Char("2024-01-01".to_string()));

    let nested_structure = create_struct(nested_struct);

    match &nested_structure {
        MatType::Struct(fields) => {
            println!("    Created nested structure with {} top-level fields:", fields.len());
...
fn demonstrate_performance_and_errors() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (rows, cols, description) in sizes {
        ...
        vars.insert("performance_matrix".to_string(), MatType::Double(large_array));
        ...
        println!("    {}: Write {:.2}ms, Read {:.2}ms, Size {:.1}KB", 
                description, 
    ...
    match read_mat(&missing_file) {
        ...
        Err(e) => println!("    Expected error reading missing file: {}", e.to_string().chars().take(50).collect::<String>()),
    }
    ...
    match write_mat(invalid_file, &empty_vars) {
        ...
        Err(e) => println!("    Expected error writing to invalid path: {}", e.to_string().chars().take(50).collect::<String>()),
    }
