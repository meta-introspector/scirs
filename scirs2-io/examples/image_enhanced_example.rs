...
use ndarray::Array3;
use scirs2_io::image::{
    ImageData, ImageFormat, ImageMetadata, ColorMode,
    load_image, save_image,
};
use scirs2_io::image::enhanced::{
    EnhancedImageProcessor, ImagePyramid, PyramidConfig, CompressionOptions, CompressionQuality,
    InterpolationMethod, create_image_pyramid, save_lossless, save_high_quality,
...
fn demonstrate_image_pyramids() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Pyramid creation time: {:.2}ms", pyramid_time.as_secs_f64() * 1000.0);
    ...
    for (target_width, target_height) in target_sizes {
        ...
        if let Some(level_image) = pyramid.get_level(best_level) {
            println!("    Target {}x{} -> Level {} ({}x{})", 
                target_width, target_height, best_level,
                level_image.metadata.width, level_image.metadata.height);
        }
    }
...
fn demonstrate_lossless_compression() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, quality) in quality_settings {
        ...
        // Test with JPEG
        let jpeg_path = temp_dir.path().join(format!("test_{}.jpg", name.to_lowercase()));
        let jpeg_start = Instant::now();
        processor.save_with_compression(&photo_image, &jpeg_path, ImageFormat::JPEG, Some(compression.clone()))?;
        let jpeg_time = jpeg_start.elapsed();
        let jpeg_size = std::fs::metadata(&jpeg_path)?.len();

        // Test with PNG
        let png_path = temp_dir.path().join(format!("test_{}.png", name.to_lowercase()));
        let png_start = Instant::now();
        processor.save_with_compression(&photo_image, &png_path, ImageFormat::PNG, Some(compression))?;
        let png_time = png_start.elapsed();
        let png_size = std::fs::metadata(&png_path)?.len();

        results.push((name, quality.value(), jpeg_size, jpeg_time, png_size, png_time));
        
        println!("    {}: Quality {} - JPEG: {:.1}KB ({:.1}ms), PNG: {:.1}KB ({:.1}ms)",
            name, quality.value(),
            jpeg_size as f64 / 1024.0, jpeg_time.as_secs_f64() * 1000.0,
            png_size as f64 / 1024.0, png_time.as_secs_f64() * 1000.0);
    }
    ...
    println!("    Lossless PNG: {:.1}KB ({:.1}ms)", 
        lossless_size as f64 / 1024.0, lossless_time.as_secs_f64() * 1000.0);
    println!("    High Quality JPEG: {:.1}KB ({:.1}ms)", 
        hq_size as f64 / 1024.0, hq_time.as_secs_f64() * 1000.0);

    // Analyze compression efficiency
    println!("  🔹 Compression efficiency analysis:");
    if let Some((_, _, best_jpeg_size, _, _, _)) = results.iter().min_by_key(|(_, _, jpeg_size, _, _, _)| *jpeg_size) {
        if let Some((_, _, worst_jpeg_size, _, _, _)) = results.iter().max_by_key(|(_, _, jpeg_size, _, _, _)| *jpeg_size) {
...
fn demonstrate_advanced_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Grayscale conversion time: {:.2}ms", gray_time.as_secs_f64() * 1000.0);
    ...
    println!("    Histogram equalization time: {:.2}ms", eq_time.as_secs_f64() * 1000.0);
    ...
    for radius in blur_radii {
        ...
        let blur_path = temp_dir.path().join(format!("processing_blur_{:.1}.png", radius));
        save_image(&blurred, &blur_path, Some(ImageFormat::PNG))?;
        println!("    Blur radius {:.1}: {:.2}ms", radius, blur_time.as_secs_f64() * 1000.0);
    }
    ...
    for (amount, radius) in sharpen_settings {
        ...
        let sharpen_path = temp_dir.path().join(format!("processing_sharpen_{:.1}_{:.1}.png", amount, radius));
        save_image(&sharpened, &sharpen_path, Some(ImageFormat::PNG))?;
        println!("    Sharpen amount {:.1}, radius {:.1}: {:.2}ms", 
            amount, radius, sharpen_time.as_secs_f64() * 1000.0);
    }
    ...
    for (name, method) in interpolation_methods {
        ...
        let resized = processor.resize_with_interpolation(&source_image, target_size.0, target_size.1, method)?;
        let interp_time = interp_start.elapsed();
        
        let interp_path = temp_dir.path().join(format!("processing_interp_{}.png", name.to_lowercase()));
        save_image(&resized, &interp_path, Some(ImageFormat::PNG))?;
        println!("    {} interpolation: {:.2}ms", name, interp_time.as_secs_f64() * 1000.0);
    }
...
fn demonstrate_batch_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    batch_convert_with_compression(&input_dir, &lossless_output, ImageFormat::PNG, lossless_compression)?;
    let batch_time = batch_start.elapsed();
    println!("    Lossless batch conversion time: {:.2}ms", batch_time.as_secs_f64() * 1000.0);
    ...
    let hq_time = hq_start.elapsed();
    println!("    High quality batch conversion time: {:.2}ms", hq_time.as_secs_f64() * 1000.0);
    ...
    for (name, _) in &test_images {
        ...
        println!("    {}: Original {:.1}KB, Lossless {:.1}KB, HQ JPEG {:.1}KB",
            name,
...
fn demonstrate_performance_features() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pyramid_configs = vec![
        ...
        ("More Levels", PyramidConfig {
            levels: 8,
            scale_factor: 0.6,
            min_size: 8,
            interpolation: InterpolationMethod::Linear,
        }),
        ("High Quality", PyramidConfig {
            levels: 6,
            scale_factor: 0.5,
            min_size: 16,
            interpolation: InterpolationMethod::Lanczos,
    ...
    for (name, config) in pyramid_configs {
        ...
        println!("    {} pyramid: {:.2}ms, {} levels",
            name, pyramid_time.as_secs_f64() * 1000.0, pyramid.num_levels());
    }
    ...
    let operations = vec![
        ("Grayscale", Box::new(|p: &EnhancedImageProcessor, img: &ImageData| p.to_grayscale(img)) as Box<dyn Fn(&EnhancedImageProcessor, &ImageData) -> Result<ImageData, crate::error::IoError>>),
        ("Histogram Eq", Box::new(|p: &EnhancedImageProcessor, img: &ImageData| p.histogram_equalization(img))),
        ("Blur (r=2.0)", Box::new(|p: &EnhancedImageProcessor, img: &ImageData| p.gaussian_blur(img, 2.0))),
        ("Sharpen (1.0)", Box::new(|p: &EnhancedImageProcessor, img: &ImageData| p.sharpen(img, 1.0, 1.5))),
    ];
    ...
    let compression_tests = vec![
        ("PNG Lossless", ImageFormat::PNG, CompressionQuality::Lossless),
    ...
    for (name, format, quality) in compression_tests {
        ...
        let comp_path = temp_dir.path().join(format!("perf_test_{}.{}", 
            name.to_lowercase().replace(" ", "_"), format.extension()));
        ...
        println!("    {}: {:.2}ms, {:.1}KB", 
            name, comp_time.as_secs_f64() * 1000.0, file_size as f64 / 1024.0);
    }
    ...
    println!("    Cache initialized: {} items, {}MB max", initial_count, max_size);
    ...
    for i in 0..3 {
        ...
        println!("    Cache test {}: {:.2}ms", i + 1, cache_time.as_secs_f64() * 1000.0);
    }
