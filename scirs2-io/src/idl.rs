...
impl IdlReader {
    ...
    pub fn new<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        reader.read_exact(&mut signature).map_err(|e| IoError::FileError(e.to_string()))?;
        
        if &signature != IDL_SIGNATURE {
            return Err(IoError::FormatError("Invalid IDL file signature".to_string()));
        }
        
        // Read format byte to determine endianness
        let format = reader.read_u8().map_err(|e| IoError::FileError(e.to_string()))?;
        ...
        reader.seek(SeekFrom::Start(4)).map_err(|e| IoError::FileError(e.to_string()))?;
    ...
    fn read_record(&mut self) -> Result<(RecordType, Option<String>, Option<IdlType>)> {
        ...
        let record_type = match rec_type {
            ...
            _ => return Err(IoError::FormatError(format!("Unknown record type: {}", rec_type))),
        };
        ...
        match record_type {
            ...
            _ => {
                ...
                if next_offset > 0 {
                    self.reader.seek(SeekFrom::Start(next_offset as u64))
    ...
    fn read_variable_data(&mut self) -> Result<IdlType> {
        ...
        match type_code {
            ...
            _ => Err(IoError::FormatError(format!("Unknown type code: {}", type_code))),
        }
    }
    ...
    fn read_byte_array(&mut self) -> Result<IdlType> {
        ...
        self.reader.read_exact(&mut data).map_err(|e| IoError::FileError(e.to_string()))?;
    ...
    fn read_string_data(&mut self) -> Result<IdlType> {
        ...
        self.reader.read_exact(&mut buffer).map_err(|e| IoError::FileError(e.to_string()))?;
...
impl IdlWriter {
    ...
    pub fn new<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        // Write signature
        writer.write_all(IDL_SIGNATURE).map_err(|e| IoError::FileError(e.to_string()))?;
        
        // Write format byte (little-endian)
        writer.write_u8(0x00).map_err(|e| IoError::FileError(e.to_string()))?;
    ...
    pub fn write_all(&mut self, variables: &HashMap<String, IdlType>) -> Result<()> {
        ...
        self.writer.flush().map_err(|e| IoError::FileError(e.to_string()))?;
        Ok(())
    }
    ...
    fn write_variable_data(&mut self, data: &IdlType) -> Result<()> {
        match data {
            ...
            _ => Err(IoError::Other("Unsupported IDL type for writing".to_string())),
        }
    }
    ...
    fn write_byte_array(&mut self, array: &ArrayD<u8>) -> Result<()> {
        ...
        for &val in array.iter() {
            self.writer.write_u8(val).map_err(|e| IoError::FileError(e.to_string()))?;
        }
    ...
    fn write_string(&mut self, string: &str) -> Result<()> {
        ...
        self.writer.write_all(bytes).map_err(|e| IoError::FileError(e.to_string()))?;
        
        // Pad to 4-byte boundary
        let padding = (4 - (bytes.len() % 4)) % 4;
        for _ in 0..padding {
            self.writer.write_u8(0).map_err(|e| IoError::FileError(e.to_string()))?;
        }
...
mod tests {
    ...
    fn test_idl_type_creation() {
        ...
        match byte_array {
            IdlType::Byte(array) => assert_eq!(array.len(), 4),
            _ => panic!("Wrong type"),
        }
        
        let double_array = IdlType::Double(arr1(&[1.0, 2.0, 3.0]).into_dyn());
        match double_array {
            IdlType::Double(array) => assert_eq!(array.len(), 3),
            _ => panic!("Wrong type"),
        }
    }
    ...
    fn test_structure_creation() {
        ...
        fields.insert("x".to_string(), IdlType::Double(arr1(&[1.0, 2.0]).into_dyn()));
        fields.insert("y".to_string(), IdlType::Float(arr1(&[3.0, 4.0]).into_dyn()));
