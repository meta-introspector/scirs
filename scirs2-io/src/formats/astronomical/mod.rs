...
use ndarray::{Array, ArrayD, Array1, Array2, Array3, IxDyn};
    ...
    pub fn get_bool(&self, keyword: &str) -> Result<bool> {
        match self.get_card(keyword) {
            Some(card) => match &card.value {
                ...
                _ => Err(IoError::ParseError(format!("Keyword {} is not a boolean", keyword))),
            },
            None => Err(IoError::ParseError(format!("Keyword {} not found", keyword))),
        }
    }
    ...
    pub fn get_i64(&self, keyword: &str) -> Result<i64> {
        match self.get_card(keyword) {
            Some(card) => match &card.value {
                ...
                _ => Err(IoError::ParseError(format!("Keyword {} is not an integer", keyword))),
            },
            None => Err(IoError::ParseError(format!("Keyword {} not found", keyword))),
        }
    }
    ...
    pub fn get_f64(&self, keyword: &str) -> Result<f64> {
        match self.get_card(keyword) {
            Some(card) => match &card.value {
                ...
                _ => Err(IoError::ParseError(format!("Keyword {} is not a number", keyword))),
            },
            None => Err(IoError::ParseError(format!("Keyword {} not found", keyword))),
        }
    }
    ...
    pub fn get_string(&self, keyword: &str) -> Result<String> {
        match self.get_card(keyword) {
            Some(card) => match &card.value {
                ...
                _ => Err(IoError::ParseError(format!("Keyword {} is not a string", keyword))),
            },
            None => Err(IoError::ParseError(format!("Keyword {} not found", keyword))),
        }
    }
...
impl FitsDataType {
    ...
    pub fn from_bitpix(bitpix: i32) -> Result<Self> {
        match bitpix {
            ...
            _ => Err(IoError::ParseError(format!("Invalid BITPIX value: {}", bitpix))),
        }
    }
}
...
impl FitsFile {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let mut file = File::open(path.as_ref())
            .map_err(|e| IoError::FileNotFound(file_path.clone(), e))?;
        ...
        loop {
            ...
            // Check for END
            if hdus.last().unwrap().header.cards.iter().any(|c| c.keyword == "END") {
    ...
    fn read_header<R: Read>(reader: &mut R) -> Result<FitsHeader> {
        ...
        loop {
            reader.read_exact(&mut card_buf)
    ...
    pub fn read_hdu_image<T: FitsNumeric>(&self, hdu_index: usize) -> Result<Array2<T>> {
        ...
        if hdu.hdu_type != HDUType::Primary && hdu.hdu_type != HDUType::Image {
            return Err(IoError::ParseError(format!("HDU {} is not an image", hdu_index)));
        }
        ...
        if naxis != 2 {
            return Err(IoError::ParseError(format!("Expected 2D image, got {}D", naxis)));
        }
        ...
        let mut file = File::open(&self.file_path)
            .map_err(|e| IoError::FileNotFound(self.file_path.clone(), e))?;
...
impl FitsNumeric for f32 {
    fn read_fits<R: Read>(reader: &mut R, data_type: FitsDataType) -> Result<Self> {
        match data_type {
            ...
            _ => Err(IoError::ParseError(format!("Unsupported conversion from {:?} to f32", data_type))),
        }
    }
        ...
        match data_type {
            FitsDataType::Float32 => {
                writer.write_f32::<BigEndian>(*self)
                    .map_err(|e| IoError::WriteError(format!("Failed to write f32: {}", e)))
            }
            _ => Err(IoError::WriteError(format!("Unsupported conversion from f32 to {:?}", data_type))),
        }
    }
}
...
impl FitsNumeric for f64 {
    fn read_fits<R: Read>(reader: &mut R, data_type: FitsDataType) -> Result<Self> {
        match data_type {
            ...
            _ => Err(IoError::ParseError(format!("Unsupported conversion from {:?} to f64", data_type))),
        }
    }
        ...
        match data_type {
            FitsDataType::Float64 => {
                writer.write_f64::<BigEndian>(*self)
                    .map_err(|e| IoError::WriteError(format!("Failed to write f64: {}", e)))
            }
            _ => Err(IoError::WriteError(format!("Unsupported conversion from f64 to {:?}", data_type))),
        }
    }
}
...
impl FitsWriter {
    ...
    pub fn write_image_2d<T: FitsNumeric>(&mut self, data: &Array2<T>, data_type: FitsDataType) -> Result<()> {
        let mut header = FitsHeader::new();
        ...
        if padding > 0 {
            ...
            self.writer.write_all(&pad_bytes)
                .map_err(|e| IoError::WriteError(format!("Failed to write padding: {}", e)))?;
        }
    ...
    fn write_header(&mut self, header: &FitsHeader) -> Result<()> {
        ...
        if remaining < cards_per_block {
            ...
            for _ in 0..remaining {
                self.writer.write_all(&blank_card)
                    .map_err(|e| IoError::WriteError(format!("Failed to write blank card: {}", e)))?;
            }
        }
    ...
    fn write_header_card(&mut self, card: &HeaderCard) -> Result<()> {
        ...
        self.writer.write_all(card_str.as_bytes())
            .map_err(|e| IoError::WriteError(format!("Failed to write header card: {}", e)))
    }
...
impl VOTable {
    ...
    pub fn get_column_data(&self, column_index: usize) -> Result<Vec<&VOTableValue>> {
        if column_index >= self.columns.len() {
            return Err(IoError::ParseError(format!("Column index {} out of range", column_index)));
        }
    ...
    pub fn write<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        ...
        writeln!(writer, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>")
            .map_err(|e| IoError::WriteError(format!("Failed to write XML header: {}", e)))?;
        writeln!(writer, "<VOTABLE version=\"1.4\" xmlns=\"http://www.ivoa.net/xml/VOTable/v1.3\">")
            .map_err(|e| IoError::WriteError(format!("Failed to write VOTABLE tag: {}", e)))?;
        
        // Write resource
        writeln!(writer, "  <RESOURCE>")
            .map_err(|e| IoError::WriteError(format!("Failed to write RESOURCE tag: {}", e)))?;
        writeln!(writer, "    <TABLE>")
            .map_err(|e| IoError::WriteError(format!("Failed to write TABLE tag: {}", e)))?;
        
        // Write fields
        for column in &self.columns {
            write!(writer, "      <FIELD name=\"{}\" datatype=\"{}\"", column.name, column.datatype)
                .map_err(|e| IoError::WriteError(format!("Failed to write FIELD: {}", e)))?;
            
            if let Some(unit) = &column.unit {
                write!(writer, " unit=\"{}\"", unit)
                    .map_err(|e| IoError::WriteError(format!("Failed to write unit: {}", e)))?;
            }
            
            writeln!(writer, "/>")
                .map_err(|e| IoError::WriteError(format!("Failed to write FIELD close: {}", e)))?;
        }
        
        // Write data
        writeln!(writer, "      <DATA>")
            .map_err(|e| IoError::WriteError(format!("Failed to write DATA tag: {}", e)))?;
        writeln!(writer, "        <TABLEDATA>")
            .map_err(|e| IoError::WriteError(format!("Failed to write TABLEDATA tag: {}", e)))?;
        
        for row in &self.data {
            write!(writer, "          <TR>")
                .map_err(|e| IoError::WriteError(format!("Failed to write TR: {}", e)))?;
            
            for value in row {
                match value {
                    VOTableValue::String(s) => write!(writer, "<TD>{}</TD>", s),
                    VOTableValue::Integer(i) => write!(writer, "<TD>{}</TD>", i),
                    VOTableValue::Float(f) | VOTableValue::Double(f) => write!(writer, "<TD>{}</TD>", f),
                    VOTableValue::Boolean(b) => write!(writer, "<TD>{}</TD>", if *b { "true" } else { "false" }),
                    VOTableValue::Null => write!(writer, "<TD/>"),
                    VOTableValue::Array(_) => write!(writer, "<TD>[]</TD>"), // Simplified
                }.map_err(|e| IoError::WriteError(format!("Failed to write TD: {}", e)))?;
            }
            
            writeln!(writer, "</TR>")
                .map_err(|e| IoError::WriteError(format!("Failed to write TR close: {}", e)))?;
        }
        
        writeln!(writer, "        </TABLEDATA>")
            .map_err(|e| IoError::WriteError(format!("Failed to close TABLEDATA: {}", e)))?;
        writeln!(writer, "      </DATA>")
            .map_err(|e| IoError::WriteError(format!("Failed to close DATA: {}", e)))?;
        writeln!(writer, "    </TABLE>")
            .map_err(|e| IoError::WriteError(format!("Failed to close TABLE: {}", e)))?;
        writeln!(writer, "  </RESOURCE>")
            .map_err(|e| IoError::WriteError(format!("Failed to close RESOURCE: {}", e)))?;
        writeln!(writer, "</VOTABLE>")
            .map_err(|e| IoError::WriteError(format!("Failed to close VOTABLE: {}", e)))?;
