...
impl PipelineStage for FileReadStage {
    fn execute(&self, mut input: PipelineData<Box<dyn Any + Send + Sync>>) -> Result<PipelineData<Box<dyn Any + Send + Sync>>> {
        let data = match self.format {
            FileFormat::Csv => {
                let data = read_csv(&self.path, None, None)?;
        ...
        input.data = data;
        input.metadata.set("source_file", self.path.to_string_lossy().to_string());
        Ok(input)
    }
...
impl<T: 'static + Send + Sync> PipelineStage for AggregationStage<T> {
    fn execute(&self, mut input: PipelineData<Box<dyn Any + Send + Sync>>) -> Result<PipelineData<Box<dyn Any + Send + Sync>>> {
        if let Ok(data) = input.data.downcast::<Vec<T>>() {
            ...
            Err(IoError::Other("Type mismatch in aggregation stage".to_string()))
        }
    }
...
pub trait ErrorHandler: Send + Sync {
    fn handle_error(&self, error: IoError, data: PipelineData<Box<dyn Any + Send + Sync>>) -> Result<PipelineData<Box<dyn Any + Send + Sync>>>;
}
...
mod tests {
    ...
    fn test_validation_stage() {
        let stage = ValidationStage::new()
            .add_validator(Box::new(SimpleValidator));
        
        let data = PipelineData::new(Box::new(vec![1, 2, 3]) as Box<dyn Any + Send + Sync>);
        let result = stage.execute(data);
        assert!(result.is_ok());
        
        let empty_data = PipelineData::new(Box::new(vec![] as Vec<i32>) as Box<dyn Any + Send + Sync>);
