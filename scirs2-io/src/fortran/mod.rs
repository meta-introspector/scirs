...
use ndarray::{Array, ArrayD, Array1, Array2, Array3, IxDyn, ShapeBuilder};
use std::fs::File;
    ...
    fn read_record(&mut self) -> Result<Vec<u8>> {
        match self.config.access_mode {
            ...
            AccessMode::Direct { record_length } => {
                ...
                self.reader.read_exact(&mut data)
                    .map_err(|e| IoError::ParseError(format!("Failed to read direct access record: {}", e)))?;
                Ok(data)
            }
            AccessMode::Stream => {
                // Stream access requires explicit size specification
                Err(IoError::ParseError("Stream access requires explicit read size".to_string()))
            }
        }
    }
    ...
    pub fn read_array_1d<T: ScientificNumber>(&mut self, n: usize) -> Result<Array1<T>> {
        ...
        for i in 0..n {
            ...
            let value = match self.config.endian {
                ...
                _ => {
                    T::from_be_bytes(&data[start..end])
                }
            };
            values.push(value);
        }
    ...
    pub fn skip_record(&mut self) -> Result<()> {
        match self.config.access_mode {
            AccessMode::Sequential => {
                ...
                let skip_size = size + match self.config.marker_size {
                    RecordMarkerSize::FourByte => 4,
                    RecordMarkerSize::EightByte => 8,
                };
                
                self.reader.seek(SeekFrom::Current(skip_size as i64))
                    .map_err(|e| IoError::ParseError(format!("Failed to skip record: {}", e)))?;
                
                Ok(())
            }
            AccessMode::Direct { record_length } => {
                self.reader.seek(SeekFrom::Current(record_length as i64))
                    .map_err(|e| IoError::ParseError(format!("Failed to skip record: {}", e)))?;
                Ok(())
            }
            AccessMode::Stream => {
                Err(IoError::ParseError("Cannot skip record in stream access mode".to_string()))
            }
        }
    }
}
    ...
    fn write_record_marker(&mut self, size: usize) -> Result<()> {
        match self.config.marker_size {
            RecordMarkerSize::FourByte => {
                ...
                match self.config.endian {
                    EndianMode::Little | EndianMode::Native if cfg!(target_endian = "little") => {
                        self.reader.write_u32::<LittleEndian>(marker)
                            .map_err(|e| IoError::WriteError(format!("Failed to write record marker: {}", e)))
                    }
                    _ => {
                        self.reader.write_u32::<BigEndian>(marker)
                            .map_err(|e| IoError::WriteError(format!("Failed to write record marker: {}", e)))
                    }
                }
            }
            RecordMarkerSize::EightByte => {
                ...
                match self.config.endian {
                    EndianMode::Little | EndianMode::Native if cfg!(target_endian = "little") => {
                        self.reader.write_u64::<LittleEndian>(marker)
                            .map_err(|e| IoError::WriteError(format!("Failed to write record marker: {}", e)))
                    }
                    _ => {
                        self.reader.write_u64::<BigEndian>(marker)
                            .map_err(|e| IoError::WriteError(format!("Failed to write record marker: {}", e)))
                    }
                }
            }
        ...
        match self.config.access_mode {
            AccessMode::Sequential => {
                ...
                self.reader.write_all(data)
                    .map_err(|e| IoError::WriteError(format!("Failed to write record data: {}", e)))?;
            ...
            AccessMode::Stream => {
                ...
                self.reader.write_all(data)
                    .map_err(|e| IoError::WriteError(format!("Failed to write stream data: {}", e)))
            }
        }
    }
...
pub fn detect_fortran_format<P: AsRef<Path>>(path: P) -> Result<(EndianMode, RecordMarkerSize)> {
    ...
    Err(IoError::ParseError("Unable to detect Fortran file format".to_string()))
}
