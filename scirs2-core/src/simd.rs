...
pub fn simd_add_aligned_f32(a: &ArrayView1<f32>, b: &ArrayView1<f32>) -> Array1<f32> {
    ...
    assert_eq!(a.len(), b.len(), "Arrays must have the same length for SIMD operations");
    ...
    while i + 16 <= n {
        ...
        let a_chunk1 = [
            a_slice[i], a_slice[i+1], a_slice[i+2], a_slice[i+3],
            a_slice[i+4], a_slice[i+5], a_slice[i+6], a_slice[i+7],
        ];
        let b_chunk1 = [
            b_slice[i], b_slice[i+1], b_slice[i+2], b_slice[i+3],
            b_slice[i+4], b_slice[i+5], b_slice[i+6], b_slice[i+7],
        ];
        
        let a_chunk2 = [
            a_slice[i+8], a_slice[i+9], a_slice[i+10], a_slice[i+11],
            a_slice[i+12], a_slice[i+13], a_slice[i+14], a_slice[i+15],
        ];
        let b_chunk2 = [
            b_slice[i+8], b_slice[i+9], b_slice[i+10], b_slice[i+11],
            b_slice[i+12], b_slice[i+13], b_slice[i+14], b_slice[i+15],
        ];
...
pub fn simd_sum_f32_enhanced(input: &ArrayView1<f32>) -> f32 {
    ...
    while i + 32 <= n {
        let chunk1 = [
            input_slice[i], input_slice[i+1], input_slice[i+2], input_slice[i+3],
            input_slice[i+4], input_slice[i+5], input_slice[i+6], input_slice[i+7],
        ];
        let chunk2 = [
            input_slice[i+8], input_slice[i+9], input_slice[i+10], input_slice[i+11],
            input_slice[i+12], input_slice[i+13], input_slice[i+14], input_slice[i+15],
        ];
        let chunk3 = [
            input_slice[i+16], input_slice[i+17], input_slice[i+18], input_slice[i+19],
            input_slice[i+20], input_slice[i+21], input_slice[i+22], input_slice[i+23],
        ];
        let chunk4 = [
            input_slice[i+24], input_slice[i+25], input_slice[i+26], input_slice[i+27],
            input_slice[i+28], input_slice[i+29], input_slice[i+30], input_slice[i+31],
        ];
        
        sum1 = sum1 + f32x8::new(chunk1);
        sum2 = sum2 + f32x8::new(chunk2);
        sum3 = sum3 + f32x8::new(chunk3);
        sum4 = sum4 + f32x8::new(chunk4);
...
#[cfg(feature = "simd")]
pub fn simd_fused_multiply_add_f32(a: &ArrayView1<f32>, b: &ArrayView1<f32>, c: &ArrayView1<f32>) -> Array1<f32> {
    let n = a.len();
    let mut result = Array1::zeros(n);
    ...
    while i + 8 <= n {
        ...
        let c_chunk = [
            c_slice[i], c_slice[i+1], c_slice[i+2], c_slice[i+3],
            c_slice[i+4], c_slice[i+5], c_slice[i+6], c_slice[i+7],
        ];
