...
pub enum CrossDeviceError {
    ...
    TransferFailed { from: String, to: String, reason: String },
...
impl From<CrossDeviceError> for ScirsCoreError {
...
pub trait Device: Send + Sync {
    ...
    fn allocate(&self, size: usize) -> ScirsCoreResult<usize>;
    
    /// Deallocate memory on this device
    fn deallocate(&self, address: usize) -> ScirsCoreResult<()>;
    
    /// Copy data to this device from CPU
    fn copy_from_host(&self, src: *const u8, dst: usize, size: usize) -> ScirsCoreResult<()>;
    
    /// Copy data from this device to CPU
    fn copy_to_host(&self, src: usize, dst: *mut u8, size: usize) -> ScirsCoreResult<()>;
    
    /// Copy data between devices (if supported)
    fn copy_peer(&self, src: usize, dst_device: &dyn Device, dst: usize, size: usize) -> ScirsCoreResult<()>;
    
    /// Synchronize device operations
    fn synchronize(&self) -> ScirsCoreResult<()>;
    
    /// Get available memory in bytes
    fn available_memory(&self) -> ScirsCoreResult<usize>;
    
    /// Get total memory in bytes
    fn total_memory(&self) -> ScirsCoreResult<usize>;
}
...
impl Device for CpuDevice {
    ...
    fn allocate(&self, size: usize) -> ScirsCoreResult<usize> {
        let layout = std::alloc::Layout::from_size_align(size, 64)
            .map_err(|e| CrossDeviceError::AllocationFailed {
    ...
    fn copy_peer(&self, _src: usize, _dst_device: &dyn Device, _dst: usize, _size: usize) -> ScirsCoreResult<()> {
...
/// GPU device wrapper
pub struct GpuDeviceWrapper {
    inner: Arc<GpuDevice>,
    device_type: DeviceType,
}

impl GpuDeviceWrapper {
    /// Create a new GPU device wrapper
    pub fn new(gpu_device: Arc<GpuDevice>, device_type: DeviceType) -> Self {
        Self {
            inner: gpu_device,
            device_type,
        }
...
impl Device for GpuDeviceWrapper {
    fn device_type(&self) -> DeviceType {
        self.device_type.clone()
    }
    ...
    fn deallocate(&self, _address: usize) -> ScirsCoreResult<()> {
        // GPU buffers are automatically freed when dropped
        Ok(())
    }
    
    fn copy_from_host(&self, _src: *const u8, _dst: usize, _size: usize) -> ScirsCoreResult<()> {
        // Would use GPU-specific memory copy operations
        Ok(())
    }
    
    fn copy_to_host(&self, _src: usize, _dst: *mut u8, _size: usize) -> ScirsCoreResult<()> {
        // Would use GPU-specific memory copy operations
        Ok(())
    }
    ...
    fn available_memory(&self) -> ScirsCoreResult<usize> {
        self.inner.get_available_memory()
            .ok_or_else(|| CrossDeviceError::DeviceNotFound("GPU memory info unavailable".to_string()).into())
    }
...
impl CrossDeviceMemoryManager {
    ...
    pub fn register_device(&self, device: Arc<dyn Device>) -> ScirsCoreResult<()> {
    ...
    pub fn set_default_device(&self, device_type: DeviceType) -> ScirsCoreResult<()> {
    ...
    pub fn allocate<T: 'static>(
        ...
        device_type: &DeviceType,
        count: usize,
    ) -> ScirsCoreResult<CrossDeviceBuffer<T>> {
        let devices = self.devices.read().unwrap();
        let device = devices.get(device_type)
    ...
    pub fn allocate_default<T: 'static>(&self, count: usize) -> ScirsCoreResult<CrossDeviceBuffer<T>> {
    ...
    pub fn transfer<T: 'static + Copy>(
        ...
        let devices = self.devices.read().unwrap();
        let src_device = devices.get(&src_buffer.device_type)
            .ok_or_else(|| CrossDeviceError::DeviceNotFound(format!("{:?}", src_buffer.device_type)))?;
        let dst_device_obj = devices.get(dst_device)
    ...
    pub fn synchronize_all(&self) -> ScirsCoreResult<()> {
        let devices = self.devices.read().unwrap();
    ...
    pub fn get_memory_statistics(&self) -> MemoryStatistics {
        ...
        for allocation in allocations.values() {
            let device_stats = stats_by_device.entry(allocation.device.clone())
...
impl<T> CrossDeviceBuffer<T> {
    ...
    pub fn transfer_to(&self, device_type: &DeviceType) -> ScirsCoreResult<CrossDeviceBuffer<T>>
    where
        T: Copy + 'static,
    {
    ...
    pub fn copy_from_host(&self, data: &[T]) -> ScirsCoreResult<()>
    where
        T: Copy,
    {
        if data.len() != self.count {
            return Err(CrossDeviceError::InvalidDeviceType(
                format!("Data length {} doesn't match buffer capacity {}", data.len(), self.count)
            ).into());
        }
        
        let devices = self.manager.devices.read().unwrap();
        let device = devices.get(&self.device_type)
    ...
    pub fn copy_to_host(&self) -> ScirsCoreResult<Vec<T>>
    where
        T: Copy + Default,
    {
...
static GLOBAL_MANAGER: std::sync::OnceLock<Arc<CrossDeviceMemoryManager>> = std::sync::OnceLock::new();
...
pub fn initialize_with_gpu_devices(gpu_devices: Vec<Arc<GpuDevice>>) -> ScirsCoreResult<()> {
    let manager = global_manager();
    
    for (i, gpu_device) in gpu_devices.into_iter().enumerate() {
        let device_type = DeviceType::CudaGpu(i as u32); // Assume CUDA for now
        let wrapper = Arc::new(GpuDeviceWrapper::new(gpu_device, device_type));
...
pub mod utils {
    ...
    pub fn allocate_optimal<T: 'static>(count: usize) -> ScirsCoreResult<CrossDeviceBuffer<T>> {
        let manager = global_manager();
        let stats = manager.get_memory_statistics();
        
        // Find device with most available memory
        let best_device = stats.device_stats.iter()
