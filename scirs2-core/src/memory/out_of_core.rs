...
impl From<OutOfCoreError> for ScirsCoreError {
...
impl ChunkId {
    ...
    pub fn new(array_id: String, coordinates: Vec<usize>) -> Self {
        Self { array_id, coordinates }
    }
    
    /// Convert to string representation
    pub fn to_string(&self) -> String {
        format!("{}:{}", self.array_id, 
    ...
    T: Clone + Default + 'static,
{
    ...
    pub fn put(&self, chunk_id: ChunkId, chunk: Array<T, IxDyn>, metadata: ChunkMetadata) -> ScirsCoreResult<()> {
    ...
    fn ensure_cache_space(&self, new_metadata: &ChunkMetadata) -> ScirsCoreResult<()> {
        ...
        let needs_eviction = current_memory + new_metadata.size_bytes > self.config.max_cache_memory
    ...
    fn evict_chunks(&self, count: usize) -> ScirsCoreResult<()> {
    ...
    fn select_eviction_candidates(&self, count: usize) -> ScirsCoreResult<Vec<ChunkId>> {
    ...
    pub fn get_statistics(&self) -> CacheStatistics {
        ...
        let dirty_count = metadata_map.values()
            .filter(|m| m.is_dirty)
            .count();
...
pub trait StorageBackend: Send + Sync {
    ...
    fn read_chunk(&self, metadata: &ChunkMetadata) -> ScirsCoreResult<Vec<u8>>;
    
    /// Write chunk data to storage
    fn write_chunk(&self, metadata: &ChunkMetadata, data: &[u8]) -> ScirsCoreResult<()>;
    
    /// Allocate space for a new chunk
    fn allocate_chunk(&self, chunk_id: &ChunkId, size: usize) -> ScirsCoreResult<ChunkMetadata>;
    
    /// Deallocate chunk space
    fn deallocate_chunk(&self, chunk_id: &ChunkId) -> ScirsCoreResult<()>;
    
    /// Flush any pending writes
    fn flush(&self) -> ScirsCoreResult<()>;
}
...
impl FileStorageBackend {
    ...
    pub fn new<P: AsRef<Path>>(base_path: P) -> ScirsCoreResult<Self> {
    ...
    fn get_file_handle(&self, array_id: &str) -> ScirsCoreResult<Arc<Mutex<File>>> {
    ...
    T: Clone + Default + 'static,
{
    ...
    fn calculate_chunk_coords(&self, indices: &[usize]) -> ScirsCoreResult<Vec<usize>> {
        if indices.len() != self.shape.len() {
            return Err(OutOfCoreError::InvalidChunkSize(
                format!("Index dimensions {} don't match array dimensions {}", 
                    indices.len(), self.shape.len())
            ).into());
        }
        
        let chunk_coords: Vec<usize> = indices.iter()
    ...
    fn get_chunk(&self, chunk_coords: &[usize]) -> ScirsCoreResult<Array<T, IxDyn>> {
    ...
    fn load_chunk_from_storage(&self, chunk_id: &ChunkId) -> ScirsCoreResult<Array<T, IxDyn>> {
    ...
    /// Create a new chunk
    fn create_new_chunk(&self, chunk_coords: &[usize]) -> ScirsCoreResult<Array<T, IxDyn>> {
    ...
    fn deserialize_chunk_data(&self, _data: &[u8], shape: &[usize]) -> ScirsCoreResult<Array<T, IxDyn>> {
    ...
    pub fn view_region(&self, ranges: &[(usize, usize)]) -> ScirsCoreResult<RegionView<T>> {
        RegionView::new(self, ranges)
    }
    
    /// Process the array in chunks with a user-provided function
    pub fn process_chunks<F>(&self, mut processor: F) -> ScirsCoreResult<()>
    where
        F: FnMut(&Array<T, IxDyn>, &[usize]) -> ScirsCoreResult<()>,
    {
        let total_chunks: Vec<usize> = self.shape.iter()
            .zip(self.config.chunk_shape.iter())
            .map(|(&total, &chunk_size)| (total + chunk_size - 1) / chunk_size)
            .collect();
        
        // Iterate through all possible chunk coordinates
        self.iterate_chunk_coords(&total_chunks, &mut processor, &mut vec![0; total_chunks.len()], 0)
    }
    ...
    pub fn flush(&self) -> ScirsCoreResult<()> {
    ...
    T: Clone + Default + 'static,
{
    ...
    fn new(array: &'a OutOfCoreArray<T>, ranges: &[(usize, usize)]) -> ScirsCoreResult<Self> {
        if ranges.len() != array.shape.len() {
            return Err(OutOfCoreError::InvalidChunkSize(
                format!("Range dimensions {} don't match array dimensions {}", 
                    ranges.len(), array.shape.len())
            ).into());
        }
    ...
    pub fn process_intersecting_chunks<F>(&self, mut processor: F) -> ScirsCoreResult<()>
    where
        F: FnMut(&Array<T, IxDyn>, &[usize], &[(usize, usize)]) -> ScirsCoreResult<()>,
    {
        // Calculate which chunks intersect with this region
        let chunk_ranges: Vec<(usize, usize)> = self.ranges.iter()
        ...
        self.iterate_region_chunks(&chunk_ranges, &mut processor, &mut vec![0; chunk_ranges.len()], 0)
    }
...
impl OutOfCoreManager {
        ...
        T: Clone + Default + 'static,
    {
        ...
        let array = Arc::new(OutOfCoreArray::new(array_id.clone(), shape, storage, config));
...
pub fn global_manager() -> Arc<OutOfCoreManager> {
    GLOBAL_MANAGER.get_or_init(|| {
        Arc::new(OutOfCoreManager::default())
    }).clone()
}
...
pub mod utils {
        ...
        T: Clone + Default + 'static,
    {
        ...
        let manager = global_manager();
        let out_of_core_array = manager.create_array(
            array_id,
            array.shape().to_vec(),
            None,
            Some(config),
...
mod tests {
    ...
    #[test]
    fn test_file_storage_backend() -> ScirsCoreResult<()> {
    ...
    #[test]
    fn test_out_of_core_manager() -> ScirsCoreResult<()> {
        ...
        let array: Arc<OutOfCoreArray<f64>> = manager.create_array(
            array_id.clone(),
            shape.clone(),
            None,
            None,
