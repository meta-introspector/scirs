...
impl AdaptiveController {
    ...
    pub fn next_interval(&self, current_progress: f64) -> Duration {
        ...
        adjusted_interval.max(self.min_interval).min(self.max_interval)
    }
    ...
    fn calculate_stability_factor(&self) -> f64 {
        ...
        let mean: f64 = self.processing_speeds.iter().sum::<f64>() / self.processing_speeds.len() as f64;
        
        if mean <= 0.0 {
            return 1.0;
        }
        
        let variance: f64 = self.processing_speeds
            .iter()
            .map(|&speed| (speed - mean).powi(2))
            .sum::<f64>() / self.processing_speeds.len() as f64;
    ...
    pub fn speed_trend(&self) -> SpeedTrend {
        ...
        let early_speeds: f64 = self.processing_speeds
            .iter()
            .take(recent_half)
            .sum::<f64>() / recent_half as f64;
        
        let late_speeds: f64 = self.processing_speeds
            .iter()
            .skip(recent_half)
            .sum::<f64>() / (self.processing_speeds.len() - recent_half) as f64;
...
impl PredictiveETA {
    ...
    pub fn calculate_eta(&self, current_processed: u64, total: u64) -> Duration {
        ...
        let combined_seconds = 
            linear_eta.as_secs_f64() * weights.0 +
    ...
    fn linear_regression_eta(&self, remaining: u64) -> Duration {
        ...
        let (sum_x, sum_y, sum_xy, sum_x2) = self.progress_history
            .iter()
            .fold((0.0, 0.0, 0.0, 0.0), |(sx, sy, sxy, sx2), &(time, progress)| {
    ...
    fn moving_average_eta(&self, remaining: u64) -> Duration {
        ...
        let recent_speeds: Vec<_> = self.speed_history
            .iter()
            .rev()
            .take(recent_count)
            .collect();
        
        let avg_speed: f64 = recent_speeds.iter().map(|&&s| s).sum::<f64>() / recent_speeds.len() as f64;
