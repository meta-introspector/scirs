...
use super::statistics::{ProgressStats, format_duration, format_rate};
...
impl ProgressTemplate {
    ...
    pub fn render(&self, description: &str, stats: &ProgressStats, bar: Option<&str>) -> String {
        ...
        if let Some(captures) = extract_format_spec(&result, "percentage") {
            let formatted = format!("{:width$.precision$}", 
                stats.percentage,
    ...
    if let Some(start) = text.find(&pattern) {
        if let Some(end) = text[start..].find('}') {
            ...
            if let Some(colon_pos) = spec_str.find(':') {
                ...
                // Parse alignment
                if format_part.starts_with('<') || format_part.starts_with('>') || format_part.starts_with('^') {
...
mod tests {
    ...
    fn test_progress_template_render() {
        let template = ProgressTemplate::new("{description}: {percentage:.1}% ({processed}/{total})");
