...
use crate::gpu::{GpuDevice, GpuBuffer, GpuKernel};
...
impl From<GpuRngError> for ScirsCoreError {
...
impl GpuRandomGenerator {
    ...
    pub fn new(device: Arc<GpuDevice>, generator_type: GpuGeneratorType) -> ScirsCoreResult<Self> {
    ...
    fn generate_seeds(count: usize) -> ScirsCoreResult<Vec<u64>> {
        ...
        let base_seed = SystemTime::now()
            ...
            .map_err(|e| ScirsCoreError::ComputationError(e.to_string()))?
            .as_nanos() as u64;
    ...
    fn compile_kernels(&mut self, generator_type: &GpuGeneratorType) -> ScirsCoreResult<()> {
        match generator_type {
            ...
            _ => {
                return Err(ScirsCoreError::ComputationError(
                    format!("Generator type {:?} not yet implemented", generator_type)
                ));
            }
        }
    ...
    fn compile_xorshift_kernels(&mut self) -> ScirsCoreResult<()> {
        ...
        self.kernels.insert("normal".to_string(), Arc::new(normal_kernel));
        
        Ok(())
    }
    
    /// Compile Linear Congruential Generator kernels
    fn compile_lcg_kernels(&mut self) -> ScirsCoreResult<()> {
        let uniform_kernel_source = r#"
    ...
    fn compile_philox_kernels(&mut self) -> ScirsCoreResult<()> {
        let uniform_kernel_source = r#"
    ...
    pub fn generate(
        ...
        count: usize
    ) -> ScirsCoreResult<Array<f32, IxDyn>> {
        match distribution {
            ...
            GpuDistribution::Exponential { lambda } => {
                self.generate_exponential(count, *lambda)
            }
            _ => Err(ScirsCoreError::ComputationError(
                format!("Distribution {:?} not yet implemented", distribution)
            )),
        }
    }
    
    /// Generate uniform random numbers [0, 1)
    pub fn generate_uniform(&self, count: usize) -> ScirsCoreResult<Array<f32, IxDyn>> {
        let kernel = self.kernels.get("uniform")
            .ok_or_else(|| ScirsCoreError::ComputationError("Uniform kernel not found".to_string()))?;
        ...
        let global_size = (count + self.work_group_size - 1) / self.work_group_size * self.work_group_size;
        ...
        
        {
            ...
            state.seeds[..num_threads.min(updated_seeds.len())].copy_from_slice(
                &updated_seeds[..num_threads.min(updated_seeds.len())]
            );
            state.counters[..num_threads.min(updated_counters.len())].copy_from_slice(
                &updated_counters[..num_threads.min(updated_counters.len())]
            );
        }
    ...
    pub fn generate_normal(&self, count: usize, mean: f32, std_dev: f32) -> ScirsCoreResult<Array<f32, IxDyn>> {
        if let Some(kernel) = self.kernels.get("normal") {
            ...
            let global_size = (count / 2 + self.work_group_size - 1) / self.work_group_size * self.work_group_size;
            kernel.execute(&[global_size], &[self.work_group_size])?;
    ...
    pub fn generate_exponential(&self, count: usize, lambda: f32) -> ScirsCoreResult<Array<f32, IxDyn>> {
        // Generate uniform samples and transform using inverse CDF
        let uniform_samples = self.generate_uniform(count)?;
        let exponential_samples: Vec<f32> = uniform_samples.iter()
            .map(|&u| -(-u.ln()) / lambda)
            .collect();
    ...
    fn box_muller_transform(&self, uniform_samples: &Array<f32, IxDyn>, mean: f32, std_dev: f32) -> ScirsCoreResult<Array<f32, IxDyn>> {
    ...
    pub fn reset(&self) -> ScirsCoreResult<()> {
        let mut state = self.state.lock().unwrap();
    ...
    pub fn seed(&self, base_seed: u64) -> ScirsCoreResult<()> {
        let mut state = self.state.lock().unwrap();
...
pub struct GpuRngManager {
    ...
    default_device: Option<Arc<GpuDevice>>,
}
...
impl GpuRngManager {
    ...
    pub fn set_default_device(&mut self, device: Arc<GpuDevice>) {
    ...
    ) -> ScirsCoreResult<Arc<GpuRandomGenerator>> {
        let device = device.or_else(|| self.default_device.clone())
            .ok_or_else(|| ScirsCoreError::ComputationError("No GPU device available".to_string()))?;
    ...
    pub fn get_overall_statistics(&self) -> ManagerStatistics {
        ...
        let total_memory: usize = self.generators.values()
            .map(|gen| gen.get_statistics().memory_usage)
            .sum();
        let total_generated: u64 = self.generators.values()
...
pub mod utils {
    ...
    pub fn benchmark_gpu_vs_cpu(
        device: Arc<GpuDevice>,
        count: usize,
    ) -> ScirsCoreResult<BenchmarkResults> {
...
mod tests {
    ...
    fn test_distribution_types() {
        ...
        let normal = GpuDistribution::Normal { mean: 0.0, std_dev: 1.0 };
