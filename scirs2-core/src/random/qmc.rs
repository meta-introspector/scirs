...
impl FaureGenerator {
    ...
    fn initialize_pascal_matrix(&mut self) {
        ...
        for i in 0..size {
            ...
            for j in 1..=i {
                let prev_row = if i > 0 { self.pascal_matrix[i - 1][j - 1] } else { 0 };
                let prev_diag = if i > 0 && j < size { self.pascal_matrix[i - 1][j] } else { 0 };
    ...
    fn scrambled_radical_inverse(&self, n: u64, dimension: usize) -> f64 {
        ...
        while index > 0 {
            ...
            let scrambled_digit = if dimension < self.pascal_matrix.len() {
                (digit + self.pascal_matrix[dimension % self.pascal_matrix.len()][digit as usize % self.pascal_matrix.len()] as u64) % self.base as u64
            } else {
                digit
            };
...
pub mod integration {
        ...
        F: Fn(&[f64]) -> f64 + Send + Sync + 'static,
    {
        ...
        let (total_sum, total_sum_sq) = results.iter().fold((0.0, 0.0), |(s, ss), (sum, sum_sq)| {
            (s + sum, ss + sum_sq)
        });
...
pub fn create_generator(
    ...
    match sequence_type {
        QmcSequenceType::Sobol => {
            Ok(Box::new(SobolGenerator::new(dimension)?))
        }
        QmcSequenceType::Halton => {
            Ok(Box::new(HaltonGenerator::with_prime_bases(dimension)?))
        }
        QmcSequenceType::Faure => {
            Ok(Box::new(FaureGenerator::new(dimension)?))
        }
...
mod tests {
    ...
    fn test_latin_hypercube_sampling() {
        let mut lhs = LatinHypercubeSampler::new(2);
