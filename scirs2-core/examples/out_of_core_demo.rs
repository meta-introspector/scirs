...
use scirs2_core::memory::out_of_core::{
    FileStorageBackend, OutOfCoreArray, OutOfCoreConfig, OutOfCoreManager, CachePolicy,
};
use std::sync::Arc;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let managed_array: Arc<OutOfCoreArray<f64>> = manager.create_array(
    ...
    let in_memory = Array::<f64, IxDyn>::from_elem(IxDyn(&[200, 200]), 3.14);
    ...
    println!("   - Total chunks: {}", stats.total_chunks);
    println!("   - Cached chunks: {}", stats.cache_stats.cached_chunks);
    println!("   - Memory usage: {} bytes", stats.cache_stats.memory_usage);
