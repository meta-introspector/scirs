...
use scirs2_core::gpu::kernels::{KernelParams, DataType};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let context = match GpuContext::new(GpuBackend::preferred()) {
        Ok(ctx) => {
            println!("Successfully initialized GPU context with backend: {}", ctx.backend_name());
            ctx
        }
        Err(e) => {
...
fn demo_ml_operations(context: &GpuContext) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Sigmoid output range: [{:.4}, {:.4}]", 
             sigmoid_result.iter().fold(f32::INFINITY, |a, &b| a.min(b)),
             sigmoid_result.iter().fold(f32::NEG_INFINITY, |a, &b| a.max(b)));
...
fn demo_transform_operations(context: &GpuContext) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..size {
        ...
        real_data.push((2.0 * std::f32::consts::PI * 5.0 * t).sin() + 
    ...
    println!("   Note: This is a placeholder implementation - real FFT would show frequency domain");
...
fn demo_kernel_specialization(context: &GpuContext) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let axpy_params = KernelParams::new(DataType::Float32)
        .with_numeric_param("alpha", 3.14159);
