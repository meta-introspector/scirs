...
use scirs2_core::logging::progress::{
    ProgressBuilder, ProgressStyle, ProgressSymbols, 
    MultiProgress, ProgressGroup
};
...
fn demo_multi_progress() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..100 {
        ...
        if i == 60 { multi.finish(task3_id); }
        if i == 75 { multi.finish(task2_id); }
        if i == 100 { multi.finish(task1_id); }
    }
...
fn demo_progress_groups() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let extract_id = group.add_tracker(
        ProgressBuilder::new("Extract Data", 100).build()
    );
    
    let transform_id = group.add_tracker(
        ProgressBuilder::new("Transform Data", 150).build()
    );
    
    let load_id = group.add_tracker(
        ProgressBuilder::new("Load Data", 80).build()
    );
