...
fn main() {
    ...
    println!("Run with: cargo run --example arbitrary_precision_example --features arbitrary-precision");
}
...
fn demo_arbitrary_integers() -> CoreResult<()> {
    ...
    // Prime testing
    let prime_candidate = ArbitraryInt::from_str_radix("170141183460469231731687303715884105727", 10)?;
    println!("\nIs {} prime? {}", prime_candidate, 
...
fn demo_arbitrary_floats() -> CoreResult<()> {
    ...
    let one_third = ArbitraryFloat::from_f64_prec(1.0, prec)? / 
                    ArbitraryFloat::from_f64_prec(3.0, prec)?;
    println!("1/3 with {} bits precision:", prec);
    println!("{}", one_third);
    println!("Decimal precision: {} digits", one_third.decimal_precision());
...
fn demo_complex_numbers() -> CoreResult<()> {
    ...
    let i_pi = ArbitraryComplex::from_parts(
        &ArbitraryFloat::from_f64_prec(0.0, prec)?,
        &pi
    );
...
fn demo_constants() -> CoreResult<()> {
    ...
    println!("Computing constants to {} bits ({} decimal digits):", 
             prec, (prec as f64 / 3.32) as u32);
...
fn demo_numerical_analysis() -> CoreResult<()> {
    ...
    for i in 0..5 {
        let x_new = (x.clone() + two.clone() / x.clone()) / 
                    ArbitraryFloat::from_f64_prec(2.0, prec)?;
...
fn demo_large_numbers() -> CoreResult<()> {
    ...
    for n in [13, 17, 19, 31, 61] {
        ...
        let _mersenne = two.mod_pow(&exp, &ArbitraryInt::from_str_radix(&"1".repeat(1000), 10)?)? - one;
