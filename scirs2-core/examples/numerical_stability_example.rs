...
fn demo_stable_summation() -> CoreResult<()> {
    ...
    println!("Adding {} small values ({}) to a large value ({}):", n, small, large);
    ...
    println!("Error (naive):   {}", (naive_sum - (large + (n as f64) * small)).abs());
    println!("Error (Kahan):   {}", (kahan.sum() - (large + (n as f64) * small)).abs());
    println!("Error (Neumaier): {}", (neumaier_result - (large + (n as f64) * small)).abs());
...
fn demo_stable_variance() -> CoreResult<()> {
    ...
    println!("Dataset: {} points with large mean ({}) and small variance", n, mean);
    println!("Expected variance: ~{}", std * std);
    println!("Naive single-pass variance: {}", naive_var);
    println!("Welford's algorithm variance: {}", welford.variance().unwrap());
...
fn demo_log_sum_exp() -> CoreResult<()> {
    ...
    let large_values = vec![700.0, 701.0, 702.0, 703.0, 704.0];
    ...
    println!("Sum of probabilities: {}", softmax_result.iter().sum::<f64>());
...
fn demo_numerical_differentiation() -> CoreResult<()> {
    ...
    for &tol in &tolerances {
        ...
        println!("  Tolerance {}: result = {} (error: {})", tol, result, error);
    }
...
fn demo_stable_special_functions() -> CoreResult<()> {
    ...
    for &x in &small_values {
        ...
        println!("  Relative error (naive): {}", ((naive - true_value) / true_value).abs());
        println!("  Relative error (stable): {}", ((stable - true_value) / true_value).abs());
    }
