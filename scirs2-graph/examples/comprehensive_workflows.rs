...
use scirs2_graph::{
    ...
    // Algorithms
    dijkstra_path, betweenness_centrality, pagerank_centrality, parallel_pagerank_centrality,
    breadth_first_search, connected_components, louvain_communities_result,
    // Graph measures
    centrality, CentralityType, clustering_coefficient,
    // I/O and utilities
    GraphError, Result,
};
...
fn workflow_basic_operations() -> Result<()> {
    ...
    if let Some(path) = dijkstra_path(&graph, &"New York".to_string(), &"Sydney".to_string())? {
        println!("  🛣️  Shortest path NYC→Sydney: {:?} (distance: {:.1}k km)", 
                 path.nodes, path.total_weight);
    }
    
    // Calculate centrality measures
    let degree_centrality = centrality(&graph, CentralityType::Degree)?;
    let most_central = degree_centrality.iter()
        .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
        .unwrap();
    println!("  🎯 Most central city (degree): {} ({:.3})", 
...
fn workflow_large_graph_analysis() -> Result<()> {
    ...
    println!("  🏗️  Generating Barabási-Albert graph ({} nodes)...", num_nodes);
    ...
    let top_nodes: Vec<_> = parallel_pagerank.iter()
        .collect::<Vec<_>>();
...
fn workflow_community_detection() -> Result<()> {
    ...
    let mut sizes: Vec<usize> = communities.communities.iter()
        .map(|c| c.len())
        .collect();
...
fn workflow_directed_graph_analysis() -> Result<()> {
    ...
    let papers = vec![
        ...
        "Paper F", "Paper G", "Paper H", "Paper I", "Paper J"
    ];
    ...
    let citations = vec![
        ...
        ("Paper B", "Paper D"), ("Paper C", "Paper D"),
        ("Paper D", "Paper E"), ("Paper E", "Paper F"),
        ("Paper F", "Paper G"), ("Paper G", "Paper H"),
        ("Paper H", "Paper I"), ("Paper I", "Paper J"),
