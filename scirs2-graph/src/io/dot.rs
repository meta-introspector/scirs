...
#[allow(dead_code)]
pub fn read_dot_format<N, E, P>(_path: P, weighted: bool) -> Result<Graph<N, E>>
where
...
/// * `Err(GraphError)` - If there was an error reading or parsing the file
#[allow(dead_code)]
pub fn read_dot_format_digraph<N, E, P>(_path: P, weighted: bool) -> Result<DiGraph<N, E>>
where
    N: Node + std::fmt::Debug + FromStr + Clone,
    E: EdgeWeight + std::marker::Copy + std::fmt::Debug + std::default::Default + FromStr,
    P: AsRef<Path>,
...
#[allow(dead_code)]
pub fn write_dot_format<N, E, Ix, P>(_graph: &Graph<N, E, Ix>, path: P, weighted: bool) -> Result<()>
where
...
#[allow(dead_code)]
fn remove_comments(_line: &str, in_multiline_comment: bool) -> (String, bool) {
    ...
    while let Some(ch) = chars.next() {
        if in_comment {
            // We're inside a multi-_line _comment, look for */
            if ch == '*' && chars.peek() == Some(&'/') {
                chars.next(); // consume '/'
                in_comment = false;
            }
            // Skip characters inside _comment
        } else {
            // Not in a _comment, check for _comment start
            if ch == '/' {
                if let Some(&next_ch) = chars.peek() {
                    if next_ch == '/' {
                        // Single-_line _comment, skip rest of _line
                        break;
                    } else if next_ch == '*' {
                        // Multi-_line _comment start
                        chars.next(); // consume '*'
                        in_comment = true;
                        continue;
                    }
...
#[allow(dead_code)]
fn parse_header(_line: &str) -> Result<Option<GraphType>> {
    let _line = _line.trim();

    if _line.starts_with("graph") && _line.contains('{') {
        return Ok(Some(GraphType::Undirected));
    }

    if _line.starts_with("digraph") && _line.contains('{') {
...
where
    ...
    E: EdgeWeight + std::marker::Copy + std::fmt::Debug + std::default::Default + FromStr,
{
    let _line = _line.trim_end_matches(';').trim();

    // Check for edge based on graph _type
    let edge_separator = match graph_type {
        GraphType::Undirected => "--",
        GraphType::Directed => "->",
    };

    if _line.contains(edge_separator) {
        parse_edge(_line, edge_separator, graph, weighted, line_num)?;
    } else if !_line.is_empty() && !_line.starts_with('}') && !_line.contains('[') {
        // Parse as node declaration (simple node without attributes)
        parse_node(_line, line_num)?;
    }
    ...
    E: EdgeWeight + std::marker::Copy + std::fmt::Debug + std::default::Default + FromStr,
{
    ...
    if _line.contains(edge_separator) {
        parse_digraph_edge(_line, edge_separator, graph, weighted, line_num)?;
    } else if !_line.is_empty() && !_line.starts_with('}') && !_line.contains('[') {
        // Parse as node declaration (simple node without attributes)
        parse_node(_line, line_num)?;
    }
...
#[allow(dead_code)]
fn parse_weight_from_attributes<E>(_attributes: &str) -> Result<E>
where
    E: EdgeWeight + std::marker::Copy + std::fmt::Debug + std::default::Default + FromStr,
{
    // Look for weight=value pattern
    if let Some(weight_start) = _attributes.find("weight=") {
        let weight_part = &_attributes[weight_start + 7..]; // Skip "weight="
