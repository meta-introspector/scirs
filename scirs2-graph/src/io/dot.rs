...
#[allow(dead_code)]
fn remove_comments(_line: &str, in_multilinecomment: bool) -> (String, bool) {
    ...
    while let Some(ch) = chars.next() {
        if in_comment {
            // We're inside a multi-_line comment, look for */
            ...
            // Not in a comment, check for _comment start
            if ch == '/' {
                if let Some(&next_ch) = chars.peek() {
                    if next_ch == '/' {
                        // Single-_line comment, skip rest of _line
                        break;
                    } else if next_ch == '*' {
...
where
    ...
    E: EdgeWeight + std::marker::Copy + std::fmt::Debug + std::default::Default + FromStr,
{
    let _line = line.trim_end_matches(';').trim();
...
#[allow(dead_code)]
fn parse_node(_line: &str, _linenum: usize) -> Result<()> {
