...
fn test_louvain_algorithm_accuracy() {
    ...
    assert_ne!(comm_0, comm_3, "The two groups should be in different communities");
...
fn test_modularity_optimization_consistency() {
    ...
    let min_communities = louvain_result.num_communities.min(greedy_result.num_communities).min(simulated_annealing_result.num_communities);
    let max_communities = louvain_result.num_communities.max(greedy_result.num_communities).max(simulated_annealing_result.num_communities);
    
    assert!(
        max_communities <= min_communities * 2,
        "Different algorithms should find similar number of communities (max: {}, min: {})",
        max_communities, min_communities
    );
...
fn test_parallel_vs_sequential_consistency() {
    ...
    if let (Some(seq_quality), Some(par_quality)) = 
        (sequential_result.quality_score, parallel_result.quality_score) {
        assert_relative_eq!(
            seq_quality,
            par_quality,
            epsilon = 0.1,
...
fn test_community_result_api_consistency() {
    ...
    for (node, &community_id) in &result.node_communities {
        assert!(
            ...
            node, community_id, result.num_communities
        );
...
fn test_edge_cases() {
    ...
    assert_eq!(empty_result.num_communities, 0, "Empty graph should have 0 communities");
    assert_eq!(empty_result.communities.len(), 0, "Empty graph should have no community sets");
    ...
    assert_eq!(single_result.num_communities, 1, "Single node should form 1 community");
    assert_eq!(single_result.communities[0].len(), 1, "Single community should have 1 node");
    assert!(single_result.communities[0].contains(&0), "Single node should be in the community");
...
fn test_modularity_bounds() {
    ...
    let graphs = vec![
        ...
        ("Random graph", generators::erdos_renyi_graph(20, 0.1, None).unwrap()),
    ];
...
fn test_algorithm_convergence() {
    ...
    if let (Some(q10), Some(q50), Some(q100)) = 
        (result_10.quality_score, result_50.quality_score, result_100.quality_score) {
...
fn create_test_graph_with_communities(nodes_per_community: usize, num_communities: usize) -> Graph<usize, f64> {
    let mut graph = Graph::new();
    ...
    for community in 0..num_communities {
        ...
        for i in start..end {
            for j in (i+1)..end {
                if (i + j) % 3 == 0 { // Add edges with some randomness but high density
                    graph.add_edge(i, j, 2.0).unwrap();
                }
            }
        }
...
fn create_karate_club_graph() -> Graph<usize, f64> {
    ...
    let group1_edges = vec![
        ...
        (1, 2), (1, 3), (1, 7), (2, 3), (2, 7), (3, 7), (4, 6), (4, 10),
        (5, 6), (5, 10), (5, 16), (6, 16), (8, 30), (8, 32), (8, 33),
    ];
    
    // Group 2: 17-33  
    let group2_edges = vec![
        (17, 18), (17, 19), (17, 20), (17, 21), (18, 19), (18, 20),
        (19, 20), (19, 21), (20, 21), (22, 23), (22, 24), (23, 24),
        (24, 25), (24, 27), (25, 27), (26, 27), (26, 28), (27, 28),
        (28, 29), (29, 30), (29, 31), (30, 31), (31, 32), (32, 33),
    ];
    ...
    // Add all edges
    for (u, v) in group1_edges.into_iter().chain(group2_edges).chain(bridge_edges) {
