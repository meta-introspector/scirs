...
use scirs2_ndimage::{
    ...
    python_interop::{
        ...
        binding_examples::{
            example_gaussian_filter_binding, 
            example_median_filter_binding, 
            generate_module_definition
        },
        setup::{generate_setup_py, generate_init_py, generate_install_instructions},
        PyError,
    },
};
...
fn main() -> NdimageResult<()> {
    ...
    match validate_array_compatibility::<f64>(&f32_info) {
        ...
        Err(e) => println!("   ✅ F32->F64 validation: FAILED as expected - {}", e.message),
    }
    ...
    println!("   📊 Generated {} filter function specifications", filter_specs.len());
    
    for spec in &filter_specs {
        println!("      • {} - {} parameters", spec.name, spec.parameters.len());
    }
    ...
    println!("   ✅ Generated Gaussian filter binding ({} chars)", gaussian_binding.len());
    println!("   ✅ Generated median filter binding ({} chars)", median_binding.len());
    println!("   ✅ Generated module definition ({} chars)", module_def.len());
    
    // Save binding examples
    std::fs::write("examples/outputs/gaussian_filter_binding.rs", &gaussian_binding)
    ...
    println!("   ✅ Generated installation instructions ({} chars)", install_instructions.len());
    
    // Save setup files
    std::fs::write("examples/outputs/setup.py", &setup_py)
        .expect("Failed to write setup.py");
    std::fs::write("examples/outputs/__init__.py", &init_py)
    ...
    println!("      Elements: {} million", large_info.shape.iter().product::<usize>() as f64 / 1_000_000.0);
    println!("      Memory: ~{:.1} MB", 
