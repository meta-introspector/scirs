...
use scirs2_ndimage::{
    error::NdimageResult,
    profiling::{
        enable_profiling, disable_profiling, get_performance_report, enable_memory_profiling,
        get_memory_report,
    },
};
...
impl BenchmarkResult {
    fn new(operation_name: String, image_size: (usize, usize), durations: &[Duration]) -> Self {
        ...
        let throughput_mpix_per_sec = if seconds > 0.0 { megapixels / seconds } else { 0.0 };
    ...
    fn display(&self) {
        println!("  {} ({}x{}):", self.operation_name, self.image_size.0, self.image_size.1);
        println!("    Mean: {:?} ± {:?}", self.mean_duration, self.std_deviation);
...
fn main() -> NdimageResult<()> {
    ...
    for &image_size in &config.image_sizes {
        println!("Benchmarking with image size: {}x{}", image_size.0, image_size.1);
    ...
    println!("Total operations: {}", perf_report.operation_breakdown.len());
...
fn create_benchmark_image(height: usize, width: usize) -> Array2<f64> {
    Array2::from_shape_fn((height, width), |(i, j)| {
        ...
        let pattern1 = (x * std::f64::consts::PI * 8.0).sin() * (y * std::f64::consts::PI * 8.0).cos();
...
fn benchmark_lbp(
    ...
    for (config_name, radii, sample_points) in lbp_configs {
        ...
        for _ in 0..config.iterations {
            let start = Instant::now();
            let _lbp_result = ultra_simd_multi_scale_lbp(
                image.view(),
                &radii,
                &sample_points,
            )?;
...
fn display_benchmark_summary(results: &[BenchmarkResult]) {
    ...
    for result in results {
        let operation_type = result.operation_name.split_whitespace().next().unwrap_or("Unknown");
        grouped.entry(operation_type.to_string()).or_default().push(result);
    }
    ...
    for (operation_type, group_results) in grouped {
        ...
        for result in group_results {
            size_groups.entry(result.image_size).or_default().push(result);
        }
        ...
        for (size, size_results) in size_groups {
            ...
            sorted_results.sort_by(|a, b| b.throughput_mpix_per_sec.partial_cmp(&a.throughput_mpix_per_sec).unwrap());
            
            for (i, result) in sorted_results.iter().enumerate() {
                let rank = if i == 0 { "🥇" } else if i == 1 { "🥈" } else if i == 2 { "🥉" } else { "  " };
                println!("    {} {}: {:.2} MPix/s ({:?})",
                         rank, result.operation_name, result.throughput_mpix_per_sec, result.mean_duration);
            }
    ...
    let avg_throughput: f64 = results.iter().map(|r| r.throughput_mpix_per_sec).sum::<f64>() / total_operations as f64;
    let max_throughput = results.iter().map(|r| r.throughput_mpix_per_sec).fold(0.0, f64::max);
...
fn main() -> NdimageResult<()> {
    ...
    println!("Please compile with: cargo run --example ultra_performance_benchmark --features simd");
    Ok(())
}