...
fn object_detection_recipes() -> NdimageResult<()> {
    ...
    println!("let (labeled, _) = label(&binary.view(), None)?;");
    ...
    println!("let (distances, _) = distance_transform_edt(&binary_dyn, None, true, false)?;");
...
fn measurement_recipes() -> NdimageResult<()> {
    ...
    println!("    assert_eq!(img1.dim(), img2.dim(), \"Images must have same dimensions\");");
    println!("    ");
...
fn transformation_recipes() -> NdimageResult<()> {
    ...
    println!("            ");
    println!("            let correlation = calculate_correlation(reference, &shifted);");
