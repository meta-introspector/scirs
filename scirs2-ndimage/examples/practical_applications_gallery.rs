...
fn medical_imaging_applications() -> NdimageResult<()> {
    ...
    let cleaned_candidates = binary_opening(&tumor_candidates.view(), Some(&structure.view()), None, None, None)?;
    ...
    println!("Results: Found {} tumor candidates out of {} initial regions", 
             tumor_regions.len(), num_candidates);
    
    for (i, tumor) in tumor_regions.iter().take(3).enumerate() {
        println!("   Tumor {}: area={:.0}, centroid=({:.0}, {:.0})", 
                i+1, tumor.area, tumor.centroid[0], tumor.centroid[1]);
    }
    ...
    println!("   Total vessel length: {:.1} pixels", vessel_stats.total_length);
    println!("   Number of branches: {}", vessel_stats.branch_points);
    println!("   Average vessel width: {:.1} pixels", vessel_stats.avg_width);
    ...
    let cell_properties = region_properties(&segmented_cells.view(), Some(&microscopy_image.view()))?;
    ...
    println!("   Average cell area: {:.1} pixels", 
             valid_cells.iter().map(|c| c.area).sum::<f64>() / valid_cells.len() as f64);
    println!("   Cell density: {:.3} cells per 1000 pixels²", 
...
fn industrial_inspection_applications() -> NdimageResult<()> {
    ...
    let linear_defects: Vec<_> = scratch_props.iter()
        .filter(|prop| {
    ...
    let circular_defects: Vec<_> = spot_props.iter()
        .filter(|prop| {
            let width = prop.bbox[3] - prop.bbox[1];
            let height = prop.bbox[2] - prop.bbox[0];
            let aspect_ratio = width.max(height) as f64 / width.min(height) as f64;
    ...
    println!("   Circular defects (dents/spots): {}", circular_defects.len());
    println!("   Total surface area analyzed: {} pixels", surface_image.len());
    ...
    let part_properties = {
        let (labeled, _) = label(&part_mask.view(), None)?;
        let props = region_properties(&labeled.view(), Some(&part_image.view()))?;
        props.into_iter().max_by(|a, b| a.area.partial_cmp(&b.area).unwrap())
    };
    ...
    println!("   Overall quality: {}", if defects.is_acceptable() { "PASS" } else { "FAIL" });
...
fn remote_sensing_applications() -> NdimageResult<()> {
    ...
    println!("   Non-vegetated area: {:.1}% of area", 100.0 - healthy_area - moderate_area - sparse_area);
    ...
    let structure = generate_binary_structure(2, 1)?;
    let cleaned_water = binary_closing(&water_mask.view(), Some(&structure.view()), None, None, None)?;
    
    println!("Step 3: Water body analysis");
    let (labeled_water, num_water_bodies) = label(&cleaned_water.view(), None)?;
    let water_properties = region_properties(&labeled_water.view(), Some(&multispectral_image.view()))?;
    ...
    println!("   Large water bodies (>500 pixels): {}", large_water_bodies.len());
    println!("   Water coverage: {:.1}% of total area", water_coverage);
    println!("   Largest water body: {:.0} pixels", 
    ...
    let shadow_coverage = cloud_shadows.iter().sum::<u8>() as f64 / cloud_shadows.len() as f64 * 100.0;
    ...
    println!("   Image quality: {}", if clear_coverage > 70.0 { "Good" } else { "Poor" });
...
fn computer_vision_applications() -> NdimageResult<()> {
    ...
    for (i, obj) in moving_objects.iter().take(3).enumerate() {
        println!("   Object {}: area={:.0}, center=({:.0}, {:.0})", 
                i+1, obj.area, obj.centroid[0], obj.centroid[1]);
    }
    ...
    println!("   Paragraphs identified: {}", layout_analysis.num_paragraphs);
    println!("   Average line height: {:.1} pixels", layout_analysis.avg_line_height);
    println!("   Text coverage: {:.1}% of document", layout_analysis.text_coverage);
    ...
    for (i, face) in validated_faces.iter().enumerate() {
        println!("   Face {}: confidence={:.2}, size={}x{}", 
                i+1, face.confidence, face.width, face.height);
    }
...
fn scientific_imaging_applications() -> NdimageResult<()> {
    ...
    let particle_properties = region_properties(&segmented_particles.view(), Some(&sem_image.view()))?;
    ...
    let sizes: Vec<f64> = valid_particles.iter().map(|p| (p.area / std::f64::consts::PI).sqrt() * 2.0).collect();
    let mean_size = sizes.iter().sum::<f64>() / sizes.len() as f64;
    let size_std = (sizes.iter().map(|&s| (s - mean_size).powi(2)).sum::<f64>() / sizes.len() as f64).sqrt();
    
    println!("Results: Particle analysis:");
    println!("   Total particles: {}", valid_particles.len());
    println!("   Mean diameter: {:.1} ± {:.1} pixels", mean_size, size_std);
    println!("   Size range: {:.1} - {:.1} pixels", 
    ...
    let bright_stars: Vec<_> = star_catalog.iter().filter(|star| star.magnitude < 15.0).collect();
    let faint_stars: Vec<_> = star_catalog.iter().filter(|star| star.magnitude >= 15.0).collect();
    ...
    if let Some(brightest) = bright_stars.iter().min_by(|a, b| a.magnitude.partial_cmp(&b.magnitude).unwrap()) {
        println!("   Brightest star: magnitude {:.1} at ({:.0}, {:.0})", 
    ...
    let point_defects = defects.iter().filter(|d| d.defect_type == DefectType::Point).count();
    let line_defects = defects.iter().filter(|d| d.defect_type == DefectType::Line).count();
    let planar_defects = defects.iter().filter(|d| d.defect_type == DefectType::Planar).count();
    ...
    println!("   Defect density: {:.3} per 1000 pixels²", 
...
fn create_medical_scan_simulation(height: usize, width: usize) -> Array2<f64> {
    Array2::from_shape_fn((height, width), |(i, j)| {
        ...
        let tumor1 = if ((x - 0.3).powi(2) + (y - 0.4).powi(2)).sqrt() < 0.08 { 0.7 } else { 0.0 };
        let tumor2 = if ((x - 0.7).powi(2) + (y - 0.6).powi(2)).sqrt() < 0.05 { 0.8 } else { 0.0 };
...
fn create_angiogram_simulation(height: usize, width: usize) -> Array2<f64> {
    Array2::from_shape_fn((height, width), |(i, j)| {
        ...
        let vessel1 = if ((x - 0.5).abs() < 0.02) && (y > 0.1 && y < 0.9) { 0.8 } else { 0.0 };
        let vessel2 = if ((y - 0.5).abs() < 0.015) && (x > 0.2 && x < 0.8) { 0.7 } else { 0.0 };
        let vessel3 = if (((x - y).abs() < 0.01) && x > 0.2 && x < 0.8) { 0.6 } else { 0.0 };
...
fn create_surface_inspection_simulation(height: usize, width: usize) -> Array2<f64> {
    Array2::from_shape_fn((height, width), |(i, j)| {
        ...
        // Add defects
        let scratch = if (x > 0.3 && x < 0.7) && ((y - 0.4).abs() < 0.005) { -0.3 } else { 0.0 };
        let dent = if ((x - 0.6).powi(2) + (y - 0.7).powi(2)).sqrt() < 0.03 { -0.2 } else { 0.0 };
...
fn detect_pcb_traces(pcb_image: &Array2<f64>) -> NdimageResult<TraceStats> {
    Ok(TraceStats { total_length: 850.0 })
}

fn analyze_pcb_defects(components: &ComponentStats, traces: &TraceStats) -> NdimageResult<PCBDefects> {
    Ok(PCBDefects {
...
fn create_satellite_bands_simulation(height: usize, width: usize) -> (Array2<f64>, Array2<f64>) {
    ...
    let nir_band = Array2::from_shape_fn((height, width), |(i, j)| {
        let x = i as f64 / height as f64;
        let y = j as f64 / width as f64;
...
fn compute_local_edge_density(edges: &Array2<f64>, window_size: usize) -> NdimageResult<Array2<f64>> {
...
fn analyze_document_layout(text_lines: &Array2<u32>, image: &Array2<f64>) -> NdimageResult<LayoutAnalysis> {
    Ok(LayoutAnalysis {
        num_text_lines: 6,
        num_paragraphs: 3,
...
fn detect_face_candidates(image: &Array2<f64>, skin_mask: &Array2<u8>) -> NdimageResult<FaceCandidates> {
    Ok(FaceCandidates { num_candidates: 3 })
}

fn validate_face_candidates(candidates: &FaceCandidates, image: &Array2<f64>) -> NdimageResult<Vec<FaceCandidate>> {
    Ok(vec![
        FaceCandidate { confidence: 0.85, width: 120, height: 150 }
    ])
}
...
fn create_particle_simulation(height: usize, width: usize) -> Array2<f64> {
    Array2::from_shape_fn((height, width), |(i, j)| {
        ...
        let particles = [
            ((0.2, 0.3), 0.03), ((0.5, 0.2), 0.05), ((0.7, 0.4), 0.02),
            ((0.3, 0.7), 0.04), ((0.8, 0.8), 0.03), ((0.1, 0.6), 0.025),
        ];
...
fn create_astronomy_simulation(height: usize, width: usize) -> Array2<f64> {
    Array2::from_shape_fn((height, width), |(i, j)| {
        ...
        // Add stars
        let stars = [
            ((0.2, 0.3), 0.8), ((0.5, 0.2), 0.6), ((0.7, 0.8), 0.9),
            ((0.1, 0.7), 0.5), ((0.8, 0.4), 0.7), ((0.3, 0.6), 0.4),
        ];
...
fn perform_star_photometry(image: &Array2<f64>, candidates: &Array2<u8>) -> NdimageResult<Vec<Star>> {
    Ok(vec![
        Star { x: 80.0, y: 120.0, magnitude: 12.5 },
        Star { x: 200.0, y: 80.0, magnitude: 14.2 },
        Star { x: 280.0, y: 320.0, magnitude: 11.8 },
    ])
}
...
fn create_crystal_simulation(height: usize, width: usize) -> Array2<f64> {
    Array2::from_shape_fn((height, width), |(i, j)| {
        ...
        // Add some defects
        let defect = if i == height / 2 && j > width / 3 && j < 2 * width / 3 {
            -0.3  // Line defect
        } else {
            0.0
        };
...
fn detect_crystal_defects(image: &Array2<f64>, lattice: &LatticeInfo) -> NdimageResult<Vec<CrystalDefect>> {
    Ok(vec![
        CrystalDefect { defect_type: DefectType::Point, x: 100.0, y: 120.0 },
        CrystalDefect { defect_type: DefectType::Line, x: 150.0, y: 150.0 },
    ])
}
