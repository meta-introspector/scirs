...
use scirs2_ndimage::{
    ...
    comprehensive_examples::{ExampleTutorial, validate_all_examples},
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Generated {} tutorial steps covering:", tutorial.step_count());
    ...
    match compatibility_tester.run_all_tests() {
        Ok(()) => {
            ...
            println!("Compatible functions: {}/{}", compatible_count, results.len());
            
            // Show any incompatible functions
            for result in results {
                if !result.compatible {
                    println!("⚠ {} - {:.1}% compatible", result.function_name, result.compatibility_score * 100.0);
    ...
    match validation_suite.run_all_validations() {
        Ok(()) => {
            ...
            let mut by_function: std::collections::HashMap<String, Vec<&_>> = std::collections::HashMap::new();
            for result in results {
            ...
            for (function, func_results) in by_function {
                ...
                for result in func_results {
                    if !result.passed {
                        println!("     ✗ {}: max_diff={:.2e}, tolerance={:.2e}", 
    ...
    match benchmark_suite.run_all_benchmarks() {
        Ok(()) => {
            ...
            let mut by_operation: std::collections::HashMap<String, Vec<&_>> = std::collections::HashMap::new();
            for result in results {
                by_operation.entry(result.operation.clone()).or_insert_with(Vec::new).push(result);
            }
            
            for (operation, op_results) in by_operation {
                let avg_time: f64 = op_results.iter().map(|r| r.execution_time_ms).sum::<f64>() / op_results.len() as f64;
                let avg_memory: f64 = op_results.iter().map(|r| r.memory_usage_bytes).sum::<usize>() as f64 / op_results.len() as f64;
                
                println!("   {}: {:.2}ms avg, {:.1}MB avg memory", 
                        operation, avg_time, avg_memory / 1_000_000.0);
            }
        },
        Err(e) => {
    ...
    println!("   - API Compatibility Report: {} characters", compatibility_report.len());
    println!("   - Numerical Validation Report: {} characters", validation_report.len());
    println!("   - Performance Benchmark Report: {} characters", benchmark_report.len());
    println!("   - Tutorial Documentation: {} characters", tutorial_markdown.len());
    ...
    println!("✓ API compatibility: {:.1}%", compatibility_tester.get_overall_score() * 100.0);
    println!("✓ Numerical validation: {:.1}%", validation_suite.get_pass_rate() * 100.0);
    println!("✓ Performance benchmarks: {} operations tested", benchmark_suite.get_results().len());
    println!("✓ Documentation: {} tutorial steps generated", tutorial.step_count());
...
fn demonstrate_custom_configurations() {
    ...
    println!("Custom API config: strict tolerance {:.0e}, test size {}", 
    ...
    println!("Custom validation config: {} random tests, max size {}", 
    ...
    println!("Custom benchmark config: {} iterations, {} warmup, {} array types",
