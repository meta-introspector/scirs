...
fn main() -> Result<()> {
    ...
    let mut optimizer = AdaptiveUltrathinkOptimizer::new(
    ...
    for (name, image) in &test_images {
        println!("   Processing {}: {}×{} array", name, image.nrows(), image.ncols());
        ...
        println!("   ✓ Consciousness level: {:.2}", result.consciousness_level);
    ...
    for (name, image) in &test_images {
        ...
        let start_time = Instant::now();
        let learning_result = enhanced_meta_learning_with_temporal_fusion(
            &image.view(),
            &mut meta_learning
        )?;
        let learning_time = start_time.elapsed();
        
        println!("   ✓ Short-term memory usage: {:.1}%", learning_result.short_term_memory_usage * 100.0);
        println!("   ✓ Long-term memory usage: {:.1}%", learning_result.long_term_memory_usage * 100.0);
        println!("   ✓ Learning convergence: {:.4}", learning_result.learning_convergence);
        println!("   ✓ Strategy evolution score: {:.4}", learning_result.strategy_evolution_score);
    ...
    for (name, image) in &test_images {
        ...
        let start_time = Instant::now();
        let scheduling_result = quantum_aware_resource_scheduling_optimization(
            &workload,
            &mut resource_scheduler
        )?;
        let scheduling_time = start_time.elapsed();
        
        println!("   ✓ Quantum resource allocation: {:.1}%", scheduling_result.quantum_allocation * 100.0);
        println!("   ✓ Classical resource allocation: {:.1}%", scheduling_result.classical_allocation * 100.0);
        println!("   ✓ Load balancing efficiency: {:.2}", scheduling_result.load_balancing_efficiency);
        println!("   ✓ Predicted speedup: {:.2}x", scheduling_result.predicted_speedup);
    ...
    let processed = ultrathink_fusion_processing(
        &large_image.view(),
    ...
    println!("   ✓ Total operations monitored: {}", profile_report.operation_count);
    println!("   ✓ Average CPU utilization: {:.1}%", profile_report.avg_cpu_utilization * 100.0);
    println!("   ✓ Peak memory usage: {:.2} MB", profile_report.peak_memory_mb);
    println!("   ✓ Processing time: {:?}", processing_time);
    ...
    println!("   ✓ Total operations analyzed: {}", optimization_report.total_operations);
    println!("   ✓ Average improvement: {:.1}%", optimization_report.average_improvement * 100.0);
    println!("   ✓ Best performing operation: {}", optimization_report.best_operation);
    println!("   ✓ Optimization accuracy: {:.2}%", optimization_report.prediction_accuracy * 100.0);
    ...
    for (i, rec) in recommendations.iter().enumerate().take(3) {
        println!("   {}. {}: {:.1}% improvement potential", 
                 i + 1, rec.operation_type, rec.improvement_potential * 100.0);
...
mod tests {
    ...
    #[test]
    fn test_ultrathink_demo_components() {
