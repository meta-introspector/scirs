    ...
    pub fn save_plot(content: &str, config: &ExportConfig) -> NdimageResult<()> {
        ...
        if config.include_metadata {
            ...
            if content.contains("</html>") {
                ...
                output_content = format!("{}\n{}", content, metadata.replace("<!--", "").replace("-->", ""));
            } else {
                output_content = format!("{}\n{}", content, metadata.replace("<!--", "").replace("-->", ""));
            }
        }

        fs::write(path, output_content).map_err(|e| {
            NdimageError::ComputationError(format!("Failed to write file: {}", e))
        })?;
...
pub mod advanced {
        ...
        T: Float + FromPrimitive + ToPrimitive + Debug + Clone,
    {
        ...
        writeln!(&mut html, r#"<select id="colormap" onchange="updateVisualization()">"#)?;
        ...
        writeln!(&mut html, r#"<select id="plotType" onchange="updateVisualization()">"#)?;
        ...
        writeln!(&mut html, r#"<button onclick="exportView()">Export View</button>"#)?;
        writeln!(&mut html, "</div>")?;
        
        writeln!(&mut html, "<div id='main-content' class='visualization-container'>")?;
        ...
        writeln!(&mut html, "<div class='stat-item'>Mean: {:.4}</div>", mean.to_f64().unwrap_or(0.0))?;
        writeln!(&mut html, "<div class='stat-item'>Min: {:.4}</div>", min_val.to_f64().unwrap_or(0.0))?;
        writeln!(&mut html, "<div class='stat-item'>Max: {:.4}</div>", max_val.to_f64().unwrap_or(0.0))?;
        writeln!(&mut html, "<div class='stat-item'>Pixels: {}</div>", width * height)?;
        writeln!(&mut html, "</div>")?;
        writeln!(&mut html, "</div>")?;
    ...
    {
        if images.is_empty() {
            return Err(NdimageError::InvalidInput("No images provided for comparison".into()));
        }
        ...
        for (name, image) in images {
            ...
            writeln!(&mut html, "<div class='stat-row'><span>Dimensions:</span><span>{}×{}</span></div>", height, width)?;
            writeln!(&mut html, "<div class='stat-row'><span>Mean:</span><span>{:.4}</span></div>", mean.to_f64().unwrap_or(0.0))?;
            writeln!(&mut html, "<div class='stat-row'><span>Range:</span><span>[{:.3}, {:.3}]</span></div>", 
                min_val.to_f64().unwrap_or(0.0), max_val.to_f64().unwrap_or(0.0))?;
            writeln!(&mut html, "</div>")?;
...
mod tests {
    ...
    fn test_comparison_view() {
        ...
        let images = vec![
            ("Original", image1.view()),
            ("Processed", image2.view()),
        ];
