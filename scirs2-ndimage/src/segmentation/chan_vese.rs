        ...
        for i in 1..height - 1 {
            for j in 1..width - 1 {
                ...
                let grad_plus = ((a_plus * a_plus).max(b_minus * b_minus) + 
                                (c_plus * c_plus).max(d_minus * d_minus)).sqrt();
                ...
                let grad_minus = ((a_minus * a_minus).max(b_plus * b_plus) + 
                                 (c_minus * c_minus).max(d_plus * d_plus)).sqrt();
                
                let grad = if sign_phi > 0.0 { grad_plus } else { grad_minus };
    ...
    for iteration in 0..params.max_iterations {
        ...
        for i in 1..height - 1 {
            for j in 1..width - 1 {
                ...
                // Update equation
                phi[[i, j]] += params.dt * delta * (
                    params.mu * curvature[[i, j]] - 
                    params.nu - 
                    params.lambda1 * f1 + 
                    params.lambda2 * f2
                );
        ...
        // Check convergence
        let change = ((phi.clone() - phi_old).mapv(|x| x.abs())).mean().unwrap_or(0.0);
    ...
    T: Float + FromPrimitive + Debug + Send + Sync + 'static,
{
    ...
    for k in 0..num_phases {
        ...
        let phi = match k {
            0 => {
                // Vertical division
                Array2::from_shape_fn((height, width), |(_, j)| {
                    if j < width / 2 { 10.0 } else { -10.0 }
                })
            }
            1 => {
                // Horizontal division
                Array2::from_shape_fn((height, width), |(i, _)| {
                    if i < height / 2 { 10.0 } else { -10.0 }
                })
            }
            2 => {
                // Diagonal division
                Array2::from_shape_fn((height, width), |(i, j)| {
                    if i + j < (height + width) / 2 { 10.0 } else { -10.0 }
                })
            }
            _ => unreachable!(),
        };
        ...
        for (phase_idx, phi) in phi_list.iter_mut().enumerate() {
            ...
            for i in 1..height - 1 {
                for j in 1..width - 1 {
                    ...
                    // Update equation
                    phi[[i, j]] += params.dt * delta * (
                        params.mu * curvature[[i, j]] - params.nu - data_term
                    );
                }
            }
...
pub fn mask_to_level_set(mask: &ArrayView2<bool>, smoothing: Option<f64>) -> NdimageResult<Array2<f64>> {
