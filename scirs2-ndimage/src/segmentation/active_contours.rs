    ...
    for _ in 0..iterations {
        ...
        for i in 1..height - 1 {
            for j in 1..width - 1 {
                ...
                let laplacian_u = u[[i + 1, j]] + u[[i - 1, j]] + u[[i, j + 1]] + u[[i, j - 1]] - 4.0 * u[[i, j]];
                let laplacian_v = v[[i + 1, j]] + v[[i - 1, j]] + v[[i, j + 1]] + v[[i, j - 1]] - 4.0 * v[[i, j]];
    ...
    T: Float + FromPrimitive,
{
    ...
    for i in 1..height - 1 {
        for j in 1..width - 1 {
            let dx = image[[i, j + 1]].to_f64().unwrap_or(0.0) - image[[i, j - 1]].to_f64().unwrap_or(0.0);
            let dy = image[[i + 1, j]].to_f64().unwrap_or(0.0) - image[[i - 1, j]].to_f64().unwrap_or(0.0);
    ...
    T: Float + FromPrimitive + Debug + Send + Sync + 'static,
{
    ...
    for iteration in 0..params.max_iterations {
        ...
        for i in 0..num_points {
            ...
            let (external_x, external_y) = if x >= 0.0 && x < u.dim().1 as f64 - 1.0 &&
                                            y >= 0.0 && y < u.dim().0 as f64 - 1.0 {
            ...
            // Update position
            contour[[i, 0]] += params.time_step * (internal_x + curvature_x + external_x + balloon_x);
            contour[[i, 1]] += params.time_step * (internal_y + curvature_y + external_y + balloon_y);
        ...
        // Check convergence
        let movement = ((contour.clone() - prev_contour).mapv(|x| x * x)).sum().sqrt();
...
pub fn smooth_contour(
    ...
    x_padded.slice_mut(ndarray::s![2..num_points + 2]).assign(&x_coords);
    y_padded.slice_mut(ndarray::s![2..num_points + 2]).assign(&y_coords);
    ...
    for i in 0..num_points {
        ...
        for j in 0..kernel_size {
            ...
            let weight = (-((j as f64 - kernel_size as f64 / 2.0).powi(2)) / (2.0 * smoothing_factor.powi(2))).exp();
