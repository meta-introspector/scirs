...
use crate::filters::{BoundaryMode, gaussian_filter, median_filter, uniform_filter};
use crate::morphology::{distance_transform_edt, binary_erosion, binary_dilation};
...
impl SciPyBenchmarkSuite {
    ...
    pub fn run_complete_benchmark(&mut self) -> NdimageResult<&BenchmarkResults> {
        ...
        println!("Testing {} operations across {} array sizes", 
                self.config.operations.len(), self.config.array_sizes.len());
        ...
        for &operation in &self.config.operations {
            for &array_size in &self.config.array_sizes {
                for &data_type in &self.config.data_types {
                    println!("Benchmarking {:?} on {}x{} {:?} array...", 
        ...
        T: Float + FromPrimitive + Clone + Default + std::fmt::Debug + Send + Sync,
    {
        ...
        let scipy_metrics = if self.config.enable_scipy_comparison {
            self.benchmark_scipy_operation::<T>(operation, &input_data).ok()
        } else {
            None
        };
        ...
        Ok(OperationBenchmarkResult {
            ...
            array_size,
            data_type: if std::any::type_name::<T>() == "f32" { DataType::F32 } else { DataType::F64 },
        ...
        T: Float + FromPrimitive + Clone + Default + std::fmt::Debug,
    {
        ...
        if !output.status.success() {
            ...
            return Err(NdimageError::InvalidInput(format!("Python script failed: {}", stderr)));
        }
...
"#,
    ...
    fn get_scipy_operation_code(&self, operation: BenchmarkOperation) -> &str {
        match operation {
            BenchmarkOperation::GaussianFilter => "result = scipy.ndimage.gaussian_filter(data, sigma=1.0, mode='reflect')",
            BenchmarkOperation::MedianFilter => "result = scipy.ndimage.median_filter(data, size=3, mode='reflect')",
            BenchmarkOperation::UniformFilter => "result = scipy.ndimage.uniform_filter(data, size=3, mode='reflect')",
            BenchmarkOperation::BinaryErosion => "result = scipy.ndimage.binary_erosion(data > 0.5)",
            BenchmarkOperation::BinaryDilation => "result = scipy.ndimage.binary_dilation(data > 0.5)",
            BenchmarkOperation::DistanceTransform => "result = scipy.ndimage.distance_transform_edt(data > 0.5)",
    ...
    fn parse_scipy_benchmark_results(&self, output: &str, array_len: usize) -> NdimageResult<PerformanceMetrics> {
        let parts: Vec<&str> = output.trim().split(',').collect();
        if parts.len() != 4 {
            return Err(NdimageError::InvalidInput("Invalid Python output format".to_string()));
        }
        
        let avg_time = Duration::from_secs_f64(parts[0].parse().map_err(|_| {
            NdimageError::InvalidInput("Failed to parse average time".to_string())
        })?);
        
        let min_time = Duration::from_secs_f64(parts[1].parse().map_err(|_| {
            NdimageError::InvalidInput("Failed to parse min time".to_string())
        })?);
        
        let max_time = Duration::from_secs_f64(parts[2].parse().map_err(|_| {
            NdimageError::InvalidInput("Failed to parse max time".to_string())
        })?);
        
        let std_dev = Duration::from_secs_f64(parts[3].parse().map_err(|_| {
            NdimageError::InvalidInput("Failed to parse std dev".to_string())
        })?);
    ...
    fn generate_test_data<T>(&self, array_size: (usize, usize), operation: BenchmarkOperation) -> Array2<T>
    where
        ...
        match operation {
            BenchmarkOperation::BinaryErosion | BenchmarkOperation::BinaryDilation => {
                ...
                for ((i, j), elem) in data.indexed_iter_mut() {
                    *elem = if (i + j) % 3 == 0 { T::one() } else { T::zero() };
                }
            }
            BenchmarkOperation::DistanceTransform => {
                ...
                for ((i, j), elem) in data.indexed_iter_mut() {
                    ...
                    let dist_sq = (i as f64 - center_y as f64).powi(2) + (j as f64 - center_x as f64).powi(2);
                    *elem = if dist_sq < (width.min(height) / 4) as f64 * (width.min(height) / 4) as f64 {
                        T::zero()
                    } else {
                        T::one()
                    };
    ...
    fn store_results(&mut self, results: Vec<OperationBenchmarkResult>) {
        for result in results {
            ...
            self.results.operation_results
    ...
    fn compute_overall_statistics(&mut self) {
        ...
        for (operation_name, results) in &self.results.operation_results {
            ...
            let avg_ratio = results.iter()
                ...
                .fold(0.0, |acc, ratio| acc + ratio) / results.len() as f64;
        ...
        for (operation_name, results) in &self.results.operation_results {
            for result in results {
                if let Some(ratio) = result.performance_ratio {
                    ...
                    if ratio > 1.1 {
                        scirs2_faster.push(format!("{} ({}x{} {:?})", 
                            operation_name, result.array_size.0, result.array_size.1, result.data_type));
                    } else if ratio < 0.9 {
                        scipy_faster.push(format!("{} ({}x{} {:?})", 
                            operation_name, result.array_size.0, result.array_size.1, result.data_type));
                    } else {
                        similar_performance.push(format!("{} ({}x{} {:?})", 
                            operation_name, result.array_size.0, result.array_size.1, result.data_type));
                    }
                }
            }
    ...
    fn find_python_executable() -> NdimageResult<String> {
        ...
        Err(NdimageError::InvalidInput("Python executable not found".to_string()))
    }
    ...
    fn generate_summary(&self) -> String {
        ...
        summary.push_str(&format!("Total Operations Tested: {}\n", 
            self.results.overall_stats.total_operations));
        summary.push_str(&format!("Average Performance Ratio: {:.2}x\n", 
        ...
        if let Some(ref comparison) = self.results.scipy_comparison {
            ...
            summary.push_str(&format!("SciPy Faster: {} operations\n", 
                comparison.scipy_faster.len()));
            summary.push_str(&format!("Similar Performance: {} operations\n", 
                comparison.similar_performance.len()));
            summary.push_str(&format!("Maximum Speedup: {:.2}x\n", 
...
impl BenchmarkReport {
    ...
    pub fn display(&self) {
        ...
        for (operation_name, results) in &self.results.operation_results {
            ...
            for result in results {
                println!("{}x{} {:?}: {:.3}ms avg (scirs2), {:.3}ms avg (scipy), ratio: {:.2}x",
                    result.array_size.0, result.array_size.1, result.data_type,
                    result.scirs2_performance.avg_time.as_secs_f64() * 1000.0,
                    result.scipy_performance.as_ref()
...
mod tests {
    ...
    fn test_test_data_generation() {
        ...
        if let Ok(suite) = SciPyBenchmarkSuite::new(config) {
            let data = suite.generate_test_data::<f64>((64, 64), BenchmarkOperation::GaussianFilter);
