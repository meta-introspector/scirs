    ...
    F: Fn(&[T]) -> T + Send + Sync,
{
    ...
    match input.ndim() {
        1 => {
            let input_1d = input.clone().into_dimensionality::<Ix1>()
                .map_err(|_| NdimageError::DimensionError("Failed to convert to 1D".to_string()))?;
            let result_1d = generic_filter_1d(&input_1d, function, size[0], border_mode, constant_value)?;
            Ok(result_1d.into_dimensionality::<D>()
                .map_err(|_| NdimageError::DimensionError("Failed to convert from 1D".to_string()))?)
        }
        2 => {
            let input_2d = input.clone().into_dimensionality::<Ix2>()
                .map_err(|_| NdimageError::DimensionError("Failed to convert to 2D".to_string()))?;
            let result_2d = generic_filter_2d(&input_2d, function, size, border_mode, constant_value)?;
            Ok(result_2d.into_dimensionality::<D>()
                .map_err(|_| NdimageError::DimensionError("Failed to convert from 2D".to_string()))?)
        }
        _ => {
            let input_dyn = input.clone().into_dimensionality::<IxDyn>()
                .map_err(|_| NdimageError::DimensionError("Failed to convert to IxDyn".to_string()))?;
            let result_dyn = generic_filter_nd(&input_dyn, function, size, border_mode, constant_value)?;
            Ok(result_dyn.into_dimensionality::<D>()
                .map_err(|_| NdimageError::DimensionError("Failed to convert from IxDyn".to_string()))?)
        }
    }
}
    ...
    F: Fn(&[T]) -> T + Send + Sync,
{
    ...
    for i in 0..rows {
        for j in 0..cols {
            ...
            // Extract the neighborhood (center the filter window)
            for di in 0..size[0] {
                for dj in 0..size[1] {
                    let pi = i + di;
                    let pj = j + dj;
                    if pi < padded.nrows() && pj < padded.ncols() {
                        neighborhood.push(padded[[pi, pj]].clone());
                    }
                }
            }
            ...
            if neighborhood.len() == size[0] * size[1] {
                ...
                // Debug: print information about incomplete neighborhoods
                eprintln!("Warning: incomplete neighborhood at ({}, {}), got {} values, expected {}", 
                         i, j, neighborhood.len(), size[0] * size[1]);
                result[[i, j]] = input[[i, j]].clone();
            }
        }
    }
    ...
    F: Fn(&[T]) -> T + Send + Sync,
{
    ...
    let pad_width: Vec<(usize, usize)> = size.iter().map(|&s| {
        let half_size = s / 2;
        (half_size, half_size)
    }).collect();
    ...
    for flat_idx in 0..total_elements {
        ...
        if neighborhood.len() == neighborhood_size {
            ...
            // Fallback
            result[indices.as_slice()] = input[indices.as_slice()].clone();
        }
    }
...
pub mod filter_functions {
    ...
    pub fn std_dev<T: Float>(values: &[T]) -> T {
        ...
        let variance = values.iter()
            ...
            .fold(T::zero(), |acc, x| acc + x) / T::from(values.len() - 1).unwrap_or(T::one());
    ...
    pub fn variance<T: Float>(values: &[T]) -> T {
        ...
        values.iter()
            .map(|&x| (x - mean_val).powi(2))
            .fold(T::zero(), |acc, x| acc + x) / T::from(values.len()).unwrap_or(T::one())
    }
}
...
mod tests {
    ...
    fn test_generic_filter_custom_function() {
        ...
        let max_func = |values: &[f64]| -> f64 {
            values.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b))
        };
        
        let result = generic_filter(&input, max_func, &[2, 2], None, None).unwrap();
        
        // All elements should be 4.0 (the maximum value in the array)
