...
mod tests {
    ...
    fn test_generic_filter_custom_function() {
        ...
        for &val in result.iter() {
            assert_abs_diff_eq!(val, 4.0, epsilon = 1e-6);
        }
    }
    ...
    fn test_generic_filter_std_dev() {
        ...
        let result = generic_filter(&input, filter_functions::std_dev, &[3, 3], None, None).unwrap();
    ...
    fn test_generic_filter_border_modes() {
        ...
        let constant = generic_filter(&input, filter_functions::mean, &[3, 3], Some(BorderMode::Constant), Some(0.0)).unwrap();
        let reflect = generic_filter(&input, filter_functions::mean, &[3, 3], Some(BorderMode::Reflect), None).unwrap();
        let nearest = generic_filter(&input, filter_functions::mean, &[3, 3], Some(BorderMode::Nearest), None).unwrap();
