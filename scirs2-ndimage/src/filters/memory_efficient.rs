...
use crate::chunked::{ChunkConfig, ChunkProcessor, ChunkPosition, process_chunked};
...
mod tests {
    ...
    fn test_uniform_filter_chunked() {
        ...
        let result = uniform_filter_chunked(&input, &size, BorderMode::Constant, Some(config)).unwrap();
    ...
    fn test_median_filter_chunked() {
        ...
        let result = median_filter_chunked(&input, &size, BorderMode::Constant, Some(config)).unwrap();
    ...
    fn test_chunked_vs_regular() {
        ...
        // Regular filter
        let regular_result = uniform_filter(&input, &size, Some(BorderMode::Reflect), None).unwrap();
        ...
        let chunked_result = uniform_filter_chunked(&input, &size, BorderMode::Reflect, Some(config)).unwrap();
