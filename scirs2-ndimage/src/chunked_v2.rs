...
use scirs2_core::memory_efficient::{
    chunk_wise_op, chunk_wise_binary_op, ChunkingStrategy, MemoryMappedArray,
    create_mmap, AccessMode,
};
    ...
    F: Fn(&ArrayView<T, IxDyn>) -> CoreResult<Array<T, IxDyn>> + Send + Sync,
{
    ...
    let input_view = mmap_input.as_array::<D>()
    ...
    F: Fn(T, T) -> T + Send + Sync + Clone + 'static,
{
    ...
    let result_dyn = chunk_wise_binary_op(
        &array1_dyn,
        &array2_dyn,
        config.strategy,
        op,
...
mod tests {
    ...
    fn test_uniform_filter_chunked_v2() {
        ...
        let result = uniform_filter_chunked_v2(
            &input,
            &size,
            BorderMode::Constant,
            Some(config),
    ...
    fn test_binary_op_chunked() {
        ...
        let result = binary_op_chunked_v2(
            &array1,
            &array2,
            |a, b| a + b,
            Some(config),
