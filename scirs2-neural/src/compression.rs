...
pub enum QuantizationBits {
    ...
    Int4,
    /// Mixed precision
    Mixed { weight_bits: u8, activation_bits: u8 },
}
...
impl<F: Float + Debug + 'static> PostTrainingQuantizer<F> {
    ...
    fn compute_quantization_params(&self, layer_name: &str) -> Result<QuantizationParams<F>> {
        let stats = self.calibration_stats.get(layer_name)
            .ok_or_else(|| NeuralError::ComputationError("Calibration stats not found".to_string()))?;
        ...
        let (qmin, qmax) = match self.bits {
            ...
            QuantizationBits::Mixed { weight_bits, .. } => {
                match weight_bits {
                    8 => (-128i32, 127i32),
                    16 => (-32768i32, 32767i32),
                    4 => (-8i32, 7i32),
                    _ => return Err(NeuralError::InvalidArchitecture("Unsupported bit width".to_string())),
                }
            }
        };
        ...
        let (min_val, max_val) = match self.calibration {
            ...
            CalibrationMethod::KLDivergence => {
                self.compute_kl_optimal_range(stats)?
            }
            CalibrationMethod::Entropy => {
                self.compute_entropy_optimal_range(stats)?
            }
        };
    ...
    fn compute_percentiles(&self, stats: &CalibrationStatistics<F>, lower: f64, upper: f64) -> Result<(F, F)> {
    ...
    fn compute_kl_divergence(&self, stats: &CalibrationStatistics<F>, start: usize, end: usize) -> F {
    ...
    pub fn quantize_tensor(&self, layer_name: &str, tensor: &ArrayD<F>) -> Result<ArrayD<i32>> {
        let params = self.layer_params.get(layer_name)
            .ok_or_else(|| NeuralError::ComputationError("Quantization params not found".to_string()))?;
        
        let quantized = tensor.mapv(|x| {
            let scaled = x / params.scale;
            let shifted = scaled + F::from(params.zero_point).unwrap();
            let clamped = shifted.max(F::from(params.qmin).unwrap())
    ...
    pub fn get_compression_ratio(&self) -> f64 {
        match self.bits {
            ...
            QuantizationBits::Mixed { weight_bits, activation_bits } => {
...
pub enum PruningMethod {
    ...
    Structured { granularity: StructuredGranularity },
...
impl<F: Float + Debug + 'static> ModelPruner<F> {
    ...
    pub fn generate_pruning_mask(
        ...
        let mask = match &self.method {
            ...
            PruningMethod::TopK { k } => {
                self.top_k_mask(weights, *k)?
            }
            ...
            PruningMethod::GradualMagnitude { 
                ...
                let current_sparsity = self.compute_gradual_sparsity(
                    *initial_sparsity, *final_sparsity, *begin_step, *end_step
                );
    ...
    fn magnitude_based_mask(&self, weights: &ArrayD<F>, threshold: f64) -> Result<ArrayD<bool>> {
        let threshold_val = F::from(threshold).ok_or_else(|| {
            NeuralError::InvalidArchitecture("Invalid threshold".to_string())
        })?;
    ...
    fn structured_mask(&self, weights: &ArrayD<F>, granularity: &StructuredGranularity) -> Result<ArrayD<bool>> {
        match granularity {
            StructuredGranularity::Channel => {
                self.channel_wise_mask(weights)
            }
            StructuredGranularity::Filter => {
                self.filter_wise_mask(weights)
            }
    ...
    fn block_wise_mask(&self, weights: &ArrayD<F>, block_size: (usize, usize)) -> Result<ArrayD<bool>> {
        if weights.ndim() != 2 {
            return Err(NeuralError::InvalidArchitecture(
    ...
    pub fn apply_pruning_mask(
        &self,
        layer_name: &str,
        weights: &mut ArrayD<F>,
    ) -> Result<()> {
        let mask = self.pruning_masks.get(layer_name)
...
impl CompressionAnalyzer {
    ...
    pub fn generate_report(&self) -> CompressionReport {
        CompressionReport {
            ...
            size_reduction_mb: (self.original_size - self.compressed_size) as f64 / (1024.0 * 1024.0),
    ...
    fn calculate_efficiency_score(&self) -> f64 {
        ...
        let accuracy_score = 1.0 - (self.accuracy_metrics.accuracy_loss / 100.0).max(0.0).min(1.0);
...
impl std::fmt::Display for CompressionReport {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(f, "Speed Improvement: {:.2}x", self.speed_metrics.speedup_ratio)?;
        writeln!(f, "Accuracy Loss: {:.2}%", self.accuracy_metrics.accuracy_loss)?;
        writeln!(f, "Memory Usage: {:.1} MB", self.speed_metrics.memory_usage_mb)?;
...
mod tests {
    ...
    fn test_post_training_quantizer() {
        ...
        quantizer.calibrate("layer1".to_string(), &activations).unwrap();
    ...
    fn test_magnitude_based_pruning() {
        let mut pruner = ModelPruner::<f64>::new(
            PruningMethod::MagnitudeBased { threshold: 0.5 }
        );

        let weights = Array2::from_shape_vec((3, 3), 
            vec![0.1, 0.6, 0.3, 0.8, 0.2, 0.7, 0.4, 0.9, 0.1]).unwrap().into_dyn();

        let mask = pruner.generate_pruning_mask("layer1".to_string(), &weights).unwrap();
    ...
    fn test_top_k_pruning() {
        let mut pruner = ModelPruner::<f64>::new(
            PruningMethod::TopK { k: 3 }
        );

        let weights = Array2::from_shape_vec((2, 3), 
            vec![0.1, 0.6, 0.3, 0.8, 0.2, 0.7]).unwrap().into_dyn();
    ...
    fn test_compression_analyzer() {
        ...
        assert!((report.size_reduction_mb - 0.75).abs() < 0.01);
    ...
    fn test_quantization_bits() {
        ...
        if let QuantizationBits::Mixed { weight_bits, activation_bits } = mixed {
