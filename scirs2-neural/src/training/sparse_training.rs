...
impl<
        F: Float + Debug + FromPrimitive + Send + Sync + Zero + PartialOrd + ndarray::ScalarOperand,
    > SparseTrainer<F>
{
    ...
    fn compute_grasp_scores<L: Layer<f32>>(
        ...
        let data = self
            ...
            .ok_or_else(|| NeuralError::InvalidState("No calibration data set".to_string()))?;
        let targets =
            self.snip_pruner.target_batch.as_ref().ok_or_else(|| {
                NeuralError::InvalidState("No calibration targets set".to_string())
            })?;
        ...
        for (i, (param, grad)) in params.iter().zip(gradients.iter()).enumerate() {
            ...
            for ((s, &p), &g) in grasp_scores.iter_mut().zip(param.iter()).zip(grad.iter()) {
                ...
                *s = (grad_weight_product * gradient_magnitude)
                    .to_f64()
                    .unwrap_or(0.0);
            }
                            ...
                            for in_ch in 0..mask_shape[1] {
                                for h in 0..mask_shape[2] {
                                    for w in 0..mask_shape[3] {
                                        if let Some(elem) =
                                            mask.mask.get_mut([filter_idx, in_ch, h, w].as_slice())
                                        {
                                            *elem = false;
                                        }
                                    }
                            ...
                            for out_ch in 0..mask_shape[0] {
                                for in_ch in start_ch..end_ch.min(mask_shape[1]) {
                                    for h in 0..mask_shape[2] {
                                        for w in 0..mask_shape[3] {
                                            if let Some(elem) =
                                                mask.mask.get_mut([out_ch, in_ch, h, w].as_slice())
                                            {
                                                *elem = false;
                                            }
                                        }
                ...
                2 => {
                    ...
                    for block_row in 0..blocks_h {
                        for block_col in 0..blocks_w {
                            ...
                            block_scores.push((
                                (block_row, block_col),
                                block_magnitude.to_f64().unwrap_or(0.0),
                            ));
                        }
                    }
                    ...
                    let target_pruned_blocks =
                        (total_blocks as f64 * self.current_sparsity) as usize;
                    ...
                    if kernel_h >= block_h && kernel_w >= block_w {
                        ...
                        for out_ch in 0..out_channels {
                            for in_ch in 0..in_channels {
                                ...
                                block_scores.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());
                                let target_pruned = (total_blocks_per_filter as f64
        ...
        if let Some((_, first_param)) = params.iter().next() {
            match first_param.ndim() {
                2 => {
                    ...
                    if let Some((weight_name, weights)) =
                        params.iter().find(|(name, _)| name.contains("weight"))
                    {
                        let weight_params = dense_layer.parameters_mut();
                        if let Some((_, dense_weights)) = weight_params
                            .iter_mut()
                            .find(|(name, _)| name.contains("weight"))
                        {
                    ...
                    if let Some((bias_name, biases)) =
                        params.iter().find(|(name, _)| name.contains("bias"))
                    {
                        let bias_params = dense_layer.parameters_mut();
                        if let Some((_, dense_biases)) = bias_params
                            .iter_mut()
                            .find(|(name, _)| name.contains("bias"))
                        {
                ...
                4 => {
                        ...
                        params.iter().find(|(name, _)| name.contains("weight"))
                    {
                        ...
                        if let Some((_, conv_weights)) = weight_params
                            .iter_mut()
                            .find(|(name, _)| name.contains("weight"))
                        {
                        ...
                        params.iter().find(|(name, _)| name.contains("bias"))
                    {
                        ...
                        if let Some((_, conv_biases)) = bias_params
                            .iter_mut()
                            .find(|(name, _)| name.contains("bias"))
                        {
                            for (i, &val) in biases.indexed_iter() {
