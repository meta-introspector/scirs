...
impl ICM {
    ...
    pub fn new(
        ...
        let forward_model = ForwardModel::new(feature_dim, action_dim, feature_dim, hidden_sizes.clone())?;
    ...
    pub fn update(
        ...
        for i in 0..batch_size {
            ...
            // Inverse model loss
            let action_pred = self.inverse_model.predict(&phi_s.view(), &phi_s_next.view())?;
...
impl ForwardModel {
    ...
    fn predict(&self, state_feat: &ArrayView1<f32>, action: &ArrayView1<f32>) -> Result<Array1<f32>> {
...
impl InverseModel {
    ...
    fn predict(&self, state_feat: &ArrayView1<f32>, next_state_feat: &ArrayView1<f32>) -> Result<Array1<f32>> {
...
impl RND {
    ...
    pub fn compute_intrinsic_reward(&self, state: &ArrayView1<f32>) -> Result<f32> {
        ...
        let error = (&target_features - &predicted_features).mapv(|x| x.powi(2)).sum();
    ...
    pub fn update(&mut self, states: &ArrayView2<f32>) -> Result<f32> {
        ...
        for i in 0..batch_size {
            ...
            let loss = (&target_features - &predicted_features).mapv(|x| x.powi(2)).sum();
            total_loss += loss;
        }
...
impl RandomNetwork {
    fn new(input_dim: usize, output_dim: usize) -> Result<Self> {
        ...
        let mut layers: Vec<Box<dyn Layer<f32>>> = Vec::new();
        layers.push(Box::new(Dense::new(input_dim, 256, Some(Activation::ReLU))?));
...
impl NoveltyExploration {
    ...
    pub fn compute_novelty_reward(&self, state: &ArrayView1<f32>) -> f32 {
        ...
        let mut distances: Vec<f32> = self.state_buffer
            .iter()
...
impl EpisodicCuriosity {
    ...
    pub fn compute_bonus(&mut self, state_embedding: &ArrayView1<f32>) -> f32 {
        ...
        for (idx, memory_state) in self.episodic_memory.iter().enumerate() {
            let distance = (memory_state - state_embedding).mapv(|x| x.powi(2)).sum().sqrt();
...
mod tests {
    ...
    fn test_icm_creation() {
        ...
        let reward = icm.compute_intrinsic_reward(&state.view(), &action.view(), &next_state.view()).unwrap();
        assert!(reward >= 0.0);
    }
