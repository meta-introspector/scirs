...
use crate::reinforcement::environments::{Environment, Observation, Action, Reward, Info};
use ndarray::prelude::*;
...
pub trait MultiAgentEnvironment: Send + Sync {
    ...
    fn step(&mut self, actions: &[Action]) -> Result<(Vec<Observation>, Vec<Reward>, Vec<bool>, Vec<Info>)>;
...
impl MultiAgentGridWorld {
    ...
    fn get_local_observation(&self, agent_id: usize) -> Array1<f32> {
        ...
        if self.communication_enabled {
            ...
            full_obs.slice_mut(s![..observation.len()]).assign(&observation);
            full_obs.slice_mut(s![observation.len()..]).assign(&comm_data);
            return full_obs;
        }
...
impl MultiAgentEnvironment for MultiAgentGridWorld {
    ...
    fn reset(&mut self) -> Result<Vec<Observation>> {
        ...
        for i in 0..self.agent_positions.len() {
            ...
            loop {
                ...
                if self.is_valid_position((gx, gy), None) && !self.goal_positions.contains(&(gx, gy)) {
    ...
    fn step(&mut self, actions: &[Action]) -> Result<(Vec<Observation>, Vec<Reward>, Vec<bool>, Vec<Info>)> {
        if actions.len() != self.num_agents() {
            return Err(NeuralError::InvalidArgument(
                format!("Expected {} actions, got {}", self.num_agents(), actions.len())
            ));
        }
        ...
        for (i, action) in actions.iter().enumerate() {
            ...
            if self.agent_positions[i] == self.goal_positions[i] {
                ...
                let old_dist = ((x as f32 - self.goal_positions[i].0 as f32).powi(2) + 
                               (y as f32 - self.goal_positions[i].1 as f32).powi(2)).sqrt();
                let new_dist = ((self.agent_positions[i].0 as f32 - self.goal_positions[i].0 as f32).powi(2) + 
                               (self.agent_positions[i].1 as f32 - self.goal_positions[i].1 as f32).powi(2)).sqrt();
    ...
    fn observation_spaces(&self) -> Vec<usize> {
        ...
        let comm_size = if self.communication_enabled { self.agent_positions.len() * 2 } else { 0 };
...
impl MultiAgentEnvironment for PursuitEvasion {
    ...
    fn step(&mut self, actions: &[Action]) -> Result<(Vec<Observation>, Vec<Reward>, Vec<bool>, Vec<Info>)> {
        ...
        for (i, action) in actions.iter().take(self.pursuers.len()).enumerate() {
            ...
            let speed = (pursuer.velocity.0 * pursuer.velocity.0 + pursuer.velocity.1 * pursuer.velocity.1).sqrt();
        ...
        for (i, action) in actions.iter().skip(self.pursuers.len()).enumerate() {
            ...
            // Clip velocity to max speed
            let speed = (evader.velocity.0 * evader.velocity.0 + evader.velocity.1 * evader.velocity.1).sqrt();
            if speed > self.max_speed {
...
impl<E: MultiAgentEnvironment> MultiAgentWrapper<E> {
    ...
    pub fn new(env: E, agent_id: usize) -> Result<Self> {
        if agent_id >= env.num_agents() {
            return Err(NeuralError::InvalidArgument(
                format!("Agent ID {} out of range (0-{})", agent_id, env.num_agents() - 1)
            ));
        }
