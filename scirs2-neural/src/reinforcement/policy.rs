...
impl PolicyNetwork {
    ...
    pub fn new(
        ...
        for hidden_size in hidden_sizes {
            layers.push(Box::new(Dense::new(input_size, hidden_size, Some(Activation::ReLU))?));
            input_size = hidden_size;
        }
        
        // Output layer
        let output_activation = if continuous { None } else { Some(Activation::Softmax) };
        layers.push(Box::new(Dense::new(input_size, action_dim, output_activation)?));
    ...
    pub fn get_distribution_params(&self, state: &ArrayView1<f32>) -> Result<(Array1<f32>, Option<Array1<f32>>)> {
        let output = self.forward(state)?;
        
        if self.continuous {
            // For continuous actions, output is mean, use learned log_std
            let std = self.log_std.as_ref()
                .ok_or_else(|| crate::error::NeuralError::InvalidArgument("Missing log_std for continuous policy".to_string()))?
                .mapv(f32::exp);
            Ok((output, Some(std)))
        } else {
    ...
    fn sample_action(&self, state: &ArrayView1<f32>) -> Result<Array1<f32>> {
        ...
        if self.continuous {
            ...
            for i in 0..self.action_dim {
                let normal = Normal::new(params[i] as f64, std[i] as f64)
                    .map_err(|e| crate::error::NeuralError::InvalidArgument(format!("Invalid normal distribution: {}", e)))?;
    ...
    fn log_prob(&self, state: &ArrayView1<f32>, action: &ArrayView1<f32>) -> Result<f32> {
        ...
        if self.continuous {
            ...
            let action_idx = action.iter()
                .position(|&a| a > 0.5)
                .ok_or_else(|| crate::error::NeuralError::InvalidArgument("Invalid discrete action".to_string()))?;
