    ...
    pub fn sample(&self, batch_size: usize) -> Result<ExperienceBatch> {
        if self.buffer.len() < batch_size {
            return Err(crate::error::NeuralError::InvalidArgument(
                format!("Not enough experiences in buffer: {} < {}", self.buffer.len(), batch_size)
            ));
        }
...
impl PrioritizedReplayBuffer {
    ...
    fn sample_indices(&self, probs: &[f32], batch_size: usize) -> Result<Vec<usize>> {
        ...
        let dist = WeightedIndex::new(probs)
            .map_err(|e| crate::error::NeuralError::InvalidArgument(format!("Invalid weights: {}", e)))?;
    ...
    /// Calculate importance sampling weights
    fn calculate_is_weights(&self, probs: &[f32], indices: &[usize], batch_size: usize) -> Array1<f32> {
        let n = self.buffer.len() as f32;
        let min_prob = probs.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(&1.0);
...
mod tests {
    ...
    fn test_replay_buffer() {
        ...
        for i in 0..10 {
            ...
            buffer.add(state, action, i as f32, next_state, false).unwrap();
        }
