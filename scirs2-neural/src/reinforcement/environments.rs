...
impl Environment for MountainCar {
    ...
    fn step(&mut self, action: &Action) -> Result<(Observation, Reward, bool, Info)> {
        ...
        Ok((Array1::from_vec(vec![self.position, self.velocity]), reward, done, info))
    }
...
impl<E: Environment + Clone> VectorizedEnvironment<E> {
    ...
    pub fn step_all(&mut self, actions: &ArrayView2<f32>) -> Result<(Array2<f32>, Array1<f32>, Array1<bool>, Vec<Info>)> {
        if actions.shape()[0] != self.num_envs {
            return Err(crate::error::NeuralError::InvalidArgument(
                format!("Expected {} actions, got {}", self.num_envs, actions.shape()[0])
            ));
        }
