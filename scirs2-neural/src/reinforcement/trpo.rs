...
impl TRPO {
    ...
    pub fn compute_gae(
        ...
        for i in (0..rewards.len()).rev() {
            ...
            let delta = rewards[i] + (if dones[i] { 0.0 } else { self.config.gamma * next_val }) - values[i];
            gae = delta + (if dones[i] { 0.0 } else { self.config.gamma * self.config.gae_lambda * gae });
    ...
    fn line_search(
        ...
        for _ in 0..self.config.max_line_search_iter {
            // Take step
            let new_params: Vec<Array2<f32>> = old_params.iter()
                .enumerate()
