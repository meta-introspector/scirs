...
impl ContinualLearner {
    ...
    pub fn train_task(
        ...
        let result = match self.config.strategy {
            ContinualStrategy::EWC => self.train_with_ewc(
                train_data, train_labels, val_data, val_labels, epochs
            )?,
            ContinualStrategy::Replay => self.train_with_replay(
                train_data, train_labels, val_data, val_labels, epochs
            )?,
            ContinualStrategy::GEM => self.train_with_gem(
                train_data, train_labels, val_data, val_labels, epochs
            )?,
            _ => self.train_standard(
                train_data, train_labels, val_data, val_labels, epochs
            )?,
        };
    ...
    fn train_with_replay(
        ...
        for epoch in 0..epochs {
            ...
            let (combined_data, combined_labels) = self.combine_with_replay(
                train_data, train_labels
            )?;
            
            // Train on combined data
            let epoch_loss = self.compute_task_loss(&combined_data.view(), &combined_labels.view())?;
            total_loss += epoch_loss;
    ...
    fn update_task_memory(
        ...
        self.memory_bank.add_task_data(self.current_task, data, labels)
    }
    ...
    pub fn evaluate_all_tasks(&self, task_data: &[(Array2<f32>, Array1<usize>)]) -> Result<Vec<f32>> {
...
impl MemoryBank {
    ...
    fn add_task_data(
        ...
        self.task_memories.insert(task_id, TaskMemory {
            data: selected_data,
...
impl MultiTaskLearner {
    ...
    pub fn new(config: MultiTaskConfig, input_dim: usize) -> Result<Self> {
        ...
        for task_name in &config.task_names {
            let task_layers = config.task_specific_layers.get(task_name)
                .cloned()
    ...
    pub fn train(
        ...
        for epoch in 0..epochs {
            ...
            for (task_name, loss) in epoch_losses {
                task_losses.entry(task_name.clone())
    ...
    fn evaluate_task(
        ...
        if let Some(head) = self.task_heads.get(task_name) {
            ...
            Err(crate::error::NeuralError::InvalidArgument(format!("Task {} not found", task_name)))
        }
    }
}
