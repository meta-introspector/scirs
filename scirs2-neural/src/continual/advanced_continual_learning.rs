...
use crate::continual::shared_backbone::{TaskType, MultiTaskArchitecture};
use crate::layers::{Layer, Dense};
use ndarray::prelude::*;
...
impl TaskColumn {
    ...
    pub fn new(task_id: usize, input_dim: usize, layer_sizes: &[usize], output_dim: usize) -> Result<Self> {
        let mut layers: Vec<Box<dyn Layer<f32> + Send + Sync>> = Vec::new();
        let mut current_dim = input_dim;
        
        for &layer_size in layer_sizes {
            let layer = Dense::new(current_dim, layer_size, Some("relu"), &mut rand::thread_rng())?;
            layers.push(Box::new(layer));
            current_dim = layer_size;
        }
        
        // Output layer
        let output_layer = Dense::new(current_dim, output_dim, Some("softmax"), &mut rand::thread_rng())?;
    ...
    pub fn forward(&self, input: &ArrayView2<f32>, lateral_inputs: &[Array2<f32>]) -> Result<(Array2<f32>, Vec<Array2<f32>>)> {
        ...
        for (i, layer) in self.layers.iter().enumerate() {
            ...
            if i < lateral_inputs.len() && !lateral_inputs[i].is_empty() {
                ...
                current_output = layer.forward(&current_output.into_dyn())?.into_dimensionality()?;
            }
    ...
    fn combine_with_lateral(&self, current: &Array2<f32>, lateral: &Array2<f32>) -> Result<Array2<f32>> {
        // Simple concatenation strategy
        if current.shape()[0] != lateral.shape()[0] {
            return Err(NeuralError::InvalidShape("Batch size mismatch in lateral connection".to_string()));
        }
        ...
        combined.slice_mut(s![.., ..current.shape()[1]]).assign(current);
        // Copy lateral activations
        combined.slice_mut(s![.., current.shape()[1]..]).assign(lateral);
...
impl LateralConnection {
    ...
    pub fn new(
        ...
        let adapter = if source_dim != target_dim {
            Some(Dense::new(source_dim, target_dim, None, &mut rand::thread_rng())?)
        } else {
            None
        };
    ...
    pub fn apply(&self, source_activation: &Array2<f32>) -> Result<Array2<f32>> {
        if let Some(ref adapter) = self.adapter {
            adapter.forward(&source_activation.clone().into_dyn())?.into_dimensionality()
                .map_err(|e| NeuralError::InvalidShape(format!("Lateral connection error: {:?}", e)))
        } else {
...
impl ProgressiveNeuralNetwork {
    ...
    pub fn forward_task(&self, input: &ArrayView2<f32>, task_id: usize) -> Result<Array2<f32>> {
        if task_id >= self.columns.len() {
            return Err(NeuralError::InvalidArgument(format!("Task {} not found", task_id)));
        }
        ...
        for prev_col_idx in 0..task_id {
            let (_, prev_outputs) = self.columns[prev_col_idx].forward(input, &vec![Array2::zeros((0, 0)); self.config.base_layers.len()])?;
        ...
        let combined_lateral: Vec<Array2<f32>> = lateral_inputs.into_iter()
            .map(|layer_inputs| {
                if layer_inputs.is_empty() {
                    ...
                    for lateral_input in layer_inputs {
                        ...
                        combined.slice_mut(s![.., offset..end_offset]).assign(&lateral_input);
                        offset = end_offset;
                    }
                    combined
                }
    ...
    pub fn evaluate_all_tasks(&self, task_data: &[(ArrayView2<f32>, ArrayView1<usize>)]) -> Result<Vec<f32>> {
    ...
    fn compute_accuracy(&self, predictions: &ArrayView2<f32>, labels: &ArrayView1<usize>) -> Result<f32> {
        let mut correct = 0;
...
impl TaskMask {
    ...
    pub fn new(task_id: usize, layer_shapes: &[(usize, usize)]) -> Self {
        let layer_masks = layer_shapes.iter()
    ...
    pub fn apply_mask(&self, parameters: &mut [Array2<f32>]) -> Result<()> {
        if parameters.len() != self.layer_masks.len() {
            return Err(NeuralError::InvalidArgument("Parameter count mismatch".to_string()));
        }
        
        for (param, mask) in parameters.iter_mut().zip(&self.layer_masks) {
            if param.shape() != mask.shape() {
                return Err(NeuralError::InvalidShape("Shape mismatch in mask application".to_string()));
            }
...
impl PackNet {
    ...
    pub fn train_task(
        ...
        // Add task to network
        self.network.add_task(task_name.clone(), &[64, 32], output_dim, task_type)?;
        
        // Create mask for this task
        let backbone_params = self.network.backbone_parameters();
        let layer_shapes: Vec<(usize, usize)> = backbone_params.iter()
    ...
    fn merge_masks(&self, target_mask: &mut TaskMask, existing_mask: &TaskMask) -> Result<()> {
        for (target_layer, existing_layer) in target_mask.layer_masks.iter_mut()
    ...
    /// Training iteration
    fn train_iteration(&mut self, task_name: &str, data: &ArrayView2<f32>, _labels: &ArrayView1<usize>) -> Result<f32> {
        // Simplified training
    ...
    pub fn evaluate_all_tasks(&self, task_data: &HashMap<String, (ArrayView2<f32>, ArrayView1<usize>)>) -> Result<HashMap<String, f32>> {
    ...
    pub fn get_utilization_stats(&self) -> HashMap<String, f32> {
        ...
        for (task_id, mask) in &self.task_masks {
            let avg_capacity: f32 = mask.available_capacity.iter().sum::<f32>() / mask.available_capacity.len() as f32;
...
impl LearningWithoutForgetting {
    ...
    pub fn train_task(
        ...
        self.model.add_task(task_name.clone(), &[64, 32], output_dim, task_type)?;
    ...
    fn compute_task_loss(&self, predictions: &ArrayView2<f32>, labels: &ArrayView1<usize>) -> Result<f32> {
        let mut loss = 0.0;
    ...
    fn compute_distillation_loss(&self, student_logits: &ArrayView2<f32>, teacher_logits: &ArrayView2<f32>) -> Result<f32> {
        let mut loss = 0.0;
        ...
        for i in 0..batch_size {
            for j in 0..num_classes {
                let student_prob = self.softmax_with_temp(student_logits.row(i), self.config.temperature)[j];
                let teacher_prob = self.softmax_with_temp(teacher_logits.row(i), self.config.temperature)[j];
