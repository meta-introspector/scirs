...
impl AggregationStrategy for FedNova {
    fn aggregate(&mut self, updates: &[ClientUpdate]) -> Result<Vec<Array2<f32>>> {
        if updates.is_empty() {
            return Err(NeuralError::InvalidArgument("No updates to aggregate".to_string()));
        }
        ...
        let total_effective_data: f32 = updates.iter()
            .zip(&effective_steps)
        ...
        for param_idx in 0..num_params {
            ...
            for (client_idx, (update, &eff_steps)) in updates.iter().zip(&effective_steps).enumerate() {
        ...
        if self.use_momentum {
            ...
            if let Some(ref mut buffers) = self.momentum_buffers {
                for (i, (update, buffer)) in aggregated_updates.iter_mut()
...
impl FedNovaClient {
    ...
    pub fn local_train(
        ...
        for epoch in 0..epochs {
            let (epoch_loss, epoch_correct) = self.train_epoch(
                global_weights,
                data,
                labels,
            )?;
...
impl FedNovaCoordinator {
    ...
    pub fn coordinate_round(
        ...
        let tau_eff = client_updates.iter()
            .map(|u| u.local_steps as f32)
            .sum::<f32>() / client_updates.len() as f32;
        ...
        if self.adaptive_tau && self.tau_history.len() > 5 {
            ...
            let recent_tau_avg = self.tau_history.iter()
                .rev()
                .take(5)
                .sum::<f32>() / 5.0;
        ...
        let standard_updates: Vec<ClientUpdate> = client_updates.into_iter()
            .map(|u| u.into())
            .collect();
    ...
    pub fn get_tau_stats(&self) -> TauStatistics {
        ...
        let variance = self.tau_history.iter()
            .map(|&tau| (tau - mean).powi(2))
            .sum::<f32>() / self.tau_history.len() as f32;
        
        TauStatistics {
            current_tau: *self.tau_history.last().unwrap(),
            mean_tau: mean,
            std_tau: variance.sqrt(),
            min_tau: self.tau_history.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).cloned().unwrap_or(0.0),
            max_tau: self.tau_history.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).cloned().unwrap_or(0.0),
        }
    }
}
