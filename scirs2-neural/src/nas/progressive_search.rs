...
use crate::nas::{
    SearchResult, SearchSpace, SearchSpaceConfig,
    architecture_encoding::ArchitectureEncoding, search_space::LayerType
};
use std::sync::Arc;
...
impl ProgressiveSearch {
    ...
    pub fn add_stage_results(&mut self, results: Vec<SearchResult>) -> Result<()> {
        ...
        if let Some(best_result) = self.get_best_result_in_stage(self.current_stage) {
            ...
            let performance = best_result.metrics.values().sum::<f64>() / best_result.metrics.len() as f64;
    ...
    pub fn should_advance_stage(&self) -> bool {
        ...
        let current_evaluations = self.stage_results.get(self.current_stage)
        ...
        if self.performance_history.len() >= 2 {
            ...
            let improvement = (current_performance - previous_performance) / previous_performance.abs();
        ...
        match &self.config.expansion_strategy {
            ...
            ExpansionStrategy::Composite(strategies) => {
                for strategy in strategies {
                    match strategy {
                        ExpansionStrategy::AdaptiveComplexity => self.expand_by_complexity(&mut expanded_config)?,
                        ExpansionStrategy::LayerTypeExpansion => self.expand_by_layer_types(&mut expanded_config)?,
                        ExpansionStrategy::ScaleExpansion => self.expand_by_scale(&mut expanded_config)?,
                        ExpansionStrategy::ConnectionExpansion => self.expand_by_connections(&mut expanded_config)?,
    ...
    fn expand_by_complexity(&self, config: &mut SearchSpaceConfig) -> Result<()> {
        let complexity_factor = 1.0 + (self.current_stage as f64 * self.config.max_complexity_increase / self.config.num_stages as f64);
    ...
    fn expand_by_layer_types(&self, config: &mut SearchSpaceConfig) -> Result<()> {
        ...
        match self.current_stage {
            1 => {
                ...
                config.layer_types.push(LayerType::Attention { num_heads: 4, key_dim: 64 });
                config.layer_types.push(LayerType::LayerNorm);
            },
            2 => {
                // Add recurrent layers
                config.layer_types.push(LayerType::LSTM { units: 128, return_sequences: false });
                config.layer_types.push(LayerType::GRU { units: 128, return_sequences: false });
            },
            3 => {
                // Add more complex convolutions
                config.layer_types.push(LayerType::Conv2D { filters: 128, kernel_size: (5, 5), stride: (1, 1) });
                config.layer_types.push(LayerType::Conv2D { filters: 256, kernel_size: (7, 7), stride: (1, 1) });
            },
            _ => {
                // Add embedding and reshape layers
                config.layer_types.push(LayerType::Embedding { vocab_size: 10000, embedding_dim: 128 });
    ...
    fn estimate_architecture_complexity(&self, architecture: &Arc<dyn ArchitectureEncoding>) -> Result<f64> {
    ...
    pub fn get_global_best(&self) -> Option<&Arc<dyn ArchitectureEncoding>> {
        ...
        for stage_results in &self.stage_results {
            if let Some(best_in_stage) = stage_results.iter().max_by(|a, b| {
                ...
                let performance = best_in_stage.metrics.values().sum::<f64>() / best_in_stage.metrics.len() as f64;
    ...
    pub fn has_converged(&self) -> bool {
        ...
        let recent_improvements: Vec<f64> = self.performance_history
            .windows(2)
            .take(3)
    ...
    pub fn generate_evolution_report(&self) -> String {
        ...
        report.push_str(&format!("Stagnation Counter: {}\n", self.stagnation_counter));
