...
impl ArchitectureEvaluator {
    ...
    pub fn evaluate(&self,
                    ...
                    data: &ArrayView2<f32>,
                    labels: &ArrayView1<usize>) -> Result<EvaluationMetrics> {
        ...
        for batch_start in (0..num_samples).step_by(self.batch_size) {
            ...
            for (i, true_label) in batch_labels.iter().enumerate() {
                ...
                let pred_label = pred_probs.iter()
                    .enumerate()
                ...
                if let Some(ref k_values) = self.metrics_config.top_k {
                    ...
                    sorted_indices.sort_by(|&a, &b| 
                        pred_probs[b].partial_cmp(&pred_probs[a]).unwrap()
                    );
    ...
    fn predict_batch(&self, batch_data: &ArrayView2<f32>) -> Result<Vec<Vec<f64>>> {
        ...
        for _ in 0..batch_size {
            ...
            use rand::Rng;
            let idx = rand::thread_rng().gen_range(0..num_classes);
            probs[idx] = 0.9;
            // Normalize
    ...
    fn compute_precision_recall_f1(&self,
                                   predictions: &[usize],
                                   true_labels: &[usize],
    ...
    pub fn evaluate_robustness(&self,
                              model: &Sequential<f32>,
                              data: &ArrayView2<f32>,
                              labels: &ArrayView1<usize>,
                              noise_levels: &[f64]) -> Result<HashMap<String, f64>> {
        ...
        for &noise_level in noise_levels {
            ...
            if let Some(accuracy) = metrics.get("accuracy") {
                robustness_metrics.insert(
                    format!("accuracy_noise_{:.2}", noise_level),
                    *accuracy
                );
            }
        }
        ...
        if !robustness_metrics.is_empty() {
            let avg_accuracy: f64 = robustness_metrics.values().sum::<f64>() 
                                  / robustness_metrics.len() as f64;
    ...
    pub fn evaluate_multi_dataset(&self,
                                model: &Sequential<f32>,
...
impl HardwareAwareEvaluator {
    ...
    pub fn evaluate_with_constraints(&self,
                                   model: &Sequential<f32>,
                                   data: &ArrayView2<f32>,
                                   labels: &ArrayView1<usize>)
        ...
        metrics.insert("constraint_violations".to_string(), constraint_violations as f64);
