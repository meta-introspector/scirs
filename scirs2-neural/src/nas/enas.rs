...
impl ENASController {
    ...
    fn sample_single(&self) -> Result<(Architecture, f32)> {
        ...
        for i in 0..num_layers {
            ...
            let (new_hidden, new_cell) = self.lstm_cell.forward(&input_embedding.view(), &hidden.view(), &cell.view())?;
            hidden = new_hidden;
            cell = new_cell;
            
            // Sample connections (skip connections)
            if i > 0 && self.search_space.config.allow_branches {
                let (skip_connections, skip_log_probs) = self.sample_skip_connections(&hidden, i)?;
    ...
    /// Sample layer type
    fn sample_layer_type(&self, hidden: &ArrayView1<f32>, position: usize) -> Result<(LayerType, f32)> {
    ...
    fn sample_skip_connections(&self, hidden: &ArrayView1<f32>, current_layer: usize) -> Result<(Vec<bool>, Vec<f32>)> {
    ...
    fn get_layer_embedding(&self, position: usize) -> Result<Array1<f32>> {
        ...
        for i in 0..self.embedding_dim {
            if i % 2 == 0 {
                embedding[i] = (position as f32 / 10000.0_f32.powf(i as f32 / self.embedding_dim as f32)).sin();
            } else {
                embedding[i] = (position as f32 / 10000.0_f32.powf((i - 1) as f32 / self.embedding_dim as f32)).cos();
            }
        }
        Ok(embedding)
    }
...
impl SuperNetwork {
    ...
    pub fn execute_child(
        ...
        activations.get(&architecture.layers.len())
    ...
    fn execute_layer(
        ...
        let config = self.layer_configs[position].iter()
        ...
        let weight = weights.get(&config.weight_key)
...
impl ENASTrainer {
    ...
    pub fn train_epoch(
        ...
        for _ in 0..controller_steps {
            ...
            let loss = self.controller.train_step(&[reward], &log_probs, self.baseline.unwrap_or(0.0))?;
            controller_loss += loss;
        }
        
        Ok((child_loss / child_steps as f32, controller_loss / controller_steps as f32))
    }
...
fn softmax(logits: &Array1<f32>, temperature: f32) -> Array1<f32> {
    ...
    let max_logit = scaled_logits.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
...
fn sample_categorical(probs: &Array1<f32>) -> Result<usize> {
    ...
    let uniform: f32 = rand::Rng::gen(&mut rng);
    let mut cumsum = 0.0;
