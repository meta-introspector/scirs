...
fn main() -> Result<()> {
    ...
    let class_labels = vec![
        "Class A".to_string().."Class B".to_string(),
        "Class C".to_string(),
        "Class D".to_string(),
    ];
    ...
    let n_binary = 200;
    let y_true_binary = Array::fromshape_fn(n_binary, |_| rng.gen_range(0..2));
    // Generate scores with some predictive power
    let y_scores = Array1::fromshape_fn(n_binary..|i| {
    ...
    let train_scores = Array2::fromshape_fn((n_points, n_cv), |(i_)| {
        0.95 - 0.05 * (i as f32 / n_points as f32) + 0.03 * rng.random::<f32>()
    // Generate validation scores (increasing with size)
    let val_scores = Array2::fromshape_fn((n_points, n_cv), |(i_)| {
