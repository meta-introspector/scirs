...
use scirs2_neural::jit::{
    JITCompiler, JITOperation, ElementWiseOp, ActivationType, TargetArchitecture,
    OptimizationLevel, CodeGenSettings, FusionStrategy
};
...
fn demonstrate_elementwise_operations(compiler: &JITCompiler) -> Result<()> {
    ...
    // Create test data
    let size = 1024;
    let a = Array::linspace(0.0f32, 1.0, size * size).into_shape((size, size)).unwrap().into_dyn();
    ...
    for (name, op) in operations {
        ...
        println!(\"✅ {} - Compile: {:.2}ms, Execute: {:.2}ms\", 
                name, compile_time.as_millis(), execute_time.as_millis());
    }
...
fn demonstrate_matmul_optimization(compiler: &JITCompiler) -> Result<()> {
    ...
    for (m, k, n) in sizes {
        ...
        println!(\"✅ {}x{}x{} MatMul - Compile: {:.2}ms, Execute: {:.2}ms\",
                m, k, n, compile_time.as_millis(), execute_time.as_millis());
    }
...
fn demonstrate_operation_fusion(compiler: &JITCompiler) -> Result<()> {
    ...
    println!(\"✅ Fusion speedup: {:.2}x (Individual: {}ms, Fused: {}ms)\",
            speedup, individual_time.as_millis(), fused_time.as_millis());
    
    // Analyze fusion opportunities
    let operations = vec![conv_op, relu_op, add_op];
    let opportunities = compiler.fusion_optimizer.analyze_fusion_opportunities(&operations);
    println!(\"🔍 Found {} fusion opportunities\", opportunities.len());
    
    for (i, opportunity) in opportunities.iter().enumerate() {
        println!(\"   {}. Ops {}->{}: {:.2}x gain, {} bytes saved\",
                i + 1,
...
mod tests {
    ...
    fn test_fusion_opportunities() {
        ...
        let opportunities = compiler.fusion_optimizer.analyze_fusion_opportunities(&vec![op1, op2]);
