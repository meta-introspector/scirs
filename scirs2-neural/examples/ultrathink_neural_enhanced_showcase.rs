...
fn quick_training_test(
    ...
    let mut rng = thread_rng();
    let initial_loss = rng.gen_range(1.0..3.0);
    let training_improvement = rng.gen_range(0.1..0.5);
...
fn training_step(
    ...
    let mut rng = thread_rng();
    let base_loss = rng.gen_range(0.1..2.0);
...
fn test_memory_strategy(strategy: MemoryStrategy) -> Result<MemoryStats> {
    ...
    let stats = match strategy {
        MemoryStrategy::Standard => MemoryStats {
            peak_memory_mb: rng.gen_range(200.0..500.0),
            throughput: rng.gen_range(1000.0..2000.0),
            efficiency_ratio: 1.0,
        },
        MemoryStrategy::GradientCheckpointing => MemoryStats {
            peak_memory_mb: rng.gen_range(100.0..250.0),
            throughput: rng.gen_range(800.0..1500.0),
            efficiency_ratio: 1.8,
        },
        MemoryStrategy::MixedPrecision => MemoryStats {
            peak_memory_mb: rng.gen_range(80.0..200.0),
            throughput: rng.gen_range(1500.0..3000.0),
            efficiency_ratio: 2.2,
        },
        MemoryStrategy::ActivationCompression => MemoryStats {
            peak_memory_mb: rng.gen_range(60.0..150.0),
            throughput: rng.gen_range(1200.0..2200.0),
...
fn collect_performance_analytics() -> Result<PerformanceAnalytics> {
    ...
    Ok(PerformanceAnalytics {
        total_operations: rng.gen_range(10000..100000),
        avg_latency_ms: rng.gen_range(0.5..5.0),
        peak_throughput: rng.gen_range(5000.0..20000.0),
        memory_efficiency: rng.gen_range(0.7..0.95),
        simd_utilization: rng.gen_range(0.6..0.9),
        ...
        performance_trend: rng.gen_range(-0.05..0.15),
    })
}
