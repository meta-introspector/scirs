...
impl Vocabulary {
    ...
    fn tokenize_text(&self, text: &str, max_length: usize) -> Vec<usize> {
        ...
        for word in words.iter().take(max_length - 2) { // Reserve space for START and END
...
impl TextDataset {
    fn new(max_length: usize, num_classes: usize) -> Self {
        ...
        let common_words = vec![
            ...
            "love", "hate", "like", "dislike", "enjoy", "fun", "boring", "interesting",
            "fast", "slow", "easy", "hard", "simple", "complex", "clear", "confusing",
            "helpful", "useless", "important", "trivial", "big", "small", "long", "short",
            "nice", "mean", "kind", "rude", "smart", "stupid", "funny", "serious",
            "happy", "sad", "excited", "disappointed", "satisfied", "frustrated",
            "the", "a", "an", "and", "or", "but", "is", "are", "was", "were",
            "this", "that", "these", "those", "very", "quite", "really", "so",
            "movie", "book", "product", "service", "experience", "quality", "price",
        ];
    ...
    fn create_synthetic_dataset(num_samples: usize, num_classes: usize, max_length: usize) -> Self {
        ...
        let positive_words = vec!["excellent", "amazing", "great", "love", "fantastic", "wonderful"];
        let negative_words = vec!["terrible", "awful", "hate", "horrible", "disappointing", "bad"];
        ...
        let patterns = vec![
            ...
            (&neutral_words, "This is an okay product with average quality"),
        ];
...
impl Dataset<f32> for TextDataset {
    ...
    fn get(&self, index: usize) -> scirs2_neural::error::Result<(ArrayD<f32>, ArrayD<f32>)> {
        ...
        let tokens = self.vocab.tokenize_text(&self.texts[index], self.max_length);
...
fn build_text_model(
    ...
    model.add(Dense::new(input_size, embedding_dim * 2, Some("relu"), rng)?);
    model.add(Dropout::new(0.1, rng)?);
    
    // Simulate LSTM processing with dense layers
    model.add(Dense::new(embedding_dim * 2, hidden_dim, Some("tanh"), rng)?);
    ...
    // Classification head
    model.add(Dense::new(hidden_dim / 2, num_classes, Some("softmax"), rng)?);
...
fn calculate_text_metrics(predictions: &ArrayD<f32>, targets: &ArrayD<f32>) -> HashMap<String, f32> {
...
fn train_text_classifier() -> StdResult<()> {
    ...
    println!("   - Classes: {} (Positive, Negative, Neutral)", num_classes);
    ...
    for i in 0..3.min(train_dataset.texts.len()) {
        println!("   [Class {}]: {}", train_dataset.labels[i], train_dataset.texts[i]);
    }
    ...
    let sample_tokens = ndarray::concatenate(ndarray::Axis(0), &batch_tokens.iter().map(|a| a.view()).collect::<Vec<_>>())?;
    let sample_targets = ndarray::concatenate(ndarray::Axis(0), &batch_targets.iter().map(|a| a.view()).collect::<Vec<_>>())?;
...
fn demonstrate_attention_model() -> StdResult<()> {
    ...
    println!("   - Parameters: {}", model.params().iter().map(|p| p.len()).sum::<usize>());
...
mod tests {
    ...
    fn test_metrics_calculation() {
        let predictions = Array::from_shape_vec((2, 3), vec![
            0.1, 0.8, 0.1,  // Class 1
            0.7, 0.2, 0.1,  // Class 0
        ]).unwrap().into_dyn();
