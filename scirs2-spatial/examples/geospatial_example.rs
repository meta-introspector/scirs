...
fn distance_calculations_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (city1, city2, coord1, coord2) in long_distance_pairs {
        ...
        println!("  Difference: {:.1} km ({:.2}%)", difference, difference / vincenty_km * 100.0);
    }
...
fn navigation_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Distance: {:.1} km ({:.1} nautical miles)", distance_km, distance_nm);
    println!("Initial bearing: {:.1}°", normalize_bearing(initial_bearing_deg));
    println!("Final bearing: {:.1}°", normalize_bearing(final_bearing_deg));
    ...
    for i in 0..=num_waypoints {
        ...
        println!("  {:.0}%: ({:.4}°, {:.4}°) - {:.0} km from departure", 
                 fraction * 100.0, waypoint.0, waypoint.1, waypoint_distance / 1000.0);
    }
    ...
    println!("Distance from departure: {:.1} km", mid_distance_from_departure);
    println!("Distance from destination: {:.1} km", mid_distance_from_destination);
...
fn coordinate_transformations_example() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if lat.abs() < 84.0 {
            ...
            match utm_result {
                Ok((easting, northing, zone, letter)) => {
                    println!("{:>20} {:>12.4} {:>12.4} {:>15.0} {:>15.0} {:>6} {:>3}", 
                             name, lat, lon, easting, northing, zone, letter);
                }
                Err(_) => {
                    println!("{:>20} {:>12.4} {:>12.4} {:>15} {:>15} {:>6} {:>3}", 
    ...
    println!("{:>20} {:>15} {:>15}", "Location", "Web Merc X", "Web Merc Y");

    for (name, (lat, lon)) in &locations {
        if lat.abs() < 85.05 { // Web Mercator limit
            ...
            match web_merc_result {
                Ok((x, y)) => {
                    ...
                    if lat_diff > 1e-6 || lon_diff > 1e-6 {
                        println!("    Round trip error: Δlat={:.8}, Δlon={:.8}", lat_diff, lon_diff);
                    }
                }
                Err(_) => {
...
fn flight_path_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, waypoint) in planned_route.iter().enumerate() {
        if i > 0 {
            ...
            println!("  Waypoint {}: ({:.2}°, {:.2}°) - {:.0} km, bearing {:.0}°", 
                     i, waypoint.0, waypoint.1, segment_distance / 1000.0, 
                     normalize_bearing(bearing * 180.0 / PI));
        } else {
            println!("  Waypoint {}: ({:.2}°, {:.2}°) - Departure", 
    ...
    println!("Great circle distance: {:.0} km", great_circle_distance / 1000.0);
    ...
    println!("Current position: ({:.2}°, {:.2}°)", current_position.0, current_position.1);
    println!("Cross-track distance: {:.1} km", cross_track / 1000.0);
    println!("Along-track distance: {:.0} km from departure", along_track / 1000.0);
    
    let remaining_distance = great_circle_distance - along_track;
    println!("Estimated remaining distance: {:.0} km", remaining_distance / 1000.0);
...
fn spherical_geometry_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (lat, lon, description) in test_points {
        ...
        println!("  ({:.1}°, {:.1}°) - {}: {}", lat, lon, description, 
    ...
    println!("\nCaribbean Sea (approximate area): {:.0} km²", caribbean_area);
...
fn gps_tracking_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut total_distance = 0.0;
    let mut elevations = vec![1200.0, 1250.0, 1320.0, 1400.0, 1520.0, 1650.0, 1520.0, 1400.0, 1320.0, 1200.0];
    ...
    for (i, ((lat, lon), elevation)) in trail_points.iter().zip(elevations.iter()).enumerate() {
        ...
        if i > 0 {
            ...
            println!("  {}: ({:.4}°, {:.4}°) - {:.0}m - Start", time, lat, lon, elevation);
        }
    }

    let total_km = total_distance / 1000.0;
    let total_elevation_gain = elevations.iter()
    ...
    println!("  Elevation range: {:.0}m - {:.0}m", min_elevation, max_elevation);
...
fn geospatial_analysis_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut station_distances: Vec<_> = monitoring_stations.iter()
        .map(|(name, coord)| {
    ...
    for (name, distance) in &station_distances {
        ...
        println!("  {}: {:.1} km - P-wave: {:.1}s, S-wave: {:.1}s", 
                 name, distance, p_wave_time, s_wave_time);
    }

    // Find the centroid of the monitoring network
    let centroid_lat = monitoring_stations.iter().map(|(_, (lat, _))| lat).sum::<f64>() / monitoring_stations.len() as f64;
    let centroid_lon = monitoring_stations.iter().map(|(_, (_, lon))| lon).sum::<f64>() / monitoring_stations.len() as f64;
    let centroid = (centroid_lat, centroid_lon);

    println!("\nNetwork analysis:");
    println!("  Network centroid: ({:.4}°, {:.4}°)", centroid.0, centroid.1);
    
    let distance_to_centroid = haversine_distance(earthquake, centroid) / 1000.0;
    println!("  Distance from earthquake to network centroid: {:.1} km", distance_to_centroid);

    // Calculate network coverage
    let mut max_station_distance = 0.0;
    for i in 0..monitoring_stations.len() {
        for j in (i+1)..monitoring_stations.len() {
            let distance = haversine_distance(monitoring_stations[i].1, monitoring_stations[j].1) / 1000.0;
            max_station_distance = max_station_distance.max(distance);
        }
    }
    
    println!("  Maximum inter-station distance: {:.1} km", max_station_distance);
    ...
    println!("  Estimated detection threshold at closest station: M{:.1}", estimated_magnitude_threshold);
