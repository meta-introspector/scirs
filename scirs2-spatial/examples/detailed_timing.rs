...
use scirs2_spatial::{
    distance::euclidean,
    simd_distance::{simd_euclidean_distance, parallel_pdist},
};
...
fn main() {
    ...
    println!("{:>8} {:>12} {:>15} {:>15}", "Size", "Operations", "Time (μs)", "Ops/sec");
    println!("{}", "-".repeat(55));
    ...
    println!("{:>8} {:>15} {:>15} {:>15}", "Size", "Time (μs)", "Ops/Million", "MB/sec");
    println!("{}", "-".repeat(60));

    for &size in &test_sizes {
        if size > 400 { continue; } // Skip very large sizes to avoid long computation
    ...
    println!("Throughput: {:.1} million operations/second", ops_per_sec / 1_000_000.0);
    ...
    println!("\nMatrix 100×10 ({} distances): {} μs", expected_ops, elapsed.as_micros());
    println!("Matrix throughput: {:.1} million distance calculations/second", 
