...
async fn test_core_functionality(points: &ndarray::Array2<f64>) -> Result<(), Box<dyn std::error::Error>> {
...
async fn test_quantum_inspired(points: &ndarray::Array2<f64>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (indices, distances) = quantum_nn.search(&points.view(), &array![2.0, 2.0].view(), 3).await?;
...
async fn test_neuromorphic(points: &ndarray::Array2<f64>) -> Result<(), Box<dyn std::error::Error>> {
...
async fn test_hybrid_algorithms(points: &ndarray::Array2<f64>) -> Result<(), Box<dyn std::error::Error>> {
...
async fn test_gpu_acceleration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Detected tensor core capabilities: {:?}", tensor_caps.has_tensor_cores);
