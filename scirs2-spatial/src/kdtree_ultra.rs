//! Ultra-optimized KD-Tree implementations with advanced performance features
//!
...
//!
//! ```
//! use scirs2_spatial::kdtree_ultra::{UltraKDTree, KDTreeConfig};
//! use ndarray::array;
//!
...
//! let (indices, distances) = kdtree.knn_search_ultra(&query.view(), 2)?;
...
pub struct UltraKDTree {
    /// Tree nodes stored in cache-friendly layout
    nodes: Vec<UltraKDNode>,
...
impl UltraKDTree {
    ...
    /// Ultra-fast k-nearest neighbors search with vectorization
    pub fn knn_search_ultra(
        &self,
    ...
    fn search_knn_ultra(
        ...
        if !node.node_info.is_leaf {
            ...
            if first_child != 0 {
                self.search_knn_ultra(first_child as usize, query, k, heap);
            }
    ...
    pub fn range_search(
        ...
        self.search_range_ultra(self.root_index.unwrap(), query, radius, &mut result);
        Ok(result)
    }

    /// Ultra-optimized range search implementation
    fn search_range_ultra(
        &self,
        node_index: usize,
        query: &ArrayView1<f64>,
        radius: f64,
        ...
        if !node.node_info.is_leaf {
            ...
            if node.node_info.left_child != 0 && query_coord - radius <= split_coord {
                self.search_range_ultra(node.node_info.left_child as usize, query, radius, result);
            }

            // Search right child
            if node.node_info.right_child != 0 && query_coord + radius >= split_coord {
                self.search_range_ultra(node.node_info.right_child as usize, query, radius, result);
            }
        }
    }
    ...
    fn calculate_depth(nodes: &[UltraKDNode], root_index: Option<usize>) -> usize {
