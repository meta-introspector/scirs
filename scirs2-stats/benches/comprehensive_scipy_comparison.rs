...
use rand::prelude::*;
use rand_distr::{Normal, StandardNormal, Uniform, Exponential, Gamma as GammaDist};
...
fn bench_descriptive_stats(c: &mut Criterion) {
    ...
    for &n in &config.sample_sizes {
        ...
        group.bench_with_input(
            ...
            &data,
            |b, data| b.iter(|| black_box(mean(&data.view())))
        );
        
        // Variance
        group.bench_with_input(
            BenchmarkId::new("variance", n),
            &data,
            |b, data| b.iter(|| black_box(var(&data.view(), 1)))
        );
        
        // Standard deviation
        group.bench_with_input(
            BenchmarkId::new("std", n),
            &data,
            |b, data| b.iter(|| black_box(std(&data.view(), 1)))
        );
        
        // Skewness
        group.bench_with_input(
            BenchmarkId::new("skewness", n),
            &data,
            |b, data| b.iter(|| black_box(skew(&data.view(), false)))
        );
        
        // Kurtosis
        group.bench_with_input(
            BenchmarkId::new("kurtosis", n),
            &data,
            |b, data| b.iter(|| black_box(kurtosis(&data.view(), false)))
        );
        
        // Median
        group.bench_with_input(
            BenchmarkId::new("median", n),
            &data,
            |b, data| b.iter(|| {
...
fn bench_correlation(c: &mut Criterion) {
    ...
    for &n in &config.sample_sizes {
        ...
        group.bench_with_input(
            ...
            &(&x, &y),
            |b, (x, y)| b.iter(|| black_box(pearsonr(&x.view(), &y.view())))
        );
        ...
        group.bench_with_input(
            ...
            &(&x, &y),
            |b, (x, y)| b.iter(|| black_box(spearmanr(&x.view(), &y.view(), None)))
        );
        
        // Kendall tau
        if n <= 1000 { // Kendall tau has O(n²) complexity
            group.bench_with_input(
                BenchmarkId::new("kendalltau", n),
                &(&x, &y),
                |b, (x, y)| b.iter(|| black_box(kendalltau(&x.view(), &y.view(), false)))
            );
        }
    }
...
fn bench_statistical_tests(c: &mut Criterion) {
    ...
    for &n in &config.sample_sizes {
        ...
        // T-tests
        group.bench_with_input(
            BenchmarkId::new("ttest_1samp", n),
            &data,
            |b, data| b.iter(|| black_box(ttest_1samp(&data.view(), 0.0, Alternative::TwoSided, "propagate")))
        );
        
        group.bench_with_input(
            BenchmarkId::new("ttest_ind", n),
            &(&data, &data2),
            |b, (d1, d2)| b.iter(|| black_box(ttest_ind(&d1.view(), &d2.view(), true, Alternative::TwoSided, "propagate")))
        );
        ...
        group.bench_with_input(
            ...
            &(&data, &data2),
            |b, (d1, d2)| b.iter(|| black_box(mann_whitney(&d1.view(), &d2.view(), true, Alternative::TwoSided, "auto")))
        );
        
        // Normality tests
        if n <= 5000 { // Shapiro-Wilk is expensive for large samples
            group.bench_with_input(
                BenchmarkId::new("shapiro_wilk", n),
                &data,
                |b, data| b.iter(|| black_box(shapiro_wilk(&data.view())))
            );
        }
        ...
        group.bench_with_input(
            ...
            &data,
            |b, data| b.iter(|| black_box(anderson_darling(&data.view(), "norm")))
        );
    }
    ...
    for &n in &config.sample_sizes {
        ...
        if n >= 50 {
            ...
            group.bench_with_input(
                ...
                &(&x_multi, &y),
                |b, (x, y)| b.iter(|| black_box(regression::linear::fit_multiple(&x.view(), &y.view())))
            );
        }
...
fn bench_quantiles(c: &mut Criterion) {
    ...
    for &n in &config.sample_sizes {
        ...
        group.bench_with_input(
            ...
            |b, data| b.iter(|| {
                let mut data_copy = data.clone();
                black_box(quantiles_array(&mut data_copy.view_mut(), &quantiles.view(), "linear"))
            })
        );
    }
...
fn bench_dispersion(c: &mut Criterion) {
    ...
    for &n in &config.sample_sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("cv", n),
            &data,
            |b, data| b.iter(|| black_box(coefficient_of_variation(&data.view(), "propagate")))
        );
        
        // Gini coefficient
        group.bench_with_input(
            BenchmarkId::new("gini", n),
            &data,
            |b, data| b.iter(|| black_box(gini(&data.view())))
        );
    }
