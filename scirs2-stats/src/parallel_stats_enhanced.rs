...
impl AdaptiveThreshold {
    ...
    pub fn calculate(&self, element_size: usize, operation_complexity: f64) -> usize {
        ...
        let adjusted_threshold = (self.base_threshold as f64 
            * simd_factor 
            / core_factor 
...
impl<F: Float + NumCast + Send + Sync> ParallelHistogram<F> {
    ...
    {
        if data.is_empty() {
            return Err(StatsError::InvalidArgument("Cannot create histogram from empty data".to_string()));
        }
        ...
        D: Data<Elem = F> + Sync,
    {
        ...
        if data.len() < parallel_threshold {
            ...
            let local_histograms: Vec<Vec<usize>> = par_chunks(data.as_slice().unwrap(), chunk_size)
                .map(|chunk| {
    ...
    D: Data<Elem = F> + Sync,
{
    ...
    let norm_const = F::one() / (F::from(2.0 * PI).unwrap().sqrt() * bandwidth * F::from(n).unwrap());
...
impl<F: Float + NumCast + Send + Sync + SimdUnifiedOps> ParallelMovingStats<F> {
        ...
        D: Data<Elem = F>,
    {
        if window_size == 0 || window_size > data.len() {
            return Err(StatsError::InvalidArgument("Invalid window size".to_string()));
        }
    ...
    pub fn moving_std(&self, ddof: usize) -> StatsResult<Array1<F>> {
        ...
        if self.window_size <= ddof {
            return Err(StatsError::InvalidArgument("Window size must be greater than ddof".to_string()));
        }
        ...
        if output_len < parallel_threshold {
            ...
            for i in 0..output_len {
                ...
                let variance = window.iter()
                    ...
                    .fold(F::zero(), |acc, x| acc + x) / divisor;
...
impl<F: Float + NumCast + Send + Sync> ParallelCrossValidation<F> {
        ...
        S: Fn(&ArrayBase<D, Ix2>, &Array1<F>) -> StatsResult<F> + Send + Sync,
    {
        ...
        if n_samples != y.len() {
            return Err(StatsError::DimensionMismatch("X and y must have same number of samples".to_string()));
        }

        // Create fold indices
        let indices: Vec<usize> = if self.shuffle {
            use crate::random::permutation_int;
            permutation_int(n_samples, self.random_state).unwrap().to_vec()
        } else {
            (0..n_samples).collect()
        };
    ...
    D: Data<Elem = F> + Sync,
{
    ...
    for &idx in control_vars {
        if idx >= n_vars {
            return Err(StatsError::InvalidArgument(
                format!("Control variable index {} out of bounds", idx)
            ));
        }
    }
