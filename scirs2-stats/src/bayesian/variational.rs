...
impl VariationalBayesianRegression {
    ...
    pub fn fit(
        ...
        if n_features != self.n_features {
            return Err(StatsError::DimensionMismatch(
                format!("x features ({}) must match model features ({})", n_features, self.n_features)
            ));
        }
        ...
        let prior_precision = scirs2_linalg::inv(&self.prior_cov_beta.view(), None)
            .map_err(|e| StatsError::ComputationError(format!("Failed to invert prior covariance: {}", e)))?;
    ...
    fn update_beta_variational(
        ...
        self.cov_beta = scirs2_linalg::inv(&precision_beta.view(), None)
    ...
    fn compute_elbo(
        ...
        // E[log p(y|X,β,τ)]
        let diff = yty - 2.0 * self.mean_beta.dot(xty) + self.mean_beta.dot(&xtx.dot(&self.mean_beta));
        ...
        let prior_det = scirs2_linalg::det(&prior_precision.view(), None)
        ...
        // -E[log q(β)]
        let var_det = scirs2_linalg::det(&self.cov_beta.view(), None)
            .map_err(|e| StatsError::ComputationError(format!("Failed to compute determinant: {}", e)))?;
        let beta_entropy = 0.5 * self.n_features as f64 * (1.0 + (2.0 * PI).ln()) + 0.5 * var_det.ln();
...
impl VariationalARD {
    ...
    fn update_alpha_ard(&mut self) -> Result<()> {
        for i in 0..self.n_features {
            ...
            self.rate_alpha[i] = self.prior_rate_alpha + 0.5 * (self.mean_beta[i].powi(2) + self.var_beta[i]);
        }
    ...
    fn update_tau_ard(
        ...
        self.rate_tau = self.prior_rate_tau + 0.5 * (yty - 2.0 * self.mean_beta.dot(&xtx.dot(&self.mean_beta)) + quadratic_term);
    ...
    fn compute_elbo_ard(
        ...
        for i in 0..self.n_features {
            ...
            prior_term += 0.5 * expected_log_alpha_i - 0.5 * (2.0 * PI).ln()
...
fn normal_ppf(p: f64) -> Result<f64> {
    if p <= 0.0 || p >= 1.0 {
        return Err(StatsError::InvalidArgument("p must be between 0 and 1".to_string()));
    }
    ...
    if p < p_low {
        ...
        -((((c[0] * q + c[1]) * q + c[2]) * q + c[3]) * q + c[4]) * q + c[5]) /
        ((((d[0] * q + d[1]) * q + d[2]) * q + d[3]) * q + 1.0)
    }.map_err(|e| StatsError::ComputationError(format!("Failed to compute normal PPF: {}", e)))
}
...
fn digamma(x: f64) -> f64 {
    ...
    x.ln() - 0.5 * inv_x - inv_x2 / 12.0 + inv_x2 * inv_x2 / 120.0 - inv_x2 * inv_x2 * inv_x2 / 252.0
}
