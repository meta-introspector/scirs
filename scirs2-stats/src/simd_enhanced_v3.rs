    ...
    D: Data<Elem = F>,
{
    ...
    if n_samples == 0 || n_features == 0 {
        return Err(StatsError::InvalidArgument("Data matrix cannot be empty".to_string()));
    }
    ...
    for i in 0..n_samples {
        for j in (i + 1)..n_samples {
            ...
            let distance = match metric {
                ...
                _ => return Err(StatsError::InvalidArgument(format!("Unknown metric: {}", metric))),
            };
    ...
    F: Float + NumCast + SimdUnifiedOps,
{
    if x.len() != y.len() {
        return Err(StatsError::DimensionMismatch(
            format!("Vectors must have the same length: {} vs {}", x.len(), y.len())
        ));
    }
...
pub fn manhattan_distance_simd<F>(
    x: &ArrayView1<F>,
    y: &ArrayView1<F>,
) -> StatsResult<F>
where
    ...
    F: Float + NumCast + SimdUnifiedOps,
{
    ...
    if norm_product <= F::epsilon() {
        return Err(StatsError::InvalidArgument("Cannot compute cosine distance for zero vectors".to_string()));
    }
    ...
    F: Float + NumCast + SimdUnifiedOps,
{
        ...
        D: Data<Elem = F>,
    {
        if data.len() < self.window_size {
            return Err(StatsError::InvalidArgument("Data length must be >= window size".to_string()));
        }
        ...
        if self.window_size > 16 {
            ...
            let mut window_sum = data.slice(ndarray::s![0..self.window_size])
                .iter()
                .fold(F::zero(), |acc, &x| acc + x);
        ...
        D: Data<Elem = F>,
    {
        ...
        if self.window_size <= ddof {
            return Err(StatsError::InvalidArgument("Window size must be > ddof".to_string()));
        }
        ...
        D: Data<Elem = F>,
    {
        ...
        for i in 0..n_windows {
            ...
            result[i] = window.iter()
                .fold(window[0], |min_val, &x| if x < min_val { x } else { min_val });
        }
        ...
        D: Data<Elem = F>,
    {
        ...
        for i in 0..n_windows {
            ...
            // Use scalar path for now (SIMD min/max need different implementation)
            result[i] = window.iter()
                .fold(window[0], |max_val, &x| if x > max_val { x } else { max_val });
        }
    ...
    D: Data<Elem = F>,
{
    ...
    if bins == 0 {
        return Err(StatsError::InvalidArgument("Number of bins must be positive".to_string()));
    }
    ...
    let (min_val, max_val) = if let Some((min, max)) = range {
        ...
        let min_val = data.iter().fold(data[0], |min, &x| if x < min { x } else { min });
        let max_val = data.iter().fold(data[0], |max, &x| if x > max { x } else { max });
        (min_val, max_val)
    };
    
    if max_val <= min_val {
        return Err(StatsError::InvalidArgument("Invalid range: max must be > min".to_string()));
    }
