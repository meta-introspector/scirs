...
impl KaplanMeierEstimator {
    ...
    pub fn fit(
        ...
        if durations.len() != event_observed.len() {
            return Err(StatsError::DimensionMismatch(
                format!("durations length ({}) must match event_observed length ({})", 
                    durations.len(), event_observed.len())
            ));
        }
        
        if durations.len() == 0 {
            return Err(StatsError::InvalidArgument("Input arrays cannot be empty".to_string()));
        }
        ...
        let mut time_event_pairs: Vec<(f64, bool)> = durations.iter()
        ...
        let confidence_intervals = if let Some(conf_level) = confidence_level {
            Some(Self::calculate_confidence_intervals(&survival_function, &at_risk, &events, conf_level)?)
        } else {
            None
        };
        
        // Calculate median survival time
        let median_survival_time = Self::calculate_median_survival(&event_times, &survival_function);
    ...
    /// Calculate median survival time
    fn calculate_median_survival(event_times: &Array1<f64>, survival_function: &Array1<f64>) -> Option<f64> {
    ...
    pub fn predict(&self, times: ArrayView1<f64>) -> Result<Array1<f64>> {
        ...
        for (i, &t) in times.iter().enumerate() {
            if t < 0.0 {
                return Err(StatsError::InvalidArgument("Times must be non-negative".to_string()));
            }
    ...
    pub fn compare_two_groups(
        ...
        while i < combined_data.len() {
            ...
            if total_events > 0.0 && total_at_risk > 0.0 {
                ...
                let var_contrib = (at_risk1 * at_risk2 * total_events * (total_at_risk - total_events)) 
...
impl CoxPHModel {
    ...
    pub fn fit(
        ...
        for iteration in 0..max_iter {
            ...
            let (log_likelihood, gradient, hessian) = Self::partial_likelihood_derivatives(
                &durations, &events, &covariates, &beta
            )?;
            
            // Check convergence
            if (log_likelihood - prev_log_likelihood).abs() < tol {
                let covariance_matrix = Self::invert_hessian(&hessian)?;
                let (baseline_times, baseline_cumulative_hazard) = Self::calculate_baseline_hazard(
                    &durations, &events, &covariates, &beta
                )?;
        ...
        Err(StatsError::ConvergenceError(
            format!("Cox model failed to converge after {} iterations", max_iter)
        ))
    }
    ...
    pub fn predict_hazard_ratio(&self, covariates: ArrayView2<f64>) -> Result<Array1<f64>> {
        ...
        if covariates.ncols() != self.coefficients.len() {
            return Err(StatsError::DimensionMismatch(
                format!("covariates has {} features, expected {}", 
                    covariates.ncols(), self.coefficients.len())
            ));
        }
...
impl AFTModel {
    ...
    pub fn fit(
        ...
        let coefficients = scirs2_linalg::solve(&xtx.view(), &xty.view(), None)
            .map_err(|e| StatsError::ComputationError(format!("Failed to solve regression: {}", e)))?;
...
impl ExtendedCoxModel {
    ...
    pub fn fit_stratified(
        ...
        for iteration in 0..max_iter {
            ...
            let (log_likelihood, gradient, hessian) = Self::stratified_partial_likelihood_derivatives(
                &durations, &events, &covariates, &beta, &strata_array, n_strata
            )?;
        ...
        Err(StatsError::ConvergenceError(
            format!("Extended Cox model failed to converge after {} iterations", max_iter)
        ))
    }
    ...
    fn stratified_partial_likelihood_derivatives(
        ...
        for stratum in 0..n_strata {
            ...
            let (stratum_ll, stratum_grad, stratum_hess) = Self::stratum_partial_likelihood(
                durations, events, covariates, beta, &sorted_indices
            )?;
...
impl CompetingRisksModel {
    ...
    pub fn fit(
        ...
        // For now, implement single risk at a time
        // In full implementation, would fit all risks simultaneously
        ...
        // Fit model for the target risk using modified data
        let (modified_durations, modified_events, modified_weights) = Self::prepare_fine_gray_data(
            &durations, &events, target_risk
        )?;
        
        // Initialize coefficients for target risk
        let mut beta = Array1::zeros(n_features);
        let mut prev_log_likelihood = f64::NEG_INFINITY;
        ...
        for iteration in 0..max_iter {
            ...
            let (log_likelihood, gradient, hessian) = Self::subdistribution_partial_likelihood(
                &modified_durations, &modified_events, &covariates, &modified_weights, &beta
            )?;
        ...
        Err(StatsError::ConvergenceError(
            format!("Competing risks model failed to converge after {} iterations", max_iter)
        ))
    }
    ...
    fn prepare_fine_gray_data(
        ...
        let mut modified_events = Array1::zeros(n_samples);
        ...
        for i in 0..n_samples {
            if events[i] == target_risk {
                ...
                let km_prob = Self::interpolate_km_probability(&censoring_km.0, &censoring_km.1, durations[i]);
    ...
    fn calculate_baseline_cif(
        ...
        Ok((Array1::from_vec(times), Array1::from_vec(cumulative_incidence)))
    }
    ...
    pub fn predict_cumulative_incidence(
        ...
        for i in 0..n_samples {
            ...
            for (j, &t) in times.iter().enumerate() {
                ...
                let baseline_value = Self::interpolate_baseline_cif(baseline_times, baseline_cif, t);
