...
impl ErrorMessages {
    ...
    pub fn length_mismatch(array1_name: &str, len1: usize, array2_name: &str, len2: usize) -> StatsError {
        StatsError::dimension_mismatch(format!(
            ...
            array1_name, len1, array2_name, len2,
    ...
    pub fn nan_detected(context: &str) -> StatsError {
        StatsError::invalid_argument(format!(
            ...
            context,
            "Remove NaN values or use functions that handle missing data explicitly."
        ))
    }
    ...
    pub fn infinite_value_detected(context: &str) -> StatsError {
        StatsError::invalid_argument(format!(
            ...
            context,
            "Check for overflow conditions or extreme values in your data."
        ))
    }
    ...
    pub fn singular_matrix(matrix_name: &str) -> StatsError {
        StatsError::computation(format!(
            ...
            matrix_name,
            "Check for linear dependencies in your data or add regularization."
        ))
    }
    ...
    pub fn validate_finite_array(data: &[f64], name: &str) -> StatsResult<()> {
        ...
        for (i, &value) in data.iter().enumerate() {
            ...
            if value.is_infinite() {
                return Err(ErrorMessages::infinite_value_detected(&format!("{}[{}]", name, i)));
            }
        }
        Ok(())
    }
    ...
    pub fn validate_same_length<T, U>(
        ...
        if arr1.len() != arr2.len() {
            return Err(ErrorMessages::length_mismatch(
                arr1_name, arr1.len(), arr2_name, arr2.len()
            ));
        }
        Ok(())
    }
...
impl RecoverySuggestions {
    ...
    pub fn get_suggestions(error: &StatsError) -> Vec<(String, PerformanceImpact)> {
        match error {
            StatsError::DimensionMismatch(_) => vec![
                ("Reshape arrays to have compatible dimensions".to_string(), PerformanceImpact::None),
                ("Use broadcasting-compatible operations".to_string(), PerformanceImpact::Minimal),
                ("Transpose matrices if needed".to_string(), PerformanceImpact::Minimal),
            ],
            StatsError::InvalidArgument(msg) if msg.contains("empty") => vec![
                ("Provide non-empty input arrays".to_string(), PerformanceImpact::None),
                ("Use default values for empty inputs".to_string(), PerformanceImpact::Minimal),
                ("Filter out empty arrays before processing".to_string(), PerformanceImpact::Minimal),
            ],
            StatsError::InvalidArgument(msg) if msg.contains("NaN") => vec![
                ("Remove NaN values using data.dropna()".to_string(), PerformanceImpact::Minimal),
                ("Use interpolation to fill NaN values".to_string(), PerformanceImpact::Moderate),
                ("Use statistical methods that handle NaN explicitly".to_string(), PerformanceImpact::Minimal),
            ],
            StatsError::ComputationError(msg) if msg.contains("singular") => vec![
                ("Add regularization (e.g., ridge regression)".to_string(), PerformanceImpact::Minimal),
                ("Use pseudo-inverse instead of inverse".to_string(), PerformanceImpact::Moderate),
                ("Check for multicollinearity in data".to_string(), PerformanceImpact::None),
            ],
            StatsError::ConvergenceError(_) => vec![
                ("Increase maximum iterations".to_string(), PerformanceImpact::Moderate),
                ("Adjust convergence tolerance".to_string(), PerformanceImpact::None),
                ("Use different initial values".to_string(), PerformanceImpact::None),
                ("Try a different optimization algorithm".to_string(), PerformanceImpact::Significant),
            ],
            StatsError::DomainError(_) => vec![
                ("Check parameter bounds and constraints".to_string(), PerformanceImpact::None),
                ("Scale or normalize input data".to_string(), PerformanceImpact::Minimal),
                ("Use robust statistical methods".to_string(), PerformanceImpact::Moderate),
            ],
            _ => vec![
                ("Check input data for validity".to_string(), PerformanceImpact::None),
                ("Refer to function documentation".to_string(), PerformanceImpact::None),
            ],
        }
    }
    ...
    pub fn get_context_suggestions(operation: &str) -> HashMap<String, Vec<String>> {
        ...
        match operation {
            "correlation" => {
                suggestions.insert("data_preparation".to_string(), vec![
                ...
                suggestions.insert("performance".to_string(), vec![
