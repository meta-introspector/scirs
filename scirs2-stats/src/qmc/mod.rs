...
impl SobolSequence {
    ...
    fn initialize_direction_numbers(dimension: usize) -> StatsResult<Vec<Vec<u32>>> {
        ...
        for dim in 1..dimension {
            ...
            for i in degree..32 {
                let mut val = direction_numbers[dim][i - degree] ^ (direction_numbers[dim][i - degree] >> degree);
    ...
    fn generate_scramble_matrices(dimension: usize, seed: Option<u64>) -> StatsResult<Vec<Array2<u32>>> {
        let mut rng = match seed {
            Some(s) => StdRng::seed_from_u64(s),
            None => {
                use std::time::{SystemTime, UNIX_EPOCH};
