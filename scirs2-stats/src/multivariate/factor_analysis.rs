...
impl FactorAnalysis {
    ...
    pub fn fit(&self, data: ArrayView2<f64>) -> Result<FactorAnalysisResult> {
        ...
        if self.n_factors >= n_features {
            return Err(StatsError::InvalidArgument(
                format!("n_factors ({}) must be less than n_features ({})", 
        ...
        for iteration in 0..self.max_iter {
            ...
            let log_likelihood = self.compute_log_likelihood(&centered_data, &new_loadings, &new_psi)?;
        ...
        if n_iter == self.max_iter {
            return Err(StatsError::ConvergenceError(
                format!("EM algorithm failed to converge after {} iterations", self.max_iter)
            ));
        }
        ...
        let final_log_likelihood = self.compute_log_likelihood(&centered_data, &rotated_loadings, &psi)?;
    ...
    fn initialize_parameters(&self, data: &Array2<f64>) -> Result<(Array2<f64>, Array1<f64>)> {
        ...
        let (_u, s, vt) = data.svd(false, true)
            .map_err(|e| StatsError::ComputationError(format!("SVD initialization failed: {}", e)))?;
    ...
    fn e_step(
        ...
        // Invert M
        let m_inv = scirs2_linalg::inv(&m.view(), None)
            .map_err(|e| StatsError::ComputationError(format!("Failed to invert M matrix: {}", e)))?;
    ...
    fn m_step(
        ...
        let sum_e_hht_inv = scirs2_linalg::inv(&sum_e_hht.view(), None)
            .map_err(|e| StatsError::ComputationError(format!("Failed to invert sum E[HH^T]: {}", e)))?;
    ...
    fn compute_log_likelihood(
        ...
        let sigma_inv = scirs2_linalg::inv(&sigma.view(), None)
            .map_err(|e| StatsError::ComputationError(format!("Failed to invert covariance: {}", e)))?;
        
        // Compute log-likelihood
        let mut log_likelihood = 0.0;
        let log_det_term = -0.5 * n_features as f64 * (2.0 * std::f64::consts::PI).ln() - 0.5 * det_sigma.ln();
    ...
    fn compute_factor_scores(
        ...
        let lt_psi_inv_l_inv = scirs2_linalg::inv(&lt_psi_inv_l.view(), None)
            .map_err(|e| StatsError::ComputationError(format!("Failed to compute factor score weights: {}", e)))?;
    ...
    fn compute_explained_variance(&self, loadings: &Array2<f64>) -> Array1<f64> {
        let factor_variances = loadings.axis_iter(Axis(1))
            .map(|col| col.dot(&col))
            .collect::<Vec<_>>();
        
        let total_variance: f64 = factor_variances.iter().sum();
        
        Array1::from_vec(factor_variances)
    ...
    pub fn transform(&self, data: ArrayView2<f64>, result: &FactorAnalysisResult) -> Result<Array2<f64>> {
        check_array_finite(&data, "data")?;
        
        if data.ncols() != result.mean.len() {
            return Err(StatsError::DimensionMismatch(
                format!("data has {} features, expected {}", data.ncols(), result.mean.len())
            ));
        }
...
pub mod efa {
    ...
    fn compute_correlation_eigenvalues(data: ArrayView2<f64>) -> Result<Array1<f64>> {
        ...
        let eigenvalues = corr.eigh(ndarray_linalg::UPLO::Upper)
    ...
    pub fn kmo_test(data: ArrayView2<f64>) -> Result<f64> {
        ...
        let corr_inv = scirs2_linalg::inv(&corr.view(), None)
            .map_err(|e| StatsError::ComputationError(format!("Failed to invert correlation matrix: {}", e)))?;
    ...
    pub fn bartlett_test(data: ArrayView2<f64>) -> Result<(f64, f64)> {
        ...
        let det_corr = scirs2_linalg::det(&corr.view(), None)
            .map_err(|e| StatsError::ComputationError(format!("Failed to compute determinant: {}", e)))?;
