...
impl Default for NamingConventions {
    fn default() -> Self {
        ...
        parameter_names.insert("data".to_string(), vec!["data".to_string(), "x".to_string(), "array".to_string()]);
        parameter_names.insert("axis".to_string(), vec!["axis".to_string(), "dim".to_string()]);
        parameter_names.insert("degrees_of_freedom".to_string(), vec!["ddof".to_string(), "df".to_string()]);
        parameter_names.insert("alpha".to_string(), vec!["alpha".to_string(), "significance".to_string()]);
        parameter_names.insert("alternative".to_string(), vec!["alternative".to_string(), "alt".to_string()]);
        parameter_names.insert("method".to_string(), vec!["method".to_string(), "algorithm".to_string()]);
        ...
        let function_patterns = vec![
            ...
            FunctionPattern {
                description: "Distribution functions should be named after the distribution".to_string(),
...
impl APIConsistencyValidator {
    ...
    fn validate_parameter_naming(&mut self) -> StatsResult<()> {
        ...
        for (module, functions) in &self.function_registry.functions_by_module {
            for function in functions {
                for param in &function.parameters {
                    let entry = parameter_analysis.entry(param.name.clone())
                        .or_insert_with(|| ParameterUsageAnalysis::new());
                    
                    entry.add_usage(module.clone(), function.name.clone(), param.param_type.clone());
                }
            }
        }
        ...
        for (param_name, analysis) in &parameter_analysis {
            if analysis.type_variations.len() > 1 {
                ...
                let inconsistency = APIInconsistency {
                    ...
                    severity: Severity::Major,
                    impact: "May cause confusion for users and complicate generic programming".to_string(),
                };
            ...
            if let Some(standard_names) = self.config.naming_conventions.parameter_names.get(param_name) {
        ...
        details.push(format!("Analyzed {} unique parameter names", parameter_analysis.len()));
        details.push(format!("Found {} type inconsistencies", inconsistencies_found));
    ...
    fn validate_return_type_consistency(&mut self) -> StatsResult<()> {
        let mut check = ValidationCheck {
            ...
            status: ValidationStatus::NotRun,
            description: "Checking consistency of return types across similar functions".to_string(),
            details: String::new(),
            severity: Severity::Major,
        };
        ...
        for (category, functions) in &self.function_registry.functions_by_category {
            ...
            if functions.len() > 3 && return_type_patterns[category].len() > functions.len() / 2 {
                let inconsistency = APIInconsistency {
                    ...
                    suggested_fix: "Standardize return types within functional categories".to_string(),
                    severity: Severity::Major,
    ...
    fn validate_error_handling_consistency(&mut self) -> StatsResult<()> {
        ...
        for (module, functions) in &self.function_registry.functions_by_module {
            for function in functions {
                ...
                if !function.return_type.is_result && function.category != FunctionCategory::Utilities {
                    let warning = ValidationWarning {
                        message: format!(
                        ...
                        suggestion: "Consider returning Result<T> for fallible operations".to_string(),
                    };
                ...
                for error_type in &function.error_types {
                    error_patterns.entry(error_type.clone())
        ...
        for (error_type, functions) in &error_patterns {
            if !expected_error_types.contains(error_type) {
                let inconsistency = APIInconsistency {
                    ...
                    severity: Severity::Major,
                    impact: "Inconsistent error handling makes error recovery difficult".to_string(),
                };
        ...
        check.details = format!(
            ...
            self.function_registry.functions_by_module.values()
                .map(|funcs| funcs.len()).sum::<usize>(),
            inconsistencies_found
        );
    ...
    fn validate_documentation_quality(&mut self) -> StatsResult<()> {
        ...
        if completion_rate < 0.8 {
            let inconsistency = APIInconsistency {
                ...
                suggested_fix: "Add comprehensive documentation to all public functions".to_string(),
                severity: Severity::Minor,
    ...
    fn validate_performance_characteristics(&mut self) -> StatsResult<()> {
            ...
            for function in functions {
                if function.category == FunctionCategory::DescriptiveStats && 
                   ...
                   !function.name.contains("parallel") {
                    ...
                    let has_simd_variant = functions.iter()
                        .any(|f| f.name == simd_variant_name);
        ...
        if !performance_issues.is_empty() {
            let warning = ValidationWarning {
                ...
                suggestion: "Consider implementing SIMD variants for performance-critical functions".to_string(),
            };
    ...
    fn validate_scipy_compatibility(&mut self) -> StatsResult<()> {
            ...
            for (module, functions) in &self.function_registry.functions_by_module {
                for function in functions {
                    if function.name == *scipy_name {
                        ...
                        if !self.check_parameter_compatibility(&function.parameters, &expected_sig.parameters) {
        ...
        if !compatibility_issues.is_empty() {
            let inconsistency = APIInconsistency {
                ...
                suggested_fix: "Implement missing functions or adjust parameters for SciPy compatibility".to_string(),
                severity: Severity::Major,
    ...
    fn get_expected_scipy_functions(&self) -> HashMap<String, FunctionSignature> {
        ...
        functions.insert("mean".to_string(), FunctionSignature {
            name: "mean".to_string(),
        ...
        functions.insert("std".to_string(), FunctionSignature {
            name: "std".to_string(),
            module: "stats".to_string(),
            category: FunctionCategory::DescriptiveStats,
            parameters: vec![
    ...
    fn check_parameter_compatibility(&self, actual: &[ParameterInfo], expected: &[ParameterInfo]) -> bool {
    ...
    fn compute_validation_summary(&mut self) {
        ...
        let passed_checks = self.results.check_results.iter()
            .filter(|c| c.status == ValidationStatus::Passed)
            .count();
        let failed_checks = self.results.check_results.iter()
            .filter(|c| c.status == ValidationStatus::Failed)
            .count();
        let warning_count = self.results.warnings.len();
        let critical_issues = self.results.inconsistencies.iter()
    ...
    pub fn register_function(&mut self, function_sig: FunctionSignature) {
        ...
        self.function_registry.functions_by_module
            .entry(function_sig.module.clone())
            .or_insert_with(Vec::new)
            .push(function_sig.clone());
        
        // Add to category registry
        self.function_registry.functions_by_category
        ...
        for param in &function_sig.parameters {
            let usage = self.function_registry.parameter_usage
...
impl ValidationReport {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        report.push_str(&format!("**Consistency Score:** {:.1}%\n\n", self.results.summary.consistency_score));
...
pub fn validate_api_consistency(config: Option<ValidationConfig>) -> StatsResult<ValidationResults> {
