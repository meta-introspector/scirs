...
impl ScipyBenchmarkComparison {
    ...
    pub fn run_comprehensive_comparison(&self) -> StatsResult<ScipyComparisonReport> {
                ...
                match self.compare_function(function_name, data_size) {
                    ...
                    Err(e) => {
                        function_comparisons.push(FunctionComparison {
                            ...
                            memory: None,
                            status: ComparisonStatus::Error { error: e.to_string() },
        ...
        let recommendations = self.generate_recommendations(&function_comparisons, &performance_analysis, &accuracy_analysis);
...
"#;
        ...
        let info: serde_json::Value = serde_json::from_str(&output_str)
            .map_err(|e| StatsError::ComputationError(format!("Failed to parse environment info: {}", e)))?;
        
        Ok(ScipyEnvironmentInfo {
            python_version: info["python_version"].as_str().unwrap_or("unknown").to_string(),
            scipy_version: info["scipy_version"].as_str().unwrap_or("unknown").to_string(),
            numpy_version: info["numpy_version"].as_str().unwrap_or("unknown").to_string(),
            blas_info: info["blas_info"].as_str().unwrap_or("unknown").to_string(),
            packages: info["packages"].as_object().unwrap_or(&serde_json::Map::new())
                .iter()
    ...
    fn compare_function(&self, function_name: &str, data_size: usize) -> StatsResult<FunctionComparison> {
        // Generate test data
        ...
        let performance = PerformanceComparison {
            ...
            ratio: scirs2_result.execution_time.as_nanos() as f64 / scipy_result.execution_time.as_nanos() as f64,
    ...
    fn generate_test_data(&self, size: usize) -> StatsResult<TestData> {
        ...
        Ok(TestData {
            ...
            secondary: Array1::from_vec(data.iter().map(|x| x + 0.1 * normal.sample(&mut rng)).collect()),
    ...
    /// Benchmark scirs2-stats function
    fn benchmark_scirs2_function(&self, function_name: &str, test_data: &TestData) -> StatsResult<BenchmarkResult> {
        let start_time = Instant::now();
        ...
        let result = match function_name {
            ...
            "kurtosis" => {
                vec![crate::descriptive::kurtosis(&test_data.primary.view(), true, false)?]
            }
            "pearsonr" => {
                let corr = crate::correlation::pearson_r(&test_data.primary.view(), &test_data.secondary.view())?;
                vec![corr]
            }
            "spearmanr" => {
                let corr = crate::correlation::spearman_r(&test_data.primary.view(), &test_data.secondary.view())?;
                vec![corr]
            }
            "ttest_1samp" => {
                let result = crate::tests::ttest::ttest_1samp(&test_data.primary.view(), 0.0, crate::tests::ttest::Alternative::TwoSided, "propagate")?;
                vec![result.statistic, result.pvalue]
            }
            _ => {
                return Err(StatsError::NotImplemented(format!("Function {} not implemented in benchmark", function_name)));
            }
        };
    ...
    /// Benchmark SciPy function
    fn benchmark_scipy_function(&self, function_name: &str, test_data: &TestData) -> StatsResult<BenchmarkResult> {
        ...
        let result: serde_json::Value = serde_json::from_str(&output_str)
            .map_err(|e| StatsError::ComputationError(format!("Failed to parse SciPy result: {}", e)))?;
        
        let execution_time = Duration::from_secs_f64(
            result["execution_time"].as_f64().unwrap_or(0.0)
        );
    ...
    fn generate_scipy_script(&self, function_name: &str, test_data: &TestData) -> StatsResult<String> {
        let data_primary: Vec<String> = test_data.primary.iter().map(|x| x.to_string()).collect();
        let data_secondary: Vec<String> = test_data.secondary.iter().map(|x| x.to_string()).collect();
...
"#,
            ...
            _ => {
                return Err(StatsError::NotImplemented(
                    format!("SciPy script generation not implemented for {}", function_name)
                ));
            }
        };
    ...
    fn compare_accuracy(&self, scirs2_result: &[f64], scipy_result: &[f64]) -> StatsResult<AccuracyComparison> {
    ...
    fn determine_comparison_status(
        ...
        // Check accuracy
        if matches!(accuracy.assessment, AccuracyAssessment::Unacceptable | AccuracyAssessment::Poor) {
        ...
        if performance.ratio > self.config.performance_tolerance {
            return ComparisonStatus::FailedPerformance {
                details: format!("Performance ratio: {:.2} (limit: {:.2})", 
        ...
        if performance.ratio > 1.5 {
            warnings.push(format!("Performance is {:.1}x slower than SciPy", performance.ratio));
        }
    ...
    fn generate_summary(&self, comparisons: &[FunctionComparison]) -> ComparisonSummary {
        ...
        let tests_passed = comparisons.iter().filter(|c| matches!(c.status, ComparisonStatus::Passed)).count();
        let tests_with_warnings = comparisons.iter().filter(|c| matches!(c.status, ComparisonStatus::PassedWithWarnings { .. })).count();
        ...
        let performance_issues: Vec<String> = comparisons.iter()
            .filter(|c| matches!(c.status, ComparisonStatus::FailedPerformance { .. }))
            .map(|c| c.function_name.clone())
            .collect();
        
        let accuracy_issues: Vec<String> = comparisons.iter()
            .filter(|c| matches!(c.status, ComparisonStatus::FailedAccuracy { .. }))
            .map(|c| c.function_name.clone())
            .collect();
        
        let avg_performance_ratio = comparisons.iter()
            .map(|c| c.performance.ratio)
        ...
        let avg_relative_diff = comparisons.iter()
    ...
    fn analyze_performance(&self, comparisons: &[FunctionComparison]) -> PerformanceAnalysis {
        ...
        let variance = ratios.iter().map(|r| (r - average_ratio).powi(2)).sum::<f64>() / ratios.len() as f64;
        let ratio_std_dev = variance.sqrt();
        
        let faster_functions: Vec<(String, f64)> = comparisons.iter()
            .filter(|c| c.performance.ratio < 1.0)
            .map(|c| (c.function_name.clone(), c.performance.ratio))
            .collect();
        
        let slower_functions: Vec<(String, f64)> = comparisons.iter()
            .filter(|c| c.performance.ratio > 1.0)
            .map(|c| (c.function_name.clone(), c.performance.ratio))
            .collect();
    ...
    fn analyze_accuracy(&self, comparisons: &[FunctionComparison]) -> AccuracyAnalysis {
        let relative_diffs: Vec<f64> = comparisons.iter()
            .map(|c| c.accuracy.relative_difference)
            .collect();
        
        let average_relative_diff = relative_diffs.iter().sum::<f64>() / relative_diffs.len() as f64;
        let max_relative_diff = relative_diffs.iter().fold(0.0, |acc, &x| acc.max(x));
        
        let problematic_functions: Vec<(String, f64)> = comparisons.iter()
        ...
        let stability_assessment = NumericalStabilityAssessment {
            ...
            unstable_functions: problematic_functions.iter().map(|(name, _)| name.clone()).collect(),
            ...
            precision_loss_analysis: PrecisionLossAnalysis {
                ...
                problematic_functions: problematic_functions.iter().map(|(name, _)| name.clone()).collect(),
            },
        };
        ...
        for comparison in comparisons {
            if comparison.performance.ratio > 5.0 {
                recommendations.push(ComparisonRecommendation {
                    priority: RecommendationPriority::High,
                    category: RecommendationCategory::Performance,
                    description: format!("Function '{}' is significantly slower than SciPy", comparison.function_name),
...
mod tests {
    ...
    fn test_accuracy_comparison() {
        ...
        let accuracy = comparison.compare_accuracy(&scirs2_result, &scipy_result).unwrap();
        assert!(matches!(accuracy.assessment, AccuracyAssessment::Excellent | AccuracyAssessment::Good));
    }
