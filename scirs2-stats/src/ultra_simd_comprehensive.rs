...
mod tests {
    ...
    fn test_comprehensive_stats() {
        ...
        let processor = UltraComprehensiveSimdProcessor::new();
        ...
        assert!(result.simd_efficiency > 0.0);
    }
    ...
    fn test_matrix_stats() {
        ...
        assert_eq!(result.col_means.len(), 3);
        assert!((result.row_means[0] - 2.0).abs() < 1e-10);
        assert!((result.col_means[0] - 4.0).abs() < 1e-10);
    }
}