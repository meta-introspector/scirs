...
impl MemoryProfiler {
    ...
    fn generate_recommendations(&self, allocations: &HashMap<String, AllocationStats>) -> Vec<String> {
        ...
        for (category, stats) in allocations {
            ...
            if stats.peak_bytes > 10 * 1024 * 1024 { // 10MB
                recommendations.push(format!(
                    "Consider streaming processing for '{}' category (large allocation: {:.1} MB)",
                    category, stats.peak_bytes as f64 / 1024.0 / 1024.0
                ));
            }
...
impl MemoryReport {
    ...
    pub fn print_report(&self) {
        ...
        println!("Peak Memory Usage: {:.2} MB", self.peak_memory as f64 / 1024.0 / 1024.0);
        println!("Current Memory Usage: {:.2} MB", self.current_memory as f64 / 1024.0 / 1024.0);
        println!();
        ...
        for (category, stats) in &self.allocations {
            ...
            println!("    Peak Allocation: {:.2} KB", stats.peak_bytes as f64 / 1024.0);
            println!("    Average Size: {:.1} bytes", stats.average_size);
            
            if !stats.allocation_times.is_empty() {
                let avg_time = stats.allocation_times.iter().sum::<Duration>().as_micros() as f64 / stats.allocation_times.len() as f64;
...
impl<F: Float + Clone> StatisticsCache<F> {
    ...
    fn evict_lru(&mut self) {
        if let Some((key_to_remove, entry_to_remove)) = self.cache
            .iter()
    ...
    F: Float + NumCast + Clone + Send + Sync,
{
    ...
    {
        // Chunked processing
        let chunk_size = self.adaptive_manager.suggest_chunk_size(data.len(), std::mem::size_of::<F>());
