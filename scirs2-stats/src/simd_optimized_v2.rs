...
impl Default for SimdConfig {
    fn default() -> Self {
        Self {
            min_size: 128,  // SIMD beneficial for arrays >= 128 elements
            use_aligned: false,  // Safe default
            unroll_factor: 4,  // 4-way unrolling
        }
    }
}
    ...
    D: Data<Elem = F>,
{
    ...
    for chunk_idx in 0..chunks {
        ...
        if chunk_min < min { min = chunk_min; }
        if chunk_max > max { max = chunk_max; }
    }
...
fn variance_scalar_welford<F, D>(
    x: &ArrayBase<D, Ix1>,
    ddof: usize,
) -> StatsResult<F>
where
...
fn stats_scalar_single_pass<F, D>(
    x: &ArrayBase<D, Ix1>,
) -> StatsResult<(F, F, F, F, F, F)>
where
    F: Float + NumCast,
