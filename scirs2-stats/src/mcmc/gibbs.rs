...
impl<C: ConditionalDistribution> GibbsSampler<C> {
    ...
    pub fn new(conditionals: C, initial: Array1<f64>) -> Result<Self> {
        ...
        if initial.len() != conditionals.dim() {
            return Err(StatsError::DimensionMismatch(
                format!("initial dimension ({}) must match conditionals dimension ({})", 
                    initial.len(), conditionals.dim())
            ));
        }
    ...
    pub fn step<R: Rng + ?Sized>(&mut self, rng: &mut R) -> Result<Array1<f64>> {
        ...
        for &var_idx in &order {
            let new_value = self.conditionals.sample_conditional(&self.current, var_idx, rng)?;
...
impl MultivariateNormalGibbs {
    ...
    pub fn from_precision(mean: Array1<f64>, precision: Array2<f64>) -> Result<Self> {
        ...
        if precision.nrows() != mean.len() || precision.ncols() != mean.len() {
            return Err(StatsError::DimensionMismatch(
                format!("precision shape ({}, {}) must be ({}, {})", 
                    precision.nrows(), precision.ncols(), mean.len(), mean.len())
            ));
        }
...
impl ConditionalDistribution for MultivariateNormalGibbs {
    fn sample_conditional<R: Rng + ?Sized>(
        ...
        if variable_index >= dim {
            return Err(StatsError::InvalidArgument(
                format!("variable_index ({}) must be less than dimension ({})", variable_index, dim)
            ));
        }
        ...
        let normal = Normal::new(conditional_mean, conditional_std)
...
impl GaussianMixtureGibbs {
    ...
    pub fn new(
        ...
        check_array_finite(&prior_alpha, "prior_alpha")?;
        
        let (n_samples, dim) = data.dim();
        
        if prior_alpha.len() != n_components {
            return Err(StatsError::DimensionMismatch(
                format!("prior_alpha length ({}) must equal n_components ({})", 
                    prior_alpha.len(), n_components)
            ));
        }
        ...
        let assignments = Array1::zeros(n_samples);
    ...
    fn sample_assignments<R: Rng + ?Sized>(&mut self, rng: &mut R) -> Result<()> {
        for i in 0..self.data.nrows() {
            ...
            for k in 0..self.n_components {
                ...
                let det = scirs2_linalg::det(&precision_k.view(), None)
    ...
    fn sample_parameters<R: Rng + ?Sized>(&mut self, rng: &mut R) -> Result<()> {
        for k in 0..self.n_components {
            ...
            let assigned_indices: Vec<usize> = self.assignments
                .iter()
                .enumerate()
    ...
    fn sample_from_prior<R: Rng + ?Sized>(&mut self, component: usize, rng: &mut R) -> Result<()> {
        ...
        for i in 0..dim {
            ...
            let normal = Normal::new(self.prior_mean[i], std)
    ...
    fn sample_posterior<R: Rng + ?Sized>(
        ...
        let posterior_mean = {
            ...
            let precision_inv = scirs2_linalg::inv(&posterior_precision.view(), None)
        ...
        for i in 0..dim {
            ...
            let std = variance.sqrt();
            let normal = Normal::new(posterior_mean[i], std)
                .map_err(|e| StatsError::ComputationError(format!("Failed to create normal: {}", e)))?;
            new_mean[i] = normal.sample(rng);
        }
    ...
    fn sample_weights<R: Rng + ?Sized>(&mut self, rng: &mut R) -> Result<()> {
        ...
        for k in 0..self.n_components {
            let gamma = Gamma::new(posterior_alpha[k], 1.0)
    ...
    pub fn new(conditionals: C, initial: Array1<f64>, blocks: Vec<Vec<usize>>) -> Result<Self> {
        ...
        for block in &blocks {
            for &idx in block {
                if idx >= dim {
                    return Err(StatsError::InvalidArgument(
                        format!("Block index {} exceeds dimension {}", idx, dim)
                    ));
                }
    ...
    pub fn step<R: Rng + ?Sized>(&mut self, rng: &mut R) -> Result<Array1<f64>> {
        ...
        for block in &self.blocks {
            for &var_idx in block {
                let new_value = self.sampler.conditionals
                    .sample_conditional(&self.sampler.current, var_idx, rng)?;
