    ...
    F: Float + NumCast + Send + Sync + std::iter::Sum,
{
    ...
    ) -> StatsResult<Vec<(F, F, F, F)>> // (mean, var, min, max)
    where
        D: Data<Elem = F> + Sync,
    {
        ...
        let results: Vec<StatsResult<(F, F, F, F)>> = datasets
            .par_iter()
            .map(|dataset| {
                self.compute_single_dataset_stats(dataset)
            })
            .collect();
    ...
    fn compute_single_dataset_stats<D>(
        &self,
        ...
        D: Data<Elem = F>,
    {
        ...
        if n < self.config.min_size {
            ...
            let min = data.iter().fold(data[0], |min_val, &x| if x < min_val { x } else { min_val });
            let max = data.iter().fold(data[0], |max_val, &x| if x > max_val { x } else { max_val });
        ...
        let results: Vec<(F, F, F, F, usize)> = par_chunks(data.as_slice().unwrap(), chunk_size)
            .map(|chunk| {
                ...
                let min = chunk.iter().fold(chunk[0], |min_val, &x| if x < min_val { x } else { min_val });
                let max = chunk.iter().fold(chunk[0], |max_val, &x| if x > max_val { x } else { max_val });
                
                // Local mean for variance calculation
                let local_mean = sum / F::from(len).unwrap();
                let sum_sq_dev = chunk.iter()
                    .map(|&x| {
        ...
        // Combine results
        let total_sum = results.iter().map(|(sum, _, _, _, _)| *sum).fold(F::zero(), |acc, x| acc + x);
        ...
        let global_min = results.iter()
            .map(|(_, _, min, _, _)| *min)
            .fold(results[0].2, |min_val, x| if x < min_val { x } else { min_val });
        let global_max = results.iter()
            .map(|(_, _, _, max, _)| *max)
            .fold(results[0].3, |max_val, x| if x > max_val { x } else { max_val });
        ...
        let global_variance = par_chunks(data.as_slice().unwrap(), chunk_size)
            ...
            .reduce(|| F::zero(), |a, b| a + b) / F::from(total_len - 1).unwrap();
    ...
    F: Float + NumCast + Send + Sync,
{
    ...
    pub fn cross_validate_correlation<D1, D2>(
        ...
        x: &ArrayBase<D1, Ix1>,
        y: &ArrayBase<D2, Ix1>,
    ) -> StatsResult<(F, F)> // (mean_correlation, std_correlation)
    where
        D1: Data<Elem = F> + Sync,
        D2: Data<Elem = F> + Sync,
    {
        if x.len() != y.len() {
            return Err(StatsError::dimension_mismatch("Arrays must have same length"));
        }
        
        let n = x.len();
        if n < self.k_folds {
            return Err(StatsError::invalid_argument("Not enough data for k-fold validation"));
        }
        ...
        let correlations: Vec<F> = (0..self.k_folds)
            ...
            .map(|fold| {
                ...
                let end = if fold == self.k_folds - 1 { n } else { (fold + 1) * fold_size };
        ...
        let mean_corr = correlations.iter().fold(F::zero(), |acc, &x| acc + x) / F::from(self.k_folds).unwrap();
        let var_corr = correlations.iter()
            .map(|&corr| {
                let diff = corr - mean_corr;
                diff * diff
            })
            .fold(F::zero(), |acc, x| acc + x) / F::from(self.k_folds - 1).unwrap();
    ...
    F: Float + NumCast + Send + Sync,
{
    ...
    pub fn bootstrap_confidence_interval<D>(
        ...
        confidence_level: F,
    ) -> StatsResult<(F, F, F)> // (estimate, lower_bound, upper_bound)
    where
        D: Data<Elem = F> + Sync,
    {
        if data.is_empty() {
        ...
        if confidence_level <= F::zero() || confidence_level >= F::one() {
            return Err(StatsError::invalid_argument("Confidence level must be between 0 and 1"));
        }
        ...
        let lower_idx = (lower_percentile * F::from(self.n_simulations - 1).unwrap()).floor().to_usize().unwrap();
        let upper_idx = (upper_percentile * F::from(self.n_simulations - 1).unwrap()).ceil().to_usize().unwrap();
...
impl ParallelMatrixOps {
        ...
        D2: Data<Elem = F> + Sync,
    {
        let (m, n) = matrix.dim();
        if n != vector.len() {
            return Err(StatsError::dimension_mismatch("Matrix columns must match vector length"));
        }
        ...
        if m < config.min_size {
            ...
            result.axis_chunks_iter_mut(Axis(0), chunk_size)
                .enumerate()
                .par_bridge()
                .for_each(|(chunk_idx, mut result_chunk)| {
                    ...
                    for (local_idx, i) in (start_row..end_row).enumerate() {
                        let row = matrix.row(i);
                        result_chunk[local_idx] = row.iter()
                            .zip(vector.iter())
                            .map(|(&a, &b)| a * b)
                            .sum();
                    }
...
mod tests {
    ...
    fn test_parallel_cross_validator() {
        ...
        let (mean_corr, std_corr) = validator.cross_validate_correlation(&x.view(), &y.view()).unwrap();
    ...
    fn test_parallel_matrix_ops() {
        ...
        let result = ParallelMatrixOps::matvec_parallel(&matrix.view(), &vector.view(), None).unwrap();
