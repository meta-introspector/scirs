...
use scirs2_stats::{
    create_cross_platform_validator, CompatibilityRating, CrossPlatformValidator,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Operating System: {}", report.platform_info.operating_system);
    ...
    println!("Tests Passed: {} / {}", report.passed_tests, report.test_results.len());
    ...
    println!("---------------------");
    println!("SIMD Speedup Factor: {:.2}x", report.performance_profile.simd_speedup_factor);
    println!("Parallel Efficiency: {:.1}%", report.performance_profile.parallel_efficiency * 100.0);
    println!("Memory Bandwidth: {:.1} GB/s", report.performance_profile.memory_bandwidth_gbps);
    println!("Cache Efficiency: {:.1}%", report.performance_profile.cache_efficiency * 100.0);
    println!("Thermal Throttling: {}", 
             if report.performance_profile.thermal_throttling_detected { "⚠️  Detected" } else { "✅ None" });
    ...
    for (category, (passed, failed)) in categories {
        ...
        let status = if success_rate == 100.0 { "✅" } else if success_rate >= 80.0 { "⚠️ " } else { "❌" };
        println!("{} {}: {}/{} ({:.0}%)", status, category, passed, total, success_rate);
    }
    println!();
    ...
    match report.compatibility_rating {
        CompatibilityRating::Excellent => {
            println!("✅ Excellent compatibility! All ultrathink optimizations should work perfectly.");
            println!("• Use OptimizationMode::{:?} for best results", report.performance_profile.recommended_optimization_mode);
        },
        CompatibilityRating::Good => {
            println!("✅ Good compatibility. Most optimizations work well.");
            println!("• Consider using OptimizationMode::{:?}", report.performance_profile.recommended_optimization_mode);
        ...
        CompatibilityRating::Incompatible => {
            println!("❌ Platform incompatibility detected. Ultrathink optimizations may not work.");
    ...
    if report.performance_profile.thermal_throttling_detected {
        println!("• Thermal throttling detected - ensure adequate cooling for sustained performance");
    }
    ...
    let mut top_tests: Vec<_> = report.test_results.iter()
        .filter(|r| r.passed)
        .collect();
    top_tests.sort_by(|a, b| b.performance_score.partial_cmp(&a.performance_score).unwrap());

    if !top_tests.is_empty() {
        println!("🏆 Top Performing Tests");
        println!("----------------------");
        for result in top_tests.iter().take(5) {
            println!("• {}: {:.1}% performance, {:.4} accuracy", 
                     result.test_name,
    ...
    println!("\n🎯 Next Steps:");
    println!("1. Use the recommended OptimizationMode::{:?} in your applications", 
