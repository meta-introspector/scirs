...
use scirs2_stats::{
    BayesianAnalysisWorkflow, DimensionalityAnalysisWorkflow, 
    QMCWorkflow, QMCSequenceType, SurvivalAnalysisWorkflow
};
...
fn bayesian_analysis_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Log marginal likelihood: {:.3}", result.model_metrics.log_marginal_likelihood);
    ...
    if let Some(ref mcmc_samples) = result.mcmc_samples {
        ...
        println!("  Posterior mean from MCMC: {:?}", mcmc_samples.mean_axis(ndarray::Axis(0)));
    }
...
fn dimensionality_analysis_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(ref pca) = result.pca {
        println!("  PCA explained variance ratio: {:?}", 
                 pca.explained_variance_ratio.slice(ndarray::s![..5]));
        println!("  Cumulative explained variance: {:.3}", 
    ...
    if let Some(ref fa) = result.factor_analysis {
        ...
        println!("  Explained variance by factors: {:?}", fa.explained_variance_ratio);
    }
    
    println!("  Recommendations:");
    println!("    Optimal PCA components: {}", result.recommendations.optimal_pca_components);
    println!("    Optimal factors: {}", result.recommendations.optimal_factors);
    println!("    Explained variance ratio: {:.3}", result.recommendations.explained_variance_ratio);
    
    println!("  Quality metrics:");
    println!("    KMO measure: {:.3}", result.comparison_metrics.kmo_measure);
    println!("    Bartlett's test: χ²={:.3}, p={:.3}", 
             result.comparison_metrics.bartlett_test.0,
             result.comparison_metrics.bartlett_test.1);
...
fn qmc_analysis_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Comparing QMC sequences for {}-dimensional integration:", dimensions);
    ...
    for sequence_type in &sequence_types {
        ...
        println!("    Star discrepancy: {:.6}", result.quality_metrics.star_discrepancy);
        println!("    Uniformity: {:.3}", result.quality_metrics.uniformity);
        println!("    Coverage efficiency: {:.3}", result.quality_metrics.coverage_efficiency);
        ...
        println!("    Integration test (∫(x₁²+x₂²+x₃²)dx): {:.6} (error: {:.6})", 
...
fn survival_analysis_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Event rate: {:.1}%", result.summary_stats.event_rate * 100.0);
    println!("    Censoring rate: {:.1}%", result.summary_stats.censoring_rate * 100.0);
