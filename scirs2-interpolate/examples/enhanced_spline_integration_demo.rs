...
fn main() -> InterpolateResult<()> {
    ...
    let integral_linear_extrap = spline.integrate_with_extrapolation(-1.0, 4.0, Some(ExtrapolateMode::Extrapolate))?;
    println!("   ∫[-1, 4] with linear extrapolation = {:.6}", integral_linear_extrap);
    
    // With constant extrapolation
    let integral_const_extrap = spline.integrate_with_extrapolation(-1.0, 4.0, Some(ExtrapolateMode::Nearest))?;
    ...
    let left_extrap = spline.integrate_with_extrapolation(-2.0, -1.0, Some(ExtrapolateMode::Extrapolate))?;
    println!("   ∫[-2, -1] (left extrapolation) = {:.6}", left_extrap);
    
    // Right extrapolation only  
    let right_extrap = spline.integrate_with_extrapolation(4.0, 5.0, Some(ExtrapolateMode::Extrapolate))?;
    println!("   ∫[4, 5] (right extrapolation) = {:.6}", right_extrap);
    
    // Mixed: left extrapolation + interior
    let mixed_left = spline.integrate_with_extrapolation(-1.0, 2.0, Some(ExtrapolateMode::Extrapolate))?;
    println!("   ∫[-1, 2] (left extrap + interior) = {:.6}", mixed_left);
    
    // Mixed: interior + right extrapolation
    let mixed_right = spline.integrate_with_extrapolation(1.0, 4.0, Some(ExtrapolateMode::Extrapolate))?;
    ...
    for (i, &(a, b)) in bounds.iter().enumerate() {
        ...
        let const_result = spline.integrate_with_extrapolation(a, b, Some(ExtrapolateMode::Nearest))?;
        let linear_result = spline.integrate_with_extrapolation(a, b, Some(ExtrapolateMode::Extrapolate))?;
        
        println!("     Constant extrapolation: {:.6}", const_result);
        println!("     Linear extrapolation:   {:.6}", linear_result);
        println!("     Difference:             {:.6}", (linear_result - const_result).abs());
    }
