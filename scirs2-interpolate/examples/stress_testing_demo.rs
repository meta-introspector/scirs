...
use scirs2_interpolate::stress_testing::{
    stress_tests, StressTestConfig, extreme_data,
};
...
fn main() {
    ...
    println!("  - Extreme extrapolation: {}", config.test_extreme_extrapolation);
    ...
    for i in 0..3.min(x_large.len()) {
        println!("  x[{}] = {:.3e}, y[{}] = {:.3e}", i, x_large[i], i, y_large[i]);
    }
    ...
    for i in 0..3.min(x_small.len()) {
        println!("  x[{}] = {:.3e}, y[{}] = {:.3e}", i, x_small[i], i, y_small[i]);
    }
    ...
    println!("  x[{}] = {:.3e}, y[{}] = {:.3e}", 
             x_range.len()-1, x_range[x_range.len()-1], 
             y_range.len()-1, y_range[y_range.len()-1]);
    ...
    println!("Well-conditioned points: condition number ≈ {:.2e}", good_cond);
    ...
    println!("Ill-conditioned points: condition number ≈ {:.2e}", bad_cond);
