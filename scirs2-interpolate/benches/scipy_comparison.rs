...
use scirs2_interpolate::{
    ...
    bspline::BSpline,
    cubic_interpolate, linear_interpolate,
...
fn generate_1d_data(n: usize, noise: bool) -> (Array1<f64>, Array1<f64>) {
    ...
    let y = if noise {
        x.mapv(|xi| (xi * 0.5).sin() + 0.1 * xi + 0.05 * (3.0 * xi).cos() + 0.01 * rand::random::<f64>())
    } else {
        x.mapv(|xi| (xi * 0.5).sin() + 0.1 * xi + 0.05 * (3.0 * xi).cos())
    };
    (x, y)
}
...
fn bench_linear_1d_comparison(c: &mut Criterion) {
    ...
    for &n_points in &[100, 1000, 10_000, 100_000] {
        ...
        group.bench_with_input(
            BenchmarkId::new("scirs2", n_points),
            &n_points,
            |b, _| {
                b.iter(|| {
                    black_box(linear_interpolate(&x.view(), &y.view(), &queries.view()))
                });
            },
        );
        ...
        {
            group.bench_with_input(
                BenchmarkId::new("scipy", n_points),
                &n_points,
                |b, _| {
                    Python::with_gil(|py| {
...
fn bench_cubic_spline_comparison(c: &mut Criterion) {
    ...
    for &n_points in &[100, 1000, 10_000, 50_000] {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    black_box(CubicSpline::new(&x.view(), &y.view()))
                });
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    black_box(spline.evaluate_array(&queries.view()))
                });
            },
        );
        ...
        for (kernel_name, kernel) in &kernels {
            ...
            group.bench_with_input(
                ...
                |b, _| {
                    ...
                    b.iter(|| {
                        black_box(interpolator.interpolate(&queries.view()))
                    });
                },
            );
            ...
            #[cfg(feature = "scipy-comparison")]
            {
                Python::with_gil(|py| {
                    ...
                    // Convert to numpy arrays
                    let points_py = numpy.call_method1("array", (points.clone().into_raw_vec(),)).unwrap();
                    ...
                    let queries_py = numpy.call_method1("array", (queries.clone().into_raw_vec(),)).unwrap();
                    ...
                    group.bench_with_input(
                        ...
                        |b, _| {
                            b.iter(|| {
                                ...
                                let interpolator = rbf.call1((points_py, values_py))
                                    .unwrap()
...
fn bench_large_scale_performance(c: &mut Criterion) {
    ...
    for &n_points in &[10_000, 100_000, 500_000, 1_000_000] {
        ...
        group.bench_with_input(
            BenchmarkId::new("linear", n_points),
            &n_points,
            |b, _| {
                b.iter(|| {
                    black_box(linear_interpolate(&x.view(), &y.view(), &queries.view()))
        ...
        if n_points <= 100_000 {
            group.bench_with_input(
                BenchmarkId::new("bspline", n_points),
                &n_points,
                |b, _| {
                    let bspline = BSpline::new(&x.view(), &y.view(), 3).unwrap();
                    b.iter(|| {
                        black_box(bspline.evaluate_array(&queries.view()))
                    });
                },
            );
        }
...
fn bench_simd_effectiveness(c: &mut Criterion) {
    ...
    for &query_size in &[16, 32, 64, 128, 256, 512, 1024, 4096] {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    black_box(cubic_interpolate(&x.view(), &y.view(), &queries.view()))
                });
            },
        );
    }
            ...
            |b, _| {
                b.iter(|| {
                    for &q in queries.iter() {
                        black_box(linear_interpolate(&x.view(), &y.view(), &Array1::from_vec(vec![q]).view()));
                    }
                });
            },
        );
