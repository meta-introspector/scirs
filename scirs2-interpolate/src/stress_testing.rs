...
impl StressTestResults {
    ...
    fn add_fail(&mut self, test_name: &str, reason: &str) {
        self.failed_tests.push((test_name.to_string(), reason.to_string()));
    }
    
    fn add_performance(&mut self, metric_name: &str, value: f64) {
        self.performance_metrics.push((metric_name.to_string(), value));
    }
    
    fn add_accuracy(&mut self, metric_name: &str, value: f64) {
        ...
        match condition {
            "large_values" => {
                ...
                for i in 0..n {
                    ...
                    values[i] = (points[[i, 0]] / scale).sin() * (points[[i, 1]] / scale).cos() * scale / 4.0;
                }
            }
            "clustered" => {
...
pub mod stress_tests {
    ...
    pub fn test_linear_interpolation(config: &StressTestConfig) -> StressTestResults {
        let mut results = StressTestResults::new("Linear Interpolation".to_string(), config.clone());
            ...
            match linear_interpolate(&x.view(), &y.view(), &queries.view()) {
                Ok(result) => {
                    ...
                    if max_error < 1e-10 {
                        ...
                        results.add_fail("small_values", &format!("Large relative error: {}", max_error));
                    }
                }
                ...
                match RBFInterpolator::new(&points.view(), &values.view(), *kernel, 1.0) {
                    Ok(interpolator) => {
                        ...
                        match interpolator.interpolate(&queries.view()) {
                            ...
                            Err(e) => results.add_fail(&format!("ill_conditioned_{}", kernel_name), &e.to_string()),
                        }
                    }
                    Err(e) => results.add_fail(&format!("ill_conditioned_{}", kernel_name), &e.to_string()),
                }
