...
impl<F: crate::traits::InterpolationFloat + ToString> CubicSpline<F> {
    ...
    pub fn find_roots(&self, tolerance: F, max_iterations: usize) -> InterpolateResult<Vec<F>> {
        ...
        for segment in 0..n_segments {
            ...
            if y_left * y_right < F::zero() {
                ...
                if let Some(root) =
                    self.find_root_in_segment(segment, x_left, x_right, tolerance, max_iterations)?
                {
                    roots.push(root);
                }
    ...
    fn evaluate_with_extrapolation(&self, x_new: F) -> InterpolateResult<F> {
        ...
        if x_new < self.x[0] {
            ...
            let dx = x_new - self.x[0];
            Ok(y0 + dy0 * dx)
        } else {
            let n = self.x.len() - 1;
            let yn = self.y[n];
            let dyn_val = self.derivative_n(self.x[n], 1)?;
            let dx = x_new - self.x[n];
            Ok(yn + dyn_val * dx)
        }
    }
    ...
    pub fn derivative_with_extrapolation(
        ...
        if !extrapolate {
            return self.derivative_n(x_new, order);
        }
        ...
        if x_new < x_min {
            ...
            let segment_idx = 0;
            let dx = x_new - self.x[segment_idx];
...
/// let y_new = cs.call_scipy(&x_new.view(), 0, true).unwrap();
///
/// // Compute derivatives
/// let dy = cs.call_scipy(&x_new.view(), 1, true).unwrap();
///
