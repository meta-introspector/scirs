...
use scirs2_optimize::unconstrained::{
    minimize_bfgs, Options, DiagnosticCollector, DiagnosticOptions, 
    LineSearchDiagnostic, ExportFormat
};
...
fn main() {
    ...
    println!("\nProblem Analysis:");
    println!("  Difficulty: {:?}", diagnostics.problem_analysis.difficulty);
    println!("  Features: {:?}", diagnostics.problem_analysis.features);
    
    // Show performance metrics
    println!("\nPerformance Metrics:");
    println!("  Total iterations: {}", diagnostics.performance_metrics.total_iterations);
    println!("  Total function evaluations: {}", diagnostics.performance_metrics.total_fev);
    println!("  Average iteration time: {:?}", diagnostics.performance_metrics.avg_iteration_time);
    println!("  Function evaluation rate: {:.2} per second", diagnostics.performance_metrics.fev_rate);
    
    // Show convergence analysis
    println!("\nConvergence Analysis:");
    println!("  Convergence rate: {:?}", diagnostics.convergence_analysis.convergence_rate);
    println!("  Convergence phase: {:?}", diagnostics.convergence_analysis.convergence_phase);
    println!("  Confidence score: {:.2}", diagnostics.convergence_analysis.confidence_score);
