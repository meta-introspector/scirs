...
impl MetaPolicyNetwork {
    ...
    pub fn new(input_size: usize, output_size: usize, hidden_sizes: Vec<usize>) -> Self {
        ...
        for layer in 0..num_layers {
            ...
            for i in 0..fan_out {
                for j in 0..fan_in {
                    policy_weights[[layer, i, j]] = rng().gen_range(-0.5..0.5) * 2.0 * xavier_std;
                    meta_weights[[layer, i, j]] =
                        rng().gen_range(-0.5..0.5) * 2.0 * xavier_std * 0.1;
                }
            }
        }
    ...
    fn get_or_create_problem_embedding(
        ...
        if let Some(embedding) = self.problem_embeddings.get(problem_class) {
            ...
            let embedding = Array1::from_shape_fn(input_size, |_| rng().gen_range(-0.05..0.05));
...
impl MetaExperienceBuffer {
    ...
    pub fn sample_meta_batch(&self, batch_size: usize) -> Vec<MetaTrajectory> {
        ...
        for _ in 0..batch_size.min(self.trajectories.len()) {
            ...
            let idx = rng().gen_range(0..self.trajectories.len());
