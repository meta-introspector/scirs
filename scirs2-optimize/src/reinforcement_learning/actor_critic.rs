...
use super::{RLOptimizationConfig, OptimizationState, OptimizationAction, Experience, RewardFunction, ImprovementReward, RLOptimizer, ExperienceBuffer, utils};
...
impl ActivationType {
    ...
    fn derivative(&self, x: f64) -> f64 {
        match self {
            ...
            ActivationType::LeakyReLU => if x > 0.0 { 1.0 } else { 0.01 },
            ActivationType::ELU => if x > 0.0 { 1.0 } else { x.exp() },
        }
    }
}
...
impl ActorNetwork {
    ...
    pub fn new(input_size: usize, hidden_size: usize, output_size: usize, activation: ActivationType) -> Self {
    ...
    pub fn action_probabilities(&self, policy_output: &ArrayView1<f64>, temperature: f64) -> Array1<f64> {
    ...
    pub fn backward_and_update(
        ...
        for i in 0..self.output_size {
            for j in 0..self.hidden_size {
                self.output_weights[[i, j]] -= learning_rate * output_raw_gradient[i] * hidden_activated[j];
            }
...
impl CriticNetwork {
    ...
    pub fn backward_and_update(
        ...
        let mut hidden_gradient = Array1::zeros(self.hidden_size);
        for j in 0..self.hidden_size {
            hidden_gradient[j] = value_error * self.output_weights[j] * self.activation.derivative(hidden_raw[j]);
        }
...
impl AdvantageActorCriticOptimizer {
    ...
    fn extract_state_features(&self, state: &OptimizationState) -> Array1<f64> {
        ...
        // Convergence metrics
        features.push(state.convergence_metrics.relative_objective_change.ln().tanh());
        ...
        if state.objective_history.len() >= 2 {
            ...
            let trend = if state.objective_history.len() >= 3 {
                let slope = (state.objective_history[state.objective_history.len() - 1] - 
                           state.objective_history[0]) / state.objective_history.len() as f64;
                slope.tanh()
            } else {
                0.0
            };
    ...
    fn select_action_with_exploration(&mut self, state: &OptimizationState) -> (OptimizationAction, Array1<f64>) {
        let state_features = self.extract_state_features(state);
        let (_, _, policy_output) = self.actor.forward(&state_features.view());
        
        // Add exploration noise
        let exploration_noise = if self.training_stats.episodes_completed < self.config.num_episodes / 2 {
            0.1 * (1.0 - self.training_stats.episodes_completed as f64 / self.config.num_episodes as f64)
        } else {
            0.01
        };
        
        let noisy_output = policy_output.mapv(|x| x + (rand::random::<f64>() - 0.5) * exploration_noise);
        let action_probs = self.actor.action_probabilities(&noisy_output.view(), self.temperature);
        
        // Sample action based on probabilities
        let cumulative_probs: Vec<f64> = action_probs.iter()
            .scan(0.0, |acc, &p| { *acc += p; Some(*acc) })
            .collect();
        
        let rand_val = rand::random::<f64>();
        let action_idx = cumulative_probs.iter()
    ...
    fn decode_action_from_index(&self, action_idx: usize, policy_output: &Array1<f64>) -> OptimizationAction {
    ...
    fn compute_advantage(&self, reward: f64, current_value: f64, next_value: f64, done: bool) -> f64 {
        let target = if done {
            reward
        } else {
    ...
    fn update_networks(&mut self, experiences: &[Experience]) -> Result<()> {
        ...
        for experience in experiences {
            ...
            let (hidden_raw, hidden_activated, current_value) = self.critic.forward(&state_features.view());
            ...
            let action_probs = self.actor.action_probabilities(&policy_output.view(), self.temperature);
            
            // Compute entropy for exploration
            let entropy = -action_probs.iter()
                .filter(|&&p| p > 1e-8)
                .map(|&p| p * p.ln())
                .sum::<f64>();
            ...
            if action_idx < actor_gradient.len() {
                actor_gradient[action_idx] = -(advantage - self.baseline) / (action_probs[action_idx] + 1e-8);
                // Add entropy bonus
        ...
        if !experiences.is_empty() {
            self.baseline = 0.9 * self.baseline + 0.1 * (total_advantage / experiences.len() as f64);
            
            // Update training statistics
            let num_exp = experiences.len() as f64;
            self.training_stats.avg_actor_loss = 0.9 * self.training_stats.avg_actor_loss + 
                                                0.1 * (total_actor_loss / num_exp);
            self.training_stats.avg_critic_loss = 0.9 * self.training_stats.avg_critic_loss + 
                                                 0.1 * (total_critic_loss / num_exp);
            self.training_stats.avg_advantage = 0.9 * self.training_stats.avg_advantage + 
                                               0.1 * (total_advantage / num_exp);
            self.training_stats.avg_entropy = 0.9 * self.training_stats.avg_entropy + 
                                             0.1 * (total_entropy / num_exp);
        }
...
impl RLOptimizer for AdvantageActorCriticOptimizer {
        ...
        F: Fn(&ArrayView1<f64>) -> f64,
    {
        ...
        for step in 0..self.config.max_steps_per_episode {
            ...
            // Compute reward
            let reward = self.reward_function.compute_reward(&current_state, &action, &new_state);
            total_reward += reward;
    ...
    F: Fn(&ArrayView1<f64>) -> f64,
{
    ...
    let mut optimizer = AdvantageActorCriticOptimizer::new(config, state_size, action_size, hidden_size);
...
mod tests {
    ...
    fn test_action_index_mapping() {
        ...
        let actions = vec![
            OptimizationAction::GradientStep { learning_rate: 0.01 },
    ...
    fn test_basic_a2c_optimization() {
        ...
        let result = actor_critic_optimize(objective, &initial.view(), Some(config), Some(16)).unwrap();
