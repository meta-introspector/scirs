...
impl ActivationType {
    pub fn apply(&self, x: f64) -> f64 {
        match self {
            ...
            ActivationType::GELU => x * 0.5 * (1.0 + (x * 0.7978845608 * (1.0 + 0.044715 * x * x)).tanh()),
    ...
    pub fn derivative(&self, x: f64) -> f64 {
        match self {
            ...
            ActivationType::GELU => {
                ...
                0.5 * (1.0 + tanh_val) + x * 0.5 * (1.0 - tanh_val * tanh_val) * 0.7978845608 * (1.0 + 0.134145 * x * x)
            },
    ...
    pub fn new(
        ...
        for &hidden_size in &hidden_sizes {
            let weights = Array2::from_shape_fn((hidden_size, prev_size), |_| {
                (rand::random::<f64>() - 0.5) * (2.0 / prev_size as f64).sqrt()
            });
...
impl ProblemEncoder {
    ...
    fn compute_dimensionality_features(&self, params: &ArrayView1<f64>, problem: &OptimizationProblem) -> Array1<f64> {
        ...
        F: Fn(&ArrayView1<f64>) -> f64,
    {
        ...
        if !gradient_components.is_empty() {
            ...
            features[4] = gradient_components.iter().map(|&g| g.abs()).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(0.0).tanh();
            features[5] = gradient_components.iter().map(|&g| g.abs()).min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(0.0).tanh();
        }
...
mod tests {
    ...
    fn test_optimization_network_creation() {
        let network = OptimizationNetwork::new(
            10, vec![32, 32], 5, true, ActivationType::GELU
        );
    ...
    fn test_network_forward_pass() {
        let network = OptimizationNetwork::new(
            5, vec![10], 3, false, ActivationType::ReLU
        );
