...
impl NeuralAdaptiveOptimizer {
    ...
    pub fn new(config: LearnedOptimizationConfig) -> Self {
        let architecture = NetworkArchitecture {
            ...
            activations: vec![ActivationType::GELU, ActivationType::GELU, ActivationType::Tanh],
            use_recurrent: true,
    ...
    pub fn adaptive_optimization_step<F>(&mut self, 
                                        objective: &F,
                                        current_params: &ArrayView1<f64>,
                                        step_number: usize) -> Result<AdaptiveOptimizationStep>
    where
        F: Fn(&ArrayView1<f64>) -> f64,
    {
        ...
        let strategy = self.adaptation_controller.select_strategy(&network_output, &performance_prediction)?;
        
        // Monitor progress and adapt if necessary
        self.adaptation_controller.monitor_and_adapt(&performance_prediction)?;
        
        // Create optimization step
        let step = AdaptiveOptimizationStep {
            strategy: strategy.clone(),
            predicted_performance: performance_prediction,
            confidence: self.performance_predictor.confidence_estimator.estimate_confidence(&state_features)?,
        ...
        F: Fn(&ArrayView1<f64>) -> f64,
    {
        ...
        self.copy_features(&mut features, &obj_features, feature_idx + param_features.len());
        
        // Temporal features
        let temporal_features = self.extract_temporal_features(step_number);
        self.copy_features(&mut features, &temporal_features, feature_idx + param_features.len() + obj_features.len());
    ...
    fn extract_parameter_features(&self, params: &ArrayView1<f64>) -> Array1<f64> {
        ...
        if !params.is_empty() {
            ...
            if std > 1e-8 {
                ...
                let kurtosis = params.iter()
                    .map(|&x| ((x - mean) / std).powi(4))
                    .sum::<f64>() / params.len() as f64 - 3.0;
                features[6] = kurtosis.tanh();
            }
            
            // Norms
            features[7] = (params.iter().map(|&x| x.abs()).sum::<f64>() / params.len() as f64).tanh(); // L1
    ...
    fn extract_temporal_features(&self, step_number: usize) -> Array1<f64> {
        ...
        if self.meta_state.performance_history.len() > 1 {
            ...
            if recent_performance.len() > 1 {
                let trend = (recent_performance[recent_performance.len() - 1] - recent_performance[0]) / 
                ...
                let variance = recent_performance.iter()
                    ...
                    (recent_performance.iter().sum::<f64>() / recent_performance.len() as f64).powi(2);
    ...
    fn update_adaptive_stats(&mut self, step: &AdaptiveOptimizationStep) -> Result<()> {
        ...
        self.adaptive_stats.avg_adaptation_rate = 
            0.9 * self.adaptive_stats.avg_adaptation_rate + 
            0.1 * step.adaptation_signal;
        
        // Update prediction accuracy (simplified)
        self.adaptive_stats.prediction_accuracy = 
            0.95 * self.adaptive_stats.prediction_accuracy + 
            0.05 * step.confidence;
    ...
    fn train_optimization_network(&mut self, trajectory: &OptimizationTrajectory) -> Result<()> {
        ...
        for (i, state) in trajectory.states.iter().enumerate() {
            if i + 1 < trajectory.actions.len() {
                ...
                self.optimization_network.backward(&loss_gradient, learning_rate)?;
            }
        }
    ...
    fn train_performance_predictor(&mut self, trajectory: &OptimizationTrajectory) -> Result<()> {
        ...
        for (i, state) in trajectory.states.iter().enumerate() {
            if i + self.performance_predictor.prediction_horizon < trajectory.performance_values.len() {
                let target_performance = trajectory.performance_values[i + self.performance_predictor.prediction_horizon];
    ...
    fn update_adaptation_controller(&mut self, trajectory: &OptimizationTrajectory) -> Result<()> {
        ...
        if trajectory.performance_values.len() > 2 {
            let performance_trend = trajectory.performance_values.last().unwrap() - 
...
impl OptimizationNetwork {
    ...
    pub fn new(architecture: NetworkArchitecture) -> Self {
        ...
        for (i, &hidden_size) in architecture.hidden_sizes.iter().enumerate() {
            let activation = architecture.activations.get(i).copied()
        ...
        let input_activation = architecture.activations.first().copied()
            .unwrap_or(ActivationType::ReLU);
        let output_activation = architecture.activations.last().copied()
            .unwrap_or(ActivationType::Tanh);
        
        let input_layer = NeuralLayer::new(architecture.input_size, architecture.input_size, input_activation);
...
impl LayerNormalization {
    ...
    pub fn normalize(&mut self, input: &Array1<f64>) -> Result<Array1<f64>> {
        ...
        self.running_mean = &self.running_mean * self.momentum + &(Array1::from_elem(input.len(), mean) * (1.0 - self.momentum));
        self.running_var = &self.running_var * self.momentum + &(Array1::from_elem(input.len(), var) * (1.0 - self.momentum));
...
impl StrategySelector {
    ...
    pub fn select(&self, network_output: &Array1<f64>, performance_prediction: f64) -> Result<OptimizationStrategy> {
        ...
        let selected_idx = strategy_scores.iter()
            .enumerate()
    ...
    pub fn reinforce_current(&mut self, strength: f64) -> Result<()> {
        ...
        if let Some((best_idx, _)) = self.strategy_weights.iter()
            .enumerate()
            .max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal))
        {
...
impl PerformancePredictor {
    ...
    pub fn predict(&self, state_features: &Array1<f64>) -> Result<f64> {
        ...
        for j in 0..prediction_features.len().min(self.prediction_network.ncols()) {
...
impl LearnedOptimizer for NeuralAdaptiveOptimizer {
    ...
    fn adapt_to_problem(&mut self, _problem: &OptimizationProblem, _initial_params: &ArrayView1<f64>) -> Result<()> {
        ...
        F: Fn(&ArrayView1<f64>) -> f64,
    {
        ...
        for step_number in 0..1000 {
            ...
            let adaptive_step = self.adaptive_optimization_step(&objective, &current_params.view(), step_number)?;
            
            // Apply the selected strategy
            let direction = self.compute_direction_for_strategy(&objective, &current_params, &adaptive_step.strategy)?;
...
impl NeuralAdaptiveOptimizer {
    fn create_trajectory_from_task(&self, task: &TrainingTask) -> Result<OptimizationTrajectory> {
        ...
        for i in 0..num_steps {
            states.push(Array1::from_shape_fn(self.optimization_network.architecture.input_size, |_| {
                rand::random::<f64>()
            }));
            
            actions.push(Array1::from_shape_fn(self.optimization_network.architecture.output_size, |_| {
                rand::random::<f64>()
            }));
            
            performance_values.push(1.0 - i as f64 / num_steps as f64);
            rewards.push(if i > 0 { performance_values[i - 1] - performance_values[i] } else { 0.0 });
        }
...
mod tests {
    ...
    fn test_optimization_network() {
        let architecture = NetworkArchitecture {
            ...
            output_size: 4,
            activations: vec![ActivationType::ReLU, ActivationType::ReLU, ActivationType::Tanh],
            use_recurrent: false,
            use_attention: false,
        };
