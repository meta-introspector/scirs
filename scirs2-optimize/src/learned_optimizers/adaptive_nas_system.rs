...
impl AdaptiveNASSystem {
    ...
    pub fn search_architectures(&mut self, training_problems: &[OptimizationProblem]) -> Result<Vec<OptimizationArchitecture>> {
    ...
    fn generate_random_architecture(&self) -> Result<OptimizationArchitecture> {
        let num_layers = 2 + (rand::random::<usize>() % 8); // 2-10 layers
        ...
        for i in 0..num_layers {
            ...
            let units = 16 + (rand::random::<usize>() % 256); // 16-272 units
            
            layers.push(LayerConfig {
                layer_type,
                units,
                dropout: rand::random::<f64>() * 0.5,
            ...
            if i > 0 {
                ...
                if i > 1 && rand::random::<f64>() < 0.3 {
                    let skip_source = rand::random::<usize>() % i;
                    connections.push(Connection {
                        from: skip_source,
                        to: i,
                        weight: 0.5,
        ...
        for _ in 0..(1 + rand::random::<usize>() % 4) {
            optimizer_components.push(self.sample_optimizer_component());
        }
        
        let id = format!("arch_{}", rand::random::<u64>());
    ...
    fn sample_layer_type(&self) -> LayerType {
        match rand::random::<usize>() % 8 {
            0 => LayerType::Dense,
            1 => LayerType::Attention { num_heads: 2 + rand::random::<usize>() % 6 },
            2 => LayerType::LSTM { hidden_size: 32 + rand::random::<usize>() % 128 },
            3 => LayerType::GRU { hidden_size: 32 + rand::random::<usize>() % 128 },
            4 => LayerType::Transformer { 
                num_heads: 2 + rand::random::<usize>() % 6,
                ff_dim: 64 + rand::random::<usize>() % 256,
            },
            5 => LayerType::Memory { memory_size: 16 + rand::random::<usize>() % 64 },
            6 => LayerType::Convolution { 
                kernel_size: 1 + rand::random::<usize>() % 5,
                stride: 1 + rand::random::<usize>() % 3,
            },
            _ => LayerType::GraphNN { aggregation: "mean".to_string() },
        }
    }
    ...
    fn sample_normalization(&self) -> NormalizationType {
        match rand::random::<usize>() % 5 {
            ...
            3 => NormalizationType::GroupNorm { groups: 2 + rand::random::<usize>() % 6 },
    ...
    fn sample_optimizer_component(&self) -> OptimizerComponent {
        match rand::random::<usize>() % 6 {
            0 => OptimizerComponent::Momentum { 
                decay: 0.8 + rand::random::<f64>() * 0.19 
            },
            1 => OptimizerComponent::AdaptiveLR {
                adaptation_rate: 0.001 + rand::random::<f64>() * 0.009,
                min_lr: 1e-8,
                max_lr: 1.0,
            },
            2 => OptimizerComponent::SecondOrder {
                hessian_approximation: HessianApprox::LBFGS { memory_size: 5 + rand::random::<usize>() % 15 },
                regularization: 1e-6 + rand::random::<f64>() * 1e-3,
            },
            3 => OptimizerComponent::TrustRegion {
                initial_radius: 0.1 + rand::random::<f64>() * 0.9,
                max_radius: 10.0,
                shrink_factor: 0.25,
                expand_factor: 2.0,
            },
            4 => OptimizerComponent::LineSearch {
                ...
                max_iterations: 10 + rand::random::<usize>() % 20,
            },
            _ => OptimizerComponent::Regularization {
                l1_weight: rand::random::<f64>() * 0.01,
                l2_weight: rand::random::<f64>() * 0.01,
                elastic_net_ratio: rand::random::<f64>(),
            },
        }
    }
    ...
    fn evaluate_population(&mut self, training_problems: &[OptimizationProblem]) -> Result<()> {
        for architecture in &mut self.architecture_population {
            ...
            for problem in training_problems.iter().take(5) { // Limit for efficiency
    ...
    fn evaluate_architecture_on_problem(&self, 
                                      architecture: &OptimizationArchitecture, 
                                      problem: &OptimizationProblem) -> Result<f64> {
            ...
            if i < self.controller.lstm_weights.len() {
                for j in 0..self.controller.lstm_weights.nrows() {
                    for k in 0..self.controller.lstm_weights.ncols() {
                        ...
                        self.controller.lstm_weights[[i % self.controller.lstm_weights.len(), j, k]] += 
    ...
    fn mutate_architecture(&self, base_arch: &OptimizationArchitecture) -> Result<OptimizationArchitecture> {
        let mut mutated = base_arch.clone();
        mutated.id = format!("mutated_{}", rand::random::<u64>());
        
        // Mutate with some probability
        if rand::random::<f64>() < 0.3 {
            // Mutate layer count
            if rand::random::<f64>() < 0.5 && mutated.layers.len() < 12 {
                mutated.layers.push(LayerConfig {
                    layer_type: self.sample_layer_type(),
                    units: 32 + rand::random::<usize>() % 128,
                    dropout: rand::random::<f64>() * 0.5,
                    normalization: self.sample_normalization(),
                    parameters: HashMap::new(),
                });
        ...
        if rand::random::<f64>() < 0.4 {
            if rand::random::<f64>() < 0.5 && mutated.optimizer_components.len() < 6 {
                mutated.optimizer_components.push(self.sample_optimizer_component());
            } else if !mutated.optimizer_components.is_empty() {
                let idx = rand::random::<usize>() % mutated.optimizer_components.len();
    ...
    /// Select best architectures for next generation
    fn select_next_generation(&mut self, mut new_architectures: Vec<OptimizationArchitecture>) -> Result<()> {
        ...
        self.architecture_population.sort_by(|a, b| {
            b.performance_metrics.convergence_rate
                .partial_cmp(&a.performance_metrics.convergence_rate)
                .unwrap_or(std::cmp::Ordering::Equal)
        });
        
        // Keep only the best architectures
        self.architecture_population.truncate(self.config.batch_size);
    ...
    fn update_search_stats(&mut self) -> Result<()> {
        ...
        let performances: Vec<f64> = self.architecture_population
            .iter()
        ...
        self.search_stats.convergence_indicators.push(self.search_stats.best_performance);
    ...
    fn check_convergence(&self) -> bool {
        ...
        let recent_improvements: Vec<f64> = self.search_stats.convergence_indicators
            .windows(2)
            .map(|w| w[1] - w[0])
            .collect();
        
        let avg_improvement = recent_improvements.iter().sum::<f64>() / recent_improvements.len() as f64;
    ...
    fn get_best_architecture_index(&self) -> usize {
        self.architecture_population
            ...
            .enumerate()
            .max_by(|(_, a), (_, b)| {
                a.performance_metrics.convergence_rate
    ...
    pub fn cache_architecture_for_problem(&mut self, 
    ...
    pub fn get_cached_architecture(&self, problem_class: &str) -> Option<&OptimizationArchitecture> {
...
impl LearnedOptimizer for AdaptiveNASSystem {
    fn meta_train(&mut self, training_tasks: &[TrainingTask]) -> Result<()> {
        let problems: Vec<OptimizationProblem> = training_tasks.iter()
        ...
        F: Fn(&ArrayView1<f64>) -> f64,
    {
        ...
        Ok(OptimizeResults {
            ...
            success: true,
            iterations,
            message: format!("NAS optimization completed using architecture: {}", best_arch.id),
        })
    }
...
impl AdaptiveNASSystem {
    fn compute_step_size(&self, architecture: &OptimizationArchitecture, iteration: usize) -> f64 {
        ...
        for component in &architecture.optimizer_components {
            match component {
                OptimizerComponent::AdaptiveLR { adaptation_rate, min_lr, max_lr } => {
    ...
    fn compute_search_direction<F>(&self, 
                                  objective: &F, 
                                  params: &Array1<f64>, 
                                  architecture: &OptimizationArchitecture) -> Array1<f64>
    where
        F: Fn(&ArrayView1<f64>) -> f64,
    {
        ...
        for component in &architecture.optimizer_components {
            match component {
                ...
                OptimizerComponent::Regularization { l1_weight, l2_weight, .. } => {
                    // Add regularization
