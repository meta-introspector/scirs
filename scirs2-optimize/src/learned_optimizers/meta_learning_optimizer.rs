...
impl MetaLearningOptimizer {
    ...
    pub fn learn_meta_strategy(&mut self, training_tasks: &[TrainingTask]) -> Result<()> {
        for task in training_tasks {
            ...
            self.task_optimizers.insert(task.problem.name.clone(), task_optimizer);
        }
    ...
    /// Create task-specific optimizer
    fn create_task_optimizer(&self, problem: &OptimizationProblem) -> Result<TaskSpecificOptimizer> {
        let param_size = self.estimate_parameter_size(problem);
        
        Ok(TaskSpecificOptimizer {
            parameters: Array1::from_shape_fn(param_size, |_| rand::random::<f64>() * 0.1),
            performance_history: Vec::new(),
    ...
    fn train_on_task(&mut self, optimizer: &TaskSpecificOptimizer, task: &TrainingTask) -> Result<f64> {
    ...
    fn update_meta_parameters(&mut self, problem: &OptimizationProblem, performance: f64) -> Result<()> {
        ...
        for i in 0..self.meta_state.meta_params.len() {
            ...
            let update = learning_rate * performance_gradient * (rand::random::<f64>() - 0.5) * 0.1;
    ...
    pub fn adapt_to_new_problem(&mut self, problem: &OptimizationProblem) -> Result<TaskSpecificOptimizer> {
    ...
    /// Find most similar task
    fn find_most_similar_task(&self, problem: &OptimizationProblem) -> Result<Option<&TaskSpecificOptimizer>> {
    ...
    fn compute_task_similarity(&self, problem: &OptimizationProblem, task_name: &str) -> Result<f64> {
    ...
    fn adapt_optimizer_parameters(&self, optimizer: &mut TaskSpecificOptimizer, problem: &OptimizationProblem) -> Result<()> {
    ...
    fn fine_tune_for_problem(&mut self, optimizer: &mut TaskSpecificOptimizer, _problem: &OptimizationProblem) -> Result<()> {
    ...
    fn update_meta_stats(&mut self) {
        ...
        if !self.meta_state.performance_history.is_empty() {
            let recent_improvements: Vec<f64> = self.meta_state.performance_history
                .windows(2)
                .map(|w| w[1] - w[0])
                .collect();
            
            if !recent_improvements.is_empty() {
                self.meta_stats.avg_adaptation_speed = recent_improvements.iter()
                    .map(|&x| if x > 0.0 { 1.0 } else { 0.0 })
                    .sum::<f64>() / recent_improvements.len() as f64;
            }
        }
        ...
        self.meta_stats.meta_gradient_norm = (self.meta_state.meta_params.iter()
            .map(|&x| x * x)
            .sum::<f64>()).sqrt();
    }
}
...
impl LearnedOptimizer for MetaLearningOptimizer {
    ...
    fn adapt_to_problem(&mut self, problem: &OptimizationProblem, _initial_params: &ArrayView1<f64>) -> Result<()> {
        ...
        self.task_optimizers.insert(problem.name.clone(), adapted_optimizer);
        Ok(())
    }
        ...
        F: Fn(&ArrayView1<f64>) -> f64,
    {
        ...
        for iter in 0..1000 {
            ...
            // Check convergence
            if direction.iter().map(|&x| x.abs()).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(0.0) < 1e-8 {
                break;
            }
        }
...
mod tests {
    ...
    fn test_meta_direction_computation() {
        ...
        let direction = optimizer.compute_meta_direction(&params, &objective).unwrap();
    ...
    fn test_task_similarity() {
        ...
        let similarity1 = optimizer.compute_task_similarity(&problem, "quadratic_task").unwrap();
        let similarity2 = optimizer.compute_task_similarity(&problem, "neural_network_task").unwrap();
