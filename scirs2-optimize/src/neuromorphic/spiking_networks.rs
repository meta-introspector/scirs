...
impl SpikingNeuron {
    ...
    pub fn update(&mut self, dt: f64, external_current: f64) -> Option<f64> {
        ...
        let dv_dt = (-(self.membrane_potential - self.resting_potential) + total_current) / self.tau_membrane;
            ...
            for synapse in &self.synapses[source_neuron] {
                if synapse.target == target_neuron {
                    ...
                    if let Some(last_spike) = self.neurons[source_neuron].last_spike_time {
                        ...
                        if time_since_spike >= synapse.delay && time_since_spike < synapse.delay + self.config.dt {
