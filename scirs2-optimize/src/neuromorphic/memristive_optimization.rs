...
impl Memristor {
    ...
    pub fn new(params: MemristorParameters, model: MemristorModel) -> Self {
        let initial_resistance = params.r_on + (params.r_off - params.r_on) * (1.0 - params.initial_x);
    ...
    fn update_resistance(&mut self) {
        ...
        let base_resistance = self.params.r_on * self.state + self.params.r_off * (1.0 - self.state);
...
impl MemristiveCrossbar {
    ...
    pub fn new(rows: usize, cols: usize, params: MemristorParameters, model: MemristorModel) -> Self {
    ...
    pub fn multiply(&mut self, input: &ArrayView1<f64>) -> Array1<f64> {
        ...
        self.stats.avg_read_time_ns = (self.stats.avg_read_time_ns * (self.stats.operations - 1) as f64 + elapsed) / self.stats.operations as f64;
    ...
    fn multiply_simd(&self, input: &ArrayView1<f64>, output: &mut Array1<f64>) {
        for i in 0..self.rows {
            ...
            let conductances: Vec<f64> = (0..self.cols)
                .map(|j| if self.fault_map[[i, j]] { 0.0 } else { self.memristors[i][j].conductance() })
                .collect();
    ...
    pub fn update(&mut self, input: &ArrayView1<f64>, target: &ArrayView1<f64>, learning_rate: f64) -> Result<()> {
        let start_time = std::time::Instant::now();
        ...
        for i in 0..self.rows.min(error.len()) {
            for j in 0..self.cols.min(input.len()) {
                if !self.fault_map[[i, j]] {
                    ...
                    let programming_voltage = self.conductance_change_to_voltage(desired_delta_g, i, j);
        ...
        // Update statistics
        let elapsed = start_time.elapsed().as_nanos() as f64;
        self.stats.avg_write_time_ns = (self.stats.avg_write_time_ns * self.stats.operations as f64 + elapsed) / (self.stats.operations + 1) as f64;
...
impl MemristiveOptimizer {
    ...
    pub fn new(
        ...
        let crossbar = MemristiveCrossbar::new(crossbar_size, crossbar_size, memristor_params, model);
    ...
    fn apply_momentum_update(&mut self, update: &Array1<f64>) -> Result<()> {
        ...
        self.momentum_buffer = &(self.momentum * &self.momentum_buffer) + &((1.0 - self.momentum) * update);
    ...
    fn update_crossbar_weights(&mut self, gradient: &Array1<f64>, objective_value: f64) -> Result<()> {
        ...
        self.crossbar.update(&encoded_gradient.view(), &target_output.view(), 0.01)?;
    ...
    fn check_convergence(&self, history: &[f64]) -> bool {
        ...
        let variance = recent.iter().fold(0.0, |acc, &x| acc + (x - recent[0]).powi(2)) / recent.len() as f64;
    ...
    F: Fn(&ArrayView2<f64>) -> f64,
{
    ...
    for i in 0..rows {
        for j in 0..cols {
            ...
            let voltage = if initial_weights[[i, j]] > 0.0 { 1.0 } else { -1.0 };
    ...
    for _iter in 0..max_iterations {
        ...
        for i in 0..rows {
            ...
            crossbar.update(&row_input.view(), &target.view(), learning_rate).ok();
        }
    }
