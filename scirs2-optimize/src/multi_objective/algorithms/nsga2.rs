...
use super::{MultiObjectiveConfig, MultiObjectiveOptimizer, utils};
use crate::error::OptimizeError;
use crate::multi_objective::solutions::{MultiObjectiveSolution, MultiObjectiveResult, Population};
use crate::multi_objective::crossover::{SimulatedBinaryCrossover, CrossoverOperator};
use crate::multi_objective::mutation::{PolynomialMutation, MutationOperator};
use crate::multi_objective::selection::{TournamentSelection, SelectionOperator};
...
impl NSGAII {
    ...
    pub fn new(
        ...
        if n_objectives == 0 {
            return Err(OptimizeError::InvalidInput("Number of objectives must be > 0".to_string()));
        }
        if n_variables == 0 {
            return Err(OptimizeError::InvalidInput("Number of variables must be > 0".to_string()));
        }
        if config.population_size == 0 {
            return Err(OptimizeError::InvalidInput("Population size must be > 0".to_string()));
        }
        ...
        let crossover = SimulatedBinaryCrossover::new(
        ...
        F: Fn(&ArrayView1<f64>) -> Array1<f64> + Send + Sync,
    {
        ...
        if objectives.len() != self.n_objectives {
            return Err(OptimizeError::InvalidInput(
                format!("Expected {} objectives, got {}", self.n_objectives, objectives.len())
            ));
        }
        ...
        F: Fn(&ArrayView1<f64>) -> Array1<f64> + Send + Sync,
    {
        ...
        while offspring.len() < self.config.population_size {
            ...
            let bounds: Vec<(f64, f64)> = if let Some((lower, upper)) = &self.config.bounds {
                lower.iter().zip(upper.iter()).map(|(&l, &u)| (l, u)).collect()
            } else {
    ...
    fn environmental_selection(&mut self, combined_population: Vec<MultiObjectiveSolution>) -> Vec<MultiObjectiveSolution> {
...
impl MultiObjectiveOptimizer for NSGAII {
    ...
    fn check_convergence(&self) -> bool {
        ...
        if self.convergence_history.len() >= 10 {
            ...
            let max_hv = recent_history.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
