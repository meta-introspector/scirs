...
use ndarray::{Array, Array1, Array2, ArrayBase, Data, DataMut, Dimension, s};
...
impl<T: Float + Default + Clone + Send + Sync> LSTMOptimizer<T> {
    ...
    pub fn new(config: LearnedOptimizerConfig) -> Result<Self, OptimizerError> {
        ...
        D: Dimension + Clone,
    {
        ...
        self.history_buffer.update(&flat_params, &flat_gradients, loss);
        ...
        let learning_rate = self.lr_controller.compute_lr(
            &flat_gradients,
            loss,
            &self.history_buffer,
        )?;
    ...
    ) -> Result<TransferResults<T>, OptimizerError> {
        self.meta_learner.transfer_learner.transfer_to_domain(
            target_tasks,
    ...
    fn prepare_lstm_input(&self, gradients: &Array1<T>) -> Result<Array1<T>, OptimizerError> {
        ...
        if let Some(prev_gradients) = self.history_buffer.get_recent_gradients(5) {
            for prev_grad in prev_gradients {
                ...
                let grad_diff: Vec<T> = gradients.iter()
                    .zip(prev_grad.iter())
                    .map(|(&g1, &g2)| g1 - g2)
                    .collect();
        ...
        let grad_std = {
            let variance = gradients.iter()
                ...
                .sum::<T>() / T::from(gradients.len()).unwrap();
            variance.sqrt()
        };
    ...
    fn update_lstm_stats(&mut self) {
        ...
        if let Some(ref attention) = self.lstm_network.attention {
            if let Some(ref attention_weights) = attention.attention_weights {
                self.metrics.attention_stats = Some(self.compute_attention_stats(attention_weights));
            }
        }
    }
    ...
    fn compute_state_stats(&self, state: &Array1<T>) -> StateStatistics {
        ...
        let saturation_count = values.iter()
            .filter(|&&x| x.abs() > 0.95)
            .count();
    ...
    fn compute_attention_stats(&self, attention_weights: &Array2<T>) -> AttentionStats {
        let weights: Vec<f64> = attention_weights.iter()
    ...
    fn estimate_memory_usage(&self) -> f64 {
        ...
        let parameter_memory = self.config.hidden_size as f64 * self.config.num_layers as f64 * 8.0 / 1024.0 / 1024.0;
        let history_memory = self.config.gradient_history_size as f64 * self.config.input_features as f64 * 8.0 / 1024.0 / 1024.0;
        let lstm_state_memory = self.config.hidden_size as f64 * self.config.num_layers as f64 * 2.0 * 8.0 / 1024.0 / 1024.0;
        
        parameter_memory + history_memory + lstm_state_memory
    }
    
    /// Validate configuration
    fn validate_config(config: &LearnedOptimizerConfig) -> Result<(), OptimizerError> {
        if config.hidden_size == 0 {
            return Err(OptimizerError::InvalidConfig("Hidden size must be positive".to_string()));
        }
        ...
        if config.input_features == 0 {
            return Err(OptimizerError::InvalidConfig("Input features must be positive".to_string()));
        }
        
        if config.meta_learning_rate <= 0.0 {
            return Err(OptimizerError::InvalidConfig("Meta learning rate must be positive".to_string()));
        }
...
impl<T: Float + Default + Clone> LSTMNetwork<T> {
    ...
    fn new(config: &LearnedOptimizerConfig) -> Result<Self, OptimizerError> {
        ...
        for i in 0..config.num_layers {
            let input_size = if i == 0 { config.input_features } else { config.hidden_size };
    ...
    fn apply_dropout(&self, input: &Array1<T>) -> Result<Array1<T>, OptimizerError> {
...
impl<T: Float + Default + Clone> LSTMLayer<T> {
    ...
    fn new(input_size: usize, hidden_size: usize) -> Result<Self, OptimizerError> {
    ...
    fn forward(&mut self, input: &Array1<T>) -> Result<Array1<T>, OptimizerError> {
        ...
        let forget_gate = Self::sigmoid(&gates.slice(s![self.hidden_size..2*self.hidden_size]).to_owned());
        let cell_gate = Self::tanh(&gates.slice(s![2*self.hidden_size..3*self.hidden_size]).to_owned());
        let output_gate = Self::sigmoid(&gates.slice(s![3*self.hidden_size..4*self.hidden_size]).to_owned());
...
impl<T: Float + Default + Clone> HistoryBuffer<T> {
    ...
    fn new(max_length: usize) -> Self {
        Self {
            gradients: VecDeque::with_capacity(max_length),
            parameters: VecDeque::with_capacity(max_length),
            losses: VecDeque::with_capacity(max_length),
            learning_rates: VecDeque::with_capacity(max_length),
            update_magnitudes: VecDeque::with_capacity(max_length),
            max_length,
            feature_cache: None,
        }
    }
...
impl<T: Float + Default + Clone> MetaLearner<T> {
    ...
    fn step(&mut self, _tasks: &[MetaTask<T>], _network: &mut LSTMNetwork<T>) -> Result<T, OptimizerError> {
...
impl<T: Float + Default + Clone> OptimizationStateTracker<T> {
    ...
    fn update(&mut self, _gradients: &Array1<T>, _updates: &Array1<T>, _loss: Option<T>) {
