...
use ndarray::{Array, Array1, Array2, Array3, ArrayBase, Data, DataMut, Dimension, Ix1, Ix2, Ix3};
    ...
    A: Float + Default + Clone + Send + Sync + std::fmt::Debug,
{
    ...
    fn prepare_input_features(&self, gradients: &Array1<A>) -> Result<Array1<A>, OptimizerError> {
        ...
        if let Some(prev_grad) = self.gradient_history.back() {
            // Gradient difference
            let grad_diff: Vec<A> = gradients.iter()
                .zip(prev_grad.iter())
                .map(|(&g1, &g2)| g1 - g2)
                .collect();
            ...
            // Gradient ratio
            let grad_ratio: Vec<A> = gradients.iter()
                .zip(prev_grad.iter())
                .map(|(&g1, &g2)| {
        ...
        let grad_std = {
            let variance = gradients.iter()
                ...
                .sum::<A>() / A::from(gradients.len()).unwrap();
            variance.sqrt()
        };
    ...
    fn lstm_forward(&mut self, input: &Array1<A>) -> Result<Array1<A>, OptimizerError> {
        ...
        let output = self.parameters.output_weights.dot(&current_input) + &self.parameters.output_bias;
    ...
    fn lstm_cell_forward(
        ...
        let ih_linear = self.parameters.weight_ih[layer].dot(input) + &self.parameters.bias_ih[layer];
        let hh_linear = self.parameters.weight_hh[layer].dot(hidden) + &self.parameters.bias_hh[layer];
    ...
    fn compute_learned_lr(&self, gradients: &Array1<A>) -> Result<A, OptimizerError> {
        if let Some(ref lr_params) = self.parameters.lr_params {
            ...
            let grad_norm = gradients.iter().map(|&g| g * g).sum::<A>().sqrt();
            let adaptive_factor = lr_params.adaptive_factors[0] / (grad_norm + A::from(1e-8).unwrap());
    ...
    fn compute_meta_gradients(&self, _meta_batch: &[MetaTask<A>]) -> Result<HashMap<String, Array1<A>>, OptimizerError> {
        ...
        meta_grads.insert("weight_ih_0".to_string(), Array1::zeros(self.config.hidden_size * 4));
        meta_grads.insert("weight_hh_0".to_string(), Array1::zeros(self.config.hidden_size * 4));
        
        Ok(meta_grads)
    }
    
    fn update_meta_parameters(&mut self, _meta_gradients: &HashMap<String, Array1<A>>) -> Result<(), OptimizerError> {
    ...
    fn reshape_array<D>(&self, flat_array: &Array1<A>, shape: D) -> Result<Array<A, D>, OptimizerError>
    where
        D: Dimension + Clone,
    {
    ...
    pub fn transfer_to_domain(&mut self, target_tasks: &[MetaTask<A>]) -> Result<TransferResults<A>, OptimizerError> {
        ...
        for _ in 0..100 { // 100 fine-tuning steps
...
impl<A: Float + Default + Clone> LSTMParameters<A> {
    fn new(config: &LearnedOptimizerConfig) -> Result<Self, OptimizerError> {
        ...
        for layer in 0..config.num_layers {
            let input_size = if layer == 0 { config.input_features } else { config.hidden_size };
    ...
    fn random_array_2d(rows: usize, cols: usize, scale: f64) -> Array2<A> {
        ...
        Array2::zeros((rows, cols)).mapv(|_| {
            A::from(scale * (rand::random::<f64>() - 0.5)).unwrap()
        })
    }
}
