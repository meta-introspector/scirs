...
impl<T: Float, P: PolicyNetwork<T>, V: ValueNetwork<T>> PolicyGradientOptimizer<T, P, V> {
    ...
    pub fn update(&mut self, trajectory: TrajectoryBatch<T>) -> Result<RLOptimizationMetrics<T>, OptimizerError> {
    ...
    fn update_ppo_clip(&mut self, mut trajectory: TrajectoryBatch<T>) -> Result<RLOptimizationMetrics<T>, OptimizerError> {
        ...
        let old_policy_eval = self.policy_network.evaluate_actions(
        ...
        for epoch in 0..n_epochs {
            ...
            for mini_batch in mini_batches {
                ...
                let policy_eval = self.policy_network.evaluate_actions(
                ...
                let policy_loss = -surr1.iter().zip(surr2.iter())
                    .map(|(&s1, &s2)| s1.min(s2))
                    .sum::<T>() / T::from(mini_batch.observations.nrows()).unwrap();
                ...
                let value_loss = if let Some(ref value_net) = self.value_network {
                    ...
                    if self.config.ppo_config.value_clip {
                        ...
                        let value_loss_1 = (&predicted_values - &mini_batch.returns).mapv(|x| x * x);
                        let value_loss_2 = (&value_pred_clipped - &mini_batch.returns).mapv(|x| x * x);
                        
                        value_loss_1.iter().zip(value_loss_2.iter())
                            .map(|(&v1, &v2)| v1.max(v2))
                            .sum::<T>() / T::from(mini_batch.observations.nrows()).unwrap()
                    } else {
                        // Standard MSE loss
                        (&predicted_values - &mini_batch.returns).mapv(|x| x * x)
                            .mean().unwrap_or(T::zero())
                    }
                } else {
                    T::zero()
                };
                ...
                let n_clipped = ratio.iter().filter(|&&r| {
                    let clip_eps = self.config.ppo_config.clip_epsilon;
                    r < T::one() - clip_eps || r > T::one() + clip_eps
                }).count();
                clip_fraction = clip_fraction + T::from(n_clipped).unwrap() / T::from(ratio.len()).unwrap();
        ...
        // Update metrics
        let n_updates = T::from(n_epochs * ((trajectory.observations.nrows() + mini_batch_size - 1) / mini_batch_size)).unwrap();
    ...
    /// PPO with adaptive KL penalty
    fn update_ppo_adaptive_kl(&mut self, trajectory: TrajectoryBatch<T>) -> Result<RLOptimizationMetrics<T>, OptimizerError> {
    ...
    fn update_trpo(&mut self, trajectory: TrajectoryBatch<T>) -> Result<RLOptimizationMetrics<T>, OptimizerError> {
    ...
    /// REINFORCE algorithm
    fn update_reinforce(&mut self, trajectory: TrajectoryBatch<T>) -> Result<RLOptimizationMetrics<T>, OptimizerError> {
        let policy_eval = self.policy_network.evaluate_actions(
            &trajectory.observations,
            &trajectory.actions,
        )?;
        ...
        let policy_loss = if self.config.use_baseline && self.value_network.is_some() {
            ...
            -(policy_eval.log_probs * trajectory.advantages).mean().unwrap_or(T::zero())
        } else {
            // Pure REINFORCE
            -(policy_eval.log_probs * trajectory.returns).mean().unwrap_or(T::zero())
        };
    ...
    /// Actor-Critic update
    fn update_actor_critic(&mut self, trajectory: TrajectoryBatch<T>) -> Result<RLOptimizationMetrics<T>, OptimizerError> {
    ...
    fn update_networks_with_loss(
        ...
        let clipped_policy_grads = self.clip_gradients(&policy_gradients, self.config.base_config.max_grad_norm)?;
    ...
    fn compute_policy_gradients(&self, loss: T) -> Result<HashMap<String, Array1<T>>, OptimizerError> {
        ...
        for (param_name, param_values) in policy_params {
            let grad = Array1::ones(param_values.len()) * loss / T::from(param_values.len()).unwrap();
    ...
    fn compute_value_gradients(&self, loss: T) -> Result<HashMap<String, Array1<T>>, OptimizerError> {
        let mut gradients = HashMap::new();
    ...
    fn clip_gradients(&self, gradients: &HashMap<String, Array1<T>>, max_norm: T) -> Result<HashMap<String, Array1<T>>, OptimizerError> {
    ...
    fn update_policy_parameters(&mut self, gradients: &HashMap<String, Array1<T>>) -> Result<(), OptimizerError> {
    ...
    fn update_value_parameters(&mut self, gradients: &HashMap<String, Array1<T>>) -> Result<(), OptimizerError> {
    ...
    pub fn update_from_buffer(&mut self) -> Result<RLOptimizationMetrics<T>, OptimizerError> {
        if self.trajectory_buffer.is_empty() {
            return Err(OptimizerError::InvalidConfig("No trajectories in buffer".to_string()));
        }
    ...
    fn combine_trajectories(&self) -> Result<TrajectoryBatch<T>, OptimizerError> {
        if self.trajectory_buffer.is_empty() {
            return Err(OptimizerError::InvalidConfig("No trajectories to combine".to_string()));
        }
        
        let total_size: usize = self.trajectory_buffer.iter()
        ...
        for trajectory in &self.trajectory_buffer {
            ...
            combined_obs.slice_mut(s![offset..offset+size, ..])
                .assign(&trajectory.observations);
            combined_actions.slice_mut(s![offset..offset+size, ..])
                .assign(&trajectory.actions);
            combined_log_probs.slice_mut(s![offset..offset+size])
                .assign(&trajectory.log_probs);
            combined_rewards.slice_mut(s![offset..offset+size])
                .assign(&trajectory.rewards);
            combined_values.slice_mut(s![offset..offset+size])
