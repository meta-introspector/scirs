...
use super::{
    PolicyNetwork, ValueNetwork, PolicyEvaluation, TrajectoryBatch,
    RLOptimizerConfig, RLScheduler, RLOptimizationMetrics, ScheduleType,
    ActionDistribution, DistributionType
};
...
impl<T: Float> ExperienceReplayBuffer<T> {
    ...
    pub fn sample(&self, batch_size: usize) -> Vec<Experience<T>> {
        let available_size = if self.is_full { self.max_size } else { self.buffer.len() };
    ...
    pub fn len(&self) -> usize {
        if self.is_full { self.max_size } else { self.buffer.len() }
    }
...
impl<T: Float, P: PolicyNetwork<T>, V: ValueNetwork<T>> ActorCriticOptimizer<T, P, V> {
    ...
    pub fn new(
        ...
        if critics.is_empty() {
            return Err(OptimizerError::InvalidConfig("At least one critic required".to_string()));
        }
    ...
    pub fn update_from_replay(&mut self, batch_size: usize) -> Result<ActorCriticMetrics<T>, OptimizerError> {
        if self.replay_buffer.len() < batch_size {
            return Err(OptimizerError::InvalidConfig("Not enough experiences in buffer".to_string()));
        }
        ...
        match self.config.method {
            ...
            _ => Err(OptimizerError::InvalidConfig("Method not implemented".to_string())),
        }
    }
    
    /// Update using trajectory (on-policy methods)
    pub fn update_from_trajectory(&mut self, trajectory: TrajectoryBatch<T>) -> Result<ActorCriticMetrics<T>, OptimizerError> {
        match self.config.method {
            ActorCriticMethod::A2C => self.update_a2c(trajectory),
            ActorCriticMethod::A3C => self.update_a3c(trajectory),
            _ => Err(OptimizerError::InvalidConfig("Method requires experience replay".to_string())),
        }
    }
    
    /// SAC update
    fn update_sac(&mut self, experiences: &[Experience<T>]) -> Result<ActorCriticMetrics<T>, OptimizerError> {
    ...
    /// TD3 update
    fn update_td3(&mut self, experiences: &[Experience<T>]) -> Result<ActorCriticMetrics<T>, OptimizerError> {
        let batch_size = experiences.len();
        ...
        if self.critics.len() >= 2 {
            ...
            let target_actions = if let Some(ref target_actor) = self.target_actor {
                ...
                let mut target_actions = self.sample_actions_from_distribution(&target_action_dist)?;
            ...
            for i in 0..rewards.len() {
                td_targets[i] = rewards[i] + gamma * min_target_q[i] * 
                    T::from(if dones[i] { 0.0 } else { 1.0 }).unwrap();
            }
    ...
    /// DDPG update
    fn update_ddpg(&mut self, experiences: &[Experience<T>]) -> Result<ActorCriticMetrics<T>, OptimizerError> {
        let batch_size = experiences.len();
        ...
        let mut td_targets = Array1::zeros(rewards.len());
        for i in 0..rewards.len() {
            td_targets[i] = rewards[i] + gamma * target_q[i] * 
                T::from(if dones[i] { 0.0 } else { 1.0 }).unwrap();
        }
    ...
    fn compute_actor_loss_td3(&self, states: &Array2<T>) -> Result<T, OptimizerError> {
        ...
        let actor_loss = -q_values.mean().unwrap_or(T::zero());
    ...
    fn update_a2c(&mut self, trajectory: TrajectoryBatch<T>) -> Result<ActorCriticMetrics<T>, OptimizerError> {
        // Compute advantages
        ...
        let critic_loss = (&values - &traj_copy.returns).mapv(|x| x * x).mean().unwrap_or(T::zero());
        
        // Update actor
        let policy_eval = self.actor.evaluate_actions(&traj_copy.observations, &traj_copy.actions)?;
        let actor_loss = -(policy_eval.log_probs * traj_copy.advantages).mean().unwrap_or(T::zero());
        
        // Update metrics
        self.metrics.actor_loss = actor_loss;
        self.metrics.critic_losses = vec![critic_loss];
        self.metrics.policy_entropy = policy_eval.entropy.mean().unwrap_or(T::zero());
    ...
    fn update_a3c(&mut self, trajectory: TrajectoryBatch<T>) -> Result<ActorCriticMetrics<T>, OptimizerError> {
    ...
    fn update_a2c_from_experiences(&mut self, experiences: &[Experience<T>]) -> Result<ActorCriticMetrics<T>, OptimizerError> {
    ...
    fn extract_states(&self, experiences: &[Experience<T>]) -> Result<Array2<T>, OptimizerError> {
        if experiences.is_empty() {
            return Err(OptimizerError::InvalidConfig("Empty experience batch".to_string()));
        }
    ...
    fn extract_next_states(&self, experiences: &[Experience<T>]) -> Result<Array2<T>, OptimizerError> {
        let batch_size = experiences.len();
    ...
    fn compute_q_values(&self, critic: &V, states: &Array2<T>, actions: &Array2<T>) -> Result<Array1<T>, OptimizerError> {
        // Simplified Q-value computation
        critic.evaluate_value(states)
    }
    
    fn compute_target_q_sac(&self, next_states: &Array2<T>, rewards: &Array1<T>, dones: &Array1<bool>) -> Result<Array1<T>, OptimizerError> {
        // Simplified target Q computation for SAC
        Ok(rewards.clone())
    }
    
    fn compute_critic_loss(&self, q_values: &Array1<T>, target_q: &Array1<T>) -> Result<T, OptimizerError> {
        Ok((q_values - target_q).mapv(|x| x * x).mean().unwrap_or(T::zero()))
    }
    ...
    fn compute_actor_loss_sac(&self, states: &Array2<T>) -> Result<T, OptimizerError> {
        ...
        let actor_loss = -actor_objective.mean().unwrap_or(T::zero());
    ...
    fn update_temperature_sac(&mut self, states: &Array2<T>) -> Result<T, OptimizerError> {
        ...
        let sampled_actions = self.sample_actions_from_distribution(&action_dist)?;
        let log_probs = self.compute_log_probabilities(&action_dist, &sampled_actions)?;
        let current_entropy = -log_probs.mean().unwrap_or(T::zero());
        
        // Target entropy (typically -action_dim for continuous actions)
        let target_entropy = self.config.sac_config.target_entropy
        ...
        self.temperature = (self.temperature - temp_lr * temp_gradient).max(T::from(0.001).unwrap());
        
        Ok(temperature_loss)
    }
    
    /// Sample actions from action distribution
    fn sample_actions_from_distribution(&self, action_dist: &ActionDistribution<T>) -> Result<Array2<T>, OptimizerError> {
        match action_dist.distribution_type {
            DistributionType::Gaussian => {
                if let (Some(ref mean), Some(ref std)) = (&action_dist.mean, &action_dist.std) {
                    ...
                    Err(OptimizerError::InvalidConfig("Invalid Gaussian distribution".to_string()))
                }
            }
            DistributionType::Categorical => {
                if let Some(ref logits) = action_dist.logits {
                    ...
                    for i in 0..logits.nrows() {
                        ...
                        let exp_logits: Vec<T> = row.iter().map(|&x| (x - max_logit).exp()).collect();
                    ...
                    Err(OptimizerError::InvalidConfig("Invalid categorical distribution".to_string()))
                }
            }
            _ => Err(OptimizerError::InvalidConfig("Unsupported distribution type".to_string())),
        }
    }
    
    /// Compute log probabilities of actions under distribution
    fn compute_log_probabilities(&self, action_dist: &ActionDistribution<T>, actions: &Array2<T>) -> Result<Array1<T>, OptimizerError> {
        match action_dist.distribution_type {
            DistributionType::Gaussian => {
                if let (Some(ref mean), Some(ref std)) = (&action_dist.mean, &action_dist.std) {
                    ...
                    for i in 0..actions.nrows() {
                        ...
                        for j in 0..actions.ncols() {
                            ...
                            let log_prob_term = -T::from(0.5).unwrap() * normalized_diff * normalized_diff 
                                - sigma.ln() - T::from(0.5 * 2.0 * std::f64::consts::PI).unwrap().ln();
            ...
            DistributionType::Categorical => {
                if let Some(ref logits) = action_dist.logits {
                    ...
                    for i in 0..actions.nrows() {
                        ...
                        let row = logits.row(i);
                        let max_logit = row.iter().fold(T::neg_infinity(), |acc, &x| acc.max(x));
                        let log_sum_exp = (row.iter().map(|&x| (x - max_logit).exp()).sum::<T>()).ln() + max_logit;
    ...
    fn experiences_to_trajectory(&self, experiences: &[Experience<T>]) -> Result<TrajectoryBatch<T>, OptimizerError> {
        let states = self.extract_states(experiences)?;
        let actions = self.extract_actions(experiences)?;
        let rewards = self.extract_rewards(experiences)?;
        let dones = self.extract_dones(experiences)?;
