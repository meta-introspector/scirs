...
use super::{PolicyNetwork, RLOptimizerConfig, RLOptimizationMetrics, TrajectoryBatch};
...
impl<T: Float, P: PolicyNetwork<T>> NaturalPolicyGradient<T, P> {
    ...
    pub fn update(&mut self, trajectory: TrajectoryBatch<T>, gradients: Array1<T>) -> Result<RLOptimizationMetrics<T>, OptimizerError> {
    ...
    /// Update Fisher Information Matrix
    fn update_fisher_information(&mut self, trajectory: &TrajectoryBatch<T>) -> Result<(), OptimizerError> {
        match self.config.fisher_method {
            ...
            FisherEstimationMethod::BlockDiagonal => self.update_block_diagonal_fisher(trajectory)?,
            FisherEstimationMethod::KroneckerFactored => self.update_kronecker_factors(trajectory)?,
            _ => {
    ...
    fn update_empirical_fisher(&mut self, trajectory: &TrajectoryBatch<T>) -> Result<(), OptimizerError> {
    ...
    fn update_true_fisher(&mut self, trajectory: &TrajectoryBatch<T>) -> Result<(), OptimizerError> {
    ...
    fn update_diagonal_fisher(&mut self, trajectory: &TrajectoryBatch<T>) -> Result<(), OptimizerError> {
    ...
    fn update_block_diagonal_fisher(&mut self, _trajectory: &TrajectoryBatch<T>) -> Result<(), OptimizerError> {
    ...
    fn update_kronecker_factors(&mut self, _trajectory: &TrajectoryBatch<T>) -> Result<(), OptimizerError> {
    ...
    /// Compute natural gradients
    fn compute_natural_gradients(&self, gradients: &Array1<T>) -> Result<Array1<T>, OptimizerError> {
    ...
    fn solve_fisher_system(&self, fisher: &Array2<T>, rhs: &Array1<T>) -> Result<Array1<T>, OptimizerError> {
        let n = rhs.len();
    ...
    /// Apply natural gradient update
    fn apply_natural_gradient_update(&mut self, natural_gradients: &Array1<T>) -> Result<(), OptimizerError> {
    ...
    fn apply_trust_region_constraint(&self, update: &Array1<T>) -> Result<Array1<T>, OptimizerError> {
    ...
    /// Compute log probability gradients
    fn compute_log_prob_gradients(&self, _obs: &Array1<T>, _action: &Array1<T>) -> Result<Array1<T>, OptimizerError> {
    ...
    fn add_to_empirical_fisher(&mut self, gradient: &Array1<T>) -> Result<(), OptimizerError> {
        ...
        self.empirical_fisher_accumulator.gradient_history.push(gradient.clone());
        
        Ok(())
    }
    
    /// Finalize empirical Fisher matrix computation
    fn finalize_empirical_fisher(&mut self) -> Result<(), OptimizerError> {
