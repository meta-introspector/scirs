...
use ndarray::{Array1, Array2, ArrayBase, Data, Dimension, s};
...
use super::{
    ...
    architecture_space::{OptimizerComponent, ComponentType}
};
...
pub trait ArchitectureController<T: Float>: Send + Sync {
    ...
    fn generate_architecture(&mut self, context: &SearchContext<T>) -> Result<OptimizerArchitecture<T>, OptimizerError>;
    ...
    fn reset(&mut self) -> Result<(), OptimizerError>;
}
...
impl<T: Float + Default + Clone + Send + Sync> RNNController<T> {
    ...
    pub fn new(hidden_size: usize, num_layers: usize, vocab_size: usize) -> Result<Self, OptimizerError> {
    ...
    fn forward(&mut self, input_sequence: &[usize]) -> Result<Array2<T>, OptimizerError> {
        ...
        // Reset states at beginning of sequence
    ...
    fn sample_architecture(&mut self) -> Result<OptimizerArchitecture<T>, OptimizerError> {
        ...
        for _ in 0..self.config.max_sequence_length {
            ...
            // Check for end token
            if token == 1 { // Assuming 1 is end token
                break;
            }
    ...
    fn sample_from_logits(&self, logits: &Array1<T>) -> Result<usize, OptimizerError> {
        // Apply temperature scaling
        let scaled_logits = logits.mapv(|x| x / T::from(self.config.temperature).unwrap());
        
        // Softmax
        let max_logit = scaled_logits.iter().cloned().fold(T::neg_infinity(), |a, b| if a > b { a } else { b });
    ...
    fn create_component_from_type(&self, component_type: ComponentType) -> Result<OptimizerComponent<T>, OptimizerError> {
        let mut hyperparameters = HashMap::new();
        
        // Set default hyperparameters based on component type
...
impl<T: Float + Default + Clone + Send + Sync> ArchitectureController<T> for RNNController<T> {
    fn initialize(&mut self, search_space: &SearchSpaceConfig) -> Result<(), OptimizerError> {
        ...
        // Initialize action space from search space
        self.action_space = ActionSpace {
            component_types: search_space.optimizer_components.iter()
    ...
    fn generate_architecture(&mut self, _context: &SearchContext<T>) -> Result<OptimizerArchitecture<T>, OptimizerError> {
    ...
    fn update_with_results(&mut self, results: &[SearchResult<T>]) -> Result<(), OptimizerError> {
        ...
        let avg_reward = if !results.is_empty() {
            results.iter()
                .filter_map(|r| r.evaluation_results.metric_scores.get(&super::EvaluationMetric::FinalPerformance))
                .sum::<T>() / T::from(results.len()).unwrap()
        } else {
            T::zero()
        };
        
        self.statistics.controller_metrics.insert("avg_reward".to_string(), avg_reward);
...
impl<T: Float + Default + Clone + Send + Sync> RNNController<T> {
    fn architecture_to_sequence(&self, architecture: &OptimizerArchitecture<T>) -> Result<Vec<usize>, OptimizerError> {
        let mut sequence = vec![0]; // Start token
        
        for component in &architecture.components {
            // Find component type index
            if let Some(pos) = self.action_space.component_types.iter().position(|&x| x == component.component_type) {
...
impl<T: Float + Default + Clone + Send + Sync> TransformerController<T> {
    ...
    pub fn new(model_dim: usize, num_heads: usize, num_layers: usize) -> Result<Self, OptimizerError> {
...
impl<T: Float + Default + Clone + Send + Sync> ArchitectureController<T> for TransformerController<T> {
    fn initialize(&mut self, search_space: &SearchSpaceConfig) -> Result<(), OptimizerError> {
        self.search_space = Some(search_space.clone());
        self.generation_count = 0;
        Ok(())
    ...
    fn update_with_results(&mut self, _results: &[SearchResult<T>]) -> Result<(), OptimizerError> {
...
impl<T: Float + Default + Clone + Send + Sync> RandomController<T> {
    ...
    pub fn new(vocab_size: usize) -> Result<Self, OptimizerError> {
        Ok(Self {
            component_types: vec![ComponentType::Adam, ComponentType::SGD, ComponentType::AdaGrad],
            search_space: None,
            statistics: ControllerStatistics::default(),
            seed: None,
            generation_count: 0,
...
impl<T: Float + Default + Clone + Send + Sync> ArchitectureController<T> for RandomController<T> {
    fn initialize(&mut self, search_space: &SearchSpaceConfig) -> Result<(), OptimizerError> {
        self.search_space = Some(search_space.clone());
        self.component_types = search_space.optimizer_components.iter()
            .map(|c| c.component_type)
            .collect();
        self.generation_count = 0;
        Ok(())
    ...
    fn generate_architecture(&mut self, _context: &SearchContext<T>) -> Result<OptimizerArchitecture<T>, OptimizerError> {
        ...
        let component_type = self.component_types[rand::random::<usize>() % self.component_types.len()];
        
        let mut hyperparameters = HashMap::new();
        match component_type {
            ComponentType::Adam => {
                hyperparameters.insert("learning_rate".to_string(), T::from(rand::random::<f64>() * 0.01).unwrap());
                hyperparameters.insert("beta1".to_string(), T::from(0.8 + rand::random::<f64>() * 0.19).unwrap());
                hyperparameters.insert("beta2".to_string(), T::from(0.9 + rand::random::<f64>() * 0.099).unwrap());
            },
            ComponentType::SGD => {
                hyperparameters.insert("learning_rate".to_string(), T::from(rand::random::<f64>() * 0.1).unwrap());
                hyperparameters.insert("momentum".to_string(), T::from(rand::random::<f64>() * 0.99).unwrap());
            },
            _ => {
                hyperparameters.insert("learning_rate".to_string(), T::from(rand::random::<f64>() * 0.01).unwrap());
            }
    ...
    fn update_with_results(&mut self, _results: &[SearchResult<T>]) -> Result<(), OptimizerError> {
        // Random controller doesn't learn from results
        Ok(())
    }
...
impl<T: Float + Default + Clone> RNNLayer<T> {
    fn new(layer_type: RNNType, input_size: usize, hidden_size: usize) -> Result<Self, OptimizerError> {
    ...
    fn forward(
        ...
        hidden: &Array1<T>,
        cell: &Array1<T>,
    ) -> Result<(Array1<T>, Array1<T>), OptimizerError> {
    ...
    fn lstm_forward(
        ...
        let forget_gate = Self::sigmoid(&combined.slice(s![hidden_size..2*hidden_size]).to_owned());
        let cell_gate = Self::tanh(&combined.slice(s![2*hidden_size..3*hidden_size]).to_owned());
        let output_gate = Self::sigmoid(&combined.slice(s![3*hidden_size..4*hidden_size]).to_owned());
    ...
    fn gru_forward(
        ...
        let reset_gate = Self::sigmoid(&(gi.slice(s![0..hidden_size]).to_owned() + gh.slice(s![0..hidden_size]).to_owned()));
        let update_gate = Self::sigmoid(&(gi.slice(s![hidden_size..2*hidden_size]).to_owned() + gh.slice(s![hidden_size..2*hidden_size]).to_owned()));
        
        let new_gate = Self::tanh(&(gi.slice(s![2*hidden_size..3*hidden_size]).to_owned() + &reset_gate * &gh.slice(s![2*hidden_size..3*hidden_size]).to_owned()));
...
impl<T: Float + Default + Clone> OutputLayer<T> {
    fn new(input_size: usize, output_size: usize, activation: ActivationType) -> Result<Self, OptimizerError> {
        Ok(Self {
...
impl<T: Float + Default + Clone> EmbeddingLayer<T> {
    fn new(vocab_size: usize, embedding_dim: usize) -> Result<Self, OptimizerError> {
        Ok(Self {
    ...
    fn forward(&self, token: usize) -> Result<Array1<T>, OptimizerError> {
        if token >= self.vocab_size {
            return Err(OptimizerError::InvalidConfig(format!("Token {} out of vocabulary", token)));
        }
...
impl<T: Float + Default + Clone> TransformerLayer<T> {
    fn new(model_dim: usize, num_heads: usize, ff_dim: usize) -> Result<Self, OptimizerError> {
        Ok(Self {
            attention: MultiHeadAttention::new(model_dim, num_heads)?,
            feed_forward: FeedForward::new(model_dim, ff_dim)?,
...
impl<T: Float + Default + Clone> MultiHeadAttention<T> {
    fn new(model_dim: usize, num_heads: usize) -> Result<Self, OptimizerError> {
        let head_dim = model_dim / num_heads;
        
        Ok(Self {
            num_heads,
            head_dim,
            query_proj: Array2::zeros((model_dim, model_dim)),
            key_proj: Array2::zeros((model_dim, model_dim)),
            value_proj: Array2::zeros((model_dim, model_dim)),
            output_proj: Array2::zeros((model_dim, model_dim)),
            dropout: 0.1,
        })
    }
}
...
impl<T: Float + Default + Clone> FeedForward<T> {
    fn new(input_dim: usize, ff_dim: usize) -> Result<Self, OptimizerError> {
        Ok(Self {
            linear1: LinearLayer::new(input_dim, ff_dim)?,
...
impl<T: Float + Default + Clone> PositionalEncoding<T> {
    fn new(max_length: usize, model_dim: usize) -> Result<Self, OptimizerError> {
