...
use ndarray::{Array, Dimension, Array1, Array2};
...
impl<A: Float + 'static, D: Dimension + 'static> SelfTuningOptimizer<A, D> {
    ...
    pub fn new(config: SelfTuningConfig) -> Result<Self> {
        ...
        optimizer_candidates.push(OptimizerCandidate {
            name: "Adam".to_string(),
            factory: Box::new(|| {
                Box::new(AdamOptimizerWrapper::new(0.001, 0.9, 0.999, 1e-8, 0.0))
            }),
        ...
        optimizer_candidates.push(OptimizerCandidate {
            name: "SGD".to_string(),
            factory: Box::new(|| {
                Box::new(SGDOptimizerWrapper::new(0.01, 0.9, 0.0, false))
            }),
            performance_history: Vec::new(),
            usage_count: 0,
            average_performance: 0.0,
    ...
    fn should_adapt_optimizer(&self, stats: &PerformanceStats) -> bool {
        ...
        let older_performance: Vec<f64> = self.performance_history
            .iter()
            .rev()
        ...
        for (i, candidate) in self.optimizer_candidates.iter().enumerate() {
            let ucb_score = if self.bandit_state.selection_counts[i] == 0 {
                ...
                let confidence = (self.bandit_state.exploration_param * 
                    ...
                    self.bandit_state.selection_counts[i] as f64).sqrt();
    ...
    fn select_epsilon_greedy(&self) -> usize {
        ...
        if rng.gen::<f64>() < self.config.exploration_rate {
            ...
            self.bandit_state.reward_estimates
                .iter()
                .enumerate()
    ...
    fn select_optimizer_performance_based(&self) -> usize {
        self.optimizer_candidates
            .iter()
            .enumerate()
            .max_by(|a, b| a.1.average_performance.partial_cmp(&b.1.average_performance).unwrap())
            .map(|(idx, _)| idx)
            .unwrap_or(0)
    }
    
    /// Meta-learning based optimizer selection
    fn select_optimizer_meta_learning(&self, _stats: &PerformanceStats) -> usize {
    ...
    fn maybe_adapt_learning_rate(&mut self, stats: &PerformanceStats) -> Result<()> {
        ...
        let clamped_lr = new_lr.max(self.search_state.lr_bounds.0)
                             .min(self.search_state.lr_bounds.1);
        
        if (clamped_lr - current_lr).abs() > current_lr * 0.01 {
            self.current_optimizer.set_learning_rate(A::from(clamped_lr).unwrap());
    ...
    fn maybe_adapt_hyperparameters(&mut self, _stats: &PerformanceStats) -> Result<()> {
    ...
    pub fn get_statistics(&self) -> SelfTuningStatistics {
        let optimizer_usage: HashMap<String, usize> = self.optimizer_candidates
            .iter()
        ...
        SelfTuningStatistics {
            ...
            total_optimizer_switches: self.optimizer_candidates.iter().map(|c| c.usage_count).sum(),
            optimizer_usage,
            current_learning_rate: self.search_state.learning_rate,
            average_step_time: self.performance_history
                .iter()
...
impl<A: Float + 'static, D: Dimension + 'static> OptimizerTrait<A, D> for AdamOptimizerWrapper<A, D> {
    fn name(&self) -> &str { "Adam" }
    
    fn step(&mut self, params: &mut [Array<A, D>], grads: &[Array<A, D>]) -> Result<()> {
        self.inner.step(params, grads)
    }
    
    fn learning_rate(&self) -> A { self.inner.learning_rate() }
    
    fn set_learning_rate(&mut self, lr: A) { self.inner.set_learning_rate(lr); }
    
    fn get_state(&self) -> HashMap<String, Vec<u8>> { HashMap::new() }
    
    fn set_state(&mut self, _state: HashMap<String, Vec<u8>>) -> Result<()> { Ok(()) }
...
impl<A: Float + 'static, D: Dimension + 'static> OptimizerTrait<A, D> for SGDOptimizerWrapper<A, D> {
...
impl<A: Float + 'static, D: Dimension + 'static> OptimizerTrait<A, D> for AdamWOptimizerWrapper<A, D> {
