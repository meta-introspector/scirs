...
enum BackwardFunction<T: Float> {
    ...
    MatMulBackward { transpose_lhs: bool, transpose_rhs: bool },
...
enum SavedValues<T: Float> {
    ...
    Indices { start: Vec<usize>, end: Vec<usize> },
}
...
impl<T: Float + Default + Clone> ReverseModeEngine<T> {
    ...
    pub fn subtract(&mut self, lhs: usize, rhs: usize) -> Result<usize, OptimizerError> {
        self.binary_op(ReverseOpType::Subtract, lhs, rhs, BackwardFunction::SubtractBackward)
    }
    ...
    pub fn sin(&mut self, input: usize) -> Result<usize, OptimizerError> {
        self.unary_op(
            ReverseOpType::Sin,
            input,
            BackwardFunction::TrigBackward { function: TrigFunction::Sin },
        )
    }
    ...
    pub fn cos(&mut self, input: usize) -> Result<usize, OptimizerError> {
        self.unary_op(
            ReverseOpType::Cos,
            input,
            BackwardFunction::TrigBackward { function: TrigFunction::Cos },
        )
    }
    ...
    pub fn tanh(&mut self, input: usize) -> Result<usize, OptimizerError> {
        self.unary_op(
            ReverseOpType::Tanh,
            input,
            BackwardFunction::ActivationBackward { function: ActivationFunction::Tanh },
        )
    }
    ...
    pub fn sigmoid(&mut self, input: usize) -> Result<usize, OptimizerError> {
        self.unary_op(
            ...
            input,
            BackwardFunction::ActivationBackward { function: ActivationFunction::Sigmoid },
        )
    }
    ...
    pub fn relu(&mut self, input: usize) -> Result<usize, OptimizerError> {
        self.unary_op(
            ReverseOpType::ReLU,
            input,
            BackwardFunction::ActivationBackward { function: ActivationFunction::ReLU },
        )
    }
    ...
    pub fn dot(&mut self, lhs: usize, rhs: usize) -> Result<usize, OptimizerError> {
        self.binary_op(ReverseOpType::Dot, lhs, rhs, BackwardFunction::MultiplyBackward)
    }
    ...
    pub fn backward(&mut self, output_id: usize, gradient: Option<Array1<T>>) -> Result<(), OptimizerError> {
    ...
    fn binary_op(&mut self, op_type: ReverseOpType, lhs: usize, rhs: usize, backward_fn: BackwardFunction<T>) -> Result<usize, OptimizerError> {
        let output_id = self.tape.len();
    ...
    fn unary_op(&mut self, op_type: ReverseOpType, input: usize, backward_fn: BackwardFunction<T>) -> Result<usize, OptimizerError> {
        let input_val = self.get_value(input)?;
    ...
    fn get_value(&self, var_id: usize) -> Result<Array1<T>, OptimizerError> {
        if var_id >= self.tape.len() {
            return Err(OptimizerError::InvalidConfig("Invalid variable ID".to_string()));
        }
    ...
    fn compute_backward_pass(&self, op: &ReverseOperation<T>, output_grad: &Array1<T>) -> Result<Vec<Array1<T>>, OptimizerError> {
        match &op.backward_fn {
            BackwardFunction::Identity => {
            ...
            BackwardFunction::MultiplyBackward => {
                if let SavedValues::TensorPair(ref lhs_val, ref rhs_val) = op.saved_values {
                    ...
                    Err(OptimizerError::InvalidConfig("Missing saved values for multiplication".to_string()))
                }
            }
            ...
            BackwardFunction::DivideBackward => {
                if let SavedValues::TensorPair(ref lhs_val, ref rhs_val) = op.saved_values {
                    ...
                    Err(OptimizerError::InvalidConfig("Missing saved values for division".to_string()))
                }
            }
            ...
            BackwardFunction::PowerBackward { exponent } => {
                if let SavedValues::Tensor(ref base_val) = op.saved_values {
                    ...
                    Err(OptimizerError::InvalidConfig("Missing saved values for power".to_string()))
                }
            }
            ...
            BackwardFunction::ExpBackward => {
                if let SavedValues::Tensor(ref input_val) = op.saved_values {
                    ...
                    Err(OptimizerError::InvalidConfig("Missing saved values for exp".to_string()))
                }
            }
            ...
            BackwardFunction::LogBackward => {
                if let SavedValues::Tensor(ref input_val) = op.saved_values {
                    ...
                    Err(OptimizerError::InvalidConfig("Missing saved values for log".to_string()))
                }
            }
            ...
            BackwardFunction::TrigBackward { function } => {
                if let SavedValues::Tensor(ref input_val) = op.saved_values {
                    ...
                    Err(OptimizerError::InvalidConfig("Missing saved values for trig function".to_string()))
                }
            }
            ...
            BackwardFunction::ActivationBackward { function } => {
                if let SavedValues::Tensor(ref input_val) = op.saved_values {
                    ...
                    Err(OptimizerError::InvalidConfig("Missing saved values for activation".to_string()))
                }
            }
            
            BackwardFunction::ReductionBackward { reduction_type, input_shape, axis: _ } => {
                match reduction_type {
                    ...
                    ReductionType::Norm => {
                        if let SavedValues::Tensor(ref input_val) = op.saved_values {
                            ...
                            Err(OptimizerError::InvalidConfig("Missing saved values for norm".to_string()))
                        }
                    }
    ...
    fn estimate_memory_usage(&self) -> usize {
        ...
        let gradient_size = self.gradients.iter()
