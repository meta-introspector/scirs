...
use ndarray::{Array, Array1, Array2, Array3, ArrayBase, Data, Dimension, s};
...
impl<T: Float + Default + Clone> HigherOrderEngine<T> {
    ...
    pub fn hessian_forward_over_reverse(
        ...
        for i in 0..n {
            let grad_fn = |x: &Array1<T>| -> Array1<T> {
                self.gradient_at_point(&function, x).unwrap_or_else(|_| Array1::zeros(n))
            };
            ...
            let hessian_row = self.directional_derivative_of_gradient(&grad_fn, point, &direction)?;
    ...
    pub fn hessian_reverse_over_forward(
        ...
        for j in 0..n {
            ...
            let partial_fn = |x: &Array1<T>| -> T {
                ...
                (function(&x_plus) - function(&x_minus)) / (T::from(2.0).unwrap() * self.finite_diff_eps)
            };
    ...
    pub fn mixed_partial(
        ...
        if variables.len() != orders.len() {
            return Err(OptimizerError::InvalidConfig("Variables and orders length mismatch".to_string()));
        }
    ...
    pub fn hessian_vector_product(
        ...
        let grad_fn = |x: &Array1<T>| -> Array1<T> {
            self.gradient_at_point(&function, x).unwrap_or_else(|_| Array1::zeros(x.len()))
        };
    ...
    pub fn hessian_vector_product_advanced(
        ...
        match selected_mode {
            HvpMode::ForwardOverReverse => {
                self.hvp_forward_over_reverse(&function, point, vector)
            }
            HvpMode::ReverseOverForward => {
                self.hvp_reverse_over_forward(&function, point, vector)
            }
            HvpMode::FiniteDifference => {
                self.hvp_finite_difference(&function, point, vector)
            }
            HvpMode::PearLman => {
                self.hvp_pearlman(&function, point, vector)
            }
        }
    }
    ...
    fn gradient_at_point(
        ...
        for i in 0..n {
            ...
            gradient[i] = (function(&x_plus) - function(&x_minus)) / (T::from(2.0).unwrap() * self.finite_diff_eps);
        }
    ...
    fn finite_difference_hessian(
        ...
        for i in 0..n {
            for j in i..n { // Only compute upper triangle due to symmetry
    ...
    fn mixed_partial_finite_difference(
        ...
        if total_order > 3 {
            return Err(OptimizerError::InvalidConfig("Mixed partial order too high".to_string()));
        }
        ...
        if total_order == 2 && variables.len() == 2 {
            ...
            let mixed_partial = (function(&x_pp) - function(&x_pm) - function(&x_mp) + function(&x_mm)) /
                               (T::from(4.0).unwrap() * eps * eps);
    ...
    fn verify_hessian_accuracy(&self, computed: &Array2<T>, reference: &Array2<T>) -> Result<(), OptimizerError> {
        let max_error = self.compute_max_error(computed, reference);
        let threshold = T::from(1e-3).unwrap();
        
        if max_error > threshold {
            return Err(OptimizerError::InvalidConfig(
                format!("Hessian verification failed: max error {}", max_error.to_f64().unwrap_or(0.0))
            ));
        }
    ...
    fn estimate_memory_usage(&self) -> usize {
        let cache_size = self.derivative_cache.len() * std::mem::size_of::<(DerivativeKey, DerivativeValue<T>)>();
        let engine_size = std::mem::size_of::<Self>();
        let profiler_size = self.profiler.hessian_timings.len() * std::mem::size_of::<ComputationTiming>();
    ...
    fn select_optimal_hvp_mode(&self, problem_size: usize) -> HvpMode {
    ...
    fn apply_adaptive_sparsity(&self, mut matrix: Array2<T>, threshold: f64) -> Result<Array2<T>, OptimizerError> {
    ...
    fn compute_activation_factor(&self, activation: &Array1<T>) -> Result<Array2<T>, OptimizerError> {
    ...
    fn kronecker_product_approximation(
        &self,
        a: &Array2<T>,
        g: &Array2<T>,
    ) -> Result<Array2<T>, OptimizerError> {
    ...
    fn combine_kfac_blocks(&self, blocks: &[Array2<T>]) -> Result<Array2<T>, OptimizerError> {
        if blocks.is_empty() {
            return Err(OptimizerError::InvalidConfig("Empty K-FAC blocks".to_string()));
        }
    ...
    fn solve_linear_system(&self, matrix: &Array2<T>, rhs: &Array1<T>) -> Result<Array1<T>, OptimizerError> {
        ...
        if n != rhs.len() || n != matrix.ncols() {
            return Err(OptimizerError::InvalidConfig("Matrix dimension mismatch".to_string()));
        }
...
impl<T: Float + Default + Clone> ComputationProfiler<T> {
    ...
    fn get_summary(&self) -> PerformanceProfile {
        let avg_hessian_time = if self.hessian_timings.is_empty() {
            ...
            self.hessian_timings.iter()
                .map(|t| t.duration_us as f64)
                .sum::<f64>() / self.hessian_timings.len() as f64
        };
    ...
    fn compute_parallel_efficiency(&self) -> f64 {
        ...
        let parallel_times: Vec<_> = self.hessian_timings.iter()
            .filter(|t| t.parallel)
            .collect();
        let sequential_times: Vec<_> = self.hessian_timings.iter()
            .filter(|t| !t.parallel)
            .collect();
        ...
        let avg_parallel = parallel_times.iter()
            .map(|t| t.duration_us as f64)
            .sum::<f64>() / parallel_times.len() as f64;
        let avg_sequential = sequential_times.iter()
            .map(|t| t.duration_us as f64)
...
mod tests {
    ...
    fn test_mixed_partial() {
        ...
        let mixed_partial = engine.mixed_partial(
            function,
            &point,
            &[0, 1],
            &[1, 1],
