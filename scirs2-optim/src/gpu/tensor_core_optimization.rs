...
use ndarray::{Array2, Array, Dimension};
use num_traits::Float;
use std::sync::Arc;

#[cfg(feature = "gpu")]
use scirs2_core::gpu::{GpuContext, GpuKernel, CudaStream};

use crate::gpu::{GpuOptimizerError, GpuOptimizerConfig};
...
impl TensorCoreOptimizer {
    ...
    fn compile_kernels(
        ...
        let sparse_gemm = CudaKernel::from_ptx(context, SPARSE_TENSOR_CORE_PTX, "sparse_wmma_gemm")?;
        let fused_adam_tc = CudaKernel::from_ptx(context, FUSED_ADAM_TC_PTX, "fused_adam_tensor_core")?;
        let fused_lamb_tc = CudaKernel::from_ptx(context, FUSED_LAMB_TC_PTX, "fused_lamb_tensor_core")?;
        ...
        let fp8_gemm = if compute_capability >= (9, 0) && config.use_hopper_cores {
            Some(CudaKernel::from_ptx(context, TENSOR_CORE_FP8_PTX, "wmma_fp8_gemm")?)
        } else {
            None
        };
    ...
    fn compute_optimal_layout(&self, m: usize, n: usize, k: usize) -> OptimalLayout {
        ...
        let alignment_factor = if padding_m + padding_n + padding_k == 0 { 3.0 } else { 2.0 };
        ...
        #[cfg(feature = "gpu")]
        {
            ...
            let kernel = match precision {
                ...
                TensorCorePrecision::FP8 => {
                    self.kernels.fp8_gemm.as_ref()
                        .ok_or_else(|| GpuOptimizerError::InvalidParameters(
    ...
    pub fn fused_adam_tensor_core<T: Float>(
        ...
        #[cfg(feature = "gpu")]
        {
            ...
            self.kernels.fused_adam_tc.set_parameter("params", params.as_mut_ptr() as *mut std::ffi::c_void);
            self.kernels.fused_adam_tc.set_parameter("grads", grads.as_ptr() as *const std::ffi::c_void);
            self.kernels.fused_adam_tc.set_parameter("exp_avg", exp_avg.as_mut_ptr() as *mut std::ffi::c_void);
            self.kernels.fused_adam_tc.set_parameter("exp_avg_sq", exp_avg_sq.as_mut_ptr() as *mut std::ffi::c_void);
            self.kernels.fused_adam_tc.set_parameter("lr", &lr as *const _ as *const std::ffi::c_void);
            self.kernels.fused_adam_tc.set_parameter("beta1", &beta1 as *const _ as *const std::ffi::c_void);
            self.kernels.fused_adam_tc.set_parameter("beta2", &beta2 as *const _ as *const std::ffi::c_void);
            self.kernels.fused_adam_tc.set_parameter("eps", &eps as *const _ as *const std::ffi::c_void);
            self.kernels.fused_adam_tc.set_parameter("weight_decay", &weight_decay as *const _ as *const std::ffi::c_void);
            self.kernels.fused_adam_tc.set_parameter("step", &step as *const _ as *const std::ffi::c_void);
            self.kernels.fused_adam_tc.set_parameter("M", &m as *const _ as *const std::ffi::c_void);
            self.kernels.fused_adam_tc.set_parameter("N", &n as *const _ as *const std::ffi::c_void);
            
            self.kernels.fused_adam_tc.launch_3d(grid_dim, block_dim, 0, Some(&self.stream))?;
    ...
    fn calculate_grid_dimensions(&self, m: usize, n: usize, padding_m: usize, padding_n: usize) -> (u32, u32, u32) {
    ...
    pub fn create_mixed_precision_trainer(&self) -> Result<MixedPrecisionTrainer, GpuOptimizerError> {
    ...
    pub fn sparse_tensor_core_gemm<T: Float>(
        ...
        #[cfg(feature = "gpu")]
        {
            ...
            self.kernels.sparse_gemm.set_parameter("A", a.as_ptr() as *const std::ffi::c_void);
            self.kernels.sparse_gemm.set_parameter("B", b_sparse.values_ptr() as *const std::ffi::c_void);
            self.kernels.sparse_gemm.set_parameter("C", c.as_mut_ptr() as *mut std::ffi::c_void);
            self.kernels.sparse_gemm.set_parameter("metadata", b_sparse.metadata_ptr() as *const std::ffi::c_void);
            self.kernels.sparse_gemm.set_parameter("alpha", &alpha as *const _ as *const std::ffi::c_void);
            self.kernels.sparse_gemm.set_parameter("beta", &beta as *const _ as *const std::ffi::c_void);
            self.kernels.sparse_gemm.set_parameter("M", &m as *const _ as *const std::ffi::c_void);
            self.kernels.sparse_gemm.set_parameter("N", &n as *const _ as *const std::ffi::c_void);
            self.kernels.sparse_gemm.set_parameter("K", &k_a as *const _ as *const std::ffi::c_void);
            
            self.kernels.sparse_gemm.launch_3d(grid_dim, block_dim, 0, Some(&self.stream))?;
    ...
    pub fn benchmark_tensor_core_performance(&self) -> Result<TensorCorePerformanceBenchmark, GpuOptimizerError> {
        let mut benchmark = TensorCorePerformanceBenchmark::new();
        
        // Test different matrix sizes and precisions
        let test_sizes = vec![(512, 512, 512), (1024, 1024, 1024), (2048, 2048, 2048), (4096, 4096, 4096)];
        let precisions = vec![
    ...
    fn estimate_tensor_core_utilization(&self, m: usize, n: usize, k: usize, precision: TensorCorePrecision) -> f64 {
        let tile_m = self.config.wmma_tile_m;
        let tile_n = self.config.wmma_tile_n;
        let tile_k = self.config.wmma_tile_k;
...
impl MixedPrecisionTrainer {
    ...
    pub fn new(tensor_core_info: TensorCoreInfo, config: &TensorCoreConfig) -> Result<Self, GpuOptimizerError> {
        Ok(Self {
    ...
    pub fn select_optimal_precision(&self, operation_type: TensorCoreOperation) -> TensorCorePrecision {
    ...
    pub fn from_dense(dense: &Array2<T>) -> Self {
        ...
        for row in 0..m {
            for col_group in (0..n).step_by(4) {
                ...
                let mut indexed_values: Vec<(usize, T)> = group_indices
                    .into_iter()
                    .zip(group_values)
                    .collect();
...
pub struct TensorCorePerformanceBenchmark {
    results: std::collections::HashMap<(usize, usize, usize, TensorCorePrecision), TensorCorePerformanceResult>,
}
...
impl TensorCorePerformanceBenchmark {
    ...
    pub fn get_best_precision_for_size(&self, m: usize, n: usize, k: usize) -> Option<TensorCorePrecision> {
        ...
        for precision in [TensorCorePrecision::FP16, TensorCorePrecision::BF16, TensorCorePrecision::TF32, TensorCorePrecision::FP8] {
    ...
    pub fn generate_report(&self) -> String {
        ...
        for ((m, n, k, precision), result) in &self.results {
            ...
            report.push_str(&format!(
                ...
                result.avg_time_ms, result.tflops, result.memory_bandwidth_gb_s, result.tensor_core_utilization
            ));
        }
...
mod tests {
    ...
    fn test_precision_selection() {
        ...
        assert!(matches!(gemm_precision, TensorCorePrecision::FP16 | TensorCorePrecision::BF16 | TensorCorePrecision::TF32 | TensorCorePrecision::FP8));
        assert!(matches!(conv_precision, TensorCorePrecision::FP16 | TensorCorePrecision::BF16 | TensorCorePrecision::TF32 | TensorCorePrecision::FP8));
        assert!(matches!(attn_precision, TensorCorePrecision::FP16 | TensorCorePrecision::BF16 | TensorCorePrecision::TF32 | TensorCorePrecision::FP8));
    }
