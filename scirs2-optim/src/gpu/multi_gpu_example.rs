...
impl<T: Float> DistributedOptimizer<T> {
    ...
    pub fn register_parameters(&mut self, params: &HashMap<String, Array1<T>>) -> Result<()> {
        ...
        for (name, param) in params {
            ...
            self.gradient_buffers.insert(name.clone(), Array1::zeros(param.len()));
    ...
    pub fn step(
        ...
        for (name, param) in params.iter_mut() {
            if let Some(gradient) = self.gradient_buffers.get(name) {
                ...
                self.local_optimizer.step_parameter(param, &scaled_gradient)?;
            }
        }
...
pub fn distributed_training_example() -> Result<()> {
    ...
    for step in 0..100 {
        ...
        gradients.insert("layer1_weights".to_string(), Array1::ones(param_size) * 0.01);
        gradients.insert("layer1_bias".to_string(), Array1::ones(100) * 0.001);
        gradients.insert("layer2_weights".to_string(), Array1::ones(param_size) * 0.005);
...
pub fn benchmark_multi_gpu_communication() -> Result<()> {
    ...
    println!("Average Bandwidth: {:.2} GB/s", metrics.average_bandwidth / 1e9);
...
mod tests {
    ...
    fn test_parameter_registration() {
        ...
        assert!(distributed_optimizer.parameter_registry.contains_key("test_param"));
        assert!(distributed_optimizer.gradient_buffers.contains_key("test_param"));
    }
