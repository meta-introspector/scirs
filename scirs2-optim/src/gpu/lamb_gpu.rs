...
use crate::optimizers::{LAMB, Optimizer};
...
impl<A: Float + ScalarOperand + Debug> LAMBGpu<A> {
    ...
    pub fn new_with_config(
        ...
        Self {
            cpu_optimizer: LAMB::new_with_config(learning_rate, beta1, beta2, epsilon, weight_decay),
            gpu_memory: None,
            kernel_handle: None,
            norm_kernel_handle: None,
            on_gpu: false,
...
impl<A: Float + ScalarOperand + Debug> BatchLAMBGpu<A> {
        ...
        D: Dimension,
    {
        ...
        for (i, (opt, (params, grads))) in self.optimizers.iter_mut()
