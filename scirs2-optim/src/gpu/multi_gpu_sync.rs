...
impl MultiGpuCommunicator {
    ...
    pub fn register_gradient_buffer<T: Float>(
        ...
        self.gradient_buffers.insert(name.to_string(), gradient_buffer);
        Ok(())
    }
    
    /// Perform all-reduce operation on gradients
    pub fn all_reduce_gradients(&mut self, gradient_names: &[&str]) -> Result<()> {
    ...
    pub fn async_all_reduce_gradients(&mut self, gradient_names: &[&str]) -> Result<()> {
        ...
        let operation = CommOperation {
            ...
            expected_duration: self.estimate_operation_duration(
                CommOpType::AllReduce,
                gradient_names.len(),
            ),
        };
        
        self.comm_streams[stream_idx].active_operations.push_back(operation);
    ...
    fn allocate_device_buffer(&self, device_id: i32, size: usize) -> Result<*mut c_void> {
        #[cfg(feature = "gpu")]
        {
    ...
    fn record_communication_metrics(
        ...
        metrics.average_bandwidth = (metrics.average_bandwidth * (metrics.total_operations - 1) as f64 + bandwidth) / metrics.total_operations as f64;
        
        let latency_us = duration.as_micros() as f64;
        metrics.average_latency_us = (metrics.average_latency_us * (metrics.total_operations - 1) as f64 + latency_us) / metrics.total_operations as f64;
    ...
    fn all_reduce_nccl(&mut self, gradient_names: &[&str]) -> Result<()> {
        #[cfg(feature = "gpu")]
        {
        ...
        #[cfg(not(feature = "gpu"))]
        {
            Err(OptimError::UnsupportedOperation("NCCL not available".to_string()))
        }
    }
    
    fn all_reduce_p2p(&mut self, gradient_names: &[&str]) -> Result<()> {
    ...
    fn all_reduce_cpu(&mut self, gradient_names: &[&str]) -> Result<()> {
    ...
    fn all_reduce_rccl(&mut self, gradient_names: &[&str]) -> Result<()> {
        #[cfg(feature = "rocm")]
        {
    ...
    fn broadcast_nccl(&mut self, param_names: &[&str], master_rank: i32) -> Result<()> {
        #[cfg(feature = "gpu")]
        {
    ...
    fn broadcast_p2p(&mut self, param_names: &[&str], master_rank: i32) -> Result<()> {
    ...
    fn broadcast_cpu(&mut self, param_names: &[&str], master_rank: i32) -> Result<()> {
        // CPU-based broadcast
        for param_name in param_names {
            if let Some(buffer) = self.param_buffers.get_mut(param_name) {
                buffer.sync_status = SyncStatus::Syncing;
    ...
    fn broadcast_rccl(&mut self, param_names: &[&str], master_rank: i32) -> Result<()> {
        // RCCL broadcast implementation
        Ok(())
    }
    
    fn async_all_reduce_nccl(&mut self, gradient_names: &[&str], stream_idx: usize) -> Result<()> {
        // Asynchronous NCCL all-reduce
        Ok(())
    }
    
    fn async_all_reduce_p2p(&mut self, gradient_names: &[&str], stream_idx: usize) -> Result<()> {
    ...
    fn p2p_broadcast(&self, buffer: &ParameterBuffer, master_rank: i32) -> Result<()> {
        // P2P broadcast implementation
    ...
    fn cpu_broadcast(&self, buffer: &ParameterBuffer, master_rank: i32) -> Result<()> {
    ...
    fn initialize_rccl_communicator(&mut self) -> Result<()> {
        ...
        #[cfg(not(feature = "rocm"))]
        {
            Err(OptimError::UnsupportedOperation("ROCm not available".to_string()))
        }
    }
    ...
    fn compress_gradients(&self, gradients: &mut [f32], algorithm: CompressionAlgorithm) -> Result<f64> {
        match algorithm {
            ...
            CompressionAlgorithm::SketchedSGD { sketch_size } => self.compress_sketched_sgd(gradients, sketch_size),
    ...
    fn compress_top_k(&self, gradients: &mut [f32], k_ratio: f32) -> Result<f64> {
    ...
    fn compress_sketched_sgd(&self, gradients: &mut [f32], sketch_size: usize) -> Result<f64> {
        let n = gradients.len();
    ...
    pub fn adaptive_compression_selection(&self) -> CompressionAlgorithm {
        ...
        if metrics.average_bandwidth < 1e9 { // Low bandwidth
            CompressionAlgorithm::OneBitSGD
        } else if metrics.average_latency_us > 1000.0 { // High latency
            CompressionAlgorithm::TopK
        } else if metrics.total_operations > 1000 { // Stable training
    ...
    pub fn get_optimal_sync_strategy(&self) -> SyncStrategy {
        match self.topology.topology_type {
            TopologyType::FullyConnected if self.devices.len() <= 8 => {
                SyncStrategy::AllReduceTree
            }
            TopologyType::Ring | TopologyType::Custom => {
...
pub fn create_multi_gpu_communicator(world_size: usize, local_rank: i32) -> Result<MultiGpuCommunicator> {
...
mod tests {
    ...
    fn test_data_type_detection() {
        assert_eq!(MultiGpuCommunicator::get_data_type::<f32>(), DataType::Float32);
        assert_eq!(MultiGpuCommunicator::get_data_type::<f64>(), DataType::Float32);
    }
}