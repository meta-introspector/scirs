    ...
    D: Dimension,
{
    ...
    current_lr: F,
    /// Metric adapter
    metric_adapter: scirs2, _metrics: integration::optim::MetricOptimizer<F>,
    ...
    D: Dimension + 'static,
{
    ...
    pub fn new<O>(_optimizer: O, metric_name: &str, maximize: bool) -> Self
    where
        O: Optimizer<F, D> + 'static,
    {
        let initial_lr = _optimizer.get_learning_rate();
        Self {
            base_optimizer: Box::new(_optimizer),
            current_lr: initial_lr,
            metric_adapter: scirs2, _metrics: integration::optim::MetricOptimizer::new(
                metric_name,
                maximize,
            ),
            history: Vec::new(),
            best_params: None, _phantom: PhantomData,
        }
    }
