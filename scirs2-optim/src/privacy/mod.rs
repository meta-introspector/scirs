...
use ndarray::{Array, Dimension, ArrayBase, Data, DataMut};
    ...
    O: Optimizer<A>,
{
    ...
    pub fn get_privacy_budget(&self) -> PrivacyBudget {
        let (epsilon_consumed, delta_consumed) = self.accountant
    ...
    pub fn get_clipping_stats(&self) -> ClippingStats {
        ...
        let clipped_steps = self.gradient_history.iter()
            .filter(|stats| stats.clipping_ratio < 1.0)
            .count();
        
        let avg_clipping_ratio: f64 = self.gradient_history.iter()
            .map(|stats| stats.clipping_ratio)
            .sum::<f64>() / total_steps as f64;
        
        let avg_pre_clip_norm: f64 = self.gradient_history.iter()
    ...
    pub fn validate_privacy(&self) -> PrivacyValidation {
        ...
        if clipping_stats.clipping_frequency < 0.1 {
            warnings.push("Low clipping frequency may indicate sub-optimal privacy-utility tradeoff".to_string());
        }
    ...
    fn generate_recommendations(&self, budget: &PrivacyBudget, clipping: &ClippingStats) -> Vec<String> {
...
impl MomentsAccountant {
    ...
    fn compute_log_moments(&self, sigma: f64, q: f64, t: f64, alpha_max: f64) -> Vec<f64> {
        ...
        for alpha_int in 2..=(alpha_max as usize) {
            ...
            let log_moment = t * (q * q * alpha * (alpha - 1.0) / (2.0 * sigma * sigma)).exp().ln();
