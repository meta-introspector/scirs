...
impl OptimizationMetric {
    ...
    pub fn get_recent_improvement(&self, window_size: usize) -> Option<f64> {
        ...
        let recent_avg: f64 = self.values.iter().rev().take(window_size).sum::<f64>() / window_size as f64;
        let older_avg: f64 = self.values.iter().rev().skip(window_size).take(window_size).sum::<f64>() / window_size as f64;
...
impl OptimizationVisualizer {
    ...
    pub fn add_metric(&mut self, name: String, value: f64, higher_is_better: bool, units: String) {
        let metric = self.metrics.entry(name.clone()).or_insert_with(|| {
            OptimizationMetric::new(name, higher_is_better, units)
        });
    ...
    pub fn plot_loss_curve(&self, metric_name: &str) -> Result<String> {
        let metric = self.metrics.get(metric_name)
            .ok_or_else(|| OptimError::InvalidConfig(format!("Metric '{}' not found", metric_name)))?;
    ...
    pub fn plot_learning_rate_schedule(&self) -> Result<String> {
        if let Some(lr_metric) = self.metrics.get("learning_rate") {
            ...
            Err(OptimError::InvalidConfig("Learning rate metric not found".to_string()))
        }
    }
    ...
    pub fn plot_optimizer_comparison(&self, metric_name: &str) -> Result<String> {
        if self.comparisons.is_empty() {
            return Err(OptimError::InvalidConfig("No optimizer comparisons available".to_string()));
        }
    ...
    pub fn plot_gradient_norm(&self) -> Result<String> {
        if let Some(grad_metric) = self.metrics.get("gradient_norm") {
            ...
            Err(OptimError::InvalidConfig("Gradient norm metric not found".to_string()))
        }
    }
    ...
    pub fn plot_throughput(&self) -> Result<String> {
        if let Some(throughput_metric) = self.metrics.get("throughput") {
            ...
            Err(OptimError::InvalidConfig("Throughput metric not found".to_string()))
        }
    }
    ...
    pub fn plot_memory_usage(&self) -> Result<String> {
        if let Some(memory_metric) = self.metrics.get("memory_usage") {
            ...
            Err(OptimError::InvalidConfig("Memory usage metric not found".to_string()))
        }
    }
    
    /// Create hyperparameter sensitivity analysis
    pub fn plot_hyperparameter_sensitivity(&self, param_name: &str, metric_name: &str) -> Result<String> {
        ...
        if param_values.is_empty() {
            return Err(OptimError::InvalidConfig(
                format!("No data available for hyperparameter '{}' and metric '{}'", param_name, metric_name)
            ));
        }
        ...
        self.save_plot(&plot_data, &format!("sensitivity_{}_{}", param_name, metric_name))
    }
...
"#);
            ...
            for (name, _) in &self.metrics {
                if plot_id >= 4 { break; } // Limit to 4 plots in 2x2 grid
...
"#, title, title))
    ...
    fn save_plot(&self, plot_data: &str, filename: &str) -> Result<String> {
        let extension = if self.config.interactive_html { "html" } else { "txt" };
        ...
        let mut file = std::fs::File::create(&filepath).map_err(|e| {
            OptimError::InvalidConfig(format!("Failed to create file {}: {}", filepath.display(), e))
        })?;
        
        file.write_all(plot_data.as_bytes()).map_err(|e| {
            OptimError::InvalidConfig(format!("Failed to write to file {}: {}", filepath.display(), e))
        })?;
...
mod tests {
    ...
    fn test_optimization_metric() {
        let mut metric = OptimizationMetric::new(
            "loss".to_string(),
            false,
            "nats".to_string()
        );
