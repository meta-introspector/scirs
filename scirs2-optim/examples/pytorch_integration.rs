...
impl PyTorchSciRS2Optimizer {
            ...
            if let Ok(data) = param_dict.get_item("data") {
                if let Ok(tensor) = data.call_method0("cpu") {
                    if let Ok(numpy_array) = tensor.call_method0("numpy") {
                        ...
                        let scirs2_param = Parameter::new(
                            Array1::from_vec(array),
                            &param_name,
                        );
            ...
            if let Ok(grad) = self.get_gradient_from_pytorch(py, model, name)? {
                ...
                self.optimizer.step_param(param)
                    .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(
                        format!("Optimization step failed: {}", e)
                    ))?;
    ...
    fn add_scheduler(&mut self, scheduler_type: &str, 
                     total_steps: Option<usize>, 
                     gamma: Option<f64>) -> PyResult<()> {
        let scheduler: Box<dyn LearningRateScheduler> = match scheduler_type.to_lowercase().as_str() {
            ...
            _ => return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
                format!("Unknown scheduler type: {}", scheduler_type)
            )),
        };
...
impl PyTorchSciRS2Optimizer {
    fn get_gradient_from_pytorch(&self, py: Python, model: &PyAny, param_name: &str) -> PyResult<Option<Array1<f64>>> {
    ...
    fn update_pytorch_parameter(&self, py: Python, model: &PyAny, param_name: &str, param: &Parameter<f64>) -> PyResult<()> {
