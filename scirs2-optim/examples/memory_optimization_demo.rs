...
fn run_memory_optimization_demo() -> Result<()> {
    ...
    for (category, usage) in &usage_summary.current_usage.by_category {
        println!("  {:?}: {:.2} MB", category, *usage as f64 / 1024.0 / 1024.0);
    }
...
fn run_optimization_recommendations() -> Result<()> {
    ...
    if analysis_report.optimization_recommendations.is_empty() {
        ...
        println!("Found {} optimization opportunities:", 
                 analysis_report.optimization_recommendations.len());
        
        for (i, recommendation) in analysis_report.optimization_recommendations.iter().enumerate() {
            println!("\n{}. {} (Priority: {:?})", i + 1, recommendation.title, recommendation.priority);
    ...
    println!("Total Potential Savings: ${:.2}", cost_benefit.total_potential_savings);
    println!("Implementation Costs: ${:.2}", cost_benefit.implementation_costs);
    
    if !cost_benefit.roi_estimates.is_empty() {
        println!("\nROI Estimates:");
        for roi in &cost_benefit.roi_estimates {
            println!("  {:?}: {:.1}x ROI, Break-even in {} days", 
...
fn run_realtime_monitoring() -> Result<()> {
    ...
    if alerts.is_empty() {
        ...
        for (i, alert) in alerts.iter().enumerate() {
            println!("\n{}. {:?} Alert (Severity: {:?})", i + 1, alert.alert_type, alert.severity);
