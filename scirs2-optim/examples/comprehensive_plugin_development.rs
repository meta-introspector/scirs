...
use scirs2_optim::{
    plugin::{
        OptimizerPlugin, ExtendedOptimizerPlugin, OptimizerPluginFactory,
        PluginInfo, PluginCapabilities, PluginCategory, DataType,
        OptimizerConfig, OptimizerState, ConfigValue, StateValue,
        MemoryUsage, PerformanceMetrics, ConvergenceMetrics,
        PluginRegistry, PluginQuery, PluginQueryBuilder, PluginStatus,
        PluginFactoryWrapper, ConfigSchema, FieldSchema, FieldType, ValidationConstraint,
        RegistryConfig, PluginRegistration,
...
impl<A: Float + Clone + std::fmt::Debug + Send + Sync> OptimizerPlugin<A> for CustomMomentumOptimizer<A> {
    ...
    fn plugin_info(&self) -> PluginInfo {
        PluginInfo {
            ...
            tags: vec!["momentum".to_string(), "custom".to_string(), "tracking".to_string()],
    ...
    fn set_config(&mut self, config: OptimizerConfig) -> Result<()> {
        self.learning_rate = A::from(config.learning_rate).ok_or_else(|| {
            OptimError::InvalidConfig("Invalid learning rate type".to_string())
        })?;
        
        self.weight_decay = A::from(config.weight_decay).ok_or_else(|| {
            OptimError::InvalidConfig("Invalid weight decay type".to_string())
        })?;
        
        self.momentum = A::from(config.momentum).ok_or_else(|| {
            OptimError::InvalidConfig("Invalid momentum type".to_string())
        })?;
    ...
    fn get_state(&self) -> Result<OptimizerState> {
        ...
        if let Some(ref velocity) = self.velocity {
            state.state_vectors.insert(
                "velocity".to_string(),
                velocity.iter().map(|&x| x.to_f64().unwrap_or(0.0)).collect(),
            );
        }
    ...
    fn memory_usage(&self) -> MemoryUsage {
        let velocity_size = self.velocity.as_ref()
...
impl<A: Float + Clone + std::fmt::Debug + Send + Sync> ExtendedOptimizerPlugin<A> for CustomMomentumOptimizer<A> {
    fn batch_step(&mut self, params: &Array2<A>, gradients: &Array2<A>) -> Result<Array2<A>> {
        ...
        for (i, (param_row, grad_row)) in params.outer_iter().zip(gradients.outer_iter()).enumerate() {
...
impl<A: Float + Clone + std::fmt::Debug + Send + Sync + 'static> OptimizerPluginFactory<A> for CustomMomentumFactory {
    fn create_optimizer(&self, config: OptimizerConfig) -> Result<Box<dyn OptimizerPlugin<A>>> {
        ...
        let momentum = A::from(config.momentum).ok_or_else(|| {
            OptimError::InvalidConfig("Invalid momentum type".to_string())
        })?;
        
        let weight_decay = A::from(config.weight_decay).ok_or_else(|| {
            OptimError::InvalidConfig("Invalid weight decay type".to_string())
        })?;
...
impl scirs2_optim::plugin::RegistryEventListener for PluginEventLogger {
    fn on_plugin_registered(&mut self, info: &PluginInfo) {
        self.events.push(format!("Registered plugin: {} v{}", info.name, info.version));
    }
    ...
    fn on_plugin_load_failed(&mut self, name: &str, error: &str) {
        self.events.push(format!("Failed to load plugin {}: {}", name, error));
    }
    
    fn on_plugin_status_changed(&mut self, name: &str, status: &PluginStatus) {
        self.events.push(format!("Plugin {} status changed to: {:?}", name, status));
    }
}
...
fn main() -> Result<()> {
    ...
    let registry_config = RegistryConfig {
        ...
        allowed_sources: vec![
            scirs2_optim::plugin::PluginSource::BuiltIn,
        ],
    };
    ...
    println!("Search found {} plugin(s) in {:.2}ms:",
    ...
    for step in 0..10 {
        ...
        if step % 3 == 0 {
            ...
            println!("Step {}: params = {:?}, grad_norm = {:.6}, conv_score = {:.3}",
                step + 1,
                params.iter().map(|&x| format!("{:.3}", x)).collect::<Vec<_>>(),
    ...
    println!("  - CPU utilization: {:.1}%", perf_metrics.cpu_utilization * 100.0);
    ...
    println!("  - Parameter change norm: {:.6}", conv_metrics.parameter_change_norm);
    println!("  - Convergence score: {:.3}", conv_metrics.convergence_score);
    
    // Step 7: Test state serialization
    println!("\n💾 Step 7: Testing State Serialization");
    let state = optimizer.get_state()?;
    println!("Saved optimizer state with {} state vectors and {} custom fields",
    ...
    if let Ok(extended_opt) = optimizer.as_any_mut().downcast_mut::<CustomMomentumOptimizer<f64>>() {
    ...
    if let Ok(extended_opt) = optimizer.as_any_mut().downcast_mut::<CustomMomentumOptimizer<f64>>() {
        ...
        println!("Original gradients: {:?}", 
            test_gradients.iter().map(|&x| format!("{:.3}", x)).collect::<Vec<_>>());
        println!("Preprocessed gradients: {:?}", 
            preprocessed_grads.iter().map(|&x| format!("{:.3}", x)).collect::<Vec<_>>());
        ...
        if !trajectory.is_empty() {
            println!("Final trajectory point: {:?}", 
                trajectory.last().unwrap().iter().map(|&x| format!("{:.3}", x)).collect::<Vec<_>>());
        }
    }
