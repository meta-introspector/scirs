...
fn run_performance_profiling_demo() -> Result<()> {
    ...
    println!("Performance Score: {:.2}%", performance_report.performance_score * 100.0);
    println!("Session Duration: {:.2}s", performance_report.session_duration.as_secs_f64());
    println!("Total Steps: {}", performance_report.total_steps);
    
    println!("\nMemory Analysis:");
    println!("  Peak Usage: {:.2} MB", performance_report.memory_analysis.peak_usage_bytes as f64 / 1024.0 / 1024.0);
    println!("  Average Usage: {:.2} MB", performance_report.memory_analysis.average_usage_bytes / 1024.0 / 1024.0);
    println!("  Efficiency Score: {:.2}%", performance_report.memory_analysis.efficiency_score * 100.0);
    
    if performance_report.memory_analysis.leak_indicators.suspected_leak {
        println!("  ⚠️  Memory leak detected with {:.1}% confidence!", 
                 performance_report.memory_analysis.leak_indicators.confidence * 100.0);
    } else {
        println!("  ✅ No memory leaks detected");
    }

    println!("\nComputational Analysis:");
    println!("  Average FLOPS: {:.2} GFLOPS", performance_report.computational_analysis.average_flops / 1e9);
    println!("  Peak FLOPS: {:.2} GFLOPS", performance_report.computational_analysis.peak_flops / 1e9);
    println!("  Arithmetic Intensity: {:.2}", performance_report.computational_analysis.arithmetic_intensity);
    println!("  Vectorization Efficiency: {:.2}%", performance_report.computational_analysis.vectorization_efficiency * 100.0);

    println!("\nHardware Analysis:");
    println!("  Average CPU Utilization: {:.1}%", performance_report.hardware_analysis.cpu_utilization_avg * 100.0);
    println!("  Peak CPU Utilization: {:.1}%", performance_report.hardware_analysis.cpu_utilization_peak * 100.0);
    println!("  Memory Bandwidth Utilization: {:.1}%", performance_report.hardware_analysis.memory_bandwidth_utilization * 100.0);
    println!("  Hardware Efficiency Score: {:.2}%", performance_report.hardware_analysis.hardware_efficiency_score * 100.0);

    // Display performance recommendations
    if !performance_report.efficiency_recommendations.is_empty() {
        println!("\n💡 PERFORMANCE RECOMMENDATIONS:");
        for (i, recommendation) in performance_report.efficiency_recommendations.iter().enumerate() {
            println!("{}. {} - {}", i + 1, recommendation.title, recommendation.description);
            println!("   Expected Impact: {:.1}%", recommendation.estimated_impact * 100.0);
        }
    }
...
fn run_documentation_analysis_demo() -> Result<()> {
    ...
    println!("Overall Quality Score: {:.1}%", analysis_results.overall_quality_score * 100.0);
    
    // Coverage analysis
    println!("\nCoverage Analysis:");
    println!("  Total Public Items: {}", analysis_results.coverage.total_public_items);
    println!("  Documented Items: {}", analysis_results.coverage.documented_items);
    println!("  Coverage Percentage: {:.1}%", analysis_results.coverage.coverage_percentage);
    
    if !analysis_results.coverage.undocumented_by_category.is_empty() {
    ...
    println!("\nExample Verification:");
    println!("  Total Examples: {}", analysis_results.example_verification.total_examples);
    println!("  Compiled Examples: {}", analysis_results.example_verification.compiled_examples);
    
    if !analysis_results.example_verification.failed_examples.is_empty() {
        println!("  Failed Examples: {}", analysis_results.example_verification.failed_examples.len());
        for failed in analysis_results.example_verification.failed_examples.iter().take(3) {
    ...
    // Link checking
    println!("\nLink Checking:");
    println!("  Total Links: {}", analysis_results.link_checking.total_links);
    println!("  Valid Links: {}", analysis_results.link_checking.valid_links);
    
    if !analysis_results.link_checking.broken_links.is_empty() {
        println!("  Broken Links: {}", analysis_results.link_checking.broken_links.len());
    ...
    // Style analysis
    println!("\nStyle Analysis:");
    println!("  Consistency Score: {:.1}%", analysis_results.style_analysis.consistency_score * 100.0);
    ...
    for (i, recommendation) in doc_report.actionable_recommendations.iter().enumerate() {
        ...
        println!("   Estimated Effort: {:.1} hours", recommendation.estimated_effort_hours);
        println!("   Expected Impact: {:.1}%", recommendation.expected_impact * 100.0);
    }

    println!("\n💼 EFFORT ESTIMATION:");
    println!("Total Effort Required: {:.1} hours", doc_report.estimated_effort.total_hours);
    println!("Confidence Level: {:.1}%", doc_report.estimated_effort.confidence_level * 100.0);
...
fn run_enhanced_cross_framework_benchmarking() -> Result<()> {
    ...
    println!("Running cross-framework comparison with {} SciRS2 optimizers...", optimizers.len());
    ...
    for result in &simulated_results {
        println!("\nTest Function: {} ({}D)", result.function_name, result.problem_dim);
...
fn run_memory_leak_detection_demo() -> Result<()> {
    ...
    for step in 0..200 {
        ...
        if step % 50 == 0 {
            ...
            println!("  Step {}: Memory usage = {:.2} MB", 
                     step, 
    ...
    println!("Memory Efficiency Score: {:.2}%", 
    ...
    if leak_indicators.suspected_leak {
        ...
        println!("   Growth Rate: {:.2} bytes/step", leak_indicators.growth_rate);
        println!("   Evidence:");
    ...
    println!("  Current Ratio: {:.2}%", fragmentation.current_ratio * 100.0);
    println!("  Average Ratio: {:.2}%", fragmentation.average_ratio * 100.0);
