...
fn run_comprehensive_security_audit() -> Result<()> {
    ...
    println!("=========================");
    println!("Overall Security Score: {:.1}/100", audit_results.overall_security_score);
    println!("Total Vulnerabilities Found: {}", audit_results.total_vulnerabilities);
    ...
    if !audit_results.recommendations.is_empty() {
        ...
        for (i, recommendation) in audit_results.recommendations.iter().take(3).enumerate() {
            ...
            println!("   Estimated Effort: {:.1} dev hours, {:.1} test hours", 
    ...
    println!("\n📋 COMPLIANCE STATUS:");
    println!("Best Practices Compliance: {:.1}%", 
...
fn test_infinity_injection_attack() -> Result<()> {
    ...
    match sgd.step(&malicious_params, &malicious_gradients) {
        Ok(_) => {
            println!("    ⚠️  VULNERABILITY: Optimizer accepted infinity values without validation");
        }
        Err(e) => {
...
fn test_privacy_budget_exhaustion() -> Result<()> {
    ...
    loop {
        if !dp_optimizer.has_privacy_budget()? {
            println!("    ✅ Privacy budget exhausted after {} steps (protection working)", step_count);
            break;
        }
...
fn test_privacy_parameter_manipulation() -> Result<()> {
    ...
    for (i, config) in invalid_configs.iter().enumerate() {
        ...
        match DifferentiallyPrivateOptimizer::new(sgd, config.clone()) {
            Ok(_) => {
                println!("    ⚠️  VULNERABILITY {}: Invalid privacy config accepted", i + 1);
            }
            Err(e) => {
                println!("    ✅ Invalid privacy config {} properly rejected: {}", i + 1, e);
            }
        }
    }
...
fn test_noise_generation_security() -> Result<()> {
    ...
    if entropy_estimate >= 7.5 {
        println!("    ✅ Noise entropy is acceptable: {:.2} bits/byte", entropy_estimate);
    } else {
        println!("    ⚠️  LOW ENTROPY: Only {:.2} bits/byte detected", entropy_estimate);
    }
    
    if autocorrelation.abs() <= 0.05 {
        println!("    ✅ Noise autocorrelation is acceptable: {:.3}", autocorrelation);
    } else {
        println!("    ⚠️  HIGH AUTOCORRELATION: {:.3} detected (should be ~0)", autocorrelation);
    }
...
fn test_memory_exhaustion_attack() -> Result<()> {
    ...
    println!("    Attempting to allocate large array ({} elements)...", large_size);
    
    // This is a simulation - we don't actually allocate to avoid issues
    if large_size > 100_000_000 { // 100M element limit
...
fn test_memory_leak_detection() -> Result<()> {
    ...
    if growth_rate > 10_000.0 { // 10KB per step threshold
...
fn test_precision_loss() -> Result<()> {
    ...
    if relative_error > 1e-10 {
        println!("    ⚠️  Significant precision loss detected: {:.2e} relative error", relative_error);
    } else {
...
fn test_ill_conditioning() -> Result<()> {
    ...
    if condition_number > 1e10 {
        println!("    ⚠️  Ill-conditioned system detected (condition number: {:.2e})", condition_number);
...
fn generate_security_report_demo() -> Result<()> {
    ...
    println!("  Overall Security Score: {:.1}/100", security_report.overall_security_score);
    
    println!("\n📊 Executive Summary:");
    println!("  {}", security_report.executive_summary);
    
    println!("\n🔍 Vulnerability Summary:");
    println!("  Total Vulnerabilities: {}", security_report.vulnerability_summary.total_vulnerabilities);
    
    for (severity, count) in &security_report.vulnerability_summary.by_severity {
        println!("    {:?}: {} vulnerabilities", severity, count);
    }
    
    println!("\n💡 Recommendations ({} total):", security_report.recommendations.len());
    ...
    println!("  Best Practices: {:.1}%", 
             security_report.compliance_assessment.best_practices_compliance);
    
    println!("\n🎯 Risk Assessment:");
    println!("  Overall Risk Level: {:?}", security_report.risk_assessment.overall_risk_level);
    println!("  Risk Factors: {} identified", security_report.risk_assessment.risk_factors.len());
    
    println!("\n📅 Remediation Timeline:");
    if !security_report.remediation_timeline.immediate_actions.is_empty() {
        println!("  Immediate Actions: {} items", security_report.remediation_timeline.immediate_actions.len());
    }
    if !security_report.remediation_timeline.short_term.is_empty() {
        println!("  Short-term Actions: {} items", security_report.remediation_timeline.short_term.len());
    }
    
    println!("\n✅ Verification Plan:");
    println!("  Strategy: {}", security_report.verification_plan.verification_strategy);
    println!("  Timeline: {} days", security_report.verification_plan.timeline.as_secs() / (24 * 3600));
