...
fn main() {
    ...
    let validations = vec![
        ...
        ("Neural RL Step Control", "../scirs2-integrate/src/neural_rl_step_control.rs"),
        ("Neural Adaptive Shader", "../scirs2-integrate/src/shaders/neural_adaptive_step_rl.comp"),
        ("API Stability Analysis", "../scirs2-interpolate/examples/api_stability_analysis.rs"),
        ("Security Audit Scanner", "src/bin/security_audit_scanner.rs"),
        ("Dependency Scanner", "src/bin/dependency_vulnerability_scanner.rs"),
    ];
    ...
    for (name, path) in validations {
        ...
        if Path::new(path).exists() {
            ...
            if let Ok(metadata) = std::fs::metadata(path) {
                if metadata.len() > 100 {
                    println!("   ✅ File exists and has content ({} bytes)", metadata.len());
    ...
    match output {
        Ok(result) => {
            if result.status.success() {
                ...
                Err(format!("Parse errors: {}", stderr))
            }
        }
        Err(e) => {
