...
use scirs2_fft::{
    create_ultrathink_coordinator, UltrathinkFftConfig, 
    FftAlgorithmType, OptimizationStrategy
};
use ndarray::Array1;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  - Algorithm Selection: {}", config.enable_algorithm_selection);
    println!("  - Adaptive Tuning: {}", config.enable_adaptive_tuning);
    println!("  - Quantum Optimization: {}", config.enable_quantum_optimization);
    println!("  - Knowledge Transfer: {}", config.enable_knowledge_transfer);
    ...
    println!("  - Recommended Algorithm: {:?}", recommendation.recommended_algorithm);
    println!("  - Confidence Score: {:.2}%", recommendation.confidence_score * 100.0);
    println!("  - Expected Execution Time: {:.2} μs", recommendation.expected_performance.execution_time);
    println!("  - Expected Memory Usage: {:.2} MB", recommendation.expected_performance.memory_usage as f64 / 1024.0 / 1024.0);
    println!("  - Expected Accuracy: {:.4}", recommendation.expected_performance.accuracy);
    ...
    println!("  - Recommended Algorithm: {:?}", chirp_recommendation.recommended_algorithm);
    println!("  - Confidence Score: {:.2}%", chirp_recommendation.confidence_score * 100.0);
    println!("  - Expected Performance Improvement: {:.1}x over naive approach", 
    ...
    println!("  - Recommended Algorithm: {:?}", sparse_recommendation.recommended_algorithm);
    println!("  - Confidence Score: {:.2}%", sparse_recommendation.confidence_score * 100.0);
    println!("  - Memory Strategy: {:?}", sparse_recommendation.memory_strategy);
    
    let start_time = Instant::now();
    let sparse_result = coordinator.execute_optimized_fft(&sparse_signal, &sparse_recommendation)?;
    ...
    println!("  - Memory Efficiency Gain: {:.1}x", estimate_memory_efficiency(&sparse_recommendation));
    println!();
    ...
    println!("  - Average Execution Time: {:.2} μs", metrics.average_execution_time);
    println!("  - Memory Efficiency Score: {:.2}", metrics.memory_efficiency);
    println!("  - Cache Hit Ratio: {:.2}%", metrics.cache_hit_ratio * 100.0);
    
    println!("\nAlgorithm Usage Statistics:");
    for (algorithm, stats) in &metrics.algorithm_distribution {
        println!("  - {:?}: {} uses, {:.2} μs avg, {:.1}% success rate",
                 algorithm, stats.usage_count, stats.avg_execution_time, stats.success_rate * 100.0);
    }
    
    println!("\nPerformance Trends:");
    println!("  - Execution Time Trend: {:.2}% improvement", 
             -metrics.performance_trends.execution_time_trend * 100.0);
    println!("  - Memory Usage Trend: {:.2}% optimization", 
             -metrics.performance_trends.memory_usage_trend * 100.0);
    println!("  - Overall Performance Score: {:.2}/10", 
        ...
        (0..length)
            .map(|_| {
                if rng.gen::<f64>() < sparsity {
                    let magnitude = rng.gen_range(-1.0..1.0);
...
fn demonstrate_adaptive_learning(
    ...
    for (i, signal) in signals.iter().enumerate() {
        ...
        println!("  Signal {}: {:?} (confidence: {:.2}%)", 
                 i + 1, execution_time, recommendation.confidence_score * 100.0);
    }
    ...
    if execution_times.len() >= 2 {
        ...
        if improvement > 0.0 {
            println!("✅ Adaptive learning detected: {:.1}% performance improvement", improvement);
        } else {
...
fn demonstrate_knowledge_transfer(
    ...
    println!("  Image Domain: {:?} (confidence: {:.2}%)", 
             img_recommendation.recommended_algorithm, img_recommendation.confidence_score * 100.0);
    ...
    println!("  Audio Domain: {:?} (confidence: {:.2}%)", 
             audio_recommendation.recommended_algorithm, audio_recommendation.confidence_score * 100.0);
    ...
    println!("  Scientific Domain: {:?} (confidence: {:.2}%)", 
             sci_recommendation.recommended_algorithm, sci_recommendation.confidence_score * 100.0);
...
fn demonstrate_quantum_optimization(
    ...
    println!("  Quantum Features Detected: {}", 
             matches!(recommendation.recommended_algorithm, 
                     scirs2_fft::FftAlgorithmType::QuantumInspiredFft));
    
    if matches!(recommendation.recommended_algorithm, scirs2_fft::FftAlgorithmType::QuantumInspiredFft) {
        ...
        println!("📊 Classical optimization selected (signal characteristics favor traditional methods)");
    }
