...
fn basic_memory_efficient_stft() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Multi-tone signal
    let signal: Vec<f64> = t.iter().map(|&t| {
        (2.0 * PI * 50.0 * t).sin() + 
    ...
    let mem_stft = MemoryEfficientStft::new(&window, hop_size, fs, Some(stft_config), memory_config)?;

    // Process STFT
    let result = mem_stft.stft_chunked(&signal)?;
    
    println!("   Signal length: {} samples", signal.len());
    println!("   STFT result shape: {} x {} (freq x time)", result.shape()[0], result.shape()[1]);
    println!("   Memory estimate: {:.2} MB", mem_stft.memory_estimate(signal.len()));
...
fn auto_memory_management() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("     Small signal ({} samples) -> {} x {} STFT", 
             small_signal.len(), small_result.shape()[0], small_result.shape()[1]);
    
    let medium_result = mem_stft.stft_auto(&medium_signal)?;
    println!("     Medium signal ({} samples) -> {} x {} STFT", 
             medium_signal.len(), medium_result.shape()[0], medium_result.shape()[1]);
    
    let large_result = mem_stft.stft_auto(&large_signal)?;
    println!("     Large signal ({} samples) -> {} x {} STFT", 
             large_signal.len(), large_result.shape()[0], large_result.shape()[1]);
...
fn large_signal_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Creating large signal: {} samples ({:.1} MB)", n, n as f64 * 8.0 / 1_000_000.0);
    ...
    println!("     Memory without chunking: {:.2} MB", info.total_memory_mb);
    ...
    println!("     Spectrogram shape: {} x {} (freq x time)", spec_result.shape()[0], spec_result.shape()[1]);
    println!("     Processing time: {:.2} seconds", processing_time.as_secs_f64());
    println!("     Processing rate: {:.2} x real-time", duration / processing_time.as_secs_f64());
...
fn magnitude_only_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let configs = vec![
        ("Complex values", MemoryEfficientStftConfig {
            max_memory_mb: 50,
            chunk_size: Some(4000),
            parallel: false,
            magnitude_only: false,
        }),
        ("Magnitude only", MemoryEfficientStftConfig {
            max_memory_mb: 50,
            chunk_size: Some(4000),
            parallel: false,
            magnitude_only: true,
...
fn parallel_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("     Sequential time: {:.3} seconds", sequential_time.as_secs_f64());
    println!("     Parallel time: {:.3} seconds", parallel_time.as_secs_f64());
    println!("     Speedup: {:.2}x", sequential_time.as_secs_f64() / parallel_time.as_secs_f64());
    
    // Verify results are equivalent
    assert_eq!(sequential_result.shape(), parallel_result.shape());
    let max_diff = sequential_result.iter()
        .zip(parallel_result.iter())
        .map(|(a, b)| (a - b).norm())
        .fold(0.0, f64::max);
    
    println!("     Maximum difference: {:.2e}", max_diff);
    println!("     Results are numerically equivalent: {}", max_diff < 1e-10);
