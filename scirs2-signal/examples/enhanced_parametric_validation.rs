...
fn test_ar_estimation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  True AR coefficients: [{:.3}, {:.3}]", true_ar_coeffs[0], true_ar_coeffs[1]);
    ...
    for (idx, method_name) in methods.iter().enumerate() {
        ...
        println!("    Estimated: [{:.3}, {:.3}]", estimated_coeffs[0], estimated_coeffs[1]);
...
fn test_ma_estimation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  True MA coefficients: [{:.1}, {:.1}, {:.1}]", 
...
fn test_arma_estimation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Estimated parameters: AR={:.2}, MA={:.2}", estimated_ar, estimated_ma);
    println!("  Estimation errors: AR={:.3}, MA={:.3}", ar_error, ma_error);
...
fn test_spectral_analysis() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &f in &freqs {
        ...
        let omega = 2.0 * PI * f;
        let magnitude_squared = (1.0 - ar_coeff * omega.cos()).powi(2) + 
                               (ar_coeff * omega.sin()).powi(2);
        let psd = variance / magnitude_squared;
        spectrum.push(psd);
    }
    
    println!("  AR(1) spectrum computed at {} frequency points", freqs.len());
    ...
    if valid_spectrum && low_freq_power > high_freq_power {
        ...
        println!("    Low freq power: {:.3}, High freq power: {:.3}", 
    ...
    println!("    Expected pole magnitude: {:.2}", expected_pole_magnitude);
