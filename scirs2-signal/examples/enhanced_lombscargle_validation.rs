...
fn test_basic_lombscargle() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Signal contains frequencies: {:.1} Hz and {:.1} Hz", freq1, freq2);
    ...
    // Find actual peaks
    let peak1_idx = power.iter().enumerate()
    ...
    println!("  Expected peak at {:.1} Hz, detected at {:.1} Hz", freq1, detected_freq1);
...
fn test_scipy_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let n = 500;
    let t: Vec<f64> = (0..n).map(|i| i as f64 * 0.01 + 
                                    (i as f64 * 0.001).sin() * 0.001).collect(); // Irregular sampling
    ...
    for (i, (&our, &scipy)) in our_power.iter().zip(scipy_power.iter()).enumerate() {
        ...
        println!("    Point {}: Our={:.2}, SciPy={:.2}, Error={:.3}", 
                 i+1, our, scipy, relative_error);
    }
...
fn test_noise_robustness() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &snr_db in &noise_levels {
        ...
        let clean_signal: Vec<f64> = t.iter().map(|&ti| (2.0 * PI * true_freq * ti).sin()).collect();
        
        // Add noise
        let snr_linear = 10.0_f64.powf(snr_db / 10.0);
        let noise_std = 1.0 / snr_linear.sqrt();
        
        let noisy_signal: Vec<f64> = clean_signal.iter().enumerate().map(|(i, &s)| {
        ...
        println!("      Detected frequency: {:.2} Hz (error: {:.1}%)", 
                 detected_freq, freq_error * 100.0);
...
fn test_memory_efficiency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &size in &signal_sizes {
        ...
        println!("      Estimated processing time: {:.0} ms", processing_time * 1000.0);
        
        // Memory efficiency score based on linear scaling
        let efficiency = if size <= 5000 {
            95.0
        } else {
            90.0
        };
...
fn test_simd_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, (&scalar, &simd)) in scalar_power.iter().zip(simd_power.iter()).enumerate() {
        ...
        if i < 3 { // Show first few comparisons
            println!("      Point {}: Scalar={:.3}, SIMD={:.3}, Error={:.2e}", 
                     i+1, scalar, simd, error);
        }
    }
