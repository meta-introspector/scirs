...
use scirs2_signal::{
    ...
    ultrathink_simd_resample, generate_simd_performance_report, UltrathinkSimdConfig,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, signal_complex) in &test_signals.complex_signals {
        ...
        println!("    ⚡ SIMD Acceleration: {:.1}x", fft_result.performance_metrics.simd_acceleration);
        println!("    💾 Memory Bandwidth: {:.1} GB/s", fft_result.performance_metrics.memory_bandwidth);
        println!("    📊 Vectorization: {:.1}%", fft_result.simd_stats.vectorization_ratio * 100.0);
        println!("    ⏱️ Time: {:.2} ms", elapsed.as_secs_f64() * 1000.0);
        
        fft_results.push((name.clone(), fft_result));
        
        // Performance comparison indicator
        let perf_level = if fft_results.last().unwrap().1.performance_metrics.simd_acceleration > 3.0 {
            "🌟 EXCELLENT"
        } else if fft_results.last().unwrap().1.performance_metrics.simd_acceleration > 2.0 {
            "⭐ VERY GOOD"
        } else {
            "⚠️ GOOD"
        };
    ...
    for (name, signal_real) in &test_signals.real_signals {
        ...
        println!("    ⚡ Real FFT Speedup: {:.1}x", rfft_result.performance_metrics.simd_acceleration);
        println!("    💾 Cache Efficiency: {:.1}%", rfft_result.performance_metrics.cache_hit_ratio * 100.0);
        println!("    📊 Vector Width: {} elements", rfft_result.simd_stats.vector_width);
        println!("    ⏱️ Time: {:.2} ms", elapsed.as_secs_f64() * 1000.0);
    ...
    for (name, signal_real) in &test_signals.real_signals {
        if signal_real.len() >= 512 { // STFT needs sufficient length
            println!("  🔍 Testing {}: {} samples", name, signal_real.len());
            ...
            let start_time = Instant::now();
            let stft_result = ultrathink_simd_stft(signal_real, window_size, hop_size, None, &config)?;
            let elapsed = start_time.elapsed();
            
            println!("    📊 Spectrogram: {} x {} (freq x time)", 
                     stft_result.magnitude.shape()[0], stft_result.magnitude.shape()[1]);
            println!("    ⚡ Per-frame Time: {:.1} μs", stft_result.performance_metrics.per_frame_time_ns / 1000.0);
            println!("    💾 Overlap Efficiency: {:.1}%", stft_result.performance_metrics.overlap_efficiency * 100.0);
            println!("    📈 SIMD Utilization: {:.1}%", stft_result.performance_metrics.simd_utilization * 100.0);
            ...
            let rt_status = if real_time_capable { "✅ REAL-TIME CAPABLE" } else { "⚠️ NOT REAL-TIME" };
            println!("    {}", rt_status);
    ...
    for (name, signal_real) in &test_signals.real_signals {
        if signal_real.len() >= 64 { // DWT needs power-of-2 or sufficient length
            println!("  🔍 Testing {}: {} samples", name, signal_real.len());
            ...
            println!("    ⚡ SIMD Speedup: {:.1}x", wavelet_result.performance_metrics.simd_speedup);
            println!("    💾 Memory Efficiency: {:.3}", wavelet_result.performance_metrics.memory_efficiency);
            println!("    ⏱️ Time: {:.2} ms", elapsed.as_secs_f64() * 1000.0);
    ...
    for (name, signal_real) in &test_signals.real_signals {
        ...
        let length_accuracy = 1.0 - (resampled.len() as f64 - expected_length as f64).abs() / expected_length as f64;
        
        println!("    📊 Ratio: {:.3}:1 ({} Hz → {} Hz)", ratio, original_rate as u32, target_rate as u32);
        println!("    📏 Output Length: {} samples (expected: {})", resampled.len(), expected_length);
    ...
    let avg_fft_acceleration = fft_results.iter()
        .map(|(_, r)| r.performance_metrics.simd_acceleration)
        .sum::<f64>() / fft_results.len() as f64;
    
    let avg_vectorization = fft_results.iter()
    ...
    println!("  - Average Vectorization: {:.1}%", avg_vectorization * 100.0);
    ...
    if avg_vectorization < 0.8 {
        println!("  1. 🔧 Improve vectorization ratio (currently {:.1}%)", avg_vectorization * 100.0);
    ...
    if let Some((_, rfft)) = rfft_results.first() {
        if rfft.performance_metrics.cache_hit_ratio < 0.9 {
            println!("  2. 💾 Optimize cache utilization (currently {:.1}%)", 
    ...
    if let Some((_, stft)) = stft_results.first() {
        if stft.performance_metrics.simd_utilization < 0.9 {
            println!("  3. ⚡ Improve STFT SIMD utilization (currently {:.1}%)", 
    ...
    if let Some((_, fft)) = fft_results.first() {
        for capability in &fft.simd_stats.capabilities_used {
            match capability.as_str() {
                "AVX512" => println!("     - AVX512 detected: Use 512-bit vectors for maximum performance"),
    ...
    println!("  - **Scientific Computing**: Large-scale signal analysis, time-frequency decomposition");
    println!("  - **Machine Learning**: Feature extraction, preprocessing pipelines");
    
    // Memory and power efficiency
    println!("\n⚡ **Efficiency Metrics**:");
    if let Some((_, fft)) = fft_results.first() {
        println!("  - Memory Bandwidth: {:.1} GB/s", fft.performance_metrics.memory_bandwidth);
        println!("  - Cache Efficiency: {:.1}%", fft.performance_metrics.cache_hit_ratio * 100.0);
        println!("  - Vector Width Utilization: {} elements", fft.simd_stats.vector_width);
    }
...
fn create_test_signals() -> TestSignals {
    ...
    test_signals.real_signals.push(("Small (256 samples)".to_string(), small_real));
    test_signals.complex_signals.push(("Small (256 samples)".to_string(), small_complex));
    ...
    test_signals.real_signals.push(("Medium (1024 samples)".to_string(), medium_real));
    test_signals.complex_signals.push(("Medium (1024 samples)".to_string(), medium_complex));
    ...
    test_signals.real_signals.push(("Large (4096 samples)".to_string(), large_real));
    test_signals.complex_signals.push(("Large (4096 samples)".to_string(), large_complex));
    ...
    test_signals.real_signals.push(("Extra Large (16384 samples)".to_string(), xl_real));
    test_signals.complex_signals.push(("Extra Large (16384 samples)".to_string(), xl_complex));
