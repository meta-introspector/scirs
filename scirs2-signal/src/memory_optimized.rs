...
pub fn memory_optimized_fir_filter(
    ...
    let file_size = input_file_handle.metadata()
    ...
    while total_processed < samples_count {
        ...
        let samples_to_read = (chunk_size + config.overlap_size).min(samples_count - total_processed);
        
        // Read binary data (assuming f64 samples)
        let mut raw_buffer = vec![0u8; samples_to_read * sample_size];
        input_reader.read_exact(&mut raw_buffer)
        ...
        for (i, chunk) in raw_buffer.chunks_exact(sample_size).enumerate() {
            if i < input_buffer.len() {
                input_buffer[i] = f64::from_le_bytes(
                    chunk.try_into()
                        .map_err(|_| SignalError::ComputationError("Invalid data format".to_string()))?
                );
            }
        }
        ...
        for i in 0..(filter_length - 1).min(effective_length) {
            if memory_start + i < filter_memory.len() {
                filter_memory[memory_start + i] = input_buffer[effective_length - (filter_length - 1) + i];
            }
        }
...
fn memory_fft_in_core(
    ...
    for i in 0..n {
        ...
        input_reader.read_exact(&mut real_bytes)
            .map_err(|e| SignalError::ComputationError(format!("Read error: {}", e)))?;
        input_reader.read_exact(&mut imag_bytes)
            .map_err(|e| SignalError::ComputationError(format!("Read error: {}", e)))?;
    ...
    for sample in &data {
        output_writer.write_all(&sample.re.to_le_bytes())
            .map_err(|e| SignalError::ComputationError(format!("Write error: {}", e)))?;
        output_writer.write_all(&sample.im.to_le_bytes())
            .map_err(|e| SignalError::ComputationError(format!("Write error: {}", e)))?;
    }
...
fn memory_fft_out_of_core(
    ...
    for stage in 0..disk_stages {
        ...
        let stage_result = process_fft_stage_disk(
            &current_input,
            &current_output,
            n,
            stage,
    ...
    if in_memory_stages > 0 {
        let final_input = if disk_stages > 0 { &current_output } else { input_file };
...
fn process_fft_stage_disk(
    ...
    for chunk_start in (0..num_groups).step_by(groups_per_chunk) {
        ...
        input_reader.seek(SeekFrom::Start(byte_offset as u64))
        ...
        // Write chunk back
        output_writer.seek(SeekFrom::Start(byte_offset as u64))
...
pub fn memory_optimized_spectrogram(
    ...
    let file_size = input_handle.metadata()
        .map_err(|e| SignalError::ComputationError(format!("Cannot get file size: {}", e)))?
        .len() as usize;
    ...
    let window: Vec<f64> = (0..window_size)
        .map(|i| 0.5 * (1.0 - ((2.0 * std::f64::consts::PI * i as f64) / (window_size as f64 - 1.0)).cos()))
        .collect();
    ...
    for frame in 0..n_frames {
        ...
        for &mag in &magnitude_buffer {
            output_writer.write_all(&mag.to_le_bytes())
                .map_err(|e| SignalError::ComputationError(format!("Write error: {}", e)))?;
        }
