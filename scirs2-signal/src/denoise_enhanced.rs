...
use ndarray::{Array1, Array2, ArrayView1, ArrayViewMut1, Axis, s};
...
use scirs2_core::cache::LruCache;
...
pub fn denoise_total_variation_1d(
    ...
    for iter in 0..max_iter {
        ...
        for i in 1..n-1 {
            ...
            let left_sign = if left_diff.abs() > 1e-12 { left_diff.signum() } else { 0.0 };
            let right_sign = if right_diff.abs() > 1e-12 { right_diff.signum() } else { 0.0 };
        ...
        if n > 1 {
            ...
            tv_term[0] = -lambda * if first_diff.abs() > 1e-12 { first_diff.signum() } else { 0.0 };
            
            let last_diff = denoised[n-1] - denoised[n-2];
            tv_term[n-1] = lambda * if last_diff.abs() > 1e-12 { last_diff.signum() } else { 0.0 };
        }
...
pub fn denoise_wiener_1d(
    ...
    let complex_signal: Vec<_> = padded_signal.iter()
    ...
    let noise_psd = match config.noise_estimation {
        ...
        WienerNoiseEstimation::HighFrequency => {
            ...
            let high_freq_power: f64 = signal_fft[high_freq_start..]
                .iter()
                .map(|c| c.norm_sqr())
                .sum::<f64>() / (padded_size - high_freq_start) as f64;
            high_freq_power
        }
    ...
    let filtered: Vec<f64> = filtered_complex[..n]
        .iter()
        .map(|c| c.re)
        .collect();
...
fn standard_denoise_1d(
    ...
    let wavedec_result = wavedec(signal.as_slice().unwrap(), config.wavelet, Some(levels), None)?;
...
fn validate_denoise_config(config: &DenoiseConfig) -> SignalResult<()> {
    ...
    check_positive(config.n_shifts, "n_shifts")?;
...
fn estimate_memory_usage(signal_length: usize) -> usize {
    ...
    let wavelet_coeffs = signal_length * 2; // Approx 2x for all levels
    let intermediate_buffers = signal_length * 4; // Various temp arrays
...
fn compute_numerical_stability(signal: &Array1<f64>) -> SignalResult<f64> {
    ...
    let dynamic_range = if min_val != 0.0 { max_val / min_val.abs() } else { max_val };
    ...
    let range_score = if dynamic_range < 1e12 { 1.0 } else { 1e12 / dynamic_range };
    let variance_score = if variance > 1e-12 { 1.0 } else { variance * 1e12 };
...
fn adaptive_noise_estimation(coeffs: &crate::dwt::DecompositionResult) -> SignalResult<f64> {
    ...
    if n_levels == 0 {
        return Err(SignalError::ValueError("No detail coefficients found".to_string()));
    }
    ...
    let median_noise = if noise_estimates.len() % 2 == 0 {
        (noise_estimates[noise_estimates.len() / 2 - 1] + noise_estimates[noise_estimates.len() / 2]) / 2.0
    } else {
...
fn simd_weighted_sum(weights: &[f64], arrays: &[Array1<f64>]) -> SignalResult<f64> {
    ...
    let mut total = 0.0;
    let weights_view = ArrayView1::from(weights);
...
fn simd_average_unshifted_results(
    ...
    if shift_results.is_empty() {
        return Err(SignalError::ValueError("No shift results provided".to_string()));
    }
...
fn compute_snr_improvement(original: &Array1<f64>, denoised: &Array1<f64>, noise_sigma: f64) -> f64 {
...
fn memory_optimized_denoise_1d(
    ...
    Ok(DenoiseResult {
        ...
        retention_rate,
        effective_df: 0.0, // Simplified for memory-optimized version
        risk_estimate: None,
        processing_time_ms,
        memory_usage_bytes,
        stability_score,
