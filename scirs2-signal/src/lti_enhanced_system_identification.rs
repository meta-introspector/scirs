...
pub fn ultra_enhanced_system_identification(
    ...
    let parameter_estimates = if config.enable_quantum_optimization {
        quantum_inspired_parameter_estimation(input, output, structure_selection.selected_order, config)?
    } else {
        classical_parameter_estimation(input, output, structure_selection.selected_order, config)?
    };

    // Step 3: Build system model
    let system_model = build_system_model(&parameter_estimates, structure_selection.selected_order)?;
...
fn classical_parameter_estimation(
    ...
    let (params, uncertainties) = regularized_least_squares(input, output, order, config.regularization_strength)?;
...
fn perform_ultra_validation(
    ...
    let tf = model.transfer_function.as_ref().ok_or_else(|| {
        SignalError::ValueError("Transfer function not available".to_string())
    })?;
    ...
    // Normalized RMSE
    let output_range = output.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() 
                    - output.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
...
fn quick_parameter_estimation(
    ...
    params.into_iter().enumerate().map(|(i, param)| {
...
fn solve_linear_system(a: &Array2<f64>, b: &Array1<f64>) -> SignalResult<Array1<f64>> {
    ...
    if n != a.ncols() || n != b.len() {
        return Err(SignalError::ValueError("Matrix dimensions mismatch".to_string()));
    }
...
fn compute_parameter_uncertainties(
    ...
    for i in 0..params.len() {
        ...
        let second_deriv = (cost_plus - 2.0 * compute_identification_cost(input, output, params, order)? + cost_minus) / (perturbation * perturbation);
...
fn compute_parameter_sensitivity(
    ...
    let param_idx = all_params.iter().position(|&x| (x - param).abs() < 1e-12).unwrap_or(0);
...
fn compute_aic(model: &SystemModel, input: &Array1<f64>, output: &Array1<f64>) -> SignalResult<f64> {
...
fn compute_bic(model: &SystemModel, input: &Array1<f64>, output: &Array1<f64>) -> SignalResult<f64> {
    if let Some(tf) = &model.transfer_function {
        let predicted = simulate_model_response(tf, input)?;
        let mse = compute_mse(output, &predicted);
        let n = output.len() as f64;
...
fn compute_mdl(model: &SystemModel, input: &Array1<f64>, output: &Array1<f64>) -> SignalResult<f64> {
    // Minimum Description Length
    compute_bic(model, input, output) // Simplified - MDL often similar to BIC
}

fn compute_cross_validation_score(model: &SystemModel, input: &Array1<f64>, output: &Array1<f64>) -> SignalResult<f64> {
...
fn compute_stability_score(model: &SystemModel) -> SignalResult<f64> {
    if let Some(tf) = &model.transfer_function {
        ...
        let poles = tf.poles()?;
        let stable_count = poles.iter().filter(|pole| {
            if tf.dt.is_some() {
...
fn analyze_complexity_accuracy_tradeoff(order_criteria: &HashMap<usize, ModelCriteria>) -> ComplexityAccuracyTradeoff {
    ...
    let recommended_point = pareto_frontier.iter()
        .max_by(|a, b| (a.1 / (1.0 + a.0)).partial_cmp(&(b.1 / (1.0 + b.0))).unwrap())
        .copied()
        .unwrap_or((1.0, 0.5));
...
fn compute_order_confidence(order_criteria: &HashMap<usize, ModelCriteria>, best_order: usize) -> f64 {
    if let Some(best_criteria) = order_criteria.get(&best_order) {
        ...
        let next_best_score = order_criteria.values()
...
fn find_alternative_orders(order_criteria: &HashMap<usize, ModelCriteria>, best_order: usize) -> Vec<(usize, f64)> {
    let mut alternatives: Vec<_> = order_criteria.iter()
...
fn analyze_convergence(parameters: &[ParameterWithUncertainty], config: &UltraEnhancedSysIdConfig) -> SignalResult<ConvergenceInfo> {
    // Simplified convergence analysis
    let avg_uncertainty = parameters.iter().map(|p| p.standard_error).sum::<f64>() / parameters.len() as f64;
...
fn compute_stability_margin(tf: &TransferFunction) -> SignalResult<f64> {
    ...
    if tf.dt.is_some() {
        ...
        Ok(1.0 - poles.iter().map(|p| p.norm()).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(0.0))
    } else {
        // Continuous-time: distance from imaginary axis
        Ok(-poles.iter().map(|p| p.re).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(-1.0))
    }
}
...
fn compute_model_confidence(parameters: &[ParameterWithUncertainty]) -> f64 {
    let avg_relative_uncertainty = parameters.iter()
...
mod tests {
    ...
    fn test_model_validation() {
        ...
        let output = simulate_model_response(model.transfer_function.as_ref().unwrap(), &input).unwrap();
