...
use crate::dwt2d_enhanced::{
    enhanced_dwt2d_decompose, EnhancedDwt2dResult, Dwt2dConfig, BoundaryMode,
...
fn compute_ssim(
    ...
    for i in (0..rows-window_size).step_by(4) {
        for j in (0..cols-window_size).step_by(4) {
            ...
            let var1 = window1.iter().map(|&x| (x - mu1).powi(2)).sum::<f64>() / (window_size * window_size) as f64;
            let var2 = window2.iter().map(|&x| (x - mu2).powi(2)).sum::<f64>() / (window_size * window_size) as f64;
...
fn check_symmetry_preservation(
    ...
    if h_symmetry < symmetry_threshold || v_symmetry < symmetry_threshold {
        ...
        for i in 0..a_rows/2 {
            for j in 0..a_cols {
                approx_symmetry += (result.approx[[i, j]] - result.approx[[a_rows-1-i, j]]).abs();
            }
        }
        
        let preservation_score = 1.0 - (approx_symmetry / (h_symmetry + v_symmetry + 1e-10)).min(1.0);
...
pub fn run_comprehensive_validation(wavelet: Wavelet) -> SignalResult<()> {
    ...
    for (name, image) in test_images {
        ...
        println!("  Reconstruction error: {:.2e}", result.reconstruction_error.max_error);
        println!("  Energy ratio: {:.6}", result.energy_conservation.energy_ratio);
