...
fn test_edge_cases(tolerance: f64) -> SignalResult<EdgeCaseResults> {
    ...
    let single_sample_valid = match lombscargle(&t_single, &y_single, None, None, None, None, None, None) {
        Ok((freqs, psd)) => {
    ...
    let duplicate_timestamps_handled = match lombscargle(&t_dup, &y_dup, None, None, None, None, None, None) {
        Ok((_, psd)) => {
    ...
    let zero_signal_handled = match lombscargle(&t_zero, &y_zero, None, None, None, None, None, None) {
        Ok((_, psd)) => {
    ...
    let constant_signal_handled = match lombscargle(&t_const, &y_const, None, None, None, None, None, None) {
        Ok((freqs, psd)) => {
    ...
    let extreme_frequencies_handled = match lombscargle(&t_extreme, &y_extreme, None, None, None, None, None, None) {
        Ok((freqs, psd)) => {
            let valid = freqs.iter().all(|&f| f.is_finite()) && psd.iter().all(|&p| p.is_finite());
            if !valid {
...
fn test_numerical_accuracy(tolerance: f64) -> SignalResult<NumericalAccuracyResults> {
    ...
    let signal_nyquist: Vec<f64> = t.iter()
        .enumerate()
...
fn test_algorithm_consistency(tolerance: f64) -> SignalResult<ConsistencyResults> {
    ...
    let signal: Vec<f64> = t.iter().map(|&ti| (0.2 * ti).sin() + (0.5 * ti).cos()).collect();
    
    // Test 1: Implementation agreement
    let (_, psd_standard) = lombscargle(&t, &signal, None, Some("standard"), None, None, None, None)?;
    ...
    let config_os1 = LombScargleConfig { oversample: 1.0, ..Default::default() };
    let config_os5 = LombScargleConfig { oversample: 5.0, ..Default::default() };
    let config_os10 = LombScargleConfig { oversample: 10.0, ..Default::default() };
    ...
    let config_no_window = LombScargleConfig { window: WindowType::None, ..Default::default() };
    let config_hann = LombScargleConfig { window: WindowType::Hann, ..Default::default() };
    ...
    let (_, psd_reversed) = lombscargle(&t_reversed, &signal_reversed, None, None, None, None, None, None)?;
    
    let time_reversal_symmetry = compute_agreement(&psd_forward, &psd_reversed);
    
    // Test 5: Frequency shift invariance
    let freq_shift = 10.0;
    let signal_shifted: Vec<f64> = t.iter()
        .zip(signal.iter())
        .map(|(&ti, &si)| si * (2.0 * PI * freq_shift * ti).cos())
        .collect();
    
    let (freqs_shifted, psd_shifted) = lombscargle(&t, &signal_shifted, None, None, None, None, None, None)?;
    let frequency_shift_invariance = test_frequency_shift_invariance(&freqs_shifted, &psd_shifted, freq_shift)?;
...
fn test_phase_accuracy(t: &[f64], tolerance: f64) -> SignalResult<f64> {
    ...
    let freq = 5.0;
    let signal: Vec<f64> = t.iter().map(|&ti| (2.0 * PI * freq * ti + phase_true).sin()).collect();
...
fn test_amplitude_recovery(t: &[f64], tolerance: f64) -> SignalResult<f64> {
    ...
    let freq = 7.0;
    let signal: Vec<f64> = t.iter().map(|&ti| amplitude_true * (2.0 * PI * freq * ti).sin()).collect();
...
fn compare_at_common_frequencies(
    ...
    let f_max = f1.last().unwrap().min(f5.last().unwrap()).min(f10.last().unwrap());
    ...
    for i in 0..n_samples {
        ...
        let max_diff = (p1_interp - mean_p).abs()
...
fn generate_validation_summary(
    ...
    total_tests += 6;
    if edge_cases.single_sample_valid { tests_passed += 1; }
    if edge_cases.two_sample_valid { tests_passed += 1; }
    if edge_cases.duplicate_timestamps_handled { tests_passed += 1; }
    if edge_cases.zero_signal_handled { tests_passed += 1; }
    if edge_cases.constant_signal_handled { tests_passed += 1; }
    if edge_cases.extreme_frequencies_handled { tests_passed += 1; }
    ...
    total_tests += 6;
    if numerical.nyquist_precision < 0.01 { tests_passed += 1; }
    if numerical.low_freq_precision < 0.01 { tests_passed += 1; }
    if numerical.phase_accuracy > 0.9 { tests_passed += 1; }
    if numerical.amplitude_accuracy > 0.95 { tests_passed += 1; }
    if numerical.frequency_bin_accuracy > 0.99 { tests_passed += 1; }
    if numerical.fp_stability_score > 0.8 { tests_passed += 1; }
    ...
    total_tests += 5;
    if consistency.implementation_agreement > 0.95 { tests_passed += 1; }
    if consistency.oversample_consistency > 0.95 { tests_passed += 1; }
    if consistency.window_consistency > 0.9 { tests_passed += 1; }
    if consistency.time_reversal_symmetry > 0.95 { tests_passed += 1; }
    if consistency.frequency_shift_invariance > 0.9 { tests_passed += 1; }
    ...
    total_tests += 4;
    if stress.max_data_size >= 65536 { tests_passed += 1; }
    if stress.performance_degradation < 2.0 { tests_passed += 1; }
    if stress.memory_efficiency > 0.7 { tests_passed += 1; }
    if stress.stability_under_stress { tests_passed += 1; }
    ...
    if numerical.low_freq_precision > 0.05 {
        recommendations.push("Improve low-frequency precision by increasing oversample factor".to_string());
    }
