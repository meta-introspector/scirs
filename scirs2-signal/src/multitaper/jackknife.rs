...
pub fn jackknife_confidence_intervals(
    ...
    for i_out in 0..k {
        ...
        for j in 0..n_freq {
            ...
            if weight_sum > 0.0 {
                ...
                jackknife_estimates[[i_out, j]] = if use_log {
                    estimate.ln()
                } else {
                    estimate
                };
    ...
    let t_dist = StudentsT::new(0.0, 1.0, dof)
        .map_err(|e| SignalError::ComputationError(format!("Failed to create t-distribution: {}", e)))?;
...
pub fn weighted_jackknife(
    ...
    let (k, n_freq) = eigenspectra.dim();
    check_shape(adaptive_weights, (Some(k), Some(n_freq)), "adaptive_weights")?;
...
pub fn cross_spectrum_jackknife(
    ...
    eigenvalues: &Array1<f64>,
    confidence: Option<f64>,
) -> SignalResult<(Array1<f64>, (Array1<f64>, Array1<f64>), Array1<f64>, (Array1<f64>, Array1<f64>))> {
    ...
    for j in 0..n_freq {
        ...
        let z_var: f64 = z_vals.iter()
            .map(|&z| (z - z_mean).powi(2))
            .sum::<f64>() * (k - 1) as f64 / k as f64;
        let z_se = z_var.sqrt();
    ...
    Ok((coherence, (coherence_lower, coherence_upper), phase, (phase_lower, phase_upper)))
}