...
use crate::parametric::{estimate_ar, ar_spectrum, ARMethod};
use crate::parametric_arma::{estimate_arma, ArmaModel, ArmaMethod};
        ...
        for &n in &config.signal_lengths {
            for &snr_db in &config.noise_levels {
                ...
                for method in [ARMethod::YuleWalker, ARMethod::Burg, ARMethod::LeastSquares] {
                    ...
                    if is_ar_model_stable(&estimated_ar) {
                        stability_count += 1;
                    }
                    
                    // Check prediction accuracy
                    let pred_error = compute_prediction_error(&signal, &estimated_ar, variance);
                    prediction_errors.push(pred_error);
                    
                    // Check spectral accuracy
                    let spec_error = compute_spectral_error(&true_ar, &estimated_ar)?;
...
fn validate_arma_models(config: &ValidationConfig) -> SignalResult<ArmaValidationMetrics> {
    ...
    for (&p, &q) in [(2, 1), (3, 2), (4, 3)].iter() {
        ...
        for &n in &config.signal_lengths {
            if n < (p + q) * 10 { continue; } // Skip if too short
...
fn check_method_consistency(config: &ValidationConfig) -> SignalResult<ConsistencyMetrics> {
    ...
    let (freqs, psd_yw) = ar_spectrum(&ar_yw, 1.0, 512)?;
    let (_, psd_burg) = ar_spectrum(&ar_burg, 1.0, 512)?;
...
fn test_numerical_stability(config: &ValidationConfig) -> SignalResult<StabilityMetrics> {
    ...
    for &snr_db in &[40.0, 30.0, 20.0, 10.0] {
        ...
        let sensitivity = compute_coefficient_error(&ar_clean, &ar_noisy) * (10.0_f64.powf(snr_db / 10.0));
    ...
    for _ in 0..n_outliers {
        ...
        contaminated[idx] += rng.random_range(-10.0..10.0) * contaminated.iter().map(|x| x.abs()).fold(0.0, f64::max);
    }
    ...
    Ok(StabilityMetrics {
        ...
        noise_sensitivity: noise_sensitivities.iter().sum::<f64>() / noise_sensitivities.len() as f64,
...
fn generate_ar_process(ar_coeffs: &Array1<f64>, n: usize, snr_db: f64) -> SignalResult<Array1<f64>> {
...
fn calculate_overall_score(
    ...
    // Stability (15 points)
    if stability.condition_number > 1e8 { score -= 5.0; }
    if stability.noise_sensitivity > 0.5 { score -= 5.0; }
    if !stability.precision_maintained { score -= 5.0; }
    
    // Performance (10 points)
    if performance.scalability_factor > 1.5 { score -= 5.0; }
    if performance.memory_efficiency < 0.7 { score -= 5.0; }
