...
pub trait RealtimeProcessor: Send + Sync {
    ...
    fn initialize(&mut self, _config: &RealtimeConfig) -> SignalResult<()> { Ok(()) }
    
    /// Reset processor state
    fn reset(&mut self) -> SignalResult<()> { Ok(()) }
}
...
impl StreamProcessor {
    ...
    pub fn start(&mut self) -> SignalResult<()> {
        ...
        if *running {
            return Err(SignalError::ValueError("Processor already running".to_string()));
        }
        *running = true;
            ...
            let processing_time = start_time.elapsed().as_secs_f64() * 1000.0;
            {
                ...
                if stats_guard.blocks_processed == 1 {
                    ...
                    stats_guard.avg_processing_time_ms = 
                        alpha * processing_time + (1.0 - alpha) * stats_guard.avg_processing_time_ms;
                }
                ...
                let buffer_latency = (input_buffer.lock().unwrap().len() + 
                                    output_buffer.lock().unwrap().len()) as f64 / samples_per_ms;
