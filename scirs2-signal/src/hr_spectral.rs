...
pub fn music(data: &Array2<f64>, config: &HrSpectralConfig) -> SignalResult<HrSpectralResult> {
    ...
    if n_samples == 0 || n_snapshots == 0 {
        return Err(SignalError::ValueError("Data matrix cannot be empty".to_string()));
    }
...
pub fn esprit(data: &Array2<f64>, config: &HrSpectralConfig) -> SignalResult<HrSpectralResult> {
    ...
    if n_samples < 2 {
        return Err(SignalError::ValueError("ESPRIT requires at least 2 samples".to_string()));
    }
    ...
    if signal_dim == 0 {
        return Err(SignalError::ValueError("No signal components detected".to_string()));
    }
    ...
    let s1 = signal_eigenvectors.slice(s![0..n_samples-1, ..]).to_owned();
...
pub fn minimum_variance(data: &Array2<f64>, config: &HrSpectralConfig) -> SignalResult<HrSpectralResult> {
    ...
    let inv_correlation = regularized_matrix.inv().map_err(|_| {
        SignalError::Compute("Failed to invert correlation matrix".to_string())
    })?;
...
pub fn pisarenko(data: &Array1<f64>, order: usize, config: &HrSpectralConfig) -> SignalResult<HrSpectralResult> {
    let n = data.len();
    
    if order >= n {
        return Err(SignalError::ValueError("Order must be less than data length".to_string()));
    }
...
pub fn prony(data: &Array1<f64>, order: usize, config: &HrSpectralConfig) -> SignalResult<HrSpectralResult> {
    let n = data.len();
    
    if order >= n / 2 {
        return Err(SignalError::ValueError("Order too large for Prony method".to_string()));
    }
...
fn compute_mvdr_spectrum(
    ...
    for (k, &freq) in frequencies.iter().enumerate() {
        ...
        for i in 0..n_samples {
            for j in 0..n_samples {
                quadratic_form += steering_vector[i].conj() * inv_correlation[[i, j]] * steering_vector[j];
            }
        }
...
fn solve_esprit_equation(
    ...
    let inv_s1h_s1 = s1h_s1.inv().map_err(|_| {
        SignalError::Compute("Failed to invert matrix in ESPRIT".to_string())
    })?;
...
fn create_line_spectrum(
    ...
    for &source_freq in source_frequencies {
        ...
        let closest_idx = frequency_grid
            .iter()
            .enumerate()
            .min_by(|(_, a), (_, b)| ((*a - source_freq).abs()).partial_cmp(&((*b - source_freq).abs())).unwrap())
            .unwrap()
            .0;
...
/// Create autocorrelation matrix
fn create_autocorrelation_matrix(data: &Array1<f64>, order: usize) -> SignalResult<Array2<Complex64>> {
    let n = data.len();
    
    if order > n {
        return Err(SignalError::ValueError("Order cannot exceed data length".to_string()));
    }
...
fn find_polynomial_roots(coeffs: &[Complex64]) -> SignalResult<Vec<Complex64>> {
    ...
    let (eigenvalues, _) = companion.eig().map_err(|_| {
        SignalError::Compute("Failed to find polynomial roots".to_string())
    })?;
...
fn solve_linear_system(a: &Array2<f64>, b: &Array1<f64>) -> SignalResult<Array1<f64>> {
    ...
    a.solve(b).map_err(|_| {
        SignalError::Compute("Failed to solve linear system".to_string())
    })
}
...
mod tests {
    ...
    fn test_music_basic() {
        ...
        for j in 0..n_snapshots {
            for i in 0..n_samples {
                data[[i, j]] = (2.0 * PI * freq1 * i as f64).sin() +
                              (2.0 * PI * freq2 * i as f64).sin();
            }
        }
