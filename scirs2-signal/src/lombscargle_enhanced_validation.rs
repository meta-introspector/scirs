...
pub fn run_enhanced_validation(
    ...
    let multi_frequency = if config.test_multi_frequency {
        Some(test_multi_frequency_signals(implementation, config.tolerance)?)
    } else {
        None
    };
    ...
    let cross_platform = if config.test_cross_platform {
        Some(test_cross_platform_consistency(implementation, config.tolerance)?)
    } else {
        None
    };
    ...
    let statistical_significance = if config.test_statistical_significance {
        Some(test_enhanced_statistical_significance(implementation, config.tolerance)?)
    } else {
        None
    };
    ...
    let memory_analysis = if config.test_memory_usage {
        Some(analyze_memory_usage(implementation, config.benchmark_iterations)?)
    } else {
        None
    };
    ...
    let simd_scalar_consistency = if config.test_simd_scalar_consistency {
        Some(test_simd_scalar_consistency(implementation, config.tolerance)?)
    } else {
        None
    };
    
    // New advanced validation tests
    let frequency_domain_analysis = Some(test_frequency_domain_analysis(implementation, config.tolerance)?);
    ...
    if let Some(ref freq_analysis) = frequency_domain_analysis {
        score += 5.0 * (freq_analysis.frequency_resolution_accuracy + freq_analysis.phase_coherence) / 2.0;
    }
...
fn test_extreme_parameters(
    ...
    let robustness_score = results.iter().map(|&b| if b { 1.0 } else { 0.0 }).sum::<f64>() / results.len() as f64;
...
fn test_multi_frequency_signals(
    ...
    let primary_peak = peaks.iter()
        .min_by(|(f1, _), (f2, _)| (f1 - primary_target).abs().partial_cmp(&(f2 - primary_target).abs()).unwrap())
        .map(|(f, p)| (*f, *p))
        .unwrap_or((0.0, 0.0));
    
    let primary_freq_accuracy = 1.0 - ((primary_peak.0 - primary_target).abs() / primary_target).min(1.0);
    ...
    for &target in &secondary_targets {
        let secondary_peak = peaks.iter()
            .min_by(|(f1, _), (f2, _)| (f1 - target).abs().partial_cmp(&(f2 - target).abs()).unwrap())
            .map(|(f, p)| (*f, *p))
            .unwrap_or((0.0, 0.0));
    ...
    let secondary_freq_accuracy = 1.0 - secondary_errors.iter().sum::<f64>() / secondary_errors.len() as f64;
...
fn test_cross_platform_consistency(
    ...
    let precision_consistency = if max_deviation < tolerance * 1000.0 { 1.0 } else { 0.5 };
    
    let all_consistent = numerical_consistency > 0.95 && 
                        simd_consistency > 0.95 && 
                        precision_consistency > 0.95;
...
fn test_frequency_resolution(
    ...
    for &df in &df_values {
        ...
        let signal: Vec<f64> = t.iter()
            .map(|&ti| {
                (2.0 * PI * f1 * ti).sin() + (2.0 * PI * f2 * ti).sin()
            })
            .collect();
    ...
    let min_separation = resolved_separations.iter().cloned().fold(f64::INFINITY, f64::min);
...
fn run_lombscargle(
    ...
    match implementation {
        "standard" => lombscargle(
            times, signal, None, Some("standard"), Some(true), Some(false), None, None
        ),
        "enhanced" => {
            let config = LombScargleConfig::default();
...
/// Estimate sidelobe suppression
fn estimate_sidelobe_suppression(
    implementation: &str,
    times: &[f64],
) -> SignalResult<f64> {
    // Single frequency signal
    let f0 = 10.0;
    let signal: Vec<f64> = times.iter()
...
fn estimate_window_effectiveness(times: &[f64]) -> SignalResult<f64> {
    ...
    let window_types = vec![WindowType::None, WindowType::Hann, WindowType::Hamming, WindowType::Blackman];
...
fn test_bootstrap_coverage(times: &[f64], signal: &[f64]) -> SignalResult<f64> {
    ...
    match enhanced_lombscargle(times, signal, &config) {
        Ok((_, _, Some((lower, upper)))) => {
            ...
            let coverage = lower.iter()
                .zip(upper.iter())
                .filter(|(&l, &u)| l <= u && u > l)
                .count() as f64 / lower.len() as f64;
            Ok(coverage)
        }
        _ => Ok(0.0),
    }
...
pub fn generate_validation_report(result: &EnhancedValidationResult) -> String {
    ...
    if let Some(ref freq_analysis) = result.frequency_domain_analysis {
        ...
        report.push_str(&format!("  Spectral Leakage: {:.3}\n", freq_analysis.spectral_leakage));
        report.push_str(&format!("  Dynamic Range: {:.1} dB\n", freq_analysis.dynamic_range_db));
        report.push_str(&format!("  Frequency Resolution Accuracy: {:.3}\n", freq_analysis.frequency_resolution_accuracy));
        report.push_str(&format!("  Alias Rejection: {:.1} dB\n", freq_analysis.alias_rejection_db));
        report.push_str(&format!("  Phase Coherence: {:.3}\n", freq_analysis.phase_coherence));
    ...
    if let Some(ref cv) = result.cross_validation {
        ...
        report.push_str(&format!("  Temporal Consistency: {:.3}\n", cv.temporal_consistency));
        report.push_str(&format!("  Frequency Stability: {:.3}\n", cv.frequency_stability));
        report.push_str(&format!("  Overall CV Score: {:.3}\n\n", cv.overall_cv_score));
    }
    
    // Edge case robustness
    if let Some(ref edge) = result.edge_case_robustness {
        report.push_str("Edge Case Robustness:\n");
        report.push_str(&format!("  Empty Signal Handling: {}\n", edge.empty_signal_handling));
        report.push_str(&format!("  Single Point Handling: {}\n", edge.single_point_handling));
        report.push_str(&format!("  Constant Signal Handling: {}\n", edge.constant_signal_handling));
        report.push_str(&format!("  Invalid Value Handling: {}\n", edge.invalid_value_handling));
        report.push_str(&format!("  Duplicate Time Handling: {}\n", edge.duplicate_time_handling));
        report.push_str(&format!("  Non-Monotonic Handling: {}\n", edge.non_monotonic_handling));
        report.push_str(&format!("  Overall Robustness: {:.3}\n\n", edge.overall_robustness));
    }
...
fn test_enhanced_statistical_significance(
    ...
    let empirical_fap = max_powers.iter().filter(|&&p| p > mean_max_power).count() as f64 / n_trials as f64;
...
fn estimate_statistical_power(implementation: &str, times: &[f64]) -> SignalResult<f64> {
    ...
    for _ in 0..n_trials {
        ...
        let detected = freqs.iter()
            ...
            .any(|(_, &p)| {
                let mean_power = power.iter().sum::<f64>() / power.len() as f64;
                let std_power = (power.iter().map(|&p| (p - mean_power).powi(2)).sum::<f64>() / power.len() as f64).sqrt();
...
fn test_enhanced_bootstrap_coverage(times: &[f64]) -> SignalResult<f64> {
    ...
    for _ in 0..n_tests {
        ...
        let signal: Vec<f64> = times.iter()
            .map(|&ti| {
                (2.0 * std::f64::consts::PI * f_true * ti).sin() + 
        ...
        match enhanced_lombscargle(times, &signal, &config) {
            Ok((freqs, power, Some((lower, upper)))) => {
                ...
                let (peak_idx, _) = freqs.iter()
                    .enumerate()
                    .min_by(|(_, f1), (_, f2)| (f1 - f_true).abs().partial_cmp(&(f2 - f_true).abs()).unwrap())
                    .unwrap();
...
fn analyze_memory_usage(
    ...
    for &size in &signal_sizes {
        ...
        let total_memory_kb = base_memory_kb + data_memory_kb + freq_memory_kb + 
                              power_memory_kb + intermediate_memory_kb;
    ...
    let peak_memory_mb = memory_measurements.iter()
        .map(|(_, mem)| *mem)
        .fold(0.0, f64::max);
    
    // Memory efficiency based on deviation from theoretical optimum
    let theoretical_linear_growth = memory_measurements.last().unwrap().1 / memory_measurements[0].1;
...
fn test_alias_rejection(implementation: &str, t: &[f64]) -> SignalResult<f64> {
    ...
    let signal_alias: Vec<f64> = t.iter()
...
fn calculate_spurious_free_dynamic_range(freqs: &[f64], power: &[f64], target_freqs: &[f64]) -> f64 {
    ...
    let max_legitimate = legitimate_peaks.iter()
        .map(|&idx| power[idx])
        .fold(0.0, f64::max);
...
fn test_cross_validation(
    ...
    // Overall CV score
    let overall_cv_score = (kfold_score + bootstrap_score + loo_score + temporal_consistency + frequency_stability) / 5.0;
...
fn perform_kfold_validation(
    ...
    for fold in 0..k {
        ...
        match run_lombscargle(implementation, &train_t, &train_signal) {
            Ok((freqs, power)) => {
                let (_, detected_freq) = freqs.iter()
                    .zip(power.iter())
                    .max_by(|(_, p1), (_, p2)| p1.partial_cmp(p2).unwrap())
                    .map(|(&f, _)| ((), f))
                    .unwrap_or(((), 0.0));
    ...
    Ok(if scores.is_empty() { 0.0 } else { scores.iter().sum::<f64>() / scores.len() as f64 })
}
...
fn perform_loo_validation(
    ...
    Ok(if scores.is_empty() { 0.5 } else { scores.iter().sum::<f64>() / scores.len() as f64 })
}
...
fn test_temporal_consistency(
    ...
    let variance = detected_freqs.iter()
        .map(|&f| (f - mean_freq).powi(2))
        .sum::<f64>() / detected_freqs.len() as f64;
...
fn test_frequency_stability(
    ...
    let std_dev = {
        let variance = freq_estimates.iter()
            .map(|&f| (f - mean_freq).powi(2))
            .sum::<f64>() / freq_estimates.len() as f64;
        variance.sqrt()
    };
    
    let relative_std = if mean_freq > 0.0 { std_dev / mean_freq } else { 1.0 };
...
fn test_edge_case_robustness(implementation: &str) -> SignalResult<EdgeCaseRobustnessResults> {
    ...
    let overall_robustness = results.iter().map(|&b| if b { 1.0 } else { 0.0 }).sum::<f64>() / results.len() as f64;
...
fn test_invalid_values(implementation: &str) -> bool {
    ...
    let signal = vec![1.0, f64::NAN, 2.0, f64::INFINITY, 0.5, -1.0, 3.0, f64::NEG_INFINITY, 1.5, 0.0];
...
fn test_precision_robustness(
    ...
    for &scale in &scaling_factors {
        ...
        match run_lombscargle(implementation, &t, &scaled_signal) {
            Ok((_, power)) => {
                ...
                let normalized_power: Vec<f64> = power.iter().map(|&p| p / (scale * scale)).collect();
                
                // Calculate relative deviation
                let max_deviation = ref_power.iter()
    ...
    let scaling_stability = 1.0 - scaling_deviations.iter().cloned().fold(0.0, f64::max).min(1.0);
...
fn test_f32_f64_consistency(
    ...
    let (_, power_f32_converted) = run_lombscargle(implementation, &t_f64_from_f32, &signal_f64_from_f32)?;
    
    // Calculate consistency metric
    let max_relative_error = power_f64.iter()
...
fn test_condition_number_robustness(
    implementation: &str,
    t: &[f64],
) -> SignalResult<f64> {
    ...
    let signal_ill: Vec<f64> = t_ill.iter().map(|&ti| (2.0 * PI * 5.0 * ti).sin()).collect();
    ...
    match run_lombscargle(implementation, &t_ill, &signal_ill) {
        Ok((_, power)) => {
            ...
            if has_invalid {
                ...
                Ok(if dynamic_range.is_finite() && dynamic_range < 1e12 { 0.9 } else { 0.3 })
            }
        }
...
fn test_catastrophic_cancellation(
    ...
    let signal_cancel: Vec<f64> = t.iter()
        .map(|&ti| {
            let large_val = 1e15;
...
fn test_simd_scalar_consistency(
    ...
    for &size in &signal_sizes {
        ...
        if let (Some(scalar), Some(simd)) = (scalar_result, simd_result) {
            let max_deviation = scalar.1.iter()
                .zip(simd.1.iter())
                .map(|(&s, &v)| {
                    if s.abs() > 1e-12 {
                        (s - v).abs() / s.abs()
    ...
    let performance_ratio = performance_ratios.iter().sum::<f64>() / performance_ratios.len() as f64;
    ...
    let all_consistent = max_deviation < tolerance && deviations.iter().all(|&d| d < tolerance * 10.0);
...
impl SciPyValidationResult {
    ...
    pub fn calculate_overall_score(&mut self) {
        ...
        self.overall_score = weights.iter()
            .zip(scores.iter())
            .map(|(w, s)| w * s)
            .sum::<f64>() * 100.0;
    }
...
pub fn validate_advanced_numerical_stability() -> SignalResult<AdvancedStabilityResult> {
    ...
    let small_y: Vec<f64> = small_t.iter().map(|&ti| (2.0 * PI * 1e10 * ti).sin()).collect();
    ...
    let large_y: Vec<f64> = large_t.iter().map(|&ti| (2.0 * PI * 1e-10 * ti).sin()).collect();
    ...
    let mixed_t: Vec<f64> = (0..50).map(|i| i as f64 * 0.01).chain(
        (50..100).map(|i| 0.5 + (i - 50) as f64 * 1000.0)
    ).collect();
