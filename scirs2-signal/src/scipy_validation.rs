...
fn validate_butterworth_filter(
    ...
    // Test parameters
    let orders = if config.extensive { vec![1, 2, 3, 4, 5, 6, 8] } else { vec![2, 4, 6] };
    let filter_types = vec![FilterType::Lowpass, FilterType::Highpass, FilterType::Bandpass, FilterType::Bandstop];
    ...
    let rmse = if num_cases > 0 { (rmse_sum / num_cases as f64).sqrt() } else { 0.0 };
    let execution_time = start_time.elapsed().as_secs_f64() * 1000.0;
    
    results.insert(test_name.clone(), ValidationTestResult {
        test_name,
        passed,
...
fn test_single_cheby1_filter(
    ...
    // Define filter parameters based on type
    ...
    let reference_result = reference_cheby1_filter(&test_signal, order, ripple, &critical_freq, btype, fs)?;
...
fn test_single_cheby2_filter(
    ...
    use crate::filter::{cheby2, lfilter};
    use crate::waveforms::chirp;
    ...
    // Reference implementation (simplified)
    let reference_result = reference_cheby2_filter(&test_signal, order, attenuation, &critical_freq, btype, fs)?;
...
fn validate_chebyshev_filter(
    ...
    // Test parameters
    let orders = if config.extensive { vec![2, 4, 6, 8] } else { vec![4, 6] };
            ...
            for filter_type in &filter_types {
                for &n in &config.test_lengths {
                    ...
                    for &ripple in &ripples {
                        match test_single_cheby1_filter(n, fs, order, filter_type.clone(), ripple, config) {
                            Ok((abs_err, rel_err, rmse)) => {
                                max_abs_error = max_abs_error.max(abs_err);
                                max_rel_error = max_rel_error.max(rel_err);
                                rmse_sum += rmse * rmse;
                            ...
                            Err(e) => {
                                passed = false;
                                error_message = Some(format!("Chebyshev I filter test failed: {}", e));
                            }
                        }
                    }
                    ...
                    for &attenuation in &attenuations {
                        match test_single_cheby2_filter(n, fs, order, filter_type.clone(), attenuation, config) {
                            Ok((abs_err, rel_err, rmse)) => {
                                max_abs_error = max_abs_error.max(abs_err);
                                max_rel_error = max_rel_error.max(rel_err);
                                rmse_sum += rmse * rmse;
                            ...
                            Err(e) => {
                                passed = false;
                                error_message = Some(format!("Chebyshev II filter test failed: {}", e));
                            }
                        }
                    }
                }
...
fn validate_multitaper_scipy(
    ...
    // Test parameters
    let nw_values = if config.extensive { vec![2.5, 4.0, 6.0, 8.0] } else { vec![4.0, 6.0] };
    let k_values = if config.extensive { vec![None, Some(4), Some(8)] } else { vec![None, Some(8)] };
    
    for &fs in &config.sampling_frequencies {
        for &n in &config.test_lengths {
            if n < 64 { continue; } // Skip small signals for multitaper
...
fn validate_dwt(
    ...
    let wavelets = if config.extensive {
        vec![Wavelet::Haar, Wavelet::DB(4), Wavelet::DB(8), Wavelet::Bior(2, 2), Wavelet::Coif(2)]
    } else {
        vec![Wavelet::Haar, Wavelet::DB(4), Wavelet::Bior(2, 2)]
    };
    
    // Test decomposition levels
    let levels = if config.extensive { vec![2, 3, 4, 5] } else { vec![3, 4] };
    
    for &n in &config.test_lengths {
        if n < 32 { continue; } // Skip small signals for DWT
        
        for wavelet in &wavelets {
            for &level in &levels {
                if n < (1 << level) * 2 { continue; } // Ensure signal is large enough
...
fn validate_windows(
    ...
    use crate::window::{hann, hamming, blackman, tukey};
    ...
    for &n in &config.test_lengths {
        if n < 8 { continue; } // Skip very small windows
...
fn validate_lombscargle(
    ...
    for &fs in &config.sampling_frequencies {
        for &n in &config.test_lengths {
            if n < 32 { continue; } // Skip very small signals
...
fn validate_parametric_spectral(
    ...
    let orders = if config.extensive { vec![4, 8, 12, 16] } else { vec![8, 12] };
    
    for &fs in &config.sampling_frequencies {
        for &n in &config.test_lengths {
            if n < 64 { continue; } // Skip small signals for parametric estimation
            
            for &method in &ar_methods {
                for &order in &orders {
                    if order >= n / 4 { continue; } // Ensure reasonable order
...
fn test_single_ar_estimation(
    ...
    use crate::parametric::{estimate_ar, ar_spectrum};
    use crate::waveforms::chirp;
    ...
    // Our implementation
    let (ar_coeffs, _reflection_coeffs, _variance) = estimate_ar(&signal_array, order, method)?;
...
pub fn load_reference_data(test_name: &str, parameters: &str) -> SignalResult<Vec<f64>> {
    ...
    match test_name {
        ...
        _ => Err(SignalError::ValueError(
            format!("No reference data available for test: {}", test_name)
        )),
    }
}
...
fn test_single_window(
    ...
    let our_window = match window_type {
        ...
        _ => return Err(SignalError::ValueError(format!("Unknown window type: {}", window_type))),
    };
...
fn reference_signal_generation(
    ...
    match signal_type {
        ...
        _ => return Err(SignalError::ValueError(format!("Unknown signal type: {}", signal_type))),
    }
...
mod tests {
    ...
    fn test_validation_results_all_passed() {
        let mut test_results = HashMap::new();
        test_results.insert("test1".to_string(), ValidationTestResult {
        ...
        let results = ValidationResults { test_results, summary };
    ...
    fn test_reference_signal_generation() {
        ...
        assert!(square_signal.iter().all(|&x| (x - 1.0).abs() < 1e-10 || (x + 1.0).abs() < 1e-10));
    }
}