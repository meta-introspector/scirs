...
fn validate_basic_accuracy(config: &ScipyValidationConfig) -> SignalResult<AccuracyValidationResult> {
    ...
    for &fs in &config.sampling_frequencies {
        for &n in &config.test_lengths {
            for &test_freq in &config.test_frequencies {
                if test_freq >= fs / 2.0 { continue; } // Skip frequencies above Nyquist
                ...
                match validate_single_case(n, fs, test_freq, config) {
                    ...
                    Err(e) => {
                        eprintln!("Validation case failed for n={}, fs={}, freq={}: {}", n, fs, test_freq, e);
                        total_cases += 1;
                    }
                }
            }
    ...
    let rmse = if total_cases > 0 { (rmse_sum / total_cases as f64).sqrt() } else { 0.0 };
    let correlation = if total_cases > 0 { correlation_sum / total_cases as f64 } else { 0.0 };
...
fn compute_reference_lombscargle(
    t: &[f64],
    y: &[f64],
    freqs: &[f64],
    ...
    for (i, &freq) in freqs.iter().enumerate() {
        ...
        let power = if sum_cos_den > 1e-15 && sum_sin_den > 1e-15 {
            0.5 * (sum_cos_num * sum_cos_num / sum_cos_den + 
                   sum_sin_num * sum_sin_num / sum_sin_den) / y_var
        } else {
            0.0
        };
...
fn validate_normalization_methods(
    ...
    for norm_method in &normalizations {
        ...
        for &fs in &config.sampling_frequencies[..2] { // Limit for normalization testing
            for &n in &config.test_lengths[..3] {
                for &test_freq in &config.test_frequencies[..2] {
                    if let Ok((_, _, _, corr)) = validate_single_normalization_case(
                        n, fs, test_freq, norm_method, config
                    ) {
                        total_corr += corr;
                        valid_tests += 1;
...
fn validate_single_normalization_case(
    ...
    let signal: Vec<f64> = t.iter().map(|&time| (2.0 * PI * test_freq * time).sin()).collect();
...
fn test_sparse_sampling(_config: &ScipyValidationConfig) -> SignalResult<bool> {
    ...
    let signal: Vec<f64> = t.iter().map(|&time| (2.0 * PI * 0.1 * time).sin()).collect();
...
fn test_extreme_dynamic_range(_config: &ScipyValidationConfig) -> SignalResult<bool> {
    ...
    let mut signal: Vec<f64> = t.iter().map(|&time| (2.0 * PI * 1.0 * time).sin()).collect();
    ...
    match lombscargle(
        ...
        Ok((_, power)) => {
            ...
            Ok(power.iter().all(|&p| p.is_finite() && p >= 0.0))
        }
        Err(_) => Ok(false),
    }
}
...
fn test_high_frequency_resolution(_config: &ScipyValidationConfig) -> SignalResult<bool> {
    ...
    let signal: Vec<f64> = t.iter().map(|&time| {
        (2.0 * PI * f1 * time).sin() + (2.0 * PI * f2 * time).sin()
    }).collect();
...
fn estimate_detection_power(config: &ScipyValidationConfig) -> SignalResult<f64> {
    ...
    for _ in 0..trials {
        ...
        let t: Vec<f64> = (0..n).map(|i| i as f64 / fs).collect();
        let signal: Vec<f64> = t.iter().map(|&time| {
            (2.0 * PI * signal_freq * time).sin() + 0.1 * rng.gen_range(-1.0..1.0)
        }).collect();
        ...
        if let Ok((freq_grid, power)) = lombscargle(
            ...
            // Find peak frequency
            if let Some(peak_idx) = power.iter()
                .enumerate()
...
fn calculate_overall_summary(
    ...
    let accuracy_score = (accuracy.correlation * 100.0)
        .min(100.0 - accuracy.max_relative_error * 10000.0);
    
    let performance_score = (performance.speed_ratio * 50.0 + 
                           performance.scalability_score * 0.5)
                           .min(100.0);
    
    let reliability_score = statistical.consistency_score * 100.0;
    
    let overall_score = (accuracy_score * 0.5 + 
                        performance_score * 0.3 + 
                        reliability_score * 0.2)
                        .min(100.0);
    
    let passed = overall_score >= 85.0 && 
                accuracy.max_relative_error < 0.01 &&
...
fn calculate_error_metrics(result1: &[f64], result2: &[f64]) -> SignalResult<(f64, f64, f64)> {
    ...
    for (i, (&a, &b)) in result1.iter().zip(result2.iter()).enumerate() {
        ...
        let rel_error = if b.abs() > 1e-15 { abs_error / b.abs() } else { 0.0 };
...
fn calculate_normalization_consistency(
    ...
    let correlations: Vec<f64> = method_results.values()
...
pub fn validate_lombscargle_advanced(
    ...
    let uncertainty_results = if config.bootstrap_samples > 0 {
        Some(quantify_uncertainty(&config.base, config.bootstrap_samples)?)
    } else {
        None
    };
...
fn test_numerical_conditioning(config: &ScipyValidationConfig) -> SignalResult<ConditioningTestResult> {
...
fn test_astronomical_scenarios(config: &ScipyValidationConfig) -> SignalResult<AstronomicalTestResult> {
    let mut rng = rand::thread_rng();
...
fn test_phase_coherence(config: &ScipyValidationConfig) -> SignalResult<PhaseCoherenceResult> {
    ...
    let n = 500;
    let times: Vec<f64> = (0..n).map(|i| i as f64 * 0.1 + rng.gen::<f64>() * 0.05).collect();
    ...
    let values: Vec<f64> = times.iter()
        .map(|&t| {
            (2.0 * PI * freq1 * t).sin() + 
            (2.0 * PI * freq2 * t + phase_offset).sin()
        })
        .collect();
...
fn quantify_uncertainty(
    ...
    for _ in 0..n_bootstrap {
        ...
        let freqs: Vec<f64> = (1..=100).map(|i| i as f64 * 0.01).collect();
        let periodogram = lombscargle(&boot_times, &boot_values, &freqs)?;
        
        // Find peak frequency
        let peak_idx = periodogram.iter()
            .enumerate()
    ...
    let variance_estimate = bootstrap_results.iter()
        .map(|&x| (x - mean).powi(2))
    ...
    let confidence_intervals = vec![(bootstrap_results[ci_low_idx], bootstrap_results[ci_high_idx])];
    
    // Coverage probability (simplified)
    let in_ci = bootstrap_results.iter()
...
/// Test frequency resolution limits
fn test_frequency_resolution(config: &ScipyValidationConfig) -> SignalResult<FrequencyResolutionResult> {
    let mut rng = rand::thread_rng();
...
pub fn run_comprehensive_validation() -> SignalResult<()> {
    ...
    println!("\n=== Validation Results ===");
    println!("Overall Status: {}", if results.summary.passed { "PASSED" } else { "FAILED" });
    ...
    println!("Reliability Score: {:.1}/100", results.summary.reliability_score);
    
    println!("\n=== Accuracy Metrics ===");
    println!("Maximum Relative Error: {:.2e}", results.accuracy_results.max_relative_error);
    println!("RMSE: {:.2e}", results.accuracy_results.rmse);
    println!("Correlation with SciPy: {:.6}", results.accuracy_results.correlation);
    println!("Passed Cases: {}/{}", 
             results.accuracy_results.passed_cases,
             results.accuracy_results.total_cases);
    ...
    if let Some(ref edge_results) = results.edge_case_results {
        println!("\n=== Edge Cases ===");
        println!("Sparse Sampling: {}", if edge_results.sparse_sampling { "PASS" } else { "FAIL" });
        println!("Extreme Dynamic Range: {}", if edge_results.extreme_dynamic_range { "PASS" } else { "FAIL" });
        println!("Short Time Series: {}", if edge_results.short_time_series { "PASS" } else { "FAIL" });
        println!("High Freq Resolution: {}", if edge_results.high_freq_resolution { "PASS" } else { "FAIL" });
        println!("Stability Score: {:.3}", edge_results.stability_score);
    }
    
    println!("\n=== Statistical Properties ===");
    println!("False Alarm Rate: {:.3}", results.statistical_results.false_alarm_rate);
    println!("Detection Power: {:.3}", results.statistical_results.detection_power);
    println!("CI Coverage: {:.3}", results.statistical_results.ci_coverage);
    
    println!("\n=== Performance ===");
    println!("Speed Ratio (ours/scipy): {:.2}", results.performance_results.speed_ratio);
    println!("Memory Ratio (ours/scipy): {:.2}", results.performance_results.memory_ratio);
    println!("Scalability Score: {:.1}", results.performance_results.scalability_score);
...
fn test_perturbation_stability(times: &[f64], values: &[f64], freqs: &[f64]) -> SignalResult<f64> {
    ...
    let perturbed_values: Vec<f64> = values.iter()
        .map(|&v| v + perturbation_level * rng.gen::<f64>())
        .collect();
    let perturbed = lombscargle(times, &perturbed_values, freqs)?;
    
    // Compute relative change
    let relative_changes: Vec<f64> = original.iter().zip(perturbed.iter())
        .map(|(&orig, &pert)| {
            if orig.abs() > 1e-15 {
                ((pert - orig) / orig).abs()
            } else {
...
fn test_gradient_stability(times: &[f64], values: &[f64], freqs: &[f64]) -> SignalResult<f64> {
    ...
    for i in 0..values.len().min(10) { // Test a few points
        ...
        let gradient_norm: f64 = original.iter().zip(perturbed.iter())
...
fn test_false_peak_suppression(rng: &mut impl Rng) -> SignalResult<f64> {
    ...
    let false_peak_ratio = (peaks.len().saturating_sub(1)) as f64 / peaks.len().max(1) as f64;
...
fn test_variable_star_simulation(rng: &mut impl Rng) -> SignalResult<f64> {
    ...
    let magnitudes: Vec<f64> = times.iter()
        .map(|&t| {
    ...
    let periodogram = lombscargle(&times, &magnitudes, &freqs)?;
...
fn test_exoplanet_transit_simulation(rng: &mut impl Rng) -> SignalResult<f64> {
    ...
    let flux: Vec<f64> = times.iter()
        .map(|&t| {
            ...
            let baseline = 1.0;
            let transit_flux = if in_transit { baseline - transit_depth } else { baseline };
            transit_flux + 0.001 * rng.gen::<f64>() // Low noise
        })
        .collect();
    
    let freqs: Vec<f64> = (1..=100).map(|i| i as f64 * 0.001).collect();
    let periodogram = lombscargle(&times, &flux, &freqs)?;
    
    let expected_freq = 1.0 / orbital_period;
    let closest_freq_idx = freqs.iter()
        .enumerate()
        .min_by(|(_, &f1), (_, &f2)| {
            (f1 - expected_freq).abs().partial_cmp(&(f2 - expected_freq).abs()).unwrap()
        })
        .map(|(i, _)| i)
        .unwrap_or(0);
...
fn test_rr_lyrae_simulation(rng: &mut impl Rng) -> SignalResult<f64> {
    ...
    let n = 800;
    let times: Vec<f64> = (0..n).map(|i| i as f64 * 0.015 + rng.gen::<f64>() * 0.005).collect();
...
fn test_multi_periodic_source(rng: &mut impl Rng) -> SignalResult<f64> {
    ...
    for &expected in &expected_freqs {
        if detected_freqs.iter().any(|&detected| (detected - expected).abs() < 0.01) {
            detections += 1;
        }
    }
...
fn test_phase_preservation(
    ...
    let peak_strength = periodogram.iter().sum::<f64>() / periodogram.len() as f64;
...
fn test_coherence_stability(times: &[f64], values: &[f64]) -> SignalResult<f64> {
    ...
    for start in (0..times.len()).step_by(window_size / 2) {
        let end = (start + window_size).min(times.len());
        if end - start < 10 {
            break;
        }
        
        let window_times = &times[start..end];
        let window_values = &values[start..end];
        
        let freqs: Vec<f64> = (10..=50).map(|i| i as f64 * 0.01).collect();
        let periodogram = lombscargle(window_times, window_values, &freqs)?;
        
        let peak_power = periodogram.iter().cloned().fold(0.0, f64::max);
    ...
    let variance = coherence_scores.iter()
        .map(|&c| (c - mean_coherence).powi(2))
        .sum::<f64>() / coherence_scores.len() as f64;
...
fn test_min_frequency_separation(rng: &mut impl Rng) -> SignalResult<f64> {
    ...
    if let Some(&min_sep) = resolved_separations.iter().min_by(|a, b| a.partial_cmp(b).unwrap()) {
        Ok(min_sep)
    } else {
...
fn characterize_spectral_window(rng: &mut impl Rng) -> SignalResult<f64> {
    ...
    let spectral_window = lombscargle(&times, &values, &freqs)?;
    
    // Analyze spectral window properties
    let main_lobe_idx = spectral_window.iter()
        .enumerate()
        .max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
        .map(|(i, _)| i)
        .unwrap_or(100);
    ...
    let sidelobe_power = spectral_window.iter()
        .enumerate()
        .filter(|(i, _)| (*i as i32 - main_lobe_idx as i32).abs() > 10)
        .map(|(_, &power)| power)
        .fold(0.0, f64::max);
