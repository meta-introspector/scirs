...
use crate::denoise_adaptive_ultra::{adaptive_denoise_ultra, AdaptiveDenoisingConfig};
use crate::filter::{butter, FilterType, filtfilt};
...
fn test_biomedical_signal_pipeline() -> SignalResult<()> {
    ...
    for &ti in &t {
        ...
        // P and T waves
        let p_wave = 0.3 * (2.0 * PI * heart_rate * ti + PI).sin() *
                     (-20.0 * (ti % (1.0 / heart_rate) - 0.05).powi(2)).exp();
        let t_wave = 0.4 * (2.0 * PI * heart_rate * ti - PI).sin() *
    ...
    let (b_notch, a_notch) = butter(4, (49.0 / (fs / 2.0), 51.0 / (fs / 2.0)), FilterType::Bandstop, false, Some("ba"))?;
    ...
    let denoise_config = AdaptiveDenoisingConfig {
        ...
        enable_fusion: true,
        preservation_mode: crate::denoise_adaptive_ultra::PreservationMode::EdgePreserving,
        optimization_target: crate::denoise_adaptive_ultra::OptimizationTarget::MaxSNR,
        ..Default::default()
    };
    
    let denoising_result = adaptive_denoise_ultra(&preprocessed_ecg, &denoise_config)?;
    println!("✅ Denoising completed - SNR improvement: {:.2} dB", denoising_result.snr_improvement_db);
    println!("   Quality score: {:.1}/100", denoising_result.quality_metrics.overall_quality);
    ...
    let max_power_idx = spectral_result.psd
        .iter()
        .enumerate()
    ...
    println!("   Dominant frequency: {:.2} Hz (expected ~1 Hz for heart rate)", dominant_frequency);
    ...
    println!("   Mean RR interval: {:.3} s", rr_intervals.iter().sum::<f64>() / rr_intervals.len() as f64);
    println!("   Heart rate variability (RMSSD): {:.3} ms", heart_rate_variability * 1000.0);
    
    // Validation checks
    assert!(dominant_frequency > 0.5 && dominant_frequency < 2.0, "Dominant frequency out of expected range");
    assert!(tf_result.fit_percentage > 70.0, "System identification fit too low");
    assert!(denoising_result.snr_improvement_db > 0.0, "No SNR improvement achieved");
...
fn test_audio_processing_pipeline() -> SignalResult<()> {
    ...
    for (chord_idx, frequencies) in chords.iter().enumerate() {
        ...
        for i in start_sample..end_sample.min(n) {
            let ti = t[i];
            let envelope = 0.5 * (-3.0 * (ti - chord_idx as f64 * chord_duration - chord_duration/2.0)).exp();
            
            for &freq in frequencies {
                audio_signal[i] += envelope * (2.0 * PI * freq * ti).sin() / frequencies.len() as f64;
            }
        }
    }
    ...
    println!("   Detected {} fundamental frequency estimates", fundamental_frequencies.len());
    ...
    assert!(!fundamental_frequencies.is_empty(), "Should detect fundamental frequencies");
...
fn test_geophysical_processing_pipeline() -> SignalResult<()> {
    ...
    for &t in &times {
        ...
        let noise = 0.2 * rng.gen_range(-1.0..1.0) + 
                   0.1 * (2.0 * PI * 50.0 * t).sin(); // Cultural noise
    ...
    let (frequencies, power, _confidence) = lombscargle_enhanced(&times, &seismic_signal, &ls_config)?;
    ...
    println!("   Detected {} significant frequency peaks", detected_frequencies.len());
    println!("   Peak frequencies: {:?}", detected_frequencies.iter().map(|f| format!("{:.1}", f)).collect::<Vec<_>>());
    ...
    println!("✅ Interpolated to {} regular samples", interpolated_signal.len());
    ...
    let (b_low, a_low) = butter(4, 8.0 / (regular_fs / 2.0), FilterType::Lowpass, false, Some("ba"))?;
    let low_freq_signal = filtfilt(&b_low, &a_low, &interpolated_signal)?;
    
    // High frequency band (> 8 Hz) - local events
    let (b_high, a_high) = butter(4, 8.0 / (regular_fs / 2.0), FilterType::Highpass, false, Some("ba"))?;
    ...
    println!("   ARMA model fitted with {} AR and {} MA coefficients", 
             parametric_result.ar_coeffs.len() - 1, parametric_result.ma_coeffs.len() - 1);
    
    // Validation
    assert!(detected_frequencies.len() >= 2, "Should detect at least 2 frequency peaks");
    assert!(!events.is_empty(), "Should detect some seismic events");
    assert!(interpolated_signal.len() > 1000, "Should have sufficient interpolated data");
...
fn test_image_processing_pipeline() -> SignalResult<()> {
    ...
    for i in 0..height {
        for j in 0..width {
            ...
            clean_image[[i, j]] = 
                0.5 * (2.0 * PI * 3.0 * r).sin() * (3.0 * theta).cos() +
    ...
    let wavelet_result = enhanced_dwt2d_decompose(&noisy_image, crate::dwt::Wavelet::DB(4), &dwt_config)?;
    ...
    let detail_coeffs: Vec<f64> = wavelet_result.detail_h.iter()
    ...
    println!("   Estimated PSNR improvement: {:.2} dB", estimated_psnr_improvement);
    ...
    // Validation
    assert!(wavelet_result.approx.len() > 0, "Should have approximation coefficients");
...
fn test_validation_pipeline() -> SignalResult<()> {
    ...
    println!("✅ Validation completed in {:.2}ms", validation_result.execution_time_ms);
    println!("   Overall score: {:.1}/100", validation_result.summary.overall_score);
    println!("   Pass rate: {:.1}%", validation_result.summary.pass_rate);
    println!("   Tests passed: {}/{}", validation_result.summary.passed_tests, validation_result.summary.total_tests);
    ...
    assert!(validation_result.summary.total_tests > 0, "Should run some tests");
    assert!(validation_result.summary.overall_score > 60.0, "Should achieve reasonable overall score");
    assert!(validation_result.execution_time_ms < 60000.0, "Should complete within 1 minute");
...
fn calculate_hrv(rr_intervals: &[f64]) -> SignalResult<f64> {
    ...
    Ok((sum_sq_diff / (rr_intervals.len() - 1) as f64).sqrt())
}
...
fn detect_onsets(spectrograms: &[crate::multitaper::enhanced::EnhancedMultitaperResult], fs: f64, hop_size: usize) -> SignalResult<Vec<f64>> {
    ...
    for i in 1..spectrograms.len() {
        ...
        if energy_ratio > 1.5 { // Threshold for onset detection
...
fn recognize_chords(spectrograms: &[crate::multitaper::enhanced::EnhancedMultitaperResult]) -> SignalResult<Vec<String>> {
    let mut chords = Vec::new();
    
    for spectrogram in spectrograms {
        // Find the 3 strongest peaks (simplified chord recognition)
        let mut freq_power_pairs: Vec<(f64, f64)> = spectrogram.frequencies
            .iter()
...
fn detect_seismic_events(signal: &[f64], fs: f64) -> SignalResult<Vec<f64>> {
    ...
    for i in lta_window..signal.len() - sta_window {
        // Calculate STA (Short-Term Average)
        let sta = signal[i..i + sta_window].iter().map(|&x| x * x).sum::<f64>() / sta_window as f64;
        
        // Calculate LTA (Long-Term Average)
        let lta = signal[i - lta_window..i].iter().map(|&x| x * x).sum::<f64>() / lta_window as f64;
...
fn extract_image_features(decomp: &crate::dwt2d_enhanced::EnhancedDwt2dResult) -> SignalResult<Vec<f64>> {
