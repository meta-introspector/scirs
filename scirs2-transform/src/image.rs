...
impl PatchExtractor {
    ...
    pub fn extract_patches_2d(&self, image: &Array2<f64>) -> Result<Array3<f64>> {
        ...
        if patch_height > img_height || patch_width > img_width {
            return Err(TransformError::InvalidInput(
                format!("Patch size ({}, {}) exceeds image size ({}, {})",
    ...
    pub fn extract_patches_batch(&self, images: &Array3<f64>) -> Result<Array4<f64>> {
        ...
        let mut all_patches = Array4::zeros((n_images * n_patches_per_image, patch_height, patch_width, 1));
        ...
        for (img_idx, image) in images.outer_iter().enumerate() {
            ...
            for (patch_idx, patch) in patches.outer_iter().enumerate() {
                all_patches.slice_mut(s![start_idx + patch_idx, .., .., 0])
                    .assign(&patch);
            }
        }
...
impl HOGDescriptor {
    ...
    pub fn with_block_norm(mut self, block_norm: BlockNorm) -> Self {
            ...
            ImageNormMethod::Standard => {
                if let Some((ref means, ref stds)) = self.channel_stats {
                    ...
                    for c in 0..n_channels {
                        ...
                        result.slice_mut(s![.., .., .., c])
                            .mapv_inplace(|v| (v - mean) / std);
                    }
                } else {
                    return Err(TransformError::NotFitted("ImageNormalizer must be fitted before transform".into()));
                }
            }
...
pub fn rgb_to_grayscale(images: &Array4<f64>) -> Result<Array3<f64>> {
    ...
    if shape[3] != 3 {
        return Err(TransformError::InvalidInput(
            format!("Expected 3 channels for RGB, got {}", shape[3])
        ));
    }
