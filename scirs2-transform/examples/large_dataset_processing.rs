...
use scirs2_transform::{
    ...
    out_of_core::{OutOfCoreNormalizer, OutOfCoreTransformer, csv_chunks},
    streaming::{StreamingStandardScaler, StreamingMinMaxScaler, StreamingTransformer,
...
fn example_streaming_transformations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..5 {
        ...
        println!("  Batch {}: {} samples processed", i + 1, scaler.n_samples_seen());
...
fn example_windowed_streaming() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Processing data with sliding window of {} samples:", window_size);
...
fn create_sample_csv(path: &str, n_rows: usize, n_cols: usize) -> Result<(), Box<dyn std::error::Error>> {
