...
fn main() -> Result<()> {
    ...
    DynamicImage::ImageLuma8(registered_point.clone()).save("output/simple_point_result.png").ok();
    DynamicImage::ImageLuma8(registered_intensity.clone()).save("output/simple_intensity_result.png").ok();
    ...
    println!("  Point-based: {:.2}", point_mse);
    ...
    println!("  Point-based: {:.4}", point_ncc);
...
fn create_synthetic_matches(num_matches: usize, true_transform: &Array2<f64>) -> Vec<PointMatch> {
    ...
    for _ in 0..(num_matches * 9 / 10) { // 90% good matches
        let source = Point2D::new(
            rng.random_range(20.0..180.0),
            rng.random_range(20.0..180.0),
        );
