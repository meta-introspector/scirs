...
use scirs2_vision::registration::{
    Point2D, PointMatch, TransformType, RegistrationParams,
    ransac_estimate_transform, transform_point,
    warping::{warp_image, InterpolationMethod, BoundaryMethod},
...
use image::{DynamicImage, ImageBuffer, Luma, RgbImage, Rgb};
use ndarray::Array2;
use rand::prelude::*;
...
fn main() -> Result<()> {
    ...
    println!("  Final cost: {:.6}", feature_result.registration.final_cost);
    ...
    let intensity_result = register_images_intensity(
        &ref_gray,
        &target_gray,
        None,
        &intensity_config,
    ...
    let point_result = ransac_estimate_transform(
        &point_matches,
    ...
    println!("  Inliers: {}/{}", point_result.inliers.len(), point_matches.len());
    
    // 4. Transform comparison and error analysis
    println!("\n4. Transform Analysis");
    println!("--------------------");
    
    analyze_transform_accuracy(&ground_truth_transform, &feature_result.registration.transform, "Feature-based");
    analyze_transform_accuracy(&ground_truth_transform, &intensity_result.transform, "Intensity-based");
    analyze_transform_accuracy(&ground_truth_transform, &point_result.transform, "Point-based");
    ...
    if let Ok(_) = DynamicImage::ImageLuma8(registered_feature_clone).save("output/registration_feature_result.png") {
        println!("  Saved feature-based result");
    }
    
    if let Ok(_) = DynamicImage::ImageLuma8(registered_intensity_clone).save("output/registration_intensity_result.png") {
...
fn add_circle(img: &mut ImageBuffer<Luma<u8>, Vec<u8>>, cx: u32, cy: u32, radius: u32, intensity: u8) {
...
fn add_rectangle(img: &mut ImageBuffer<Luma<u8>, Vec<u8>>, x: u32, y: u32, w: u32, h: u32, intensity: u8) {
...
fn add_corner_pattern(img: &mut ImageBuffer<Luma<u8>, Vec<u8>>, cx: u32, cy: u32, size: u32, intensity: u8) {
    let (width, height) = img.dimensions();
...
fn create_synthetic_matches(num_matches: usize) -> Vec<PointMatch> {
    ...
    let true_transform = Array2::from_shape_vec((3, 3), vec![
        1.1 * 0.8_f64.cos(), -1.1 * 0.8_f64.sin(), 20.0,
        1.1 * 0.8_f64.sin(), 1.1 * 0.8_f64.cos(), 15.0,
        0.0, 0.0, 1.0,
    ]).unwrap();
    
    for _ in 0..(num_matches * 8 / 10) { // 80% good matches
...
fn analyze_transform_accuracy(ground_truth: &Array2<f64>, estimated: &Array2<f64>, method_name: &str) {
...
fn compute_mse(img1: &ImageBuffer<Luma<u8>, Vec<u8>>, img2: &ImageBuffer<Luma<u8>, Vec<u8>>) -> f64 {
...
fn compute_ncc(img1: &ImageBuffer<Luma<u8>, Vec<u8>>, img2: &ImageBuffer<Luma<u8>, Vec<u8>>) -> f64 {
    let (width, height) = img1.dimensions();
