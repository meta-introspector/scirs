...
use scirs2_vision::{
    // Scene understanding
    analyze_scene_with_reasoning, SceneUnderstandingEngine, SpatialRelationType,
    
    // Visual reasoning
    perform_advanced_visual_reasoning, QueryType, VisualReasoningEngine, VisualReasoningQuery,
    
    // Activity recognition
    recognize_activities_comprehensive, ActivityRecognitionEngine, monitor_activities_realtime,
    
    // Visual SLAM
    process_visual_slam, VisualSLAMSystem, process_visual_slam_realtime,
    
    // AI optimization
    RLParameterOptimizer, NeuralArchitectureSearch, GeneticPipelineOptimizer, PredictiveScaler,
    
    // Quantum-inspired processing
    QuantumAdaptiveStreamPipeline, QuantumStreamProcessor,
    
    // Neuromorphic processing
    AdaptiveNeuromorphicPipeline, SpikingNeuralNetwork,
...
fn main() -> Result<()> {
    ...
    println!("✓ Detected {} objects in scene", scene_analysis.objects.len());
    println!("✓ Identified {} spatial relationships", scene_analysis.relationships.len());
    println!("✓ Scene classification: {} (confidence: {:.2})", 
             scene_analysis.scene_class, scene_analysis.scene_confidence);
    println!("✓ Scene graph contains {} nodes and {} edges", 
             scene_analysis.scene_graph.nodes.len(), scene_analysis.scene_graph.edges.len());
    
    if let Some(temporal_info) = &scene_analysis.temporal_info {
        println!("✓ Temporal analysis: {} scene changes detected", 
                 temporal_info.scene_changes.len());
    }
    
    println!("✓ Reasoning results: {} conclusions drawn", 
    ...
    let questions = vec![
        ("What is happening in this scene?", QueryType::WhatIsHappening),
        ("Why might this be occurring?", QueryType::WhyIsHappening),
        ("How are the objects related?", QueryType::HowAreObjectsRelated),
        ("What causal relationships exist?", QueryType::CausalRelationshipQuery),
    ];
    ...
    for (question, query_type) in questions {
        ...
        println!("📋 Reasoning steps: {}", reasoning_result.reasoning_steps.len());
        println!("📊 Evidence pieces: {}", reasoning_result.evidence.len());
        println!("🔀 Alternative hypotheses: {}", reasoning_result.alternatives.len());
        println!();
    }
    ...
    println!("✓ Detected {} individual activities", activity_result.activities.len());
    println!("✓ Identified {} activity sequences", activity_result.sequences.len());
    println!("✓ Found {} person interactions", activity_result.interactions.len());
    println!("✓ Scene summary:");
    println!("  - Dominant activity: {}", activity_result.scene_summary.dominant_activity);
    println!("  - Energy level: {:.2}", activity_result.scene_summary.energy_level);
    println!("  - Social interaction level: {:.2}", activity_result.scene_summary.social_interaction_level);
    println!("  - Complexity score: {:.2}", activity_result.scene_summary.complexity_score);
    println!("✓ Timeline segments: {}", activity_result.timeline.segments.len());
    println!("✓ Overall confidence: {:.2}", activity_result.confidence_scores.overall);
    ...
    for (i, frame) in test_images.iter().take(3).enumerate() {
        ...
        println!("  Frame {}: {} activities detected", 
                 i + 1, realtime_result.activities.len());
    }
    println!();
    ...
    println!("  - Trajectory smoothness: {:.2}", 
             slam_result.trajectory.smoothness_metrics.smoothness_score);
    println!("  - Velocity consistency: {:.2}", 
             slam_result.trajectory.smoothness_metrics.velocity_consistency);
    
    println!("✓ 3D Map reconstruction:");
    println!("  - {} landmarks mapped", slam_result.map_3d.landmarks.len());
    println!("  - Map resolution: {:.3}m", slam_result.map_3d.resolution);
    println!("  - Map quality score: {:.2}", slam_result.map_quality.overall_score);
    println!("  - Reconstruction accuracy: {:.2}", slam_result.map_quality.reconstruction_accuracy);
    
    println!("✓ Semantic mapping:");
    println!("  - {} semantic objects", slam_result.semantic_map.semantic_objects.len());
    println!("  - {} object relationships", slam_result.semantic_map.object_relationships.len());
    println!("  - Semantic consistency: {:.2}", 
             slam_result.semantic_map.consistency_metrics.overall_consistency);
    
    println!("✓ Loop closures: {} detected", slam_result.loop_closures.len());
    println!("✓ System performance:");
    println!("  - Tracking success rate: {:.1}%", 
             slam_result.performance_stats.tracking_success_rate * 100.0);
    println!("  - Average processing time: {:.1}ms", 
             slam_result.performance_stats.processing_times.iter().sum::<f64>() / 
             slam_result.performance_stats.processing_times.len() as f64 * 1000.0);
    println!("  - Robustness score: {:.2}", slam_result.performance_stats.robustness_score);
    ...
    for (i, frame) in test_images.iter().take(3).enumerate() {
        ...
        println!("  Frame {}: pose confidence {:.2}, {} landmarks", 
                 i + 1, 
    ...
    let quantum_stages = vec!["entanglement".to_string(), "superposition".to_string(), "annealing".to_string()];
    ...
    let (quantum_processed_frame, quantum_decision) = quantum_processor.process_quantum_frame(test_frame)?;
    ...
    println!("  - Output frame dimensions: {}x{}", quantum_processed_frame.data.nrows(), quantum_processed_frame.data.ncols());
    println!();
    ...
    println!("  - Output frame dimensions: {}x{}", neuromorphic_result.data.nrows(), neuromorphic_result.data.ncols());
    ...
    println!("  - Scene understanding: {:.2}ms", scene_time.as_secs_f64() * 1000.0);
    println!("  - Visual reasoning: {:.2}ms", reasoning_time.as_secs_f64() * 1000.0);
    println!("  - Activity recognition: {:.2}ms", activity_time.as_secs_f64() * 1000.0);
    println!("  - Total processing time: {:.2}ms", start_time.elapsed().as_secs_f64() * 1000.0);
    ...
    println!("  - Real-time capability: {}", 
             if start_time.elapsed().as_secs_f64() < 0.1 { "Excellent" } else { "Good" });
    println!();
