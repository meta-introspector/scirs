...
use crate::registration::{
    Point2D, PointMatch, RegistrationParams, RegistrationResult, TransformType,
    ransac_estimate_transform, identity_transform,
};
use crate::feature::{
    harris_corners, extract_feature_coordinates,
};
...
pub fn register_images(
    ...
    let (target_keypoints, target_descriptors) = detect_and_describe(target, &config.detector_params)?;
        ...
        if let Some(idx) = best_idx {
            ...
            if best_dist <= params.max_distance as f32 {
                ...
                let pass_ratio_test = if params.use_ratio_test {
                    second_best_dist > 0.0 && best_dist / second_best_dist < params.ratio_threshold as f32
                } else {
                    true
                };
...
fn generate_simple_descriptors(
    image: &GrayImage,
    keypoints: &[Keypoint],
) -> Result<Vec<Vec<u8>>> {
...
pub fn multi_scale_register(
    ...
    best_result.ok_or_else(|| VisionError::OperationError(
        "Multi-scale registration failed".to_string()
    ))
}
...
fn build_pyramid(image: &DynamicImage, levels: usize) -> Vec<DynamicImage> {
    ...
    for _ in 1..levels {
        ...
        let downsampled = prev.resize_exact(
            new_width,
            new_height,
...
pub fn template_register(
    ...
    let (search_x, search_y, search_width, search_height) = search_region
        .unwrap_or((0, 0, ref_width - template_width, ref_height - template_height));
