...
pub fn transform_point(point: Point2D, transform: &TransformMatrix) -> Point2D {
    ...
    if transformed[2].abs() < 1e-10 {
        ...
        Point2D::new(transformed[0] / transformed[2], transformed[1] / transformed[2])
    }
}

/// Apply transformation to multiple points
pub fn transform_points(points: &[Point2D], transform: &TransformMatrix) -> Vec<Point2D> {
    points.iter().map(|&p| transform_point(p, transform)).collect()
}
...
pub fn invert_transform(transform: &TransformMatrix) -> Result<TransformMatrix> {
    ...
    transform.inv().map_err(|e| VisionError::OperationError(
...
pub fn ransac_estimate_transform(
    ...
    if matches.len() < min_samples {
        return Err(VisionError::InvalidParameter(
            format!("Need at least {} matches for {} transformation", 
                    min_samples, format!("{:?}", transform_type))
        ));
    }
    ...
    for _iteration in 0..params.ransac_iterations {
        ...
        let sample_matches: Vec<_> = sample_indices.iter()
        ...
        for (i, m) in matches.iter().enumerate() {
            ...
            let error = ((transformed.x - m.target.x).powi(2) + 
                        (transformed.y - m.target.y).powi(2)).sqrt();
    ...
    let inlier_matches: Vec<_> = best_inliers.iter()
        .map(|&i| matches[i].clone())
        .collect();
...
fn estimate_affine_transform(matches: &[PointMatch]) -> Result<TransformMatrix> {
    ...
    let params = a.solve(&b).map_err(|e| VisionError::OperationError(
        format!("Failed to solve affine system: {}", e)
    ))?;
...
fn estimate_homography_transform(matches: &[PointMatch]) -> Result<TransformMatrix> {
    ...
    let (_u, _s, vt) = a.svd(true, true).map_err(|e| VisionError::OperationError(
        format!("SVD failed: {}", e)
    ))?;
    
    let vt = vt.ok_or_else(|| VisionError::OperationError(
        "SVD did not return Vt matrix".to_string()
    ))?;
