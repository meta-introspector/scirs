...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Initial concentrations: [A]={:.3}, [B]={:.3}", y0[0], y0[1]);
    println!("   Final concentrations: [A]={:.3}, [B]={:.3}", 
    ...
    let b_eq = total - a_eq;
    println!("   Theoretical equilibrium: [A]={:.3}, [B]={:.3}", a_eq, b_eq);
    println!();
    
    // Example 2: Michaelis-Menten Enzyme Kinetics
    println!("2. Michaelis-Menten Enzyme Kinetics");
    let result = solve_ivp(michaelis_menten_kinetics, [0.0, 3.0], array![10.0, 0.0], None)?;
    
    println!("   Initial: [S]={:.3}, [P]={:.3}", 10.0, 0.0);
    println!("   Final: [S]={:.3}, [P]={:.3}", 
             result.y.last().unwrap()[0], result.y.last().unwrap()[1]);
    println!("   Conservation check: S+P = {:.3} (should be 10.0)", 
             result.y.last().unwrap()[0] + result.y.last().unwrap()[1]);
    println!();
    ...
    println!("   Final: [A]={:.3}, [B]={:.3}", 
             result.y.last().unwrap()[0], result.y.last().unwrap()[1]);
    println!("   Total conservation: A+B = {:.3} (should be 1.1)", 
             result.y.last().unwrap()[0] + result.y.last().unwrap()[1]);
    println!();
    ...
    let result = solve_ivp(lotka_volterra_reaction, [0.0, 10.0], array![1.0, 1.0], Some(options.clone()))?;
    
    println!("   Initial: [X]={:.3}, [Y]={:.3}", 1.0, 1.0);
    println!("   Final: [X]={:.3}, [Y]={:.3}", 
             result.y.last().unwrap()[0], result.y.last().unwrap()[1]);
    ...
    let result = solve_ivp(brusselator, [0.0, 20.0], array![1.0, 1.0], Some(options.clone()))?;
    
    println!("   Initial: [X]={:.3}, [Y]={:.3}", 1.0, 1.0);
    println!("   Final: [X]={:.3}, [Y]={:.3}", 
             result.y.last().unwrap()[0], result.y.last().unwrap()[1]);
    println!("   Steps taken: {} (limit cycle oscillator)", result.t.len());
    println!();
    ...
    let result = solve_ivp(oregonator, [0.0, 5.0], array![1.0, 1.0, 1.0], Some(options_bz))?;
    
    println!("   Initial: [X]={:.3}, [Y]={:.3}, [Z]={:.3}", 1.0, 1.0, 1.0);
    println!("   Final: [X]={:.3}, [Y]={:.3}, [Z]={:.3}", 
             result.y.last().unwrap()[0], result.y.last().unwrap()[1], result.y.last().unwrap()[2]);
    ...
    println!("   Initial: [G6P]={:.3}, [F6P]={:.3}, [ATP]={:.3}", 0.1, 0.1, 2.0);
    println!("   Final: [G6P]={:.3}, [F6P]={:.3}, [ATP]={:.3}", 
             result.y.last().unwrap()[0], result.y.last().unwrap()[1], result.y.last().unwrap()[2]);
