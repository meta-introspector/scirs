...
use scirs2_integrate::pde::mesh_generation::{
    AutoMeshGenerator, MeshGenerationParams, Domain, BoundarySpecification
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   - Average element size: {:.4}", rect_quality.avg_element_size);
    println!();
    ...
    println!("   - Average element size: {:.4}", circle_quality.avg_element_size);
    println!();
    ...
    for i in 0..n_sides {
        ...
        pentagon_vertices.push(Point::new(
            radius * angle.cos(),
            radius * angle.sin(),
        ));
    }
    ...
    for (name, gen) in &generators {
        ...
        println!("   {} mesh: {} points, {} elements, avg size: {:.4}",
                 name, mesh.points.len(), mesh.elements.len(), quality.avg_element_size);
    }
    println!();
    ...
    println!("   - Poor quality elements: {}", hq_quality.poor_quality_elements);
...
mod tests {
    ...
    fn test_element_size_effect() {
        ...
        let coarse_mesh = coarse_generator.generate_mesh(&domain, &boundary_spec).unwrap();
        let fine_mesh = fine_generator.generate_mesh(&domain, &boundary_spec).unwrap();
