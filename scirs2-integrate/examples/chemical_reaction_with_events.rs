...
fn main() -> IntegrateResult<()> {
    ...
    let event_funcs: Vec<EventFunc> = vec![
        // Event 1: dx/dt = 0 (X maximum or minimum), Box::new(move |_t: f64, state: ArrayView1<f64>| -> f64 {
            let x = state[0];
            let y = state[1];
            a + x * x * y - b * x - x // dx/dt
        }),
        // Event 2: dy/dt = 0 (Y maximum or minimum), Box::new(move |_t: f64, state: ArrayView1<f64>| -> f64 {
            let x = state[0];
            let y = state[1];
            b * x - x * x * y // dy/dt
        }),
        ...
        // Event 4: Time limit reached (t = 50), Box::new(move |t: f64_state: ArrayView1<f64>| -> f64 { t - 50.0 }),
    ];
