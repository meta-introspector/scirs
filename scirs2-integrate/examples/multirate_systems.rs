...
impl MultirateSystem<f64> for StiffOscillatorSystem {
    fn slow_rhs(&self_t: f64, y_slow: ArrayView1<f64>, y_fast: ArrayView1<f64>) -> Array1<f64> {
        let x_slow = y_slow[0];
        let v_slow = y_slow[1];
        let x_fast = y_fast[0];
    ...
    fn fast_rhs(&self_t: f64, y_slow: ArrayView1<f64>, y_fast: ArrayView1<f64>) -> Array1<f64> {
        let x_slow = y_slow[0];
        let x_fast = y_fast[0];
        let v_fast = y_fast[1];
