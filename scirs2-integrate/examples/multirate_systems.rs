...
use scirs2_integrate::ode::{
    MultirateSolver, MultirateSystem, MultirateOptions, MultirateMethod, ODEMethod
};
...
impl MultirateSystem<f64> for StiffOscillatorSystem {
    ...
    fn slow_dim(&self) -> usize { 2 }
    fn fast_dim(&self) -> usize { 2 }
}
...
impl MultirateSystem<f64> for TwoTimescaleVanDerPol {
    fn slow_rhs(&self, _t: f64, y_slow: ArrayView1<f64>, y_fast: ArrayView1<f64>) -> Array1<f64> {
        ...
        let dy_dt = self.epsilon * (self.mu_slow * (1.0 - x_slow * x_slow) * y_slow_var - x_slow) + 0.1 * x_fast;
...
impl MultirateSystem<f64> for ClimateWeatherSystem {
    ...
    fn fast_rhs(&self, _t: f64, y_slow: ArrayView1<f64>, y_fast: ArrayView1<f64>) -> Array1<f64> {
        ...
        let dpressure_dt = -0.5 * self.weather_timescale * pressure + 
                          0.2 * (temp_weather - temp_climate);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let options = MultirateOptions {
        method: MultirateMethod::ExplicitMRK { macro_steps: 4, micro_steps: 50 },
        macro_step: 0.01,
        rtol: 1e-6,
        atol: 1e-9,
        max_steps: 1000,
    ...
    println!("   Initial state: [slow: {:.3}, {:.3}] [fast: {:.3}, {:.3}]", 
             y0[0], y0[1], y0[2], y0[3]);
    let final_state = result.y.last().unwrap();
    println!("   Final state: [slow: {:.3}, {:.3}] [fast: {:.3}, {:.3}]", 
    ...
    println!("   Initial concentrations: [C]={:.3}, [A]={:.3}, [B]={:.3}", 
             y0_chem[0], y0_chem[1], y0_chem[2]);
    let final_chem = result_chem.y.last().unwrap();
    println!("   Final concentrations: [C]={:.3}, [A]={:.3}, [B]={:.3}", 
             final_chem[0], final_chem[1], final_chem[2]);
    println!("   Total mass: {:.3} (conservation check)", 
             final_chem[0] + final_chem[1] + final_chem[2]);
    println!("   Fast equilibrium ratio A/B: {:.3}", final_chem[1] / final_chem[2]);
    println!();
    ...
    let options_vdp = MultirateOptions {
        method: MultirateMethod::ExplicitMRK { macro_steps: 4, micro_steps: 25 },
        macro_step: 0.02,
        rtol: 1e-8,
        atol: 1e-11,
        max_steps: 2000,
    ...
    println!("   Initial: [slow: {:.3}, {:.3}] [fast: {:.3}, {:.3}]", 
             y0_vdp[0], y0_vdp[1], y0_vdp[2], y0_vdp[3]);
    let final_vdp = result_vdp.y.last().unwrap();
    println!("   Final: [slow: {:.3}, {:.3}] [fast: {:.3}, {:.3}]", 
             final_vdp[0], final_vdp[1], final_vdp[2], final_vdp[3]);
    println!("   Steps taken: {} (complex multi-scale dynamics)", result_vdp.n_steps);
    println!();
    ...
    let options_climate = MultirateOptions {
        method: MultirateMethod::Extrapolated { base_ratio: 10, levels: 2 },
    ...
    println!("   Initial: Climate temp={:.1}°C, CO2={:.0}ppm, Weather temp={:.1}°C, Pressure={:.0}hPa", 
             y0_climate[0], y0_climate[1], y0_climate[2], y0_climate[3]);
    let final_climate = result_climate.y.last().unwrap();
    println!("   Final: Climate temp={:.1}°C, CO2={:.0}ppm, Weather temp={:.1}°C, Pressure={:.0}hPa", 
             final_climate[0], final_climate[1], final_climate[2], final_climate[3]);
    println!("   Climate warming: {:.2}°C over 100 days", final_climate[0] - y0_climate[0]);
    println!("   CO2 increase: {:.0}ppm", final_climate[1] - y0_climate[1]);
    println!();
    ...
    let methods = vec![
        ("Explicit MRK", MultirateMethod::ExplicitMRK { macro_steps: 4, micro_steps: 25 }),
        ("Compound F-S", MultirateMethod::CompoundFastSlow { 
            fast_method: ODEMethod::RK4,
            slow_method: ODEMethod::RK4,
        }),
        ("Extrapolated", MultirateMethod::Extrapolated { base_ratio: 5, levels: 2 }),
    ];
    ...
    for (name, method) in methods {
        ...
        println!("   {}: {} steps, {:.2}ms", name, result_test.n_steps, elapsed.as_secs_f64() * 1000.0);
    }
...
mod tests {
    ...
    fn test_chemical_reaction_conservation() {
        ...
        let options = MultirateOptions {
            method: MultirateMethod::ExplicitMRK { macro_steps: 4, micro_steps: 20 },
            macro_step: 0.01,
            rtol: 1e-8,
            atol: 1e-11,
            max_steps: 200,
    ...
    fn test_stiff_oscillator_energy() {
        ...
        let final_state = result.y.last().unwrap();
        let final_energy = 0.5 * (final_state[1] * final_state[1] + final_state[3] * final_state[3]) + 
                          0.5 * (1.0 * final_state[0] * final_state[0] + 400.0 * final_state[2] * final_state[2]);
