...
fn metapopulation_sir(_t: f64, y: ArrayView1<f64>) -> Array1<f64> {
    let s1 = y[0]; let i1 = y[1]; let r1 = y[2]; // Population 1
    let s2 = y[3]; let i2 = y[4]; let r2 = y[5]; // Population 2
    
    let n1 = s1 + i1 + r1; // Total population 1
    let n2 = s2 + i2 + r2; // Total population 2
    
    let beta1 = 0.3; let beta2 = 0.25; // Transmission rates
    let gamma = 0.1;                   // Recovery rate
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Theoretical final: {:.1}", y0[0] * (0.5 * t_span[1]).exp());
    println!();
    ...
    println!("   Approached carrying capacity: {:.1}%", 
    ...
    let result = solve_ivp(lotka_volterra, [0.0, 15.0], array![10.0, 5.0], Some(options.clone()))?;
    
    println!("   Initial populations: Prey={:.1}, Predator={:.1}", 10.0, 5.0);
    println!("   Final populations: Prey={:.1}, Predator={:.1}", 
    ...
    let result = solve_ivp(competitive_lotka_volterra, [0.0, 20.0], array![30.0, 20.0], None)?;
    
    println!("   Initial populations: Species1={:.1}, Species2={:.1}", 30.0, 20.0);
    println!("   Final populations: Species1={:.1}, Species2={:.1}", 
             result.y.last().unwrap()[0], result.y.last().unwrap()[1]);
    ...
    println!("   Initial: S={:.0}, I={:.0}, R={:.0}", sir_initial[0], sir_initial[1], sir_initial[2]);
    let final_state = result.y.last().unwrap();
    println!("   Final: S={:.0}, I={:.0}, R={:.0}", final_state[0], final_state[1], final_state[2]);
    
    // Calculate basic reproduction number R₀ = β/γ
    let r0 = 0.3 / 0.1;
    println!("   Basic reproduction number R₀: {:.1}", r0);
    println!("   Attack rate: {:.1}%", (sir_initial[0] - final_state[0]) / sir_initial[0] * 100.0);
    println!();
    ...
    let result = solve_ivp(seir_model, [0.0, 60.0], seir_initial.clone(), Some(options.clone()))?;
    
    println!("   Initial: S={:.0}, E={:.0}, I={:.0}, R={:.0}", 
             seir_initial[0], seir_initial[1], seir_initial[2], seir_initial[3]);
    let final_state = result.y.last().unwrap();
    println!("   Final: S={:.0}, E={:.0}, I={:.0}, R={:.0}", 
             final_state[0], final_state[1], final_state[2], final_state[3]);
    println!("   Peak exposed: {:.0}", 
             result.y.iter().map(|state| state[1]).fold(0.0_f64, f64::max));
    println!();
    ...
    let result = solve_ivp(metapopulation_sir, [0.0, 40.0], meta_initial.clone(), Some(options))?;
    
    println!("   City 1 Initial: S={:.0}, I={:.0}, R={:.0}", 
             meta_initial[0], meta_initial[1], meta_initial[2]);
    println!("   City 2 Initial: S={:.0}, I={:.0}, R={:.0}", 
             meta_initial[3], meta_initial[4], meta_initial[5]);
    
    let final_state = result.y.last().unwrap();
    println!("   City 1 Final: S={:.0}, I={:.0}, R={:.0}", 
             final_state[0], final_state[1], final_state[2]);
    println!("   City 2 Final: S={:.0}, I={:.0}, R={:.0}", 
...
mod tests {
    ...
    fn test_lotka_volterra_conservation() {
        ...
        let final_state = result.y.last().unwrap();
        let final_h = c * b * final_state[0] - d * final_state[0].ln() + a * final_state[1] - b * final_state[1].ln();
    ...
    fn test_epidemic_basic_reproduction_number() {
        ...
        // Find peak infection
        let peak_infected = result.y.iter().map(|state| state[1]).fold(0.0_f64, f64::max);
