...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Theoretical final (at 5τ): {:.3} V", 5.0 * (1.0 - (-5.0_f64).exp()));
    println!("   Time constant τ = RC = {:.3} ms", 1000.0 * 100e-6 * 1000.0);
    println!();
    ...
    println!("   Final capacitor voltage: {:.3} V", result.y.last().unwrap()[0]);
    ...
    let result = solve_ivp(van_der_pol_oscillator, t_span_vdp, y0_vdp, Some(options_vdp))?;
    
    println!("   Initial state: x={:.3}, y={:.3}", 2.0, 0.0);
    println!("   Final state: x={:.3}, y={:.3}", 
    ...
    println!("   Initial state: V_c1={:.3}, V_c2={:.3}, I_L={:.3}", 0.1, 0.0, 0.0);
    println!("   Final state: V_c1={:.3}, V_c2={:.3}, I_L={:.3}", 
...
mod tests {
    ...
    fn test_rlc_energy_conservation() {
        ...
        let final_energy = 0.5 * final_state[0] * final_state[0] / c + 0.5 * l * final_state[1] * final_state[1];
