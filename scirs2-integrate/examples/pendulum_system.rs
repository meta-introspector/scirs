...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Initial angle: {:.3} rad ({:.1}°)", y0[0], y0[0] * 180.0 / PI);
    println!("   Final angle: {:.3} rad ({:.1}°)", result.y.last().unwrap()[0], 
             result.y.last().unwrap()[0] * 180.0 / PI);
    println!("   Period (theoretical): {:.3} s", 2.0 * PI * (1.0_f64 / 9.81).sqrt());
    ...
    println!("   Final angle: {:.3} rad (energy dissipated by damping)", result.y.last().unwrap()[0]);
    println!("   Steps taken: {}", result.t.len());
    println!();
    ...
    let result = solve_ivp(driven_pendulum, t_span_long, y0_driven.clone(), Some(options_driven))?;
    
    println!("   Initial angle: {:.3} rad", y0_driven[0]);
    println!("   Final angle: {:.3} rad", result.y.last().unwrap()[0]);
    println!("   Final velocity: {:.3} rad/s", result.y.last().unwrap()[1]);
    println!("   Steps taken: {}", result.t.len());
    println!();
    ...
    let result = solve_ivp(double_pendulum, t_span_short, y0_double.clone(), Some(options_double))?;
    
    println!("   Initial angles: θ₁={:.3} rad, θ₂={:.3} rad", y0_double[0], y0_double[2]);
    println!("   Final angles: θ₁={:.3} rad, θ₂={:.3} rad", 
             result.y.last().unwrap()[0], result.y.last().unwrap()[2]);
    println!("   Steps taken: {} (high precision needed for chaotic system)", result.t.len());
    println!();
...
mod tests {
    ...
    fn test_damped_pendulum_energy_loss() {
        ...
        let final_energy = 0.5 * final_state[1] * final_state[1] + g / l * (1.0 - final_state[0].cos());
    ...
    fn test_double_pendulum_conservation() {
        ...
        let final_state = result.y.last().unwrap();
        let final_ke = 0.5 * m1 * final_state[1] * final_state[1] + 0.5 * m2 * final_state[3] * final_state[3];
        let final_pe = m1 * g * l1 * (1.0 - final_state[0].cos()) + m2 * g * l2 * (1.0 - final_state[2].cos());
