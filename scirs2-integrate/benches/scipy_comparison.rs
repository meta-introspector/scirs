...
mod ode_problems {
    ...
    pub fn lotka_volterra(a: f64, b: f64, c: f64, d: f64) -> impl Fn(f64, ndarray::ArrayView1<f64>) -> Array1<f64> + 'static {
        move |t: f64, y: ndarray::ArrayView1<f64>| {
            let x = y[0]; // prey
...
mod quadrature_problems {
    ...
    pub fn nearly_singular(x: f64) -> f64 {
        if x > 1e-10 { 1.0 / x.sqrt() } else { 1.0 / (1e-10_f64).sqrt() }
    }
    ...
    pub fn high_dim_oscillatory(x: &[f64]) -> f64 {
        let sum: f64 = x.iter().enumerate().map(|(i, &xi)| (i as f64 + 1.0) * xi).sum();
        sum.sin()
    }
}
                ...
                |b, &(prob_name, method, t_span, ref y0)| {
                    b.iter(|| {
                        ...
                        let result = match prob_name {
                            "exponential_decay" => {
                                solve_ivp(ode_problems::exponential_decay, t_span, y0_array, Some(opts))
                            },
                            "harmonic_oscillator" => {
                                solve_ivp(ode_problems::harmonic_oscillator, t_span, y0_array, Some(opts))
                            },
                            "van_der_pol_mild" => {
                                solve_ivp(ode_problems::van_der_pol(1.0), t_span, y0_array, Some(opts))
                            },
                            "van_der_pol_stiff" => {
                                solve_ivp(ode_problems::van_der_pol(100.0), t_span, y0_array, Some(opts))
                            },
                            "lotka_volterra" => {
                                solve_ivp(ode_problems::lotka_volterra(1.5, 1.0, 3.0, 1.0), t_span, y0_array, Some(opts))
                            },
                            "three_body" => {
                                solve_ivp(ode_problems::three_body_problem, t_span, y0_array, Some(opts))
                            },
            ...
            |bench, &(prob_name, a, b)| {
                bench.iter(|| {
                    ...
                    let result = match prob_name {
                        ...
                        "oscillatory" => {
                            quad(quadrature_problems::oscillatory, a, b, Some(opts))
                        },
                        "gaussian" => {
                            quad(quadrature_problems::gaussian, a, b, Some(opts))
                        },
                        "nearly_singular" => {
...
fn bench_parallel_operations(c: &mut Criterion) {
    ...
    for &dim in &dimensions {
        for &n_samples in &sample_counts {
            ...
            // Sequential version
            group.bench_with_input(
                BenchmarkId::new("monte_carlo_sequential", format!("{}d_{}k", dim, n_samples / 1000)),
                &(dim, n_samples),
                |b, &(dim, n_samples)| {
                    b.iter(|| {
                        let opts = MonteCarloOptions {
            ...
            // Parallel version
            group.bench_with_input(
                BenchmarkId::new("monte_carlo_parallel", format!("{}d_{}k", dim, n_samples / 1000)),
                &(dim, n_samples),
                |b, &(dim, n_samples)| {
                    b.iter(|| {
