...
use crate::pde::finite_element::{Point, Triangle, TriangularMesh, ElementType};
...
pub enum Domain {
    ...
    Rectangle { x_min: f64, y_min: f64, x_max: f64, y_max: f64 },
    /// Circle: (center_x, center_y, radius)
    Circle { center_x: f64, center_y: f64, radius: f64 },
    /// Ellipse: (center_x, center_y, a, b, rotation_angle)
    Ellipse { center_x: f64, center_y: f64, a: f64, b: f64, rotation: f64 },
    /// L-shaped domain
    LShape { width: f64, height: f64, notch_width: f64, notch_height: f64 },
    /// Custom polygon defined by vertices
    Polygon { vertices: Vec<Point> },
    /// Annulus (ring): (center_x, center_y, inner_radius, outer_radius)
    Annulus { center_x: f64, center_y: f64, inner_radius: f64, outer_radius: f64 },
}
...
impl AutoMeshGenerator {
    ...
    pub fn generate_mesh(&self, domain: &Domain, boundary_spec: &BoundarySpecification) -> PDEResult<TriangularMesh> {
        match domain {
            Domain::Rectangle { x_min, y_min, x_max, y_max } => {
                self.generate_rectangle_mesh(*x_min, *y_min, *x_max, *y_max, boundary_spec)
            },
            Domain::Circle { center_x, center_y, radius } => {
                self.generate_circle_mesh(*center_x, *center_y, *radius, boundary_spec)
            },
            Domain::Ellipse { center_x, center_y, a, b, rotation } => {
                self.generate_ellipse_mesh(*center_x, *center_y, *a, *b, *rotation, boundary_spec)
            },
            Domain::LShape { width, height, notch_width, notch_height } => {
                self.generate_l_shape_mesh(*width, *height, *notch_width, *notch_height, boundary_spec)
            },
            Domain::Polygon { vertices } => {
                self.generate_polygon_mesh(vertices, boundary_spec)
            },
            Domain::Annulus { center_x, center_y, inner_radius, outer_radius } => {
                self.generate_annulus_mesh(*center_x, *center_y, *inner_radius, *outer_radius, boundary_spec)
            },
        }
    }

    /// Generate rectangular mesh using structured approach
    fn generate_rectangle_mesh(&self, x_min: f64, y_min: f64, x_max: f64, y_max: f64, 
                              boundary_spec: &BoundarySpecification) -> PDEResult<TriangularMesh> {
        ...
        self.apply_boundary_markers(&mut mesh, boundary_spec, &Domain::Rectangle { x_min, y_min, x_max, y_max })?;
    ...
    fn generate_circle_mesh(&self, center_x: f64, center_y: f64, radius: f64, 
                           boundary_spec: &BoundarySpecification) -> PDEResult<TriangularMesh> {
        ...
        mesh.points = points;
        mesh.elements = triangles;
        self.apply_boundary_markers(&mut mesh, boundary_spec, &Domain::Circle { center_x, center_y, radius })?;
        self.improve_mesh_quality(&mut mesh)?;
    ...
    fn generate_ellipse_mesh(&self, center_x: f64, center_y: f64, a: f64, b: f64, rotation: f64,
                            boundary_spec: &BoundarySpecification) -> PDEResult<TriangularMesh> {
        // Generate circle mesh with radius = max(a, b)
        let max_radius = a.max(b);
        let mut mesh = self.generate_circle_mesh(0.0, 0.0, max_radius, &BoundarySpecification::default())?;
        ...
        self.apply_boundary_markers(&mut mesh, boundary_spec, &Domain::Ellipse { center_x, center_y, a, b, rotation })?;
        self.improve_mesh_quality(&mut mesh)?;
    ...
    fn generate_l_shape_mesh(&self, width: f64, height: f64, notch_width: f64, notch_height: f64,
                            boundary_spec: &BoundarySpecification) -> PDEResult<TriangularMesh> {
        // Create L-shape as combination of two rectangles
        let mesh1 = self.generate_rectangle_mesh(0.0, 0.0, width, height - notch_height, &BoundarySpecification::default())?;
        let mesh2 = self.generate_rectangle_mesh(0.0, height - notch_height, width - notch_width, height, &BoundarySpecification::default())?;
        
        // Combine meshes
        let combined_mesh = self.combine_meshes(&[mesh1, mesh2])?;
        let mut mesh = combined_mesh;
        
        self.apply_boundary_markers(&mut mesh, boundary_spec, &Domain::LShape { width, height, notch_width, notch_height })?;
        self.improve_mesh_quality(&mut mesh)?;
    ...
    fn generate_polygon_mesh(&self, vertices: &[Point], boundary_spec: &BoundarySpecification) -> PDEResult<TriangularMesh> {
        if vertices.len() < 3 {
            return Err(PDEError::FiniteElementError("Polygon must have at least 3 vertices".to_string()));
        }
        ...
        let (min_x, max_x) = vertices.iter().map(|p| p.x).fold((f64::INFINITY, f64::NEG_INFINITY), |(min, max), x| (min.min(x), max.max(x)));
        let (min_y, max_y) = vertices.iter().map(|p| p.y).fold((f64::INFINITY, f64::NEG_INFINITY), |(min, max), y| (min.min(y), max.max(y)));
        ...
        mesh.points = points;
        mesh.elements = triangles;
        self.apply_boundary_markers(&mut mesh, boundary_spec, &Domain::Polygon { vertices: vertices.to_vec() })?;
        
        Ok(mesh)
    }

    /// Generate annulus (ring) mesh
    fn generate_annulus_mesh(&self, center_x: f64, center_y: f64, inner_radius: f64, outer_radius: f64,
                            boundary_spec: &BoundarySpecification) -> PDEResult<TriangularMesh> {
        if inner_radius >= outer_radius {
            return Err(PDEError::FiniteElementError("Inner radius must be less than outer radius".to_string()));
        }
        
        // Generate structured radial mesh
        let n_theta = ((2.0 * PI * outer_radius / self.params.element_size).ceil() as usize).max(8);
        let n_r = (((outer_radius - inner_radius) / self.params.element_size).ceil() as usize).max(2);
        ...
        mesh.points = points;
        mesh.elements = triangles;
        self.apply_boundary_markers(&mut mesh, boundary_spec, &Domain::Annulus { center_x, center_y, inner_radius, outer_radius })?;
        self.improve_mesh_quality(&mut mesh)?;
    ...
    fn apply_boundary_markers(&self, mesh: &mut TriangularMesh, boundary_spec: &BoundarySpecification, _domain: &Domain) -> PDEResult<()> {
    ...
    fn find_boundary_edges(&self, mesh: &TriangularMesh) -> Vec<(usize, usize)> {
        ...
        for element in &mesh.elements {
            ...
            let edges = [(p1.min(p2), p1.max(p2)), (p2.min(p3), p2.max(p3)), (p3.min(p1), p3.max(p1))];
    ...
    fn laplacian_smoothing(&self, mesh: &mut TriangularMesh) -> PDEResult<()> {
        ...
        let boundary_edges = self.find_boundary_edges(mesh);
        let boundary_points: HashSet<usize> = boundary_edges.iter()
    ...
    fn quality_refinement(&self, mesh: &mut TriangularMesh) -> PDEResult<()> {
        ...
        for (i, element) in mesh.elements.iter().enumerate() {
            ...
            if quality.min_angle < self.params.min_angle || quality.max_angle > self.params.max_angle {
    ...
    fn element_quality(&self, mesh: &TriangularMesh, element: &Triangle) -> ElementQuality {
        ...
        let angle_a = ((bc.powi(2) + ca.powi(2) - ab.powi(2)) / (2.0 * bc * ca)).acos() * 180.0 / PI;
        let angle_b = ((ca.powi(2) + ab.powi(2) - bc.powi(2)) / (2.0 * ca * ab)).acos() * 180.0 / PI;
    ...
    fn combine_meshes(&self, meshes: &[TriangularMesh]) -> PDEResult<TriangularMesh> {
        if meshes.is_empty() {
            return Err(PDEError::FiniteElementError("Cannot combine empty mesh list".to_string()));
        }
...
mod tests {
    ...
    fn test_rectangle_mesh_generation() {
        ...
        let domain = Domain::Rectangle { x_min: 0.0, y_min: 0.0, x_max: 1.0, y_max: 1.0 };
    ...
    fn test_circle_mesh_generation() {
        ...
        let domain = Domain::Circle { center_x: 0.0, center_y: 0.0, radius: 1.0 };
        let boundary_spec = BoundarySpecification::default();
