...
pub enum MultirateMethod {
    ...
    ExplicitMRK { macro_steps: usize, micro_steps: usize },
    /// Implicit-explicit (IMEX) multirate method
    IMEX { macro_steps: usize, micro_steps: usize },
    /// Compound fast-slow method
    CompoundFastSlow { fast_method: ODEMethod, slow_method: ODEMethod },
...
impl<F: IntegrateFloat> Default for MultirateOptions<F> {
    fn default() -> Self {
        Self {
            method: MultirateMethod::ExplicitMRK { macro_steps: 4, micro_steps: 10 },
...
impl<F: IntegrateFloat> MultirateSolver<F> {
        ...
        S: MultirateSystem<F>,
    {
        ...
        if y0.len() != slow_dim + fast_dim {
            return Err(IntegrateError::ValueError(
                format!("Initial condition dimension {} does not match system dimension {}", 
                       y0.len(), slow_dim + fast_dim)
            ));
        }
        ...
        while t < tf && step_count < self.options.max_steps {
            ...
            let (new_y_slow, new_y_fast) = match &self.options.method {
                MultirateMethod::ExplicitMRK { macro_steps, micro_steps } => {
                    self.explicit_mrk_step(&system, t, dt, y_slow.view(), y_fast.view(), 
                                         *macro_steps, *micro_steps)?
                },
                MultirateMethod::IMEX { macro_steps, micro_steps } => {
                    self.imex_step(&system, t, dt, y_slow.view(), y_fast.view(), 
                                 *macro_steps, *micro_steps)?
                },
                MultirateMethod::CompoundFastSlow { fast_method: _, slow_method: _ } => {
                    self.compound_fast_slow_step(&system, t, dt, y_slow.view(), y_fast.view())?
                },
                MultirateMethod::Extrapolated { base_ratio, levels } => {
                    self.extrapolated_step(&system, t, dt, y_slow.view(), y_fast.view(), 
                                         *base_ratio, *levels)?
                },
            };
        ...
        S: MultirateSystem<F>,
    {
        ...
        let k3_slow = system.slow_rhs(t + dt / F::from(2).unwrap(), 
                                    (y_slow.to_owned() + k1_slow.clone() * dt / F::from(2).unwrap()).view(), 
                                    y_fast_current.view());
        let k4_slow = system.slow_rhs(t + dt, 
        ...
        S: MultirateSystem<F>,
    {
        ...
        for _ in 0..50 { // Allow fast system to equilibrate
        ...
        S: MultirateSystem<F>,
    {
        ...
        for level in 0..levels {
            ...
            let (y_slow_approx, y_fast_approx) = self.explicit_mrk_step(
                system, t, dt, y_slow, y_fast, 4, micro_steps
            )?;
...
mod tests {
    ...
    fn test_extrapolated_multirate_method() {
        ...
        let options = MultirateOptions {
            method: MultirateMethod::Extrapolated { base_ratio: 5, levels: 2 },
            macro_step: 0.02,
            rtol: 1e-8,
            atol: 1e-11,
            max_steps: 500,
