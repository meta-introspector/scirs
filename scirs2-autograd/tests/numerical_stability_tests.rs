...
fn test_arithmetic_stability() {
    ...
    ag::run(|ctx: &mut ag::Context<f32>| {
        ...
        let small_denom = T::convert_to_tensor(
            Array::<f32>::fromshape_vec(IxDyn(&[3]), vec![1e-20, 1.0, 1e20]).unwrap(),
            ctx,
        );
        let numerator = T::convert_to_tensor(
            Array::<f32>::fromshape_vec(IxDyn(&[3]), vec![1.0, 1.0, 1.0]).unwrap(),
            ctx,
        );
...
fn test_matrix_operation_stability() {
    ...
    ag::run(|ctx: &mut ag::Context<f32>| {
        ...
        let ill_conditioned = T::convert_to_tensor(
            Array::<f32, ndarray::Ix2>::fromshape_vec(
                (3, 3),
        ...
        let vector = T::convert_to_tensor(
            Array::<f32, ndarray::Ix2>::fromshape_vec((3, 1), vec![1.0, 2.0, 3.0]).unwrap(),
            ctx,
        );
        ...
        let well_conditioned = T::convert_to_tensor(
            Array::<f32, ndarray::Ix2>::fromshape_vec((2, 2), vec![2.0, 1.0, 1.0, 2.0]).unwrap(),
            ctx,
        );
...
fn test_gradient_numerical_stability() {
    ...
    ag::run(|ctx: &mut ag::Context<f32>| {
        ...
        let x = T::convert_to_tensor(
            Array::<f32>::fromshape_vec(IxDyn(&[3]), vec![1e-10, 1.0, 1e10]).unwrap(),
            ctx,
        );
        ...
        let matrix = T::convert_to_tensor(
            Array::<f32, ndarray::Ix2>::fromshape_vec((2, 2), vec![1.0, 2.0, 3.0, 4.0]).unwrap(),
            ctx,
        );
...
fn test_optimization_numerical_stability() {
    ...
    ag::run(|ctx: &mut ag::Context<f32>| {
        ...
        let x = T::convert_to_tensor(
            Array::<f32>::fromshape_vec(IxDyn(&[10]), (0..10).map(|i| i as f32 * 0.1).collect())
                .unwrap(),
            ctx,
        );
        ...
        let const1 = T::convert_to_tensor(
            Array::<f32>::fromshape_vec(IxDyn(&[1]), vec![0.0]).unwrap(),
            ctx,
        );
        let const2 = T::convert_to_tensor(
            Array::<f32>::fromshape_vec(IxDyn(&[1]), vec![1.0]).unwrap(),
            ctx,
        );

        // Expression: x + 0 * (large_constant) + 1 * x
        let large_constant = T::convert_to_tensor(
            Array::<f32>::fromshape_vec(IxDyn(&[10]), vec![1e10; 10]).unwrap(),
            ctx,
        );
...
fn test_memory_optimization_stability() {
    ...
    ag::run(|ctx: &mut ag::Context<f32>| {
        ...
        let addend = T::convert_to_tensor(
            Array::<f32>::fromshape_vec(IxDyn(&[1000]), vec![0.5; 1000]).unwrap(),
            ctx,
        );
