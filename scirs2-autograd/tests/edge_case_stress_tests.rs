...
use scirs2_autograd::optimization::{
    GraphOptimizer, OptimizationLevel, MemoryOptimizer, MemoryOptimizationConfig,
};
use scirs2_autograd::visualization::{
    GraphVisualizer, VisualizationConfig, OutputFormat,
};
use scirs2_autograd::parallel::{
    ThreadPoolConfig, ParallelConfig, init_thread_pool_with_config,
    parallel_ops::{ParallelElementWise, ParallelReduction, ParallelMatrix},
};
use std::f32;
...
fn test_thread_pool_extreme_load() {
    ...
    ag::run(|ctx: &mut ag::Context<f32>| {
        ...
        for i in 0..100 {
            let data = T::convert_to_tensor(
                Array::from_shape_vec(IxDyn(&[1000]), (0..1000).map(|x| (x + i) as f32).collect()).unwrap(),
                ctx,
            );
    ...
    let max_config = ThreadPoolConfig {
        num_threads: std::thread::available_parallelism().map(|n| n.get()).unwrap_or(4) * 2,
        work_stealing: true,
...
fn test_custom_activations_extreme_inputs() {
    ...
    ag::run(|ctx: &mut ag::Context<f32>| {
        ...
        let extreme_positive = T::convert_to_tensor(
            Array::from_shape_vec(IxDyn(&[5]), vec![100.0, 1000.0, 10000.0, f32::MAX / 2.0, 50.0]).unwrap(),
            ctx,
        );
        
        // Test with very large negative values
        let extreme_negative = T::convert_to_tensor(
            Array::from_shape_vec(IxDyn(&[5]), vec![-100.0, -1000.0, -10000.0, f32::MIN / 2.0, -50.0]).unwrap(),
            ctx,
        );
        
        // Test with edge values
        let edge_values = T::convert_to_tensor(
            Array::from_shape_vec(IxDyn(&[6]), vec![0.0, f32::EPSILON, -f32::EPSILON, f32::INFINITY, f32::NEG_INFINITY, f32::NAN]).unwrap(),
            ctx,
        );
        ...
        for activation in activations {
            ...
            assert!(finite_count >= 3, "Activation {} failed on extreme positive values", activation);
            ...
            assert!(finite_count >= 3, "Activation {} failed on extreme negative values", activation);
            ...
            assert!(output_edge.iter().all(|&x| x.is_finite()), 
                   "Activation {} failed on edge values", activation);
            
            println!("✅ Activation '{}' handles extreme inputs correctly", activation);
        }
    });
}
...
fn test_simd_operations_edge_cases() {
    ...
    ag::run(|ctx: &mut ag::Context<f32>| {
        ...
        let tiny_values = T::convert_to_tensor(
            Array::from_shape_vec(IxDyn(&[8]), vec![
                f32::EPSILON, f32::EPSILON * 2.0, f32::MIN_POSITIVE,
                1e-30, 1e-35, 1e-38, 0.0, -0.0
            ]).unwrap(),
            ctx,
        );
        ...
        let huge_values = T::convert_to_tensor(
            Array::from_shape_vec(IxDyn(&[8]), vec![
                f32::MAX / 2.0, f32::MAX / 4.0, 1e30, 1e35,
                -f32::MAX / 2.0, -f32::MAX / 4.0, -1e30, -1e35
            ]).unwrap(),
            ctx,
        );
        ...
        let near_overflow_a = T::convert_to_tensor(
            Array::from_shape_vec(IxDyn(&[4]), vec![1e20, -1e20, f32::MAX.sqrt(), -f32::MAX.sqrt()]).unwrap(),
            ctx,
        );
...
fn test_memory_optimization_stress() {
    ...
    ag::run(|ctx: &mut ag::Context<f32>| {
        ...
        assert!(final_result.iter().all(|&x| x.is_finite() || x.is_infinite()));
    ...
    for (i, config) in stress_configs.iter().enumerate() {
        ...
        println!("✅ Memory optimization config {} created successfully", i + 1);
    }
}
...
fn test_parallel_operations_numerical_stability() {
    ...
    let cancellation_test = Array::from_shape_vec(
        (1000,).into_dyn(),
        (0..1000).map(|i| if i % 2 == 0 { 1e10 } else { -1e10 }).collect::<Vec<f32>>()
    ).unwrap();
    ...
    assert!(sum_result.abs() < 1e6, "Parallel sum failed catastrophic cancellation test");
    
    // Test parallel operations with denormalized numbers
    let denormal_values = Array::from_shape_vec(
        (1000,).into_dyn(),
        (0..1000).map(|i| f32::MIN_POSITIVE * (i as f32 + 1.0) * 1e-10).collect::<Vec<f32>>()
    ).unwrap();
    ...
    let ill_conditioned_a = Array::from_shape_vec(
        ...
        (0..10000).map(|i| {
            let row = i / 100;
            let col = i % 100;
            if row == col { 1.0 } else if row == col + 1 { 1e-10 } else { 0.0 }
        }).collect::<Vec<f32>>()
    ).unwrap();
    
    let identity = Array::from_shape_vec(
        (100, 100).into_dyn(),
        (0..10000).map(|i| if i / 100 == i % 100 { 1.0 } else { 0.0 }).collect::<Vec<f32>>()
    ).unwrap();
    ...
    let max_error = matmul_result.iter()
        .zip(ill_conditioned_a.iter())
        .map(|(&result, &expected)| (result - expected).abs())
        .fold(0.0f32, |acc, error| acc.max(error));
    
    assert!(max_error < 1e-6, "Parallel matrix multiplication failed numerical stability test");
    ...
    assert!(finite_count >= 990, "Parallel element-wise add failed with extreme ranges");
...
fn test_cross_feature_integration_stress() {
    ...
    ag::run(|ctx: &mut ag::Context<f32>| {
        ...
        assert!(finite_ratio > 0.8, "Too many non-finite gradients: {:.2}%", 
        ...
        println!("   - Final output range: [{:.6}, {:.6}]", 
...
fn test_numerical_precision_stability() {
    ...
    ag::run(|ctx: &mut ag::Context<f32>| {
        ...
        assert!(precision_error < 1e-5, 
               "Precision degraded too much: error = {:.2e}", precision_error);
        ...
        let balanced_values = Array::from_shape_vec(
            (1000,).into_dyn(),
            (0..1000).map(|i| if i < 500 { 1.0 / 500.0 } else { -1.0 / 500.0 }).collect()
        ).unwrap();
        
        let parallel_sum = ParallelReduction::sum(&balanced_values, &config).unwrap();
        assert!(parallel_sum.abs() < 1e-6, "Parallel sum lost precision: {:.2e}", parallel_sum);
        
        // Test numerical stability of custom activations near critical points
        let critical_points = T::convert_to_tensor(
            Array::from_shape_vec(IxDyn(&[7]), vec![
                0.0, f32::EPSILON, -f32::EPSILON, 
                1.0, -1.0, 0.5, -0.5
            ]).unwrap(),
            ctx,
        );
        ...
        for activation in ["swish", "gelu", "mish"] {
            ...
            assert!(output.iter().all(|&x| x.is_finite()), 
                   "Activation {} produced non-finite values at critical points", activation);
            
            // Check monotonicity where expected (rough test)
            if activation == "gelu" || activation == "swish" {
                assert!(output[6] < output[5], // -0.5 < 0.5 
                       "Activation {} lost monotonicity", activation);
            }
        }
