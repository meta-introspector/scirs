...
#[allow(dead_code)]
fn compute_pseudo_inverse<F: Float>(_matrix: &ndarray::ArrayView2<F>) -> Result<Array2<F>, OpError> {
...
#[allow(dead_code)]
pub fn determinant<'g, F: Float + ndarray::ScalarOperand>(_matrix: &Tensor<'g, F>) -> Tensor<'g, F> {
    let g = _matrix.graph();
