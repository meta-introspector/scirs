...
static RESHAPE_CACHE: LazyLock<Mutex<ReshapeCache>> = LazyLock::new(|| Mutex::new(ReshapeCache::new()));
...
impl ReshapeCache {
    ...
    fn get(&mut self, from_shape: &[usize], to_shape: &[usize]) -> Option<bool> {
        self.cache.get(&(from_shape.to_vec(), to_shape.to_vec())).copied()
    }
    ...
    fn insert(&mut self, from_shape: &[usize], to_shape: &[usize], is_contiguous: bool) {
        ...
        self.cache.insert((from_shape.to_vec(), to_shape.to_vec()), is_contiguous);
    }
        ...
        let result = if self.allow_zero_copy {
            ...
            match input.view().into_shape_with_order(IxDyn(&self.new_shape)) {
                ...
                Err(_) => {
                    ...
                    flattened.into_shape_with_order(IxDyn(&self.new_shape)).unwrap()
                }
            }
        } else {
...
impl<F: Float> Op<F> for EfficientSliceOp {
    ...
    fn compute(&self, ctx: &mut ComputeContext<F>) -> Result<(), OpError> {
        ...
        let slice_info = unsafe { SliceInfo::<Vec<SliceInfoElem>, IxDyn, IxDyn>::new(slice_elements).unwrap() };
...
pub fn efficient_concat<'g, F: Float>(
    tensors: &[&Tensor<'g, F>],
    axis: usize
) -> Tensor<'g, F> {
    if tensors.is_empty() {
...
mod tests {
    ...
    fn test_efficient_reshape_op_creation() {
        ...
        assert_eq!(op.name(), "EfficientReshape");
    ...
    fn test_efficient_slice_op_creation() {
        ...
        let op = EfficientSliceOp { slices: slices.clone() };
        assert_eq!(op.name(), "EfficientSlice");
    ...
    fn test_efficient_concat_op_creation() {
        ...
        assert_eq!(op.name(), "EfficientConcat");
        assert_eq!(op.axis, 1);
