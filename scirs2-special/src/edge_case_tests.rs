...
use crate::{
    bessel, erf, gamma, SpecialResult,
};
use ndarray::Array1;
use std::f64;
    ...
    for (input, expected, description) in test_cases {
        ...
        results.push(EdgeCaseResult {
            ...
            error_message: if !passed {
                Some(format!("Error: {:.2e}, tolerance: {:.2e}", error, config.tolerance))
            } else {
                None
            },
        });
...
fn test_erf_extreme_values(_config: &EdgeCaseConfig) -> Vec<EdgeCaseResult> {
    ...
    results.push(EdgeCaseResult {
        ...
        passed: (output + 1.0).abs() < 1e-10,
        error_message: if (output + 1.0).abs() >= 1e-10 {
            Some(format!("erf({}) = {}, expected ≈ -1", large_neg_input, output))
        } else {
            None
        },
    });
...
pub fn run_comprehensive_edge_case_tests(config: &EdgeCaseConfig) -> SpecialResult<()> {
    ...
    println!("Gamma tests: {}/{} passed", gamma_passed, gamma_results.len());
    ...
    println!("Bessel tests: {}/{} passed", bessel_passed, bessel_results.len());
    ...
    println!("Error function tests: {}/{} passed", erf_passed, erf_results.len());
    ...
    println!("Success rate: {:.1}%", 100.0 * total_passed as f64 / total_tests as f64);
...
pub fn test_numerical_precision() -> SpecialResult<()> {
    ...
    for (input, expected) in test_cases {
        ...
        println!("γ({:.1}) = {:.10} (expected: {:.10}, rel_error: {:.2e})", 
...
pub fn test_array_edge_cases() -> SpecialResult<()> {
    ...
    let edge_values = vec![
        0.0, 1e-15, 1e-10, 0.1, 0.5, 1.0, 1.5, 2.0, 10.0, 100.0,
        f64::MIN_POSITIVE, f64::MAX / 1e10,
    ];
    ...
    for (input, output) in edge_values.iter().zip(gamma_results.iter()) {
        println!("  γ({:.2e}) = {:.6e} (finite: {})", 
                 input, output, output.is_finite());
    }
