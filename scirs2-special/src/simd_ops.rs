    ...
    if len > PARALLEL_THRESHOLD {
        ...
        if let (Some(input_slice), Some(output_slice)) = (input.as_slice(), output.as_slice_mut()) {
            (0..chunks).into_par_iter().for_each(|chunk_idx| {
                ...
                let chunk_input = &input_slice[start..end];
                let chunk_results = simd_gamma_approx_f32(chunk_input);
                
                for (i, &result) in chunk_results.iter().enumerate() {
                    output_slice[start + i] = result;
                }
            });
            
            // Handle remaining elements sequentially
            for i in (chunks * SIMD_CHUNK_SIZE)..len {
...
pub fn benchmark_parallel_performance(size: usize) -> SpecialResult<()> {
    ...
    // Create test data
    let data_f64: Array1<f64> = Array1::from_vec(
        (0..size).map(|i| (i as f64) * 0.001 + 1.0).collect()
    );
    ...
    let bessel_data: Array1<f64> = Array1::from_vec(
...
pub fn benchmark_simd_performance(size: usize) -> SpecialResult<()> {
    ...
    // Create test data
    let data_f32: Array1<f32> = Array1::from_vec(
        (0..size).map(|i| (i as f32) * 0.01 + 1.0).collect()
    );
    let data_f64: Array1<f64> = Array1::from_vec(
        (0..size).map(|i| (i as f64) * 0.01 + 1.0).collect()
    );
