...
use scirs2_special::{
    ...
    i0e, i1e, ive, j0e, j1e, jne, jve, k0e, k1e, kve, y0e, y1e, yne,
    // Dawson's integral
    dawsn,
    // Polygamma function
    polygamma,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Odd function test: dawsn({}) = {:.6}, dawsn({}) = {:.6}", 
    ...
    for n in 0..=4 {
        for &x_val in &[1.0, 2.0, 5.0] {
            println!("    polygamma({}, {}) = {:.8}", n, x_val, polygamma(n, x_val));
        }
    }
    ...
    println!("    polygamma(0, 1) = {:.10} (should be ≈ {:.10})", psi0_1, -euler_gamma);
    
    // polygamma(1, 1) = π²/6 (Basel problem)
    let pi_squared_over_6 = std::f64::consts::PI.powi(2) / 6.0;
    let psi1_1 = polygamma(1, 1.0);
    println!("    polygamma(1, 1) = {:.10} (should be ≈ {:.10})", psi1_1, pi_squared_over_6);
