    ...
    for &z in &large_z_values {
        match wright_bessel(rho, beta, z) {
            Ok(w_val) => {
                ...
                let asymptotic = z.powf((1.0 - beta) / rho) * (rho * z.powf(1.0 / rho)).exp() / (rho * (2.0 * PI).sqrt());
...
fn demonstrate_applications() {
    ...
    if rho_stable > -1.0 {
        match wright_bessel(rho_stable, beta_stable, -x_stable / time_prob.powf(stability_param)) {
            Ok(w_val) => {
...
fn demonstrate_computational_aspects() {
    ...
    println!("The defining series W_ρ,β(z) = Σ z^k/[k!Γ(ρk+β)] has different convergence properties:");
