...
fn physics_applications() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &theta in &theta_vals {
        ...
        println!("θ = {:5.2} rad ({:3.0}°): E = {:.6}", 
               theta, theta * 180.0 / PI, field_strength);
    }
...
fn engineering_applications() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (n, &zero) in j0_zeros.iter().enumerate() {
        ...
        println!("(0,{})      {:8.1}        {:8.4}", n + 1, frequency, wavelength);
    }
    
    for (n, &zero) in j1_zeros.iter().enumerate() {
        let frequency = zero * wave_speed / (2.0 * PI * radius);
        let wavelength = wave_speed / frequency;
        println!("(1,{})      {:8.1}        {:8.4}", n + 1, frequency, wavelength);
    }
    ...
    for &zeta in &damping_ratios {
        ...
        println!("{:.1}   {:7.1}%    {:8.3}s      {:6.3}s", 
               zeta, overshoot * 100.0, settling_time, peak_time);
    }
    ...
    for angle_deg in [0.0, 15.0, 30.0, 45.0, 60.0, 75.0, 90.0] {
        ...
        println!("{:6.0}    {:8.2}     {:7.4}", angle_deg, pattern_db, pattern);
    }
    ...
    for x in [0.0, 1.0, 2.5, 5.0, 7.5, 9.0, 10.0] {
        let (deflection, slope) = beam_deflection_distributed(
            x, beam_length, distributed_load, elastic_modulus, moment_inertia
        );
        println!("{:8.1}    {:10.2}      {:8.2}", 
               x, deflection * 1000.0, slope * 1000.0);
    }
...
fn signal_processing_applications() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in [5, 10, 15, 20, 25, 30, 35, 40, 45] {
        ...
        let (magnitude_db, phase_deg, group_delay) = bessel_filter_response(
            freq, cutoff_freq, filter_order
        );
        println!("{:7.3}    {:9.2}     {:8.1}     {:8.2}", 
    ...
    for &snr_db in &snr_db_values {
        ...
        println!("{:6.1}  {:9.2e}  {:9.2e}  {:9.2e}  {:9.2e}", 
    ...
    for &freq in &test_frequencies {
        ...
        println!("{:7.0}      {:11.1}        {:10.4}", freq, threshold, spread);
    }
    ...
    for &t in &time_points {
        ...
        println!("{:6.1}    {:8.4}    {:8.4}     {:8.4}", 
               t, morlet.re, morlet.im, morlet.norm());
    }
...
fn computer_graphics_applications() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..8 {
        ...
        println!("{:4.1}   {:4.1}   {:9.4}    {:9.4}", x, y, noise_val, turbulence);
    }
    ...
    for i in 0..8 {
        ...
        println!("{:7}  {:6.3}  {:6.3}  {:6.3}  {:6.3}  {:6.3}  {:6.3}", 
    ...
    let test_points = vec![
        ...
        (sphere_radius/2.0_f64.sqrt(), sphere_radius/2.0_f64.sqrt(), 0.0),
...
fn financial_applications() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("European option prices (S={}, r={:.1}%, σ={:.1}%, T={:.2}y):", 
           spot_price, risk_free_rate * 100.0, volatility * 100.0, time_to_expiry);
    ...
    for &strike in &strike_prices {
        let (call_price, put_price, call_delta, put_delta) = black_scholes_option(
            spot_price, strike, risk_free_rate, volatility, time_to_expiry
        );
        
        println!("{:6.0}    {:8.2}     {:8.2}     {:8.4}     {:8.4}", 
    ...
    println!("Merton model default probabilities (Debt = {}):", debt_value);
    ...
    for &asset_val in &asset_values {
        let (default_prob, distance_to_default) = merton_default_probability(
            asset_val, debt_value, risk_free_rate, asset_volatility, time_horizon
        );
        
        println!("{:10.0}      {:8.4}        {:11.2}", 
    ...
    for &maturity in &maturities {
        let (bond_price, yield_rate, duration) = vasicek_bond_pricing(
            ...
            rate_volatility, maturity
        );
        
        println!("{:7.2}     {:8.4}    {:6.2}    {:7.2}", 
               maturity, bond_price, yield_rate * 100.0, duration);
    }
    ...
    for i in 0..8 {
        ...
        let (portfolio_risk, sharpe_ratio) = efficient_frontier_point(
            target_return, &expected_returns, &volatilities, &correlations, risk_free_rate
        );
        
        println!("{:11.1}%      {:10.1}%      {:10.2}", 
               target_return * 100.0, portfolio_risk * 100.0, sharpe_ratio);
    }
    ...
    println!("Asian option prices (arithmetic average, {} simulations):", num_simulations);
    ...
    for &strike in &asian_strikes {
        let (call_price, put_price, std_error) = asian_option_monte_carlo(
            spot_price, strike, risk_free_rate, volatility, 
            time_to_expiry, num_time_steps, num_simulations
        );
        
        println!("{:6.0}    {:8.2}     {:8.2}     {:7.4}", 
...
fn psychoacoustic_masking(freq: f64, masker_freq: f64, masker_level: f64) -> (f64, f64) {
    ...
    let threshold = masker_level + 10.0 * spread.log10() - 15.0;
...
fn b_spline_basis(i: usize, p: usize, u: f64, knots: &[f64]) -> f64 {
    ...
    if p == 0 {
        ...
        if i + 1 < knots.len() && i + p + 1 < knots.len() && (knots[i + p + 1] - knots[i + 1]).abs() > 1e-10 {
            right = (knots[i + p + 1] - u) / (knots[i + p + 1] - knots[i + 1]) * b_spline_basis(i + 1, p - 1, u, knots);
        }
...
fn vasicek_bond_pricing(r: f64, a: f64, b: f64, sigma: f64, t: f64) -> (f64, f64, f64) {
    ...
    let at = (bt - t) * (a * a * b - 0.5 * sigma * sigma) / (a * a) - 
             0.25 * sigma * sigma * bt * bt / a;
...
fn efficient_frontier_point(target_return: f64, returns: &[f64], vols: &[f64], 
                          corr: &[Vec<f64>], rf: f64) -> (f64, f64) {
        ...
        for j in 0..=(100-i) {
            ...
            if w3 >= 0.0 {
                ...
                if (port_return - target_return).abs() < 0.001 {
                    ...
                    for i in 0..n {
                        for j in 0..n {
                            risk_squared += weights[i] * weights[j] * vols[i] * vols[j] * corr[i][j];
                        }
                    }
...
fn asian_option_monte_carlo(s: f64, k: f64, r: f64, sigma: f64, t: f64, 
                          steps: usize, simulations: usize) -> (f64, f64, f64) {
