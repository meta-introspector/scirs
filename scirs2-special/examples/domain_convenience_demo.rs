...
fn main() {
    ...
    println!("Particle in box (n=2): max ψ = {:.3}", psi.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap());
    ...
    println!("Hydrogen 2p radial: max = {:.3e}", psi_r.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap());
    ...
    println!("Array factor max: {:.3}", af.iter().map(|c| c.norm()).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap());
    ...
    println!("Shannon entropy (uniform): {:.3} bits", entropy);
    ...
    println!("Black-Scholes call price: ${:.2}", call_price);
    ...
    println!("Jukes-Cantor distance (p=0.2): {:.3}", jc_dist);
    ...
    println!("Michaelis-Menten Vmax/2 at: S = {:.1}", substrate[velocity.iter().position(|&v| v >= 50.0).unwrap()]);
    ...
    println!("Acoustic impedance: {:.0} kg/(m²·s)", z);
    
    // Richter magnitude
    let magnitude = geophysics::richter_magnitude(1000.0, 100.0).unwrap();
    println!("Richter magnitude: {:.1}", magnitude);
