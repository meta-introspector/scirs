...
use scirs2_special::{pro_cv, pro_cv_seq, obl_cv, obl_cv_seq, pro_ang1, pro_rad1, obl_ang1, obl_rad1};
...
fn demonstrate_asymptotic_behavior() {
    ...
    for &c in &large_c_values {
        let exact = pro_cv(m, n, c).unwrap();
        let asymptotic = -c*c/4.0 + (2.0*n as f64 + 1.0)*c + n as f64*(n as f64 + 1.0) - (m as f64).powi(2)/2.0;
