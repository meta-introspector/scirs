...
use scirs2_special::edge_case_tests::{
    run_comprehensive_edge_case_tests, test_numerical_precision, test_array_edge_cases,
    EdgeCaseConfig,
};
...
fn demonstrate_custom_edge_cases() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for value in near_poles {
        ...
        println!("  γ({:.6}) = {:.6e} (finite: {})", 
                 value, result, result.is_finite());
    }
...
fn analyze_edge_case_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Average time per normal value: {:.2?}", normal_time / 1000);
...
fn test_robustness() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for value in special_values {
        ...
        println!("    γ: {:.2e} (finite: {})", gamma_result, gamma_result.is_finite());
        println!("    J0: {:.2e} (finite: {})", j0_result, j0_result.is_finite());
        println!("    erf: {:.2e} (finite: {})", erf_result, erf_result.is_finite());
    }
...
fn test_gamma_monotonicity() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..test_points.len() {
        ...
        if curr <= prev {
            monotonic = false;
            println!("    ❌ Non-monotonic at {:.2}: {:.6e} -> {:.6e}", 
...
fn test_erf_symmetry() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if symmetric {
        println!("    ✅ Error function symmetry preserved (max error: {:.2e})", max_error);
    }
