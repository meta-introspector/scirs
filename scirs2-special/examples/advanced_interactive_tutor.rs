...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Setup user profile
    let mut profile = setup_user_profile()?;
...
fn setup_user_profile() -> Result<UserProfile, Box<dyn std::error::Error>> {
    ...
    let theoretical = get_yes_no_input("Are you interested in mathematical proofs and theory? (y/n): ")?;
    ...
    println!("Starting difficulty: {:?}", profile.learning_preferences.difficulty_preference);
...
fn run_quick_assessment() -> Result<u32, Box<dyn std::error::Error>> {
    ...
    if answer.to_lowercase() == "a" {
        ...
        println!("❌ Incorrect. Bessel functions satisfy a linear homogeneous differential equation.");
    }
    ...
    if score >= 2 {
        ...
        if answer.to_lowercase() == "c" {
            ...
            println!("❌ Wright functions generalize both exponential and Mittag-Leffler functions.");
        }
        ...
        if answer.to_lowercase() == "b" {
            ...
            println!("❌ Spherical harmonics are eigenfunctions of the angular momentum operator L².");
        }
    }
    ...
    match profile.learning_preferences.difficulty_preference {
        DifficultyLevel::Beginner => {
            ...
            if error < 0.01 {
                ...
                println!("   Your answer: {:.6} (error: {:.2}%)", user_answer, error * 100.0);
            }
        },
        ...
        DifficultyLevel::Intermediate => {
            ...
            let real_part: f64 = get_user_input("Real part: ")?.parse()
                .map_err(|_| "Invalid number")?;
            let imag_part: f64 = get_user_input("Imaginary part: ")?.parse()
                .map_err(|_| "Invalid number")?;
...
fn error_function_adaptive_session(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &x in &test_values {
        ...
        println!("  Verification: erf({}) + erfc({}) = {:.6} ≈ 1.0", x, x, sum);
    }
    ...
    if matches!(profile.learning_preferences.difficulty_preference, DifficultyLevel::Advanced | DifficultyLevel::Expert) {
...
fn orthogonal_polynomial_adaptive_session(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if correct {
        ...
        println!("❌ Incorrect. Legendre polynomials are used in Gauss-Legendre quadrature for [-1,1].");
...
fn advanced_topic_session(profile: &mut UserProfile, topic: &str) -> Result<(), Box<dyn std::error::Error>> {
...
fn wright_functions_session(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Verification: ω({}) × e^ω({}) = {:.6} ≈ {}", z, z, verification, z);
...
fn spheroidal_functions_session(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
...
fn mathematical_proof_walkthrough(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
...
fn gamma_half_proof(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let steps = vec![
        ("Step 1: Definition", "Start with the definition: Γ(1/2) = ∫₀^∞ t^(-1/2) e^(-t) dt"),
        ("Step 2: Substitution", "Let t = u², then dt = 2u du"),
        ("Step 3: Transform", "Γ(1/2) = ∫₀^∞ (u²)^(-1/2) e^(-u²) · 2u du = 2∫₀^∞ e^(-u²) du"),
        ("Step 4: Gaussian integral", "We know that ∫₋∞^∞ e^(-u²) du = √π"),
        ("Step 5: Symmetry", "By symmetry, ∫₀^∞ e^(-u²) du = (1/2)√π"),
        ("Step 6: Conclusion", "Therefore, Γ(1/2) = 2 · (1/2)√π = √π ✓"),
    ];
...
fn stirling_approximation_proof(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &z in &test_values {
        ...
        println!("z = {}: Γ(z) = {:.6e}, Stirling ≈ {:.6e}, Error: {:.3}%", 
    ...
    for i in 0..zeros.len() {
        for j in i..zeros.len() {
            ...
            if i == j {
                ...
                println!("∫₀¹ x J_{}({:.3}x) J_{}({:.3}x) dx = {:.6} (expected: {:.6})", 
                        nu, zeros[i], nu, zeros[j], integral, expected);
            } else {
                println!("∫₀¹ x J_{}({:.3}x) J_{}({:.3}x) dx = {:.6} (expected: 0)", 
...
fn error_function_series_proof(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for n in 0..=20 {
        ...
        if n % 5 == 4 {
            ...
            println!("n = {:2}: series = {:.10}, error = {:.2e}", n, series_val, error);
        }
    }
...
fn solve_probability_problems(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n🎯 Interactive exploration:");
    let user_k = get_user_input("Enter a value k to compute P(X = k): ")?.parse::<u32>()
        .map_err(|_| "Invalid number")?;
...
fn solve_physics_problems(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for n in 0..=3 {
        ...
        let wavefunction = normalization / (2.0_f64.powi(n as i32) * factorial(n as u64) as f64).sqrt() 
                          * hermite_val * exponential;
        println!("ψ_{}(0) = {:.6}", n, wavefunction);
    }
    
    // Interactive exploration
    println!("\n🎯 Interactive:");
    let user_n: u32 = get_user_input("Enter quantum number n: ")?.parse()
        .map_err(|_| "Invalid number")?;
    let user_x: f64 = get_user_input("Enter position x: ")?.parse()
        .map_err(|_| "Invalid number")?;
    
    let hermite_val = hermite(user_n, alpha.sqrt() * user_x);
    let exponential = (-0.5 * alpha * user_x * user_x).exp();
    let wavefunction = normalization / (2.0_f64.powi(user_n as i32) * factorial(user_n as u64) as f64).sqrt() 
                      * hermite_val * exponential;
...
fn explore_gamma_visual(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Interactive exploration
    println!("\n🎯 Interactive exploration:");
    let x: f64 = get_user_input("Enter x value to evaluate Γ(x): ")?.parse()
        .map_err(|_| "Invalid number")?;
...
fn explore_error_visual(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let x: f64 = get_user_input("Enter x to find P(Z ≤ x) for standard normal: ")?.parse()
        .map_err(|_| "Invalid number")?;
...
fn real_world_applications_lab(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
...
fn computer_graphics_lab(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let user_theta: f64 = get_user_input("Enter theta (0 to π): ")?.parse()
        .map_err(|_| "Invalid number")?;
    let user_phi: f64 = get_user_input("Enter phi (0 to 2π): ")?.parse()
        .map_err(|_| "Invalid number")?;
...
fn climate_science_lab(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &tau in &optical_depths {
        ...
        println!("τ = {}: I = {:.1} W/m² ({:.1}% attenuated)", tau, intensity, attenuation);
    }
...
fn determine_challenge_level(profile: &UserProfile) -> u32 {
    ...
    match profile.level {
        1..=3 => 1,
        4..=7 => if avg_mastery > 0.7 { 2 } else { 1 },
        8..=15 => if avg_mastery > 0.8 { 3 } else { 2 },
        _ => if avg_mastery > 0.9 { 4 } else { 3 },
    }
}
...
fn intermediate_mastery_challenge(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
...
fn peer_comparison_mode(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let subjects = ["Gamma Functions", "Bessel Functions", "Error Functions", "Orthogonal Polynomials"];
    ...
    for (i, subject) in subjects.iter().enumerate() {
        ...
        println!("{}: You: {:.2}, Peers: {:.2}", subject, user_mastery, peer_avg);
    ...
    let weakest_subject = profile.mastery_scores.iter()
...
fn profile_settings(profile: &mut UserProfile) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Visual learner: {}", profile.learning_preferences.visual_learner);
    println!("Theoretical focus: {}", profile.learning_preferences.theoretical_focus);
    println!("Practical applications: {}", profile.learning_preferences.practical_applications);
    println!("Difficulty preference: {:?}", profile.learning_preferences.difficulty_preference);
    ...
    for record in profile.performance_history.iter().rev().take(recent_sessions) {
        println!("  {} - Score: {:.2} - Time: {:.1}s", 
                record.topic, record.score, record.time_taken.as_secs_f64());
    }
...
fn select_adaptive_topic(profile: &UserProfile) -> String {
    ...
    let weakest_topic = profile.mastery_scores.iter()
        .min_by(|a, b| a.1.partial_cmp(b.1).unwrap())
    ...
    match profile.level {
        ...
        4..=7 => {
            let intermediate = ["Orthogonal Polynomials", "Hypergeometric Functions", "Elliptic Functions"];
            let covered = intermediate.iter()
                .filter(|&&topic| profile.mastery_scores.contains_key(topic))
                .count();
        ...
        _ => {
            let advanced = ["Wright Functions", "Spheroidal Functions", "Mathieu Functions"];
            let covered = advanced.iter()
                .filter(|&&topic| profile.mastery_scores.contains_key(topic))
                .count();
...
fn check_gamma_achievements(profile: &mut UserProfile, score: f64, hints_used: u32) {
    ...
    if profile.mastery_scores.get("Gamma Functions").unwrap_or(&0.0) >= &0.9 {
        profile.earn_achievement(Achievement {
...
fn display_session_summary(profile: &UserProfile) {
    ...
    if !profile.mastery_scores.is_empty() {
        let avg_mastery: f64 = profile.mastery_scores.values().sum::<f64>() / profile.mastery_scores.len() as f64;
