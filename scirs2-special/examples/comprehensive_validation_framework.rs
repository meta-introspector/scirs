...
fn validate_gamma_functions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Reflection formula: max relative error = {:.2e}\n", max_error);
    ...
    println!("✅ Duplication formula: max relative error = {:.2e}\n", max_error);
    ...
    for &z in &asymptotic_test_values {
        ...
        println!("z = {:5.0}: exact = {:.8}, Stirling = {:.8}, error = {:.2e}", 
    ...
    for &z in &complex_test_cases {
        ...
        println!("z = {:.1}+{:.1}i: Γ(z) = {:.6}+{:.6}i, symmetry error = {:.2e}", 
...
fn validate_bessel_functions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, &alpha_i) in j0_zeros.iter().enumerate() {
        for (j, &alpha_j) in j0_zeros.iter().enumerate() {
            ...
            if i <= j {  // Only print upper triangle
                println!("∫ x J₀({:.4}x) J₀({:.4}x) dx = {:.6} (expected {:.1}), error = {:.2e}",
    ...
    for &x in &large_x_values {
        ...
        println!("J₀({:.0}): exact = {:.8}, asymptotic = {:.8}, error = {:.2e}",
    ...
    for &x in &modified_test_values {
        ...
        println!("x = {:.1}: I₀K₁ + I₁K₀ = {:.6} (expected {:.6}), error = {:.2e}",
...
fn validate_error_functions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &x in &test_values {
        ...
        if error > 1e-14 {
            println!("⚠️  x = {}: erf + erfc = {:.15}, error = {:.2e}", x, sum, error);
        }
    }
    ...
    for &p in &probability_values {
        ...
        println!("p = {:.2}: erfinv -> erf gives {:.8}, error = {:.2e}", p, recovered_p, error);
    }
    println!();
    ...
    for &z in &complex_args {
        ...
        println!("z = {}+{}i: erf(z*) = [erf(z)]*, error = {:.2e}",
...
fn validate_orthogonal_polynomials() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..=max_degree {
        for j in i..=max_degree {
            ...
            let expected = if i == j { 2.0 / (2.0 * i as f64 + 1.0) } else { 0.0 };
            let error = (orthogonality_integral - expected).abs();
            
            println!("∫ P_{}(x) P_{}(x) dx = {:.8} (expected {:.8}), error = {:.2e}",
    ...
    for &x in &x_values {
        for n in 1..6 {
            ...
            if error > 1e-12 {
                println!("⚠️  Hermite recurrence error n={}, x={}: {:.2e}", n, x, error);
            }
        }
    }
    ...
    for &m in &m_values {
        for &u in &u_values {
            ...
            if identity1_error > 1e-12 || identity2_error > 1e-12 {
                println!("⚠️  Jacobi identity error at u={}, m={}: {:.2e}, {:.2e}", 
...
fn validate_hypergeometric_functions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &z in &z_values {
        ...
        println!("₂F₁(1,1;2;{:.1}) = {:.8}, -ln(1-z)/z = {:.8}, error = {:.2e}",
    ...
    for &z in &z_values {
        if z < 1.0 {
            ...
            println!("z = {:.1}: Euler transformation error = {:.2e}", z, euler_error);
        }
    }
    println!();
...
fn validate_zeta_functions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &(s, expected) in &special_cases {
        ...
        println!("ζ({:.0}) = {:.12} (expected {:.12}), rel. error = {:.2e}",
    ...
    for &s in &s_values {
        if s != 1.0 {  // Avoid pole
            ...
            let functional_right = 2.0_f64.powf(s) * PI.powf(s - 1.0) * 
                                  (PI * s / 2.0).sin() * gamma(1.0 - s) * zeta_1_minus_s;
            
            let error = (zeta_s - functional_right).abs();
            let relative_error = error / zeta_s.abs();
            
            println!("s = {:.1}: functional equation rel. error = {:.2e}", s, relative_error);
        }
    }
    println!();
    ...
    for &a in &a_values {
        if a > 0.0 {
            ...
            if hurwitz_2.is_finite() && hurwitz_4.is_finite() {
                println!("ζ(2,{:.1}) = {:.8}, ζ(4,{:.1}) = {:.8}", a, hurwitz_2, a, hurwitz_4);
            }
        }
    }
    println!();
...
fn validate_information_theory_functions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, p) in probability_distributions.iter().enumerate() {
        ...
        println!("Distribution {}: H = {:.6} bits (max = {:.6})", i + 1, h, max_entropy);
...
fn benchmark_scalar_functions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("gamma          {:10}   {:9}   {:13.2}", iterations, gamma_time, gamma_rate);
    ...
    println!("j0             {:10}   {:9}   {:13.2}", iterations, j0_time, j0_rate);
    ...
    println!("erf            {:10}   {:9}   {:13.2}", iterations, erf_time, erf_rate);
    println!();
...
fn benchmark_simd_operations() -> Result<(), Box<dyn std::error::Error>> {
    #[cfg(feature = "simd")]
    {
        ...
        println!("gamma      {:11}   {:9}   {:7.2}x", scalar_time, simd_time, speedup);
    }
...
fn benchmark_parallel_operations() -> Result<(), Box<dyn std::error::Error>> {
    #[cfg(feature = "parallel")]
    {
        ...
        println!("gamma      {:11}   {:13}   {:7.2}x", serial_time, parallel_time, speedup);
    }
...
fn extreme_parameter_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &x in &small_values {
        ...
        println!("x = {:.0e}: Γ(x) = {:.2e}, J₀(x) = {:.6}, erf(x) = {:.2e}",
        ...
        assert!(erf_result.abs() <= x * 2.0 / PI.sqrt(), "erf should be ~ 2x/√π for small x");
    }
    println!();
    ...
    println!("Γ(1.5) = {:.15}, expected = {:.15}, rel. error = {:.2e}",
...
fn complex_plane_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &z in &complex_test_points {
        ...
        println!("z = {:.1}+{:.1}i: Γ(z) = {:.6}+{:.6}i, |Γ(z)| = {:.6}",
                z.re, z.im, gamma_z.re, gamma_z.im, gamma_z.norm());
    ...
    for &z in &near_branch_points {
        ...
        println!("Near pole at {:.0}: above = {:.3}+{:.3}i, below = {:.3}+{:.3}i",
...
fn educational_demonstrations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for n in 1..=10 {
        ...
        println!("{}! = {:.0}, Γ({}) = {:.6}, error = {:.2e}",
                n, factorial_exact, n + 1, gamma_result, error);
    }
    println!();
    ...
    for n in 0..5 {
        ...
        let wavefunction_norm = 1.0 / (2.0_f64.powi(n as i32) * factorial(n) as f64).sqrt() / PI.powf(0.25);
        
        println!("n = {}: E = {:.1}, ψ norm factor = {:.6}", n, energy, wavefunction_norm);
    }
    println!();
...
fn generate_validation_report() -> Result<(), Box<dyn std::error::Error>> {
    ...
    writeln!(file, "1. **Mathematical correctness** across all tested function families")?;
    writeln!(file, "2. **High performance** with modern optimization techniques")?;
    writeln!(file, "3. **Robust implementation** handling extreme parameter ranges")?;
    writeln!(file, "4. **Production readiness** for scientific computing applications")?;
