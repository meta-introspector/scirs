...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Harmonic values:   {:?}", 
             harmonics.iter().map(|&x| format!("{:.4}", x)).collect::<Vec<_>>());
    ...
    println!("Expansion coefficients ({}x{} matrix):", max_degree + 1, max_order + 1);
    ...
    println!("E_{}^{}({:.1}+{:.1}i; {:.2}+{:.2}i) = {:.4}+{:.4}i", 
             n, p, z_complex.re, z_complex.im, h2_complex.re, h2_complex.im,
             complex_harm.re, complex_harm.im);
    ...
    println!("Ellipsoidal harmonic (spherical limit): E_{}^0({:.1}) = {:.6}", 
