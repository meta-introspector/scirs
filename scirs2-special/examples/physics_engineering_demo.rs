...
fn main() {
    ...
    println!("Sun's peak emission wavelength: {:.1} nm", wavelength_max * 1e9);
    ...
    println!("Spectral radiance at 500nm: {:.2e} W⋅sr⁻¹⋅m⁻²⋅Hz⁻¹", radiance);
    ...
    println!("8-element array factor: {:.3} + {:.3}i", array_factor.re, array_factor.im);
    ...
    println!("Brewster's angle for glass: {:.1}°", brewster * 180.0 / std::f64::consts::PI);
    ...
    println!("Gaussian beam radius at 10cm: {:.2} mm", beam_radius * 1000.0);
    ...
    println!("Thermal diffusion length at 1kHz: {:.2} μm", thermal_diff * 1e6);
    ...
    println!("Silicon intrinsic carrier concentration at 300K: {:.2e} cm⁻³", ni);
    ...
    println!("Reflection coefficient: {:.3} ∠{:.1}°", 
             gamma.norm(), gamma.arg() * 180.0 / std::f64::consts::PI);
