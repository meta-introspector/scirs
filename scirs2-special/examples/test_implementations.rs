...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result_complex = wright_bessel_complex(
        1.0, 
        Complex64::new(1.0, 0.0), 
        Complex64::new(0.0, 0.0)
    )?;
    println!("Wright Bessel complex(1.0, 1+0i, 0+0i) = {}", result_complex);
    assert!((result_complex.re - 1.0).abs() < 1e-10, "Wright Bessel complex test failed");
    assert!(result_complex.im.abs() < 1e-10, "Wright Bessel complex imaginary part test failed");
    ...
    println!("Prolate spheroidal angular(0, 0, 0.0, 0.5) = {:?}", ang_result);
    assert!((ang_result.0 - 1.0).abs() < 1e-10, "Prolate angular function test failed");
    
    // Test Wright Bessel with non-zero values
    let result2 = wright_bessel(1.0, 2.0, 0.0)?;
    println!("Wright Bessel(1.0, 2.0, 0.0) = {}", result2);
    assert!((result2 - 1.0).abs() < 1e-10, "Wright Bessel(1.0, 2.0, 0.0) test failed");
    ...
    assert!(pro_result_small_c > 2.0, "Prolate CV with small c should be > 2.0");
