...
impl ForecastingMetrics {
    ...
    pub fn evaluate_forecast(
        ...
        let forecast_bias = y_pred.iter().zip(y_true.iter())
    ...
    fn calculate_mape(&self, y_true: &Array1<f64>, y_pred: &Array1<f64>) -> Result<f64> {
        ...
        for (true_val, pred_val) in y_true.iter().zip(y_pred.iter()) {
            if true_val.abs() > 1e-10 { // Avoid division by zero
        ...
        if count > 0 {
            ...
            Err(MetricsError::ComputationError("All true values are zero".to_string()))
        }
    }
    ...
    fn calculate_smape(&self, y_true: &Array1<f64>, y_pred: &Array1<f64>) -> Result<f64> {
        ...
        if count > 0 {
            ...
            Err(MetricsError::ComputationError("All values are zero".to_string()))
        }
    }
    
    /// Calculate Mean Absolute Scaled Error using training data
    fn calculate_mase(&self, y_true: &Array1<f64>, y_pred: &Array1<f64>, y_train: &Array1<f64>) -> Result<f64> {
        if y_train.len() < 2 {
            return Err(MetricsError::InvalidInput("Training data too short for MASE".to_string()));
        }
        ...
        if naive_mae > 1e-10 {
            ...
            Err(MetricsError::ComputationError("Naive forecast MAE is zero".to_string()))
        }
    }
    
    /// Calculate MASE with provided naive forecast
    fn calculate_mase_with_naive(&self, y_true: &Array1<f64>, y_pred: &Array1<f64>, naive: &Array1<f64>) -> Result<f64> {
        if naive.len() != y_true.len() {
            return Err(MetricsError::InvalidInput("Naive forecast length mismatch".to_string()));
        }
    ...
    /// Calculate directional accuracy
    fn calculate_directional_accuracy(&self, y_true: &Array1<f64>, y_pred: &Array1<f64>, y_train: Option<&Array1<f64>>) -> Result<f64> {
        if y_true.len() < 2 {
            return Ok(0.0);
        }
    ...
    /// Calculate Theil's U statistic
    fn calculate_theil_u(&self, y_true: &Array1<f64>, y_pred: &Array1<f64>, y_train: Option<&Array1<f64>>) -> Result<f64> {
...
impl TimeSeriesAnomalyMetrics {
    ...
    pub fn evaluate_anomaly_detection(
        ...
        for (true_val, pred_val) in y_true.iter().zip(y_pred.iter()) {
            match (true_val, pred_val) {
                ...
                (0, 0) => tn += 1,
                (1, 0) => fn_count += 1,
                _ => return Err(MetricsError::InvalidInput("Invalid label values".to_string())),
            }
        }
    ...
    fn calculate_auc(&self, y_true: &Array1<i32>, y_score: &Array1<f64>) -> Result<f64> {
        ...
        let mut pairs: Vec<(f64, i32)> = y_score.iter()
            .zip(y_true.iter())
...
impl TrendAnalysisMetrics {
    ...
    pub fn evaluate_trend_analysis(&self, time_series: &Array1<f64>, period: Option<usize>) -> Result<TrendAnalysisResults> {
        if time_series.len() < 10 {
            return Err(MetricsError::InvalidInput("Time series too short for trend analysis".to_string()));
        }
    ...
    /// Calculate trend strength using R² of linear fit
    fn calculate_trend_strength(&self, time_series: &Array1<f64>) -> Result<f64> {
        let n = time_series.len();
        let x: Array1<f64> = Array1::linspace(0.0, (n - 1) as f64, n);
        
        // Calculate R² to measure trend strength
        let r2 = r2_score(time_series, &x);
        match r2 {
            Ok(r2_val) => Ok(r2_val.abs()), // Use absolute value for trend strength
            Err(_) => Ok(0.0),
        }
    }
    
    /// Calculate seasonality strength
    fn calculate_seasonality_strength(&self, time_series: &Array1<f64>, period: usize) -> Result<f64> {
...
impl DomainMetrics for TimeSeriesSuite {
    ...
    fn metric_descriptions(&self) -> HashMap<&'static str, &'static str> {
        ...
        descriptions.insert("forecasting_mape", "Mean Absolute Percentage Error for forecasting");
        descriptions.insert("forecasting_smape", "Symmetric Mean Absolute Percentage Error");
        descriptions.insert("forecasting_mase", "Mean Absolute Scaled Error");
        descriptions.insert("forecasting_directional_accuracy", "Directional accuracy for trend prediction");
...
mod tests {
    ...
    fn test_forecasting_metrics() {
        ...
        let results = metrics.evaluate_forecast(&y_true, &y_pred, Some(&y_train)).unwrap();
    ...
    fn test_anomaly_detection_metrics() {
        ...
        let results = metrics.evaluate_anomaly_detection(&y_true, &y_pred, Some(&y_score)).unwrap();
    ...
    fn test_trend_analysis_metrics() {
        ...
        let results = metrics.evaluate_trend_analysis(&trend_data, Some(4)).unwrap();
    ...
    fn test_autocorrelation_calculation() {
        ...
        let autocorr = metrics.calculate_autocorrelation(&constant_series, 1).unwrap();
