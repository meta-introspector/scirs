...
use crate::anomaly::{detection_accuracy, false_alarm_rate, miss_detection_rate, anomaly_auc_score};
...
impl DetectionMetrics {
    ...
    fn calculate_auc_pr(&self, y_true: &Array1<f64>, y_score: &Array1<f64>) -> Result<f64> {
        ...
        let mut pairs: Vec<(f64, f64)> = y_score.iter()
            .zip(y_true.iter())
...
impl DistributionMetrics {
    ...
    pub fn evaluate_distribution_anomalies(
        ...
        let kl_divergence = self.calculate_kl_divergence_from_samples(normal_samples, anomaly_samples)?;
        
        // Calculate JS divergence
        let js_divergence = self.calculate_js_divergence_from_samples(normal_samples, anomaly_samples)?;
        
        // Calculate Wasserstein distance (1D case)
        let wasserstein_distance = self.calculate_wasserstein_distance(normal_samples, anomaly_samples)?;
    ...
    fn create_histograms(
        ...
        let min_val = samples1.iter().chain(samples2.iter()).fold(f64::INFINITY, |a, &b| a.min(b));
        let max_val = samples1.iter().chain(samples2.iter()).fold(f64::NEG_INFINITY, |a, &b| a.max(b));
    ...
    fn calculate_wasserstein_distance(
        ...
        if sorted1.len() == sorted2.len() {
            let distance = sorted1.iter()
                .zip(sorted2.iter())
                .map(|(a, b)| (a - b).abs())
                .sum::<f64>() / sorted1.len() as f64;
            Ok(distance)
        } else {
    ...
    /// Calculate energy distance
    fn calculate_energy_distance(&self, samples1: &Array1<f64>, samples2: &Array1<f64>) -> Result<f64> {
        let n1 = samples1.len() as f64;
        let n2 = samples2.len() as f64;
    ...
    fn calculate_ks_statistic(&self, samples1: &Array1<f64>, samples2: &Array1<f64>) -> Result<f64> {
        let mut all_values = samples1.iter().chain(samples2.iter()).copied().collect::<Vec<_>>();
        ...
        for &value in &all_values {
            let cdf1 = samples1.iter().filter(|&&x| x <= value).count() as f64 / samples1.len() as f64;
            let cdf2 = samples2.iter().filter(|&&x| x <= value).count() as f64 / samples2.len() as f64;
    ...
    fn calculate_ad_statistic(&self, samples1: &Array1<f64>, samples2: &Array1<f64>) -> Result<f64> {
...
impl TimeSeriesAnomalyMetrics {
    ...
    pub fn evaluate_time_series_anomalies(
        ...
        let (range_precision, range_recall) = 
            self.calculate_range_based_metrics(y_true, y_pred)?;
    ...
    /// Calculate standard precision and recall
    fn calculate_standard_metrics(&self, y_true: &Array1<i32>, y_pred: &Array1<i32>) -> Result<(f64, f64)> {
        let mut tp = 0;
        let mut fp = 0;
        let mut fn_count = 0;
    ...
    fn calculate_point_adjust_metrics(&self, y_true: &Array1<i32>, y_pred: &Array1<i32>) -> Result<(f64, f64)> {
        let true_anomaly_points: Vec<usize> = y_true.iter()
            .enumerate()
            .filter(|(_, &val)| val == 1)
            .map(|(idx, _)| idx)
            .collect();
        
        let pred_anomaly_points: Vec<usize> = y_pred.iter()
            .enumerate()
            .filter(|(_, &val)| val == 1)
            .map(|(idx, _)| idx)
            .collect();
    ...
    fn calculate_range_based_metrics(&self, y_true: &Array1<i32>, y_pred: &Array1<i32>) -> Result<(f64, f64)> {
                ...
                if y_pred[i] == 1 {
                    if !range_detected {
                        ...
                        if detection_position <= range_middle {
                            ...
                            score += self.nab_weights.true_positive + self.nab_weights.early_detection_bonus;
                        } else {
                            // Late detection penalty
                            score += self.nab_weights.true_positive + self.nab_weights.late_detection_penalty;
                        }
                        range_detected = true;
                    }
...
impl DomainMetrics for AnomalyDetectionSuite {
    ...
    fn metric_descriptions(&self) -> HashMap<&'static str, &'static str> {
        ...
        descriptions.insert("distribution_kl_divergence", "Kullback-Leibler divergence between distributions");
        descriptions.insert("distribution_js_divergence", "Jensen-Shannon divergence");
        descriptions.insert("distribution_wasserstein", "Wasserstein distance between distributions");
        descriptions.insert("time_series_point_adjust_precision", "Point-adjust precision with tolerance window");
        descriptions.insert("time_series_point_adjust_recall", "Point-adjust recall with tolerance window");
        descriptions.insert("time_series_nab_score", "Numenta Anomaly Benchmark score");
        descriptions.insert("time_series_average_delay", "Average delay in anomaly detection");
        descriptions
    }
}
...
mod tests {
    ...
    fn test_distribution_metrics() {
        ...
        let results = metrics.evaluate_distribution_anomalies(&normal_samples, &anomaly_samples).unwrap();
    ...
    fn test_time_series_anomaly_metrics() {
        ...
        let results = metrics.evaluate_time_series_anomalies(&y_true, &y_pred, Some(&timestamps)).unwrap();
