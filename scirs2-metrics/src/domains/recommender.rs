...
use crate::ranking::{ndcg_score, precision_at_k, recall_at_k, mean_average_precision};
...
impl RecommenderRankingMetrics {
    ...
    pub fn evaluate_ranking(
        ...
        for &k in &self.k_values {
            ...
            for i in 0..y_true.len() {
                ...
                let top_k_items: HashSet<usize> = recommended_items[i]
                    .iter()
                    .take(k)
                    .copied()
                    .collect();
            ...
            ndcg_at_k.insert(k, ndcg_scores.iter().sum::<f64>() / ndcg_scores.len() as f64);
            precision_at_k.insert(k, precision_scores.iter().sum::<f64>() / precision_scores.len() as f64);
            recall_at_k.insert(k, recall_scores.iter().sum::<f64>() / recall_scores.len() as f64);
            hit_rate_at_k.insert(k, hit_scores.iter().sum::<f64>() / hit_scores.len() as f64);
            coverage_at_k.insert(k, recommended_items_set.len() as f64 / self.total_items as f64);
        }
        
        // Calculate MAP
        let map = if let Ok(map_score) = mean_average_precision(y_true, y_score, None) {
            map_score
        } else {
            0.0
        };
    ...
    fn calculate_mrr(&self, y_true: &[Array1<f64>], recommended_items: &[Vec<usize>]) -> Result<f64> {
...
impl RatingPredictionMetrics {
    ...
    pub fn evaluate_rating_prediction(
        ...
        for &threshold in &self.rating_thresholds {
            ...
            let (precision, recall, f1) = self.calculate_binary_metrics(&y_true_binary, &y_pred_binary)?;
    ...
    fn calculate_binary_metrics(&self, y_true: &Array1<i32>, y_pred: &Array1<i32>) -> Result<(f64, f64, f64)> {
        let mut tp = 0;
        let mut fp = 0;
        let mut tn = 0;
        let mut fn_count = 0;
        ...
        for (&true_val, &pred_val) in y_true.iter().zip(y_pred.iter()) {
            match (true_val, pred_val) {
                ...
                (0, 0) => tn += 1,
                (1, 0) => fn_count += 1,
                _ => return Err(MetricsError::InvalidInput("Invalid binary labels".to_string())),
            }
        }
    ...
    fn calculate_intra_list_diversity(
        ...
        for items in recommended_items {
            ...
            if !pairwise_distances.is_empty() {
                total_diversity += pairwise_distances.iter().sum::<f64>() / pairwise_distances.len() as f64;
                count += 1;
            }
        }
        
        Ok(if count > 0 { total_diversity / count as f64 } else { 0.0 })
    }
...
impl DomainMetrics for RecommenderSuite {
    ...
    fn metric_descriptions(&self) -> HashMap<&'static str, &'static str> {
        ...
        descriptions.insert("ranking_ndcg_at_10", "Normalized Discounted Cumulative Gain at 10");
        descriptions.insert("ranking_precision_at_10", "Precision at 10 for ranking evaluation");
        descriptions.insert("ranking_recall_at_10", "Recall at 10 for ranking evaluation");
        descriptions.insert("ranking_map", "Mean Average Precision for ranking");
        descriptions.insert("ranking_mrr", "Mean Reciprocal Rank");
        descriptions.insert("ranking_hit_rate_at_10", "Hit rate at 10 (fraction of users with relevant items in top-10)");
        descriptions.insert("rating_rmse", "Root Mean Squared Error for rating prediction");
        descriptions.insert("rating_mae", "Mean Absolute Error for rating prediction");
        descriptions.insert("diversity_intra_list", "Average intra-list diversity of recommendations");
        descriptions.insert("diversity_item_coverage", "Fraction of catalog items recommended");
        descriptions.insert("diversity_entropy", "Entropy of item recommendation distribution");
        descriptions.insert("diversity_long_tail_coverage", "Coverage of long-tail items");
        descriptions
    }
}
...
mod tests {
    ...
    fn test_ranking_metrics() {
        ...
        let results = metrics.evaluate_ranking(&y_true, &y_score, &recommended_items).unwrap();
    ...
    fn test_rating_prediction_metrics() {
        ...
        let results = metrics.evaluate_rating_prediction(&y_true, &y_pred).unwrap();
    ...
    fn test_diversity_metrics() {
        ...
        let results = metrics.evaluate_diversity(&recommended_items, 10, 3, 0.1).unwrap();
    ...
    fn test_mrr_calculation() {
        ...
        let recommended_items = vec![
            vec![2, 0, 1, 3], // First relevant item at position 1 (0-indexed) -> rank 2 -> RR = 0.5
            vec![1, 3, 0, 2], // First relevant item at position 0 (0-indexed) -> rank 1 -> RR = 1.0
        ];
    ...
    fn test_gini_coefficient() {
        ...
        // Perfect inequality: only one item recommended
