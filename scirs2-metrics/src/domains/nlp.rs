...
impl TextGenerationMetrics {
    ...
    fn calculate_bleu(&self, reference: &[String], candidate: &[String], n: usize) -> f64 {
        ...
        let geo_mean = if precisions.iter().all(|&p| p > 0.0) {
            precisions.iter().product::<f64>().powf(1.0 / precisions.len() as f64)
        } else {
            0.0
        };
...
impl TextClassificationMetrics {
    ...
    pub fn evaluate_classification(
        ...
        for &class in &classes {
            ...
            per_class_metrics.insert(class, ClassMetrics {
                precision,
                recall,
                f1_score: f1,
                support,
...
impl NERMetrics {
    ...
    pub fn evaluate_ner(
        ...
        let token_correct = true_bio.iter().zip(pred_bio.iter()).filter(|(t, p)| t == p).count();
        ...
        for entity_type in entity_types {
            ...
            per_type_metrics.insert(entity_type, ClassMetrics {
...
impl DomainMetrics for NLPSuite {
    ...
    fn metric_descriptions(&self) -> HashMap<&'static str, &'static str> {
        ...
        descriptions.insert("text_generation_bleu_4", "BLEU-4 score for text generation quality");
        descriptions.insert("text_generation_rouge_l", "ROUGE-L score for text generation quality");
        descriptions.insert("text_classification_accuracy", "Accuracy for text classification");
        descriptions.insert("text_classification_macro_f1", "Macro-averaged F1 for text classification");
        descriptions.insert("ner_entity_f1", "Entity-level F1 score for named entity recognition");
        descriptions.insert("ner_token_f1", "Token-level F1 score for named entity recognition");
        descriptions.insert("sentiment_accuracy", "Accuracy for sentiment analysis");
        descriptions.insert("sentiment_macro_f1", "Macro F1 score for sentiment analysis");
        descriptions
    }
}
...
mod tests {
    ...
    fn test_text_generation_evaluation() {
        ...
        let results = metrics.evaluate_generation(&references, &candidates).unwrap();
    ...
    fn test_ner_evaluation() {
        ...
        let results = metrics.evaluate_ner(&true_entities, &pred_entities, 15).unwrap();
