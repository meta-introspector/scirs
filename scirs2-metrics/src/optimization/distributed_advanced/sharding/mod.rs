...
pub use crate::optimization::distributed::config::{HashFunction, ShardingConfig, ShardingStrategy};
...
impl DataRange {
        ...
        match self {
            ...
            DataRange::Time { start, end } => {
                ...
                if let Ok(timestamp_str) = key.parse::<u64>() {
                    if let Some(timestamp) = SystemTime::UNIX_EPOCH.checked_add(Duration::from_secs(timestamp_str)) {
    ...
    pub fn overlaps_with(&self, other: &DataRange) -> bool {
        match (self, other) {
            ...
            (DataRange::Numeric { start: s1, end: e1 }, DataRange::Numeric { start: s2, end: e2 }) => {
                s1 <= e2 && s2 <= e1
            }
    ...
    pub fn split(&self) -> Result<(DataRange, DataRange)> {
        match self {
            DataRange::Hash { start, end } => {
                ...
                Ok((
                    DataRange::Hash { start: *start, end: mid },
                    DataRange::Hash { start: mid + 1, end: *end },
                ))
            }
            DataRange::Key { start, end } => {
                ...
                Ok((
                    DataRange::Key { start: start.clone(), end: mid.clone() },
                    DataRange::Key { start: mid, end: end.clone() },
                ))
            }
            DataRange::Numeric { start, end } => {
                let mid = start + (end - start) / 2.0;
                Ok((
                    DataRange::Numeric { start: *start, end: mid },
                    DataRange::Numeric { start: mid, end: *end },
                ))
            }
            DataRange::Time { start, end } => {
                let duration = end.duration_since(*start).map_err(|_| {
                    MetricsError::ShardingError("Invalid time range".to_string())
                })?;
                let mid_duration = duration / 2;
                let mid = *start + mid_duration;
                Ok((
                    DataRange::Time { start: *start, end: mid },
                    DataRange::Time { start: mid, end: *end },
                ))
            }
...
impl ShardManager {
    ...
    pub fn get_node_for_key(&self, key: &str) -> Result<String> {
        match self.config.strategy {
            ShardingStrategy::ConsistentHash => {
                self.get_node_consistent_hash(key)
            }
            _ => {
    ...
    pub fn migrate_shard(&mut self, shard_id: &str, target_node: Option<String>) -> Result<String> {
        ...
        let shard = shards.get_mut(shard_id).ok_or_else(|| {
            MetricsError::ShardingError("Shard not found".to_string())
        })?;
        ...
        let target = target_node.unwrap_or_else(|| {
            ...
            shard.replicas.first().cloned()
                .unwrap_or_else(|| "default_node".to_string())
        });

        let migration_id = format!("migration_{}_{}", shard_id, 
    ...
    pub fn complete_migration(&mut self, migration_id: &str) -> Result<()> {
        ...
        let migration = migrations.get_mut(migration_id).ok_or_else(|| {
            MetricsError::ShardingError("Migration not found".to_string())
        })?;
    ...
    pub fn get_stats(&self) -> ShardingStats {
        ...
        let active_migrations = migrations.values()
...
mod tests {
    ...
    fn test_data_range_contains_key() {
        let hash_range = DataRange::Hash { start: 1000, end: 2000 };
        ...
        let numeric_range = DataRange::Numeric { start: 10.0, end: 20.0 };
    ...
    fn test_data_range_split() {
        ...
        if let (DataRange::Hash { start: s1, end: e1 }, DataRange::Hash { start: s2, end: e2 }) = (left, right) {
            assert_eq!(s1, 1000);
            assert_eq!(e2, 2000);
            assert_eq!(e1 + 1, s2);
        } else {
