...
impl SimdMetrics {
    ...
    fn detect_cuda_capabilities() -> Result<GpuInfo> {
        ...
        if std::env::var("CUDA_VISIBLE_DEVICES").is_ok() 
            ...
            Err(MetricsError::ComputationError("CUDA not available".to_string()))
        }
    }
    ...
    fn detect_opencl_capabilities() -> Result<GpuInfo> {
        ...
        if std::path::Path::new("/usr/lib/x86_64-linux-gnu/libOpenCL.so").exists()
            ...
            Err(MetricsError::ComputationError("OpenCL not available".to_string()))
        }
    }
    ...
    pub fn simd_exponential_moving_average<F>(&self, values: &ArrayView1<F>, alpha: F) -> Result<Array1<F>>
    where
        F: Float + SimdUnifiedOps<F> + Send + Sync,
    {
        if values.is_empty() {
        ...
        if self.enable_simd && self.capabilities.supports_simd() && values.len() > 4 {
            ...
            for i in (1..values.len()).step_by(chunk_size) {
                ...
                if chunk_len == chunk_size {
                    ...
                    let weighted_prev = F::simd_mul(&prev_ema.view(), &one_minus_alpha_array.view())?;
        ...
        F: Float + SimdUnifiedOps<F> + Send + Sync,
    {
        if matrices.is_empty() {
            return Err(MetricsError::InvalidInput("No matrices provided".to_string()));
        }
        ...
        F: Float + SimdUnifiedOps<F>,
    {
        ...
        if nrows != ncols {
            return Err(MetricsError::InvalidInput("Matrix must be square".to_string()));
        }
        ...
        F: Float + Send + Sync,
    {
        ...
        let blocks_needed = (batch_size + threads_per_block as usize - 1) / threads_per_block as usize;
                ...
                for &batch_idx in chunk {
                    ...
                    for &metric in metrics {
                        let result = match metric {
                            ...
                            "correlation" => self.simd_correlation(&y_true_sample, &y_pred_sample)?,
                            _ => F::zero(),
                        };
                        sample_results.insert(metric.to_string(), result);
                    }
        ...
        F: Float,
    {
        let mse = y_true.iter()
            .zip(y_pred.iter())
            .map(|(&t, &p)| (t - p) * (t - p))
            .sum::<F>() / F::from(y_true.len()).unwrap();
        Ok(mse)
    }
...
mod tests {
    ...
    fn test_simd_r2_score() {
        ...
        let r2 = metrics.simd_r2_score(&y_true.view(), &y_pred.view()).unwrap();
    ...
    fn test_gpu_batch_metrics() {
        ...
        let results = metrics.gpu_batch_metrics(
            &y_true_batch.view(),
            &y_pred_batch.view(),
            &["mse", "mae"]
        ).unwrap();
    ...
    fn test_benchmark_implementations() {
        ...
        let benchmark = metrics.benchmark_implementations(
            &y_true.view(),
            &y_pred.view(),
            10
        ).unwrap();
    ...
    fn test_simd_exponential_moving_average() {
        ...
        let ema = metrics.simd_exponential_moving_average(&values.view(), alpha).unwrap();
    ...
    fn test_simd_batch_matrix_operations() {
        ...
        let results = metrics.simd_batch_matrix_operations(&matrices, MatrixOperation::All).unwrap();
