    ...
    T: Float + Debug + Display + Copy + Zero + One + Send + Sync + 'static + std::ops::AddAssign,
{
    ...
    fn find(&self) -> (Array1<usize>, Array1<usize>, Array1<T>) {
        ...
        (Array1::from_vec(rows), Array1::from_vec(cols), Array1::from_vec(data))
    }
    ...
    fn dot(&self, other: &dyn SparseArray<T>) -> SparseResult<Box<dyn SparseArray<T>>> {
        ...
        let csr = crate::csr_array::CsrArray::from_triplets(
            &rows, &cols, &data, result.dim(), false
        )?;
    ...
    fn sum(&self, axis: Option<usize>) -> SparseResult<crate::sparray::SparseSum<T>> {
        match axis {
            ...
            Some(0) => {
                ...
                Ok(crate::sparray::SparseSum::SparseArray(Box::new(result_array)))
            }
            Some(1) => {
    ...
    fn max(&self) -> T {
        self.data.iter().fold(T::neg_infinity(), |a, &b| if a > b { a } else { b })
    }

    fn min(&self) -> T {
        self.data.iter().fold(T::infinity(), |a, &b| if a < b { a } else { b })
    }
    ...
    fn slice(&self, row_range: (usize, usize), col_range: (usize, usize)) -> SparseResult<Box<dyn SparseArray<T>>> {
        ...
        if end_row > self.shape.0 || end_col > self.shape.1 {
            return Err(SparseError::ValueError("Slice bounds exceed matrix dimensions".to_string()));
        }
