    ...
    S: SparseArray<T>,
{
    ...
    for start in 0..n {
        if !visited[start] {
            ...
            dfs_component(&adj_list, start, &mut visited, component_count, &mut labels);
            component_count += 1;
        }
    }
    ...
    S: SparseArray<T>,
{
    ...
    Ok((tarjan.component_count, tarjan.labels))
}
...
where
    ...
    S: SparseArray<T>,
{
    let (n_components, _) = connected_components(graph, directed, "strong", false)?;
    Ok(n_components == 1)
}
