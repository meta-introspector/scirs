...
pub fn ultra_precision_eig<F>(a: &ArrayView2<F>, workers: Option<usize>) -> LinalgResult<(Array1<F>, Array2<F>)>
where
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
    use crate::parallel;
    ...
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
    ...
    let det_estimate = if n == 2 {
        ...
        let product = (0..n).map(|i| a[[i, i]].abs()).fold(F::one(), |acc, x| acc * x);
        if product > F::zero() {
...
fn ultra_precise_small_matrix_eig<F>(a: &ArrayView2<F>, precision_target: F) -> LinalgResult<(Array1<F>, Array2<F>)>
where
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
    let n = a.nrows();
    ...
    match n {
        ...
        _ => Err(LinalgError::InvalidInput("Matrix size not supported for ultra-precise small matrix solver".to_string()))
    }
}

/// Ultra-precise 2x2 eigenvalue computation with Kahan summation
fn ultra_precise_2x2_eig<F>(a: &ArrayView2<F>, _precision_target: F) -> LinalgResult<(Array1<F>, Array2<F>)>
where
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
    let a11 = a[[0, 0]];
...
fn ultra_precise_3x3_eig<F>(a: &ArrayView2<F>, precision_target: F) -> LinalgResult<(Array1<F>, Array2<F>)>
where
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
...
fn ultra_precise_4x4_eig<F>(a: &ArrayView2<F>, precision_target: F) -> LinalgResult<(Array1<F>, Array2<F>)>
where
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
...
fn solve_cubic_equation_ultra_precise<F>(coeffs: &[F; 4], precision_target: F) -> LinalgResult<Array1<F>>
where
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
    ...
    if discriminant > precision_target {
        ...
        let two_pi_third = F::from(2.0).unwrap() * F::from(std::f64::consts::PI).unwrap() / F::from(3.0).unwrap();
        let a_third = a * third;
...
fn newton_method_cubic_refinement<F>(coeffs: &[F; 4], initial_guess: F, tolerance: F, max_iter: usize) -> LinalgResult<F>
where
    F: Float + NumAssign,
{
    let [c0, c1, c2, c3] = *coeffs;
...
fn compute_ultra_precise_eigenvectors_2x2<F>(a: &ArrayView2<F>, eigenvalues: &Array1<F>) -> LinalgResult<Array2<F>>
where
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
...
fn compute_ultra_precise_eigenvectors_3x3<F>(a: &ArrayView2<F>, eigenvalues: &Array1<F>, precision_target: F) -> LinalgResult<Array2<F>>
where
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
...
fn enhanced_null_space_computation<F>(matrix: &ArrayView2<F>, precision_target: F) -> LinalgResult<Array1<F>>
where
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
    let n = matrix.nrows();
...
fn ultra_precise_iterative_eig<F>(a: &ArrayView2<F>, precision_target: F, _workers: Option<usize>) -> LinalgResult<(Array1<F>, Array2<F>)>
where
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
...
fn ultra_precise_qr_iteration<F>(a: &ArrayView2<F>, precision_target: F, max_iter: usize) -> LinalgResult<(Array1<F>, Array2<F>)>
where
    F: Float + NumAssign + Sum + Send + Sync + ndarray::ScalarOperand + 'static,
{
    let n = a.nrows();
    ...
    for (new_idx, &old_idx) in indices.iter().enumerate() {
        ...
        sorted_eigenvectors.column_mut(new_idx).assign(&q_total.column(old_idx));
    }
