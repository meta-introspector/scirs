...
use scirs2_linalg::{compat, basic, decomposition, eigen, solve, norm as norm_mod, matrix_functions};
...
fn bench_basic_operations(c: &mut Criterion) {
    ...
    for &size in &[10, 50, 100] {
        ...
        group.bench_with_input(
            BenchmarkId::new("det_compat", size),
            &matrix,
            |b, m| b.iter(|| compat::det(&m.view(), false, true).unwrap())
        );
        
        group.bench_with_input(
            BenchmarkId::new("det_basic", size),
            &matrix,
            |b, m| b.iter(|| basic::det(&m.view()).unwrap())
        );
        
        // Benchmark matrix inverse
        group.bench_with_input(
            BenchmarkId::new("inv_compat", size),
            &matrix,
            |b, m| b.iter(|| compat::inv(&m.view(), false, true).unwrap())
        );
        
        group.bench_with_input(
            BenchmarkId::new("inv_basic", size),
            &matrix,
            |b, m| b.iter(|| basic::inv(&m.view()).unwrap())
        );
    }
...
fn bench_matrix_norms(c: &mut Criterion) {
    ...
    for &size in &[20, 100, 200] {
        ...
        group.bench_with_input(
            ...
            &matrix,
            |b, m| b.iter(|| norm_mod::matrix_norm(&m.view(), "frobenius").unwrap())
        );
        
        // 1-norm
        group.bench_with_input(
            BenchmarkId::new("norm_1_compat", size),
            &matrix,
            |b, m| b.iter(|| compat::norm(&m.view(), Some("1"), None, false, true).unwrap())
        );
        
        group.bench_with_input(
            BenchmarkId::new("norm_1_basic", size),
            &matrix,
            |b, m| b.iter(|| norm_mod::matrix_norm(&m.view(), "1").unwrap())
        );
        ...
        group.bench_with_input(
            BenchmarkId::new("norm_inf_basic", size),
            &matrix,
            |b, m| b.iter(|| norm_mod::matrix_norm(&m.view(), "inf").unwrap())
        );
    }
...
fn bench_decompositions(c: &mut Criterion) {
    ...
    for &size in &[20, 50, 100] {
        ...
        // LU decomposition
        group.bench_with_input(
            BenchmarkId::new("lu_compat", size),
            &matrix,
            |b, m| b.iter(|| compat::lu(&m.view(), false, false, true, false).unwrap())
        );
        
        group.bench_with_input(
            BenchmarkId::new("lu_basic", size),
            &matrix,
            |b, m| b.iter(|| decomposition::lu(&m.view()).unwrap())
        );
        
        // QR decomposition
        group.bench_with_input(
            BenchmarkId::new("qr_compat", size),
            &matrix,
            |b, m| b.iter(|| compat::qr(&m.view(), false, None, "full", false, true).unwrap())
        );
        
        group.bench_with_input(
            BenchmarkId::new("qr_basic", size),
            &matrix,
            |b, m| b.iter(|| decomposition::qr(&m.view()).unwrap())
        );
        
        // SVD (smaller sizes due to computational cost)
        if size <= 50 {
            group.bench_with_input(
                BenchmarkId::new("svd_compat", size),
                &matrix,
                |b, m| b.iter(|| compat::svd(&m.view(), true, true, false, true, "gesdd").unwrap())
            );
            
            group.bench_with_input(
                BenchmarkId::new("svd_basic", size),
                &matrix,
                |b, m| b.iter(|| decomposition::svd(&m.view(), false).unwrap())
            );
        }
...
fn bench_eigenvalues(c: &mut Criterion) {
    ...
    for &size in &[20, 50, 100] {
        ...
        group.bench_with_input(
            ...
            &spd_matrix,
            |b, m| b.iter(|| compat::eigh(&m.view(), None, false, true, false, false, true, None, None, None, 1).unwrap())
        );
        ...
        if size <= 50 {
            group.bench_with_input(
                ...
                &spd_matrix,
                |b, m| b.iter(|| compat::eigh(&m.view(), None, false, false, false, false, true, None, None, None, 1).unwrap())
            );
            
            group.bench_with_input(
                BenchmarkId::new("eigh_basic", size),
                &spd_matrix,
                |b, m| b.iter(|| eigen::eigh(&m.view()).unwrap())
            );
        }
    }
...
fn bench_linear_solvers(c: &mut Criterion) {
    ...
    for &size in &[20, 50, 100] {
        ...
        group.bench_with_input(
            ...
            &(&matrix, &rhs_2d),
            |b, (m, r)| b.iter(|| compat::compat_solve(&m.view(), &r.view(), false, false, false, true, None, false).unwrap())
        );
        ...
        if size >= 20 {
            ...
            group.bench_with_input(
                ...
                |b, (m, r)| b.iter(|| compat::lstsq(&m.view(), &r.view(), None, false, false, true, None).unwrap())
            );
...
fn bench_matrix_functions(c: &mut Criterion) {
    ...
    for &size in &[10, 20, 30] { // Smaller sizes for expensive matrix functions
        ...
        // Matrix exponential
        group.bench_with_input(
            BenchmarkId::new("expm_compat", size),
            &matrix,
            |b, m| b.iter(|| compat::expm(&m.view()).unwrap())
        );
        
        group.bench_with_input(
            BenchmarkId::new("expm_basic", size),
            &matrix,
            |b, m| b.iter(|| matrix_functions::expm(&m.view()).unwrap())
        );
        
        // Matrix square root
        group.bench_with_input(
            BenchmarkId::new("sqrtm_compat", size),
            &matrix,
            |b, m| b.iter(|| compat::sqrtm(&m.view()).unwrap())
        );
        
        group.bench_with_input(
            BenchmarkId::new("sqrtm_basic", size),
            &matrix,
            |b, m| b.iter(|| matrix_functions::sqrtm(&m.view(), 100, 1e-12).unwrap())
        );
        
        // Matrix logarithm
        if size <= 20 { // Even smaller for log
            group.bench_with_input(
                BenchmarkId::new("logm_compat", size),
                &matrix,
                |b, m| b.iter(|| compat::logm(&m.view()).unwrap())
            );
            
            group.bench_with_input(
                BenchmarkId::new("logm_basic", size),
                &matrix,
                |b, m| b.iter(|| matrix_functions::logm(&m.view()).unwrap())
            );
        }
    }
...
fn bench_matrix_properties(c: &mut Criterion) {
    ...
    for &size in &[20, 50, 100] {
        ...
        // Condition number
        group.bench_with_input(
            BenchmarkId::new("cond_compat", size),
            &matrix,
            |b, m| b.iter(|| compat::cond(&m.view(), Some("2")).unwrap())
        );
        
        group.bench_with_input(
            BenchmarkId::new("cond_basic", size),
            &matrix,
            |b, m| b.iter(|| norm_mod::cond(&m.view()).unwrap())
        );
        
        // Matrix rank
        group.bench_with_input(
            BenchmarkId::new("rank_compat", size),
            &matrix,
            |b, m| b.iter(|| compat::matrix_rank(&m.view(), None, false, true).unwrap())
        );
        
        group.bench_with_input(
            BenchmarkId::new("rank_basic", size),
            &matrix,
            |b, m| b.iter(|| norm_mod::matrix_rank(&m.view(), None).unwrap())
        );
        
        // Pseudoinverse (smaller sizes)
        if size <= 50 {
            group.bench_with_input(
                BenchmarkId::new("pinv_compat", size),
                &matrix,
                |b, m| b.iter(|| compat::pinv(&m.view(), None, false, true).unwrap())
            );
        }
    }
...
fn bench_advanced_decompositions(c: &mut Criterion) {
    ...
    for &size in &[10, 20, 30] {
        ...
        // RQ decomposition
        group.bench_with_input(
            BenchmarkId::new("rq_compat", size),
            &matrix,
            |b, m| b.iter(|| compat::rq(&m.view(), false, None, "full", true).unwrap())
        );
        
        // Polar decomposition
        group.bench_with_input(
            BenchmarkId::new("polar_right", size),
            &matrix,
            |b, m| b.iter(|| compat::polar(&m.view(), "right").unwrap())
        );
        
        group.bench_with_input(
            BenchmarkId::new("polar_left", size),
            &matrix,
            |b, m| b.iter(|| compat::polar(&m.view(), "left").unwrap())
        );
    }
...
fn bench_memory_allocation(c: &mut Criterion) {
    ...
    for &size in &[50, 100, 200] {
        ...
        group.bench_function(
            ...
            |b| b.iter(|| Array2::<f64>::zeros((size, size)))
        );
...
fn bench_scalability(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("det_scaling", size),
            &matrix,
            |b, m| b.iter(|| compat::det(&m.view(), false, true).unwrap())
        );
        ...
        // Test norm scaling
        group.bench_with_input(
            BenchmarkId::new("norm_scaling", size),
            &matrix,
            |b, m| b.iter(|| compat::norm(&m.view(), Some("fro"), None, false, true).unwrap())
        );
        ...
        if size <= 100 {
            group.bench_with_input(
                BenchmarkId::new("lu_scaling", size),
                &matrix,
                |b, m| b.iter(|| compat::lu(&m.view(), false, false, true, false).unwrap())
            );
        }
