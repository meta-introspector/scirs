...
fn adaptive_algorithms_demo() -> LinalgResult<()> {
    ...
    for &(size, threshold) in &sizes_and_thresholds {
        println!("\n📊 Testing {}x{} matrix (threshold: {})", size, size, threshold);
    ...
    let start = Instant::now();
    let _result1 = algorithms::parallel_matvec(&unbalanced_matrix.view(), &vector.view(), &config_balanced)?;
    let balanced_time = start.elapsed();
    
    let start = Instant::now();
    let _result2 = algorithms::parallel_matvec(&unbalanced_matrix.view(), &vector.view(), &config_imbalanced)?;
