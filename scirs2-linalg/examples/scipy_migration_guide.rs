...
use scirs2_linalg::{
    ...
    det, inv, matrix_norm, matrix_rank,
    lu, qr, svd, cholesky,
    solve,
    eig, eigh,
};
...
fn decompositions_migration() -> LinalgResult<()> {
    ...
    println!("U shape: {:?}, s length: {}, Vt shape: {:?}", u.dim(), s.len(), vt.dim());
    ...
    match cholesky(&a.view(), None) {
        ...
        Err(e) => {
            println!("❌ Cholesky failed (matrix may not be positive definite): {}", e);
...
fn advanced_features_differences() -> LinalgResult<()> {
    ...
    let a = Array2::from_shape_fn((100, 100), |(i, j)| {
        ((i + 1) * (j + 1)) as f64
    });
    ...
    let _result = scirs2_linalg::parallel::algorithms::parallel_matvec(&a.view(), &vector.view(), &config)?;
