...
fn main() -> LinalgResult<()> {
    ...
    let (eigenvals, eigenvecs_opt) = compat::eigh(
        &a.view(), None, false, false, false, false, true,
        None, None, None, 1
    )?;
    ...
    let (eigenvals_only, _) = compat::eigh(
        &a.view(), None, false, true, false, false, true,
        None, None, None, 1
    )?;
    ...
    let (lstsq_solution, residuals_opt, lstsq_rank, sing_vals) = compat::lstsq(
        &a.view(), &b.view(), None, false, false, true, None
    )?;
